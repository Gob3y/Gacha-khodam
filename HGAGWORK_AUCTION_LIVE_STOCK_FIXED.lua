...
function HaimiyachGAG2_AuctionDisplayName(lot)
    ...
    if lot.guiDisplayName and tostring(lot.guiDisplayName) ~= "" then return tostring(lot.guiDisplayName) end
    HaimiyachGAG2_AuctionLoadModules()
    if HaimiyachGAG2_AuctioneerModule and HaimiyachGAG2_AuctioneerModule.DisplayName then
        ...
        if ok and txt and tostring(txt) ~= "" then return tostring(txt) end
    end
    local name = tostring(lot.item or lot.Name or lot.FruitName or lot.name or lot.DisplayName or lot.displayName or "Unknown")
    if lot.mutation and tostring(lot.mutation) ~= "" then name = tostring(lot.mutation) .. " " .. name end
    if lot.count and tonumber(lot.count) and tonumber(lot.count) > 1 and not string.find(name, "%(x") then
        name = name .. " (x" .. tostring(math.floor(tonumber(lot.count))) .. ")"
    end
    return name
end
...
function HaimiyachGAG2_AuctionCurrentPrice(lot)
    ...
    if tonumber(lot.guiCurrentPrice) then return tonumber(lot.guiCurrentPrice) end
    HaimiyachGAG2_AuctionLoadModules()
    if HaimiyachGAG2_AuctioneerModule and HaimiyachGAG2_AuctioneerModule.CurrentPrice then
        local ok, price = pcall(function() return HaimiyachGAG2_AuctioneerModule.CurrentPrice(lot, HaimiyachGAG2_AuctionServerNow()) end)
        if ok and type(price) == "number" then return price end
    end
    return tonumber(lot.currentPrice or lot.price or lot.startPrice or lot.startingPrice or lot.buyPrice or lot.PurchasePrice or lot.cost) or 0
end
...
function HaimiyachGAG2_AuctionCollectGuiRows(root)
    ...
    local function addRow(row)
        ...
        local stockText = HaimiyachGAG2_AuctionFirstText(row, { "Stock_Text", "StockText", "Stock", "Amount" })
...
function HaimiyachGAG2_AuctionParseRowsFromGameGui()
    ...
    for i, row in ipairs(rows) do
        ...
        local buyText = ""
        if buyButton then
            buyText = HaimiyachGAG2_AuctionFirstText(buyButton, { "Text", "Price", "Amount" })
            if buyText == "" and buyButton:IsA("TextButton") then buyText = tostring(buyButton.Text or "") end
        end
        if buyText == "" then buyText = HaimiyachGAG2_AuctionTextPattern(row, "price") end

        local lowStock = string.lower(tostring(stockText or ""))
        local stockNum = nil
        local n = tostring(stockText or ""):match("x([%d,]+)") or tostring(stockText or ""):match("([%d,]+)")
        if n then stockNum = tonumber((tostring(n):gsub(",", ""))) end
        if string.find(lowStock, "sold", 1, true) or (row:FindFirstChild("OUT_OF_STOCK", true) and row:FindFirstChild("OUT_OF_STOCK", true).Visible) then stockNum = 0 end
        ...
        if itemName ~= "" then
            lots[#lots + 1] = {
                ...
                guiCurrentPrice = HaimiyachGAG2_AuctionTextToNumber(buyText),
    ...
    if #lots > 0 then
        for _, lot in ipairs(lots) do pcall(HaimiyachGAG2_AuctionSyncLotStockFromLive, lot) end
        HaimiyachGAG2_AuctionState.lots = lots
        pcall(HaimiyachGAG2_AuctionRememberItems, lots)
        HaimiyachGAG2_AuctionState.optionVersion = (tonumber(HaimiyachGAG2_AuctionState.optionVersion) or 0) + 1
        HaimiyachGAG2_AuctionState.status = "LOADED " .. tostring(#lots) .. " LOTS FROM GAME AUCTION GUI + LIVE STOCK"
        return true
    end
...
function HaimiyachGAG2_AuctionRefresh(force)
    ...
    local state = HaimiyachGAG2_AuctionState
    local now = os.clock()
    local scanGui = force or not state.lots or #state.lots == 0
    if not scanGui and now - (tonumber(state.lastGuiScan) or 0) > 2 then
        local pg = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
        local auctionGui = pg and pg:FindFirstChild("Auction")
        scanGui = auctionGui and (not auctionGui:IsA("ScreenGui") or auctionGui.Enabled)
    end
    if scanGui then
        state.lastGuiScan = now
        if HaimiyachGAG2_AuctionParseRowsFromGameGui() then return true end
    end
    if not force and state.lastFetch and now - state.lastFetch < 3 then return false end
    state.lastFetch = now
    state.requestId = (tonumber(state.requestId) or 0) + 1
    state.status = "REQUESTING SNAPSHOT..."
    ...
    if force then
        ...
        if HaimiyachGAG2_AuctionParseRowsFromGameGui() then return true end
        if HaimiyachGAG2_AuctionTryModuleLots() then return true end
        if state.lots and #state.lots > 0 then return true end
        if ok and result == nil then
            state.status = "NO SNAPSHOT RESPONSE. OPEN AUCTION NPC ONCE, THEN REFRESH."
        end
    end
    return false
end
...
function HaimiyachGAG2_AuctionStock(lot)
    ...
    -- Controller auction uses a separate live stock table (t5) from StockUpdate.
    -- Use it first because the visible GUI Stock_Text is animated/cached and can be stale.
    local live = HaimiyachGAG2_AuctionLiveStock(lot)
    ...
    if lot.guiStockText and tostring(lot.guiStockText) ~= "" then
        local low = string.lower(tostring(lot.guiStockText))
        if string.find(low, "unlimited", 1, true) then return nil end
        if string.find(low, "sold", 1, true) then return 0 end
        local n = tostring(lot.guiStockText):match("x([%d,]+)") or tostring(lot.guiStockText):match("([%d,]+)")
        if n then return tonumber((tostring(n):gsub(",", ""))) or 0 end
    end
...
function HaimiyachGAG2_AuctionMatchesSelection(lot)
    ...
    local rawName = HaimiyachGAG2_NameKey(lot and (lot.item or lot.Name or lot.name or lot.DisplayName) or "")
    local filter = HaimiyachGAG2_NameKey(S.auctionNameFilter or "")
    if filter ~= "" and (string.find(display, filter, 1, true) or string.find(rawName, filter, 1, true)) then return true end
    ...
    for want, enabled in pairs(S.auctionBuyItems) do
        if enabled == true then
            ...
            if key == "allauctionitems" or key == "allactiveauctionitems" or key == display or key == rawName then
                return true
            end
        end
    end
