...
local function cleanContainer(container)
    ...
    for _, child in ipairs(container:GetChildren()) do
        pcall(function()
            ...
            if child:IsA("ScreenGui") and (string.find(n, "skrilya") or string.find(n, "maclib") or string.find(n, "haimiyach_gag2")) then
                child:Destroy()
            end
        end)
    end
...
local baseW = isMobile and math.min(620, math.floor(ScreenSize.X * 0.68)) or 690
local baseH = isMobile and math.min(380, math.floor(ScreenSize.Y * 0.68)) or 430
baseW = math.max(560, baseW)
baseH = math.max(340, baseH)
...
new("Frame", { Name = "TopLine", Size = UDim2.new(1,0,0,1), Position = UDim2.new(0,0,1,-1), BackgroundColor3 = Color3.fromRGB(45,45,45), BorderSizePixel = 0 }, TopBar)
...
corner(TabBar, 14); stroke(TabBar, Color3.fromRGB(60,60,60), 1, 0.35); pad(TabBar, 8, 8, 6, 6)
...
corner(Content, 14); stroke(Content, Color3.fromRGB(60,60,60), 1, 0.35)
...
local function setVisible(v)
    ...
    ShowButton.Visible = not UI.Visible
end
local function setMinimized(v)
    UI.Minimized = v and true or false
    if UI.Minimized then
        Main.Size = UDim2.fromOffset(baseW, 56)
...
MinBtn.Activated:Connect(function() setMinimized(not UI.Minimized) end)
...
local function createTab(name, order)
    local w = math.clamp((#name * 8) + 68, 112, 162)
...
local function addToggle(page, label, default, callback)
    ...
    corner(hit, 15); stroke(hit, Color3.fromRGB(80,80,80), 1, 0.35)
    ...
    hit.Activated:Connect(function() value = not value; draw(true) end)
...
local function addInput(page, label, placeholder, default, callback)
    ...
    corner(box, 8); pad(box, 10, 10, 0, 0)
...
local function addDropdown(page, label, options, default, multi, callback)
    ...
    local row = new("Frame", { Size = UDim2.new(1,0,0,62), BackgroundColor3 = T.Row, BorderSizePixel = 0, ClipsDescendants = false }, page)
    corner(row, 10)
    new("TextLabel", {
        Size = UDim2.new(0.42, -18, 1, 0),
        Position = UDim2.fromOffset(14, 0),
    ...
    local list = new("ScrollingFrame", {
        ...
        Position = UDim2.new(0.42, 10, 1, 4),
    ...
    corner(list, 8); stroke(list, Color3.fromRGB(70,70,70), 1, 0.3)
    ...
    local function selectedText()
        if multi then
            ...
            return tostring(current or options[1] or "Select")
        end
    end
    local function emit()
    ...
    local function rebuild()
        ...
        for i, op in ipairs(options) do
            local item = new("TextButton", {
                ...
                BorderSizePixel = 0,
                Text = (multi and (selected[op] and "✓ " or "") or "") .. tostring(op),
    ...
    return {
        SetOptions = function(newOptions)
            ...
            if not multi and current and not table.find(options, current) then current = options[1] end
            rebuild(); emit()
        end,
...
local BoostPage = createTab("BOOSTS", 2)
local PetsPage = createTab("PETS", 3)
local OpenPage = createTab("OPEN", 4)
local ShopPage = createTab("SHOP", 5)
local StealPage = createTab("STEAL", 6)
local MiscPage = createTab("MISC", 7)
local SettingsPage = createTab("SETTINGS", 8)

-- FARM
addSection(FarmPage, "Status")
local plotLabel = addLabel(FarmPage, "Plot: ?")
local cashLabel = addLabel(FarmPage, "Sheckles: ? · Tokens: ?")
local statLabel = addLabel(FarmPage, "bought 0 · planted 0 · harvested 0 · sold 0 (+0)")
addSection(FarmPage, "Master")
addToggle(FarmPage, "Auto-Farm (buy+plant+harvest+sell+expand)", false, function(v) S.autoFarm = v end)
addToggle(FarmPage, "Auto-Expand garden", false, function(v) S.autoExpand = v end)
addToggle(FarmPage, "Auto-Daily deals", false, function(v) S.autoDaily = v end)
addSection(FarmPage, "Buy Seeds")
addDropdown(FarmPage, "Seeds to buy", SEED_NAMES, {}, true, function(sel) pickMulti(sel, S.buySeeds) end)
addToggle(FarmPage, "Auto-Buy selected", false, function(v) S.autoBuy = v end)
addSlider(FarmPage, "Buy interval (s)", 1, 30, S.buyInterval, 0, function(v) S.buyInterval = v end)
addSlider(FarmPage, "Max buys / seed / pass", 1, 50, S.buyPerTick, 0, function(v) S.buyPerTick = v end)
addSection(FarmPage, "Plant / Harvest / Sell")
local plantOptions = { "Best owned" }; for _, n in ipairs(SEED_NAMES) do plantOptions[#plantOptions + 1] = n end
addDropdown(FarmPage, "Seed to plant", plantOptions, S.plantSeed, false, function(v) S.plantSeed = v or "Best owned" end)
addToggle(FarmPage, "Auto-Plant (fill plot)", false, function(v) S.autoPlant = v end)
addSlider(FarmPage, "Plant spacing (studs)", 2, 10, S.plantSpacing, 0, function(v) S.plantSpacing = v end)
addToggle(FarmPage, "Auto-Harvest ripe fruit", false, function(v) S.autoHarvest = v end)
addSlider(FarmPage, "Harvest pace (s/fruit)", 0, 0.2, S.harvestDelay, 3, function(v) S.harvestDelay = v end)
addToggle(FarmPage, "Auto-Sell", false, function(v) S.autoSell = v end)
addSlider(FarmPage, "Sell interval (s)", 3, 120, S.sellInterval, 0, function(v) S.sellInterval = v end)
addToggle(FarmPage, "Auto-Pot grown plants", false, function(v) S.autoPot = v end)

-- BOOSTS
addSection(BoostPage, "Sprinkler / Water")
addToggle(BoostPage, "Auto-place Sprinklers", false, function(v) S.autoSprinkler = v end)
addSlider(BoostPage, "Sprinkler interval (s)", 10, 120, S.sprinklerInterval, 0, function(v) S.sprinklerInterval = v end)
addToggle(BoostPage, "Auto-Watering Can", false, function(v) S.autoWater = v end)
addSlider(BoostPage, "Water interval (s)", 2, 60, S.waterInterval, 0, function(v) S.waterInterval = v end)
addSection(BoostPage, "Skill Points")
addDropdown(BoostPage, "Stats to level", { "BaseSpeed", "BaseJump", "ShovelPower", "MaxBackpack" }, {}, true, function(sel) pickMulti(sel, S.skillStats) end)
addToggle(BoostPage, "Auto-Spend skill points", false, function(v) S.autoSkill = v end)

-- PETS
addSection(PetsPage, "Pets")
addToggle(PetsPage, "Auto-Equip pets (to slot cap)", false, function(v) S.autoEquipPets = v end)
addToggle(PetsPage, "Auto-Buy pet slots", false, function(v) S.autoPetSlot = v end)
addToggle(PetsPage, "Auto-Buy world pets", false, function(v) S.autoBuyPets = v end)
addSlider(PetsPage, "Max pet price (Sheckles)", 1000, 1000000, S.maxPetPrice, 0, function(v) S.maxPetPrice = v end)
addToggle(PetsPage, "Teleport to pet", true, function(v) S.petTeleport = v end)
addSlider(PetsPage, "Pet buy interval (s)", 2, 60, S.petBuyInterval, 0, function(v) S.petBuyInterval = v end)
addSection(PetsPage, "Sell Pets")
local petDrop = addDropdown(PetsPage, "Pets to sell", ownedPetNames(), {}, true, function(sel) pickMulti(sel, S.sellPets) end)
addButton(PetsPage, "Refresh pet list", function() petDrop.SetOptions(ownedPetNames()) end)
addToggle(PetsPage, "Auto-Sell selected pets", false, function(v) S.autoSellPets = v end)

-- OPEN
addSection(OpenPage, "Eggs / Crates / Packs")
addToggle(OpenPage, "Auto-Open Eggs", false, function(v) S.autoEgg = v end)
addToggle(OpenPage, "Auto-Open Crates", false, function(v) S.autoCrate = v end)
addToggle(OpenPage, "Auto-Open Seed Packs", false, function(v) S.autoPack = v end)
addSlider(OpenPage, "Open interval (s)", 1, 30, S.openInterval, 0, function(v) S.openInterval = v end)
addLabel(OpenPage, "Auto-open uses owned items from your inventory. Keep this off if you want to save rare eggs, crates, or seed packs.")

-- SHOP
addSection(ShopPage, "Gear Shop")
addDropdown(ShopPage, "Gear to buy", GEAR_NAMES, {}, true, function(sel) pickMulti(sel, S.gearBuy) end)
addToggle(ShopPage, "Auto-Buy selected gear", false, function(v) S.autoGear = v end)
addSlider(ShopPage, "Gear buy interval (s)", 2, 60, S.gearInterval, 0, function(v) S.gearInterval = v end)

-- STEAL
addSection(StealPage, "Steal")
addToggle(StealPage, "Auto-Steal others' ripe fruit", false, function(v) S.autoSteal = v end)
addToggle(StealPage, "Teleport to fruit", true, function(v) S.stealTeleport = v end)
addToggle(StealPage, "Return to base after each fruit", true, function(v) S.stealReturnBase = v end)
addSlider(StealPage, "Steal speed (delay/fruit)", 0, 1, S.stealDelay, 2, function(v) S.stealDelay = v end)
addLabel(StealPage, "This works only when the game allows stealing and the fruit is available through the game prompt/remotes.")

-- MISC
addSection(MiscPage, "Mailbox / Gifts")
addToggle(MiscPage, "Auto-Claim mailbox", false, function(v) S.autoMail = v end)
addToggle(MiscPage, "Auto-Accept gifts", false, function(v) S.autoAcceptGift = v end)
addSection(MiscPage, "Server")
addToggle(MiscPage, "Anti-AFK", true, function(v) S.antiAfk = v end)
addToggle(MiscPage, "Auto server-hop", false, function(v) S.autoHop = v end)
addSlider(MiscPage, "Hop every (min, 0=off)", 0, 120, S.hopInterval / 60, 0, function(v) S.hopInterval = v * 60 end)
addSection(MiscPage, "Codes")
addInput(MiscPage, "Redeem a code", "enter code", "", function(text) S.codeText = text or "" end)
...
addToggle(MiscPage, "Auto-redeem code list", false, function(v) S.autoCodes = v end)

-- SETTINGS
addSection(SettingsPage, "Performance")
addToggle(SettingsPage, "FPS Boost (low graphics)", false, function(v) S.fpsBoost = v; applyFpsBoost(v) end)
addSlider(SettingsPage, "UI Scale", 0.6, 1.4, UI.Scale, 2, function(v)
    UI.Scale = v
    UIScaleObj.Scale = v
end)
addSection(SettingsPage, "Window")
addDropdown(SettingsPage, "UI Keybind", { "LeftControl", "RightControl", "LeftAlt", "RightAlt", "RightShift", "K", "F" }, "LeftControl", false, function(v)
    local kc = Enum.KeyCode[v or "LeftControl"]
...
addToggle(SettingsPage, "Enable reports", false, function(v) S.webhookEnabled = v end)
addSlider(SettingsPage, "Report interval (min)", 1, 60, S.webhookInterval / 60, 0, function(v) S.webhookInterval = v * 60 end)
...
addLabel(SettingsPage, "Grow a Garden 2 · HAIMIYACH HUB custom UI\nHotkey default: Left Control\nTop show button is locked at the top center.")
addButton(SettingsPage, "Unload hub (stops everything)", function()
    S.killed = true
    UI.Unloaded = true
    cleanContainer(PlayerGui)
    cleanContainer(CoreGui)
...
selectTab("FARM")
setVisible(true)
notify("HAIMIYACH HUB", "Grow A Garden 2", 5)
print("[HAIMIYACH HUB] - Grow a Garden 2")
