...
-- HIDE PLANTS
-- Simple, safe client-side hide. It is not saved to config and it does not auto-run on execute.
local HidePlantsEnabled = false
local HidePlantsBackup = {}

local function HidePlantsRemember(obj, prop)
    if not obj then return end
    if HidePlantsBackup[obj] == nil then HidePlantsBackup[obj] = {} end
    if HidePlantsBackup[obj][prop] == nil then
        local ok, val = pcall(function()
            return obj[prop]
        end)
        if ok then
            HidePlantsBackup[obj][prop] = val
        end
    end
end

local function HidePlantsSet(obj, prop, value)
    pcall(function()
        if obj then obj[prop] = value end
    end)
end

local function HidePlantsGetRoots()
    local roots = {}

    local ownPlot = myPlot()
    local ownPlants = ownPlot and ownPlot:FindFirstChild("Plants")
    if ownPlants then
        roots[#roots + 1] = ownPlants
    end
    ...
    if gardens then
        for _, plot in ipairs(gardens:GetChildren()) do
            ...
            if plants and plants ~= ownPlants then
                roots[#roots + 1] = plants
            end
        end
    end
...
local function HidePlantsApplyObject(obj)
    if not obj then return end

    local okBasePart, isBasePart = pcall(function() return obj:IsA("BasePart") end)
    if okBasePart and isBasePart then
        HidePlantsRemember(obj, "LocalTransparencyModifier")
        HidePlantsRemember(obj, "CastShadow")
        HidePlantsSet(obj, "LocalTransparencyModifier", 1)
        HidePlantsSet(obj, "CastShadow", false)
        return
    end

    local okDecal, isDecal = pcall(function() return obj:IsA("Decal") or obj:IsA("Texture") end)
    if okDecal and isDecal then
        HidePlantsRemember(obj, "Transparency")
        HidePlantsSet(obj, "Transparency", 1)
        return
    end

    local okEffect, isEffect = pcall(function()
        return obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Fire") or obj:IsA("Smoke") or obj:IsA("Sparkles")
    end)
    if okEffect and isEffect then
        HidePlantsRemember(obj, "Enabled")
        HidePlantsSet(obj, "Enabled", false)

        local okParticle, isParticle = pcall(function() return obj:IsA("ParticleEmitter") end)
        if okParticle and isParticle then
            HidePlantsRemember(obj, "Rate")
            HidePlantsSet(obj, "Rate", 0)
        end
    end
end

local function RestoreHidePlants()
    HidePlantsEnabled = false

    for obj, props in pairs(HidePlantsBackup) do
        if obj and obj.Parent and type(props) == "table" then
            for prop, value in pairs(props) do
                HidePlantsSet(obj, prop, value)
            end
        end
    end
...
local function SetHidePlantsEnabled(state)
    ...
    HidePlantsEnabled = true

    task.spawn(function()
        local processed = 0
        for _, root in ipairs(HidePlantsGetRoots()) do
            if not HidePlantsEnabled then break end

            HidePlantsApplyObject(root)
...
hidePlantsToggle = addToggle(VisualsPage, "HIDE PLANTS", false, function(v)
