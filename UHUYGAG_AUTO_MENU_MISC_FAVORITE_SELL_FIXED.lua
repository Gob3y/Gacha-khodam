...
function HaimiyachGAG2_MutationCatalog()
    local out, seen = {}, {}
    local defaults = { "Rainbow", "Gold", "Shiny", "Wet", "Chilled", "Frozen", "Moonlit", "Bloodlit", "Void", "Celestial" }
    local ok, data = pcall(function() return require(ReplicatedStorage.SharedModules.MutationData) end)
    if ok and type(data) == "table" then
        for k, v in pairs(data) do
            local name = nil
            if type(v) == "table" then
                name = v.Name or v.DisplayName or v.Mutation or v.Id
...
MUTATION_NAMES = HaimiyachGAG2_MutationCatalog()
...
local S = {
    ...
    autoSell = false, sellInterval = 15, sellProtectMutations = {},
    autoExpand = false, autoPot = false, autoDaily = false,
    autoClaimSeedEvent = false, claimUseFly = false,
    autoShovelPlants = false, autoShovelFruits = false, shovelPlantTargets = {}, shovelFruitTargets = {}, shovelMaxKg = 0, shovelNameFilter = "", shovelDelay = 2,
    autoFavoriteFruits = false, favoriteFruitTargets = {}, favoriteFruitFilter = "", favoriteMinKg = 0, favoriteMaxKg = 0, favoriteMutationTargets = {}, favoriteMutationFilter = "", unfavoriteNotMatching = false, favoriteDelay = 3,
    -- boosts
    ...
    codeText = "", autoCodes = false, antiAfk = true, flyEnabled = false, flySpeed = 45, infJump = false, noclip = false, antiRagdoll = false, antiKnockback = false, antiFling = false,
    -- perf / webhook
...
local CONFIG_KEYS = {
    ...
    "autoSell", "sellInterval", "sellProtectMutations", "autoExpand", "autoPot", "autoDaily",
    "autoClaimSeedEvent", "claimUseFly", "autoShovelPlants", "autoShovelFruits", "shovelPlantTargets", "shovelFruitTargets", "shovelMaxKg", "shovelNameFilter", "shovelDelay",
    "autoFavoriteFruits", "favoriteFruitTargets", "favoriteFruitFilter", "favoriteMinKg", "favoriteMaxKg", "favoriteMutationTargets", "favoriteMutationFilter", "unfavoriteNotMatching", "favoriteDelay",
    ...
    "autoMail", "autoAcceptGift", "autoHop", "hopInterval", "codeText", "autoCodes", "antiAfk", "flyEnabled", "flySpeed", "infJump", "noclip", "antiRagdoll", "antiKnockback", "antiFling",
...
function HaimiyachGAG2_SelectedMatch(text, selected)
    ...
    for name, enabled in pairs(selected) do
        if enabled == true then
            local selectedKey = HaimiyachGAG2_NameKey(name)
            if selectedKey == key or string.find(key, selectedKey, 1, true) or string.find(selectedKey, key, 1, true) then
                return true
            end
        end
    end
...
function HaimiyachGAG2_TextListMatch(text, filter)
    ...
    for item in string.gmatch(filter, "[^,]+") do
        ...
        item = string.gsub(item, "%s+$", "")
        if item ~= "" and string.find(text, item, 1, true) then return true end
    end
    return false
end
...
function HaimiyachGAG2_ShouldFavoriteFruit(tool)
    ...
    if picked(S.favoriteFruitTargets) then
        if not HaimiyachGAG2_SelectedMatch(fruit, S.favoriteFruitTargets) then return false end
    ...
    if picked(S.favoriteMutationTargets) then
        if not HaimiyachGAG2_SelectedMatch(mutation, S.favoriteMutationTargets) then return false end
    else
        local mutFilter = tostring(S.favoriteMutationFilter or "")
        if mutFilter ~= "" and mutFilter ~= "all" and not HaimiyachGAG2_TextListMatch(mutation, mutFilter) then return false end
    end
...
function HaimiyachGAG2_ProtectSellMutationFavorites()
    if type(S.sellProtectMutations) ~= "table" or not picked(S.sellProtectMutations) then return 0 end
    local changed = 0
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        local mutation = tostring(tool:GetAttribute("Mutation") or "")
        if HaimiyachGAG2_SelectedMatch(mutation, S.sellProtectMutations) then
...
local function sellAllNow()
    pcall(HaimiyachGAG2_ProtectSellMutationFavorites)
...
S.getShovelFruitTargets = function()
    ...
    for _, pr in ipairs(CollectionService:GetTagged("HarvestPrompt")) do
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            local m = promptCarrier(pr)
            local pid = m and m:GetAttribute("PlantId")
            if pid and S.shovelSelectedOk(m, S.shovelFruitTargets) and S.shovelKgOk(m) then
                local uid = tonumber(m:GetAttribute("UserId"))
                if uid == nil or uid == LocalPlayer.UserId then
                    out[#out + 1] = { plantId = tostring(pid), fruitId = tostring(m:GetAttribute("FruitId") or "") }
                end
...
S.getShovelPlantTargets = function()
    ...
    if not plants then return out end
    for _, m in ipairs(plants:GetChildren()) do
        if S.shovelSelectedOk(m, S.shovelPlantTargets) and S.shovelKgOk(m) then
...
-- // ============================================================ \ --
HaimiyachMove = HaimiyachMove or { bv = nil, bg = nil }
HaimiyachInput = game:GetService("UserInputService")
function HaimiyachCleanFly()
    if HaimiyachMove.bv then pcall(function() HaimiyachMove.bv:Destroy() end); HaimiyachMove.bv = nil end
    if HaimiyachMove.bg then pcall(function() HaimiyachMove.bg:Destroy() end); HaimiyachMove.bg = nil end
end
function HaimiyachEnsureFly(hrp)
    if not hrp then return end
    if not HaimiyachMove.bv or HaimiyachMove.bv.Parent ~= hrp then
        HaimiyachCleanFly()
        local bv = Instance.new("BodyVelocity")
        bv.Name = "HaimiyachFlyVelocity"
        bv.MaxForce = Vector3.new(90000, 90000, 90000)
        bv.Velocity = Vector3.new(0, 0, 0)
        bv.Parent = hrp
        local bg = Instance.new("BodyGyro")
        bg.Name = "HaimiyachFlyGyro"
...
HaimiyachInput.JumpRequest:Connect(function()
    if S.killed or not S.infJump then return end
    local hum = humanoid()
    if hum then pcall(function() hum:ChangeState(Enum.HumanoidStateType.Jumping) end) end
end)
RunService.RenderStepped:Connect(function()
    if S.killed then HaimiyachCleanFly(); return end
    local char = LocalPlayer.Character
    ...
    if S.antiRagdoll and hum then
        ...
        if st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown or st == Enum.HumanoidStateType.Physics then hum:ChangeState(Enum.HumanoidStateType.GettingUp) end
    end
    if (S.antiKnockback or S.antiFling) and hrp then
        local vel = hrp.AssemblyLinearVelocity
        local rot = hrp.AssemblyAngularVelocity
        local maxLinear = S.antiFling and 90 or 140
        if vel.Magnitude > maxLinear then hrp.AssemblyLinearVelocity = Vector3.new(0, math.min(vel.Y, 35), 0) end
        if rot.Magnitude > 80 then hrp.AssemblyAngularVelocity = Vector3.new(0, 0, 0) end
    end
    if S.flyEnabled and hrp and hum then
        HaimiyachEnsureFly(hrp)
        local cam = Workspace.CurrentCamera
        local move = hum.MoveDirection
        local up = 0
        if HaimiyachInput:IsKeyDown(Enum.KeyCode.Space) then up = up + 1 end
        if HaimiyachInput:IsKeyDown(Enum.KeyCode.LeftControl) or HaimiyachInput:IsKeyDown(Enum.KeyCode.RightControl) then up = up - 1 end
        local direction = Vector3.new(0, up, 0)
        if move.Magnitude > 0 then direction = direction + move end
        if direction.Magnitude > 0 then direction = direction.Unit else direction = Vector3.new(0, 0, 0) end
        HaimiyachMove.bv.Velocity = direction * (tonumber(S.flySpeed) or 45)
        HaimiyachMove.bg.CFrame = cam and cam.CFrame or hrp.CFrame
    else
        HaimiyachCleanFly()
    end
end)
...
-- Tabs
DashboardPage = createTab("DASHBOARD", 1)
FarmPage = createTab("FARM", 2)
ShopPage = createTab("SHOP", 3)
BoostPage = createTab("BOOSTS", 4)
PetsPage = createTab("PETS", 5)
OpenPage = createTab("OPEN", 6)
StealPage = createTab("STEAL", 7)
AutoPage = createTab("AUTO", 8)
MiscPage = createTab("MISC", 9)
VisualsPage = createTab("VISUALS", 10)
SettingsPage = createTab("SETTINGS", 11)

-- DASHBOARD
addSection(DashboardPage, "STATUS")
local plotLabel = addLabel(DashboardPage, "PLOT: ?")
...
-- FARM
addSection(FarmPage, "PLANT SETTINGS")
...
addSection(FarmPage, "SELL SETTINGS")
addSlider(FarmPage, "SELL DELAY", 3, 120, S.sellInterval, 0, function(v) S.sellInterval = v end)
addDropdown(FarmPage, "SELL KEEP MUTATIONS", MUTATION_NAMES, S.sellProtectMutations, true, function(sel) pickMulti(sel, S.sellProtectMutations) end)
addSection(FarmPage, "SHOVEL SETTINGS")
addDropdown(FarmPage, "SHOVEL PLANT TARGETS", SEED_NAMES, S.shovelPlantTargets, true, function(sel) pickMulti(sel, S.shovelPlantTargets) end)
addDropdown(FarmPage, "SHOVEL FRUIT TARGETS", SEED_NAMES, S.shovelFruitTargets, true, function(sel) pickMulti(sel, S.shovelFruitTargets) end)
addSlider(FarmPage, "SHOVEL MAX KG", 0, 100, S.shovelMaxKg, 1, function(v) S.shovelMaxKg = v end)
addSlider(FarmPage, "SHOVEL DELAY", 1, 30, S.shovelDelay, 0, function(v) S.shovelDelay = v end)
...
-- OPEN
addSection(OpenPage, "OPEN SETTINGS")
...
-- AUTO
addSection(AutoPage, "FARM AUTO")
addToggle(AutoPage, "AUTO FARM", S.autoFarm, function(v) S.autoFarm = v end)
addToggle(AutoPage, "AUTO EXPAND", S.autoExpand, function(v) S.autoExpand = v end)
addToggle(AutoPage, "AUTO PLANT", S.autoPlant, function(v) S.autoPlant = v end)
addToggle(AutoPage, "AUTO HARVEST", S.autoHarvest, function(v) S.autoHarvest = v end)
addToggle(AutoPage, "AUTO POT PLANTS", S.autoPot, function(v) S.autoPot = v end)
addToggle(AutoPage, "AUTO SELL WHEN FULL", S.autoSell, function(v) S.autoSell = v end)
addSection(AutoPage, "SEED EVENT")
addToggle(AutoPage, "AUTO CLAIM SEED EVENT", S.autoClaimSeedEvent, function(v) S.autoClaimSeedEvent = v end)
addToggle(AutoPage, "USE FLY TO SEED EVENT", S.claimUseFly, function(v) S.claimUseFly = v end)
addSection(AutoPage, "SHOP AUTO")
addToggle(AutoPage, "AUTO BUY SEEDS", S.autoBuy, function(v) S.autoBuy = v end)
addToggle(AutoPage, "AUTO BUY ALL SEEDS", S.autoBuyAllSeeds, function(v) S.autoBuyAllSeeds = v end)
addToggle(AutoPage, "AUTO BUY GEAR", S.autoGear, function(v) S.autoGear = v end)
addToggle(AutoPage, "AUTO BUY ALL GEAR", S.autoBuyAllGear, function(v) S.autoBuyAllGear = v end)
addToggle(AutoPage, "AUTO DAILY DEALS", S.autoDaily, function(v) S.autoDaily = v end)
addSection(AutoPage, "BOOST AUTO")
addToggle(AutoPage, "AUTO PLACE SPRINKLERS", S.autoSprinkler, function(v) S.autoSprinkler = v end)
addToggle(AutoPage, "AUTO WATERING CAN", S.autoWater, function(v) S.autoWater = v end)
addToggle(AutoPage, "AUTO SPEND SKILL POINTS", S.autoSkill, function(v) S.autoSkill = v end)
addSection(AutoPage, "PETS AUTO")
addToggle(AutoPage, "AUTO EQUIP PETS", S.autoEquipPets, function(v) S.autoEquipPets = v end)
addToggle(AutoPage, "AUTO BUY PET SLOTS", S.autoPetSlot, function(v) S.autoPetSlot = v end)
addToggle(AutoPage, "AUTO BUY WORLD PETS", S.autoBuyPets, function(v) S.autoBuyPets = v end)
addToggle(AutoPage, "AUTO SELL SELECTED PETS", S.autoSellPets, function(v) S.autoSellPets = v end)
addSection(AutoPage, "OPEN AUTO")
addToggle(AutoPage, "AUTO OPEN EGGS", S.autoEgg, function(v) S.autoEgg = v end)
addToggle(AutoPage, "AUTO OPEN CRATES", S.autoCrate, function(v) S.autoCrate = v end)
addToggle(AutoPage, "AUTO OPEN SEED PACKS", S.autoPack, function(v) S.autoPack = v end)
addSection(AutoPage, "SHOVEL AUTO")
addToggle(AutoPage, "AUTO SHOVEL PLANTS", S.autoShovelPlants, function(v) S.autoShovelPlants = v end)
addToggle(AutoPage, "AUTO SHOVEL FRUITS", S.autoShovelFruits, function(v) S.autoShovelFruits = v end)
addSection(AutoPage, "STEAL AUTO")
addToggle(AutoPage, "AUTO STEAL RIPE FRUIT", S.autoSteal, function(v) S.autoSteal = v end)
addSection(AutoPage, "INVENTORY FAVORITE")
addToggle(AutoPage, "AUTO FAVORITE FRUITS", S.autoFavoriteFruits, function(v) S.autoFavoriteFruits = v end)
addDropdown(AutoPage, "FAVORITE FRUIT TARGETS", SEED_NAMES, S.favoriteFruitTargets, true, function(sel) pickMulti(sel, S.favoriteFruitTargets) end)
addDropdown(AutoPage, "FAVORITE MUTATIONS", MUTATION_NAMES, S.favoriteMutationTargets, true, function(sel) pickMulti(sel, S.favoriteMutationTargets) end)
addSlider(AutoPage, "FAVORITE MIN KG", 0, 100, S.favoriteMinKg, 1, function(v) S.favoriteMinKg = v end)
addSlider(AutoPage, "FAVORITE MAX KG", 0, 100, S.favoriteMaxKg, 1, function(v) S.favoriteMaxKg = v end)
addToggle(AutoPage, "UNFAVORITE NOT MATCHING", S.unfavoriteNotMatching, function(v) S.unfavoriteNotMatching = v end)
addSlider(AutoPage, "FAVORITE DELAY", 1, 30, S.favoriteDelay, 0, function(v) S.favoriteDelay = v end)
addButton(AutoPage, "FAVORITE INVENTORY NOW", function() local n = HaimiyachGAG2_AutoFavoriteStep(); notify("Inventory Favorite", tostring(n) .. " item updated", 3) end)
addSection(AutoPage, "OTHER AUTO")
addToggle(AutoPage, "AUTO CLAIM MAILBOX", S.autoMail, function(v) S.autoMail = v end)
addToggle(AutoPage, "AUTO ACCEPT GIFTS", S.autoAcceptGift, function(v) S.autoAcceptGift = v end)
addToggle(AutoPage, "AUTO SERVER HOP", S.autoHop, function(v) S.autoHop = v end)
addSlider(AutoPage, "SERVER HOP DELAY", 0, 120, S.hopInterval / 60, 0, function(v) S.hopInterval = v * 60 end)
addToggle(AutoPage, "AUTO EXECUTE AFTER HOP", S.autoExecute, function(v) S.autoExecute = v end)
addToggle(AutoPage, "AUTO REDEEM CODE LIST", S.autoCodes, function(v) S.autoCodes = v end)
addInput(AutoPage, "REDEEM CODE", "enter code", "", function(text) S.codeText = text or "" end)
addButton(AutoPage, "REDEEM CODE NOW", function() local code = S.codeText or ""; if code ~= "" then local ok, res = fire("Settings.SubmitCode", code); notify("Code", ok and "Redeem request sent" or tostring(res or "Failed"), 3) end end)

-- MISC
addSection(MiscPage, "MOVEMENT")
addToggle(MiscPage, "FLY", S.flyEnabled, function(v) S.flyEnabled = v end)
addSlider(MiscPage, "FLY SPEED", 10, 150, S.flySpeed, 0, function(v) S.flySpeed = v end)
addToggle(MiscPage, "INFINITE JUMP", S.infJump, function(v) S.infJump = v end)
addToggle(MiscPage, "NOCLIP", S.noclip, function(v) S.noclip = v end)
addSection(MiscPage, "PROTECTION")
addToggle(MiscPage, "ANTI RAGDOLL", S.antiRagdoll, function(v) S.antiRagdoll = v end)
addToggle(MiscPage, "ANTI KNOCKBACK", S.antiKnockback, function(v) S.antiKnockback = v end)
addToggle(MiscPage, "ANTI FLING", S.antiFling, function(v) S.antiFling = v end)
...
loopOn(function() return S.autoClaimSeedEvent end, 3, function()
    claimSeedEvent()
end)
...
task.spawn(function()
    while not S.killed do
        local p = myPlot()
        safeSetText(plotLabel, "PLOT: " .. (p and p.Name or "?"))
