...
S.hidePlants = false -- keep HIDE PLANTS off on startup; enable manually from Visuals
...
-- HIDE PLANTS: lightweight client-side visual hide for plant folders only.
-- Kept off from saved config to avoid execute/freeze issues when many plant objects load.
...
local function HidePlantsRoots()
    local roots = {}
    local gardens = Workspace:FindFirstChild("Gardens")
    if gardens then
        for _, plot in ipairs(gardens:GetChildren()) do
            local plants = plot:FindFirstChild("Plants")
            if plants then roots[#roots + 1] = plants end
        end
    else
        local plot = myPlot()
        local plants = plot and plot:FindFirstChild("Plants")
...
local function IsInsideHidePlantsRoots(obj)
    if not obj then return false end
    for _, root in ipairs(HidePlantsRoots()) do
...
local function BackupPlantValue(obj, prop)
    if not HidePlantsBackup[obj] then HidePlantsBackup[obj] = {} end
    if HidePlantsBackup[obj][prop] == nil then
        pcall(function()
            HidePlantsBackup[obj][prop] = obj[prop]
        end)
    end
end
...
local function HidePlantVisualObject(obj)
    ...
    if SafeIsA(obj, "BasePart") then
        BackupPlantValue(obj, "LocalTransparencyModifier")
        BackupPlantValue(obj, "CastShadow")
        SafeSetProperty(obj, "LocalTransparencyModifier", 1)
        SafeSetProperty(obj, "CastShadow", false)

    elseif SafeIsA(obj, "Decal") or SafeIsA(obj, "Texture") then
        BackupPlantValue(obj, "Transparency")
        SafeSetProperty(obj, "Transparency", 1)

    elseif SafeIsA(obj, "ParticleEmitter") then
        BackupPlantValue(obj, "Enabled")
        BackupPlantValue(obj, "Rate")
        SafeSetProperty(obj, "Enabled", false)
        SafeSetProperty(obj, "Rate", 0)

    elseif SafeIsA(obj, "Trail") or SafeIsA(obj, "Beam") or SafeIsA(obj, "Fire") or SafeIsA(obj, "Smoke") or SafeIsA(obj, "Sparkles") then
        BackupPlantValue(obj, "Enabled")
        SafeSetProperty(obj, "Enabled", false)
    end
end

local function HidePlantTreeSafe(obj)
...
local function ApplyHidePlants()
    ...
    HidePlantsConnection = Workspace.DescendantAdded:Connect(function(obj)
        if not HidePlantsActive or not S.hidePlants then return end
        task.defer(function()
            if token ~= HidePlantsToken or not HidePlantsActive or not S.hidePlants then return end
            if IsInsideHidePlantsRoots(obj) then
                HidePlantTreeSafe(obj)
            end
        end)
    end)
end
