...
local S = {
    ...
    espGardenFruit = false, espGardenFruitValue = true,
...
local CONFIG_KEYS = {
    ...
    "fpsBoost", "highGraphics", "espGardenFruit", "espGardenFruitValue", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
-- // ============================================================ \ --
-- //                    LITE GARDEN FRUIT ESP                    \ --
-- // ============================================================ \ --
-- Step 1 only: garden fruit ESP. No require(), no WaitForChild(), no heavy bundle.
local HaimiyachGardenEspCache = {}
local HaimiyachEspBaseValue = {
    ...
    Pomegranate = 900, Beanstalk = 2000, ["Poison Apple"] = 900, ["Moon Bloom"] = 9000,
    ["Dragon's Breath"] = 3400, ["Dragon’s Breath"] = 3400, ["Poison Ivy"] = 1700,
...
local function HaimiyachHasTag(inst, tag)
    local ok, res = pcall(function() return CollectionService:HasTag(inst, tag) end)
    return ok and res or false
end

local function HaimiyachEspPart(model)
    local p = model:FindFirstChild("HarvestPart", true)
...
local function HaimiyachFruitName(model)
    return model:GetAttribute("CorePartName") or model:GetAttribute("FruitName") or model:GetAttribute("Fruit")
end

local function HaimiyachSizeText(size)
    size = tonumber(size) or 1
    if size >= 4 then return "GIANT" end
    if size >= 2.5 then return "BIG" end
    return "NORMAL"
end

local function HaimiyachGardenValue(name, size)
    local base = tonumber(HaimiyachEspBaseValue[tostring(name or "")]) or 0
    if base <= 0 then return 0 end
    size = math.max(tonumber(size) or 1, 0.1)
    local exp = 2.65
    if name == "Mushroom" then exp = 1.9 elseif name == "Bamboo" then exp = 1.75 end
    return math.floor(base * (size ^ exp))
end

local function HaimiyachIsReadyFruit(model)
    if not model or not model:IsA("Model") then return false end
    if not HaimiyachFruitName(model) then return false end
    if HaimiyachHasTag(model, "Harvestable") then return true end
    local age = tonumber(model:GetAttribute("Age")) or 0
    local maxAge = tonumber(model:GetAttribute("MaxAge")) or 0
    if maxAge > 0 and age >= maxAge then return true end
    return model:FindFirstChild("HarvestPart", true) ~= nil
end

function ClearGardenESP()
    for inst, gui in pairs(HaimiyachGardenEspCache) do
        pcall(function() if gui then gui:Destroy() end end)
        HaimiyachGardenEspCache[inst] = nil
    end
end

function UpdateGardenESP()
    if S.killed or not S.espGardenFruit then ClearGardenESP(); return end
    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then ClearGardenESP(); return end

    local alive = {}
    local made = 0
    for _, model in ipairs(gardens:GetDescendants()) do
        if made >= 160 then break end
        if HaimiyachIsReadyFruit(model) then
            local part = HaimiyachEspPart(model)
            if part then
                alive[model] = true
                made = made + 1
                local name = tostring(HaimiyachFruitName(model) or "Fruit")
                local size = tonumber(model:GetAttribute("SizeMulti") or model:GetAttribute("SizeMultiplier")) or 1
                local weight = tonumber(model:GetAttribute("Weight"))
                local kgText = weight and (string.format("%.2fkg", weight)) or ("x" .. string.format("%.2f", size))
                local text = name .. "\n" .. HaimiyachSizeText(size) .. " | " .. kgText
                if S.espGardenFruitValue then
                    local value = HaimiyachGardenValue(name, size)
                    if value > 0 then text = text .. " | " .. fmt(value) .. "¢" end
                end

                local gui = HaimiyachGardenEspCache[model]
                if not (gui and gui.Parent) then
                    ...
                    label.TextColor3 = Color3.fromRGB(255, 235, 80)
                    ...
                    label.Parent = bg
                    HaimiyachGardenEspCache[model] = gui
                end
                gui.Adornee = part
                pcall(function() gui.BG.Label.Text = text end)
            end
        end
    end

    for inst, gui in pairs(HaimiyachGardenEspCache) do
        if not alive[inst] or not inst.Parent or not S.espGardenFruit then
            pcall(function() if gui then gui:Destroy() end end)
            HaimiyachGardenEspCache[inst] = nil
        end
    end
...
task.spawn(function()
    while not S.killed do
        ...
        elseif next(HaimiyachGardenEspCache) ~= nil then
            ClearGardenESP()
        end
        task.wait(1.2)
    end
...
addToggle(UI.Pages["VISUALS"], "ESP GARDEN FRUIT", S.espGardenFruit, function(v)
    S.espGardenFruit = v
    if not v then ClearGardenESP() else task.spawn(function() pcall(UpdateGardenESP) end) end
    saveConfig(true)
end)
addToggle(UI.Pages["VISUALS"], "GARDEN VALUE SHECKLES", S.espGardenFruitValue, function(v)
    S.espGardenFruitValue = v
    if S.espGardenFruit then task.spawn(function() pcall(UpdateGardenESP) end) end
    saveConfig(true)
end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        UpdateGardenESP = UpdateGardenESP, ClearGardenESP = ClearGardenESP,
