...
local function promptCarrier(prompt)
    ...
    while node and node ~= Workspace and guard < 14 do
        ...
        if node:GetAttribute("PlantId") ~= nil and not fallback then
            fallback = node
        end
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    -- Jangan ambil KG dari parent plant untuk fruit/harvest object.
    -- Ini penyebab ABOVE KG bisa tetap harvest fruit kecil karena parent plant-nya berat.
...
-- Override KG calculator here so it captures the local GardenESP table safely.
-- The earlier function is defined before GardenESP exists, so it cannot see this local table.
function HaimiyachGAG2_GardenCalcKg(obj)
    if not (obj and obj.IsDescendantOf and obj:IsDescendantOf(Workspace)) then return nil end
    ...
    local target = fallback or obj
    local sizeMulti = tonumber(target:GetAttribute("SizeMultiplier") or target:GetAttribute("SizeMulti") or target:GetAttribute("Size"))

    local ok, weight = pcall(function()
        return GardenESP.calculateWeight(target, sizeMulti)
    end)
