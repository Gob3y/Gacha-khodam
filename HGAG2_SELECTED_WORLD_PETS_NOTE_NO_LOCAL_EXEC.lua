...
-- AUTO BUY selected world pets: walk up (teleport) to selected affordable unowned wild pet and buy it.
...
loopOn(function() return S.autoBuyPets end, function() return S.petBuyInterval end, function()
    ...
    for _, w in ipairs(wildPets()) do
        ...
        if petName ~= "" and S.buyWorldPets[petName] == true and w.owner == 0 and w.price > 0 and w.price <= S.maxPetPrice and getSheckles() >= w.price then
...
addButton(UI.Pages["PETS"], "REFRESH WORLD PET LIST", function()
    if UI.worldPetDrop and UI.worldPetDrop.SetOptions then
        UI.worldPetDrop.SetOptions(HaimiyachGAG2_WorldPetNames())
    end
end)
