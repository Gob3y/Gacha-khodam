...
local S = {
    ...
    stealSkipOwnerNear = true, stealOwnerNearDistance = 70, stealConfirmTimeout = 2.5, stealFailCooldown = 8,
    -- misc
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay", "stealSkipOwnerNear", "stealOwnerNearDistance", "stealConfirmTimeout", "stealFailCooldown",
...
local HaimiyachGAG2_DroppedCollectFailUntil = {}

local function HaimiyachGAG2_WaitDroppedItemCollected(obj, folder, timeout)
    if not obj then return false end
...
local function HaimiyachGAG2_DroppedItemKind(obj)
    if HaimiyachGAG2_IsDroppedSeed(obj) then return "seed" end
    if HaimiyachGAG2_IsDroppedFruit(obj) then return "fruit" end
    if HaimiyachGAG2_IsDroppedPet(obj) then return "pet" end
    return ""
end

local function HaimiyachGAG2_DroppedKindEnabled(kind, force)
    if force then return true end
    if kind == "seed" then return S.autoCollectSeed == true end
    if kind == "fruit" then return S.autoCollectFruit == true end
    if kind == "pet" then return S.autoCollectPet == true end
    return false
end

local function HaimiyachGAG2_UpdateDroppedCollectStats(kind, itemName)
    if kind == "seed" then
...
local function HaimiyachGAG2_ShouldCollectDroppedItem(item, wantedKind, force)
    if not (item and item.Parent and item:IsA("Model")) then return false, "" end
    if not HaimiyachGAG2_CanCollectDroppedItem(item) then return false, "" end

    local now = os.clock()
    local blockedUntil = HaimiyachGAG2_DroppedCollectFailUntil[item]
    if blockedUntil and blockedUntil > now then
        return false, ""
    end

    local kind = HaimiyachGAG2_DroppedItemKind(item)
    if kind == "" then return false, "" end
    if wantedKind and wantedKind ~= "" and kind ~= wantedKind then return false, "" end
    if not HaimiyachGAG2_DroppedKindEnabled(kind, force) then return false, "" end
    return true, kind
end

local function HaimiyachGAG2_FindBestDroppedCollectTarget(folder, wantedKind, force, root)
    local best = nil
    local bestKind = ""
    local bestDist = math.huge
    ...
    for _, item in ipairs(folder:GetChildren()) do
        local okKind, kind = HaimiyachGAG2_ShouldCollectDroppedItem(item, wantedKind, force)
        if okKind then
...
local function HaimiyachGAG2_ReturnToGardenAfterCollect(root, returnPos, fallbackCf)
    if not (root and root.Parent and S.collectSeedTeleport and S.collectSeedReturn) then return end
...
local function HaimiyachGAG2_CollectDroppedItemsSmart(wantedKind, force)
    wantedKind = string.lower(tostring(wantedKind or ""))
    if wantedKind == "all" then wantedKind = "" end
    ...
    while not S.killed and attempts < maxAttempts do
        folder = HaimiyachGAG2_DroppedItemFolder()
        if not folder then break end

        local charNow = LocalPlayer.Character
        local rootNow = charNow and charNow:FindFirstChild("HumanoidRootPart")
        if rootNow then root = rootNow end

        local item, kind = HaimiyachGAG2_FindBestDroppedCollectTarget(folder, wantedKind, force, root)
        if not item then break end

        attempts = attempts + 1
        local prompt = HaimiyachGAG2_FindEnabledPrompt(item)
        local pos = HaimiyachGAG2_InstancePos(item)
        if not (prompt and pos) then
            HaimiyachGAG2_DroppedCollectFailUntil[item] = os.clock() + 1.5
        else
            ...
            if not item.Parent or not item:IsDescendantOf(folder) then
                ...
                for _ = 1, 3 do
                    if not (item and item.Parent and item:IsDescendantOf(folder)) then
                        didCollect = true
                        break
                    end
                    prompt = HaimiyachGAG2_FindEnabledPrompt(item)
                    if not prompt then break end
                    HaimiyachGAG2_TriggerPrompt(prompt)
                    didCollect = HaimiyachGAG2_WaitDroppedItemCollected(item, folder, 1.2)
                    if didCollect then break end
                    task.wait(0.12)
                end
            end
            ...
            if didCollect then
                ...
                HaimiyachGAG2_UpdateDroppedCollectStats(kind, itemName)
                HaimiyachGAG2_DroppedCollectFailUntil[item] = nil
                task.wait(0.12)
            else
                HaimiyachGAG2_DroppedCollectFailUntil[item] = os.clock() + 3
                Stats.lastReturnGardenStatus = "WAIT COLLECT"
                Stats.lastReturnGardenAt = os.clock()
                if Stats.SetWarning then Stats.SetWarning("COLLECT", "Not confirmed: " .. tostring(itemName)) end
                task.wait(0.12)
            end
        end
    end

    -- Return once after the whole collect pass, not after every kind/item.
    -- This fixes mixed drops such as seeds + fruits and many seeds in DroppedItems.
    if collected > 0 then
        HaimiyachGAG2_ReturnToGardenAfterCollect(root, returnPos, fallbackCf)
    end
...
local function HaimiyachGAG2_CollectDroppedItemStep(kind, force)
    return HaimiyachGAG2_CollectDroppedItemsSmart(kind, force)
end
...
function HaimiyachGAG2_CollectDroppedEnabledStep()
    if not (S.autoCollectSeed or S.autoCollectFruit or S.autoCollectPet) then
        return 0
    end
    return HaimiyachGAG2_CollectDroppedItemsSmart("all", false)
end
...
local function HaimiyachGAG2_TriggerWildPetPrompt(part)
    if not part then return false end
    for _, d in ipairs(part:GetDescendants()) do
        if d:IsA("ProximityPrompt") and d.Enabled then
            if type(fireproximityprompt) == "function" then
                local ok = pcall(function() fireproximityprompt(d) end)
                if ok then return true end
            end
            local ok2 = pcall(function()
                d:InputHoldBegin()
                task.wait(math.max(0.15, tonumber(d.HoldDuration) or 0.15))
                d:InputHoldEnd()
            end)
...
local function HaimiyachGAG2_WaitWildPetConfirmed(w, beforeCount)
    ...
    while os.clock() < deadline do
        ...
        if part and not part:IsDescendantOf(Workspace) then
            return true
        end
        ...
        if part then
            ...
            if owner ~= 0 and owner ~= tonumber(w.owner or 0) then
                return true
            end
...
local function HaimiyachGAG2_TameWildPet(w)
    ...
    if S.petTeleport and w.pos and hrp then
        pcall(function() hrp.CFrame = CFrame.new(w.pos + Vector3.new(0, 4, 0)) end)
        moved = true
        task.wait(0.45)
    end
    ...
    for attempt = 1, 3 do
        ...
        fired = fired or ok
        HaimiyachGAG2_TriggerWildPetPrompt(w.part)
...
-- // ============================================================ \ --
-- Smart steal flow:
--   1) scan all steal prompts from every non-owned garden
--   2) rank gardens by total fruit value, then pick the highest-value fruit
--   3) teleport to fruit and attempt steal
--   4) return to own garden ONLY when carrying stolen fruit is confirmed
--   5) if owner is nearby or fruit cannot be stolen, skip to another fruit/garden
local function hrpNow() local c = LocalPlayer.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local HaimiyachGAG2_StealFailUntil = {}

local function HaimiyachGAG2_StealPlotOf(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
    if not (gardens and obj) then return nil end
    local cur = obj
    while cur and cur ~= Workspace do
        if cur.Parent == gardens then return cur end
        cur = cur.Parent
    end
    return nil
end
...
local function HaimiyachGAG2_StealKey(f)
    return tostring(f and f.owner or 0) .. ":" .. tostring(f and f.plantId or "") .. ":" .. tostring(f and f.fruitId or "")
end

local function HaimiyachGAG2_StealValueOf(f)
    if not (f and f.obj) then return 0 end
    local name = nil
    local okName, gotName = pcall(function()
        return GardenESP and GardenESP.name and GardenESP.name(f.obj)
    end)
    if okName then name = gotName end
    name = tostring(name or f.obj:GetAttribute("DisplayName") or f.obj:GetAttribute("ItemName") or f.obj.Name or "")
    local sm = tonumber(f.obj:GetAttribute("SizeMulti") or f.obj:GetAttribute("SizeMultiplier") or f.obj:GetAttribute("Scale") or 1) or 1
    local okValue, value = pcall(function()
        if GardenESP and GardenESP.valueFor then
            return GardenESP.valueFor(f.obj, name, sm)
        end
        return nil
    end)
    value = okValue and tonumber(value) or nil
    if value and value > 0 then return value end
    local raw = tonumber(f.obj:GetAttribute("Value") or f.obj:GetAttribute("SellValue") or f.obj:GetAttribute("Price"))
    return raw or 0
end

local function HaimiyachGAG2_OwnerNearStealFruit(f)
    if not (S.stealSkipOwnerNear and f and f.owner and f.owner > 0 and f.pos) then return false end
    local maxDist = tonumber(S.stealOwnerNearDistance) or 70
    if maxDist <= 0 then return false end
    local plr = Players:GetPlayerByUserId(f.owner)
    local char = plr and plr.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return false end
    return (root.Position - f.pos).Magnitude <= maxDist
end

local function HaimiyachGAG2_SmartStealable()
    local now = os.clock()
    local raw = stealable()
    local items, gardenTotals = {}, {}
    local ownPlot = myPlot()
    for _, f in ipairs(raw) do
        if f and f.obj and f.owner ~= LocalPlayer.UserId then
            local key = HaimiyachGAG2_StealKey(f)
            if not (HaimiyachGAG2_StealFailUntil[key] and HaimiyachGAG2_StealFailUntil[key] > now) then
                local plot = HaimiyachGAG2_StealPlotOf(f.obj)
                if plot ~= ownPlot and not HaimiyachGAG2_OwnerNearStealFruit(f) then
                    f.garden = plot
                    f.value = HaimiyachGAG2_StealValueOf(f)
                    local gkey = plot or "WORLD"
                    gardenTotals[gkey] = (gardenTotals[gkey] or 0) + math.max(tonumber(f.value) or 0, 1)
                    items[#items + 1] = f
                end
            end
        end
    ...
    for _, f in ipairs(items) do
        f.gardenValue = gardenTotals[f.garden or "WORLD"] or 0
    end
    local myRoot = hrpNow()
    table.sort(items, function(a, b)
        local ag = tonumber(a.gardenValue) or 0
        local bg = tonumber(b.gardenValue) or 0
        if ag ~= bg then return ag > bg end
        local av = tonumber(a.value) or 0
        local bv = tonumber(b.value) or 0
        if av ~= bv then return av > bv end
        if myRoot and a.pos and b.pos then
            return (myRoot.Position - a.pos).Magnitude < (myRoot.Position - b.pos).Magnitude
        end
...
local function HaimiyachGAG2_TryStealFruit(f)
    if not (f and f.plantId and f.fruitId) then return false end
    ...
    if HaimiyachGAG2_OwnerNearStealFruit(f) then
        if Stats.SetWarning then Stats.SetWarning("STEAL", "Owner near fruit, skipped") end
        return false
    end

    fire("Steal.BeginSteal", f.owner, f.plantId, f.fruitId)
    fire("Steal.CompleteSteal")
    if type(fireproximityprompt) == "function" and f.prompt and f.prompt.Parent and f.prompt.Enabled then
        pcall(function() fireproximityprompt(f.prompt) end)
    end

    local timeout = tonumber(S.stealConfirmTimeout) or 2.5
    if timeout < 0.5 then timeout = 0.5 end
    local t0 = os.clock()
    while S.autoSteal and os.clock() - t0 < timeout do
    ...
    local key = HaimiyachGAG2_StealKey(f)
    HaimiyachGAG2_StealFailUntil[key] = os.clock() + (tonumber(S.stealFailCooldown) or 8)
    if Stats.SetWarning then
        Stats.SetWarning("STEAL", "Not confirmed, moving to another garden")
    end
    return false
end

local function HaimiyachGAG2_ReturnStolenFruitHome()
    if not S.stealReturnBase then return end
    if LocalPlayer:GetAttribute("CarryingStolenFruit") ~= true then return end
    local base = myBasePos()
    local hrp = hrpNow()
    if base and hrp then
        hrp.CFrame = CFrame.new(base + Vector3.new(0, 4, 0))
        Stats.lastReturnGardenStatus = "STEAL OK"
        Stats.lastReturnGardenAt = os.clock()
        local t0 = os.clock()
        while LocalPlayer:GetAttribute("CarryingStolenFruit") == true and os.clock() - t0 < 4 and S.autoSteal do
            task.wait(0.15)
        end
        if LocalPlayer:GetAttribute("CarryingStolenFruit") == true and Stats.SetWarning then
            Stats.SetWarning("STEAL", "Still carrying after return")
        end
    else
        Stats.lastReturnGardenStatus = "STEAL NO BASE"
        Stats.lastReturnGardenAt = os.clock()
    end
end
...
loopOn(function() return S.autoSteal end, 1.5, function()
    ...
    local list = HaimiyachGAG2_SmartStealable()
    if #list == 0 then return end
    for _, f in ipairs(list) do
        if not (S.autoSteal and isNight()) then break end
        local ok = HaimiyachGAG2_TryStealFruit(f)
        if ok then
            Stats.stolen = Stats.stolen + 1
            if Stats.SetLastAction then
                local fruitName = nil
                pcall(function() fruitName = GardenESP and GardenESP.name and GardenESP.name(f.obj) end)
                Stats.SetLastAction("STEAL", tostring(fruitName or "Fruit") .. " value " .. fmt(tonumber(f.value) or 0))
            end
            HaimiyachGAG2_ReturnStolenFruitHome()
            if (S.stealDelay or 0) > 0 then task.wait(S.stealDelay) end
            break
        else
            if (S.stealDelay or 0) > 0 then task.wait(math.max(0.15, S.stealDelay)) end
        end
    end
end)
...
addToggle(UI.Pages["STEAL"], "SKIP OWNER NEAR", S.stealSkipOwnerNear, function(v) S.stealSkipOwnerNear = v; saveConfig(true) end)
addSlider(UI.Pages["STEAL"], "OWNER NEAR RANGE", 20, 150, S.stealOwnerNearDistance, 0, function(v) S.stealOwnerNearDistance = v; saveConfig(true) end)
addSlider(UI.Pages["STEAL"], "STEAL CONFIRM TIME", 1, 5, S.stealConfirmTimeout, 1, function(v) S.stealConfirmTimeout = v; saveConfig(true) end)
addSlider(UI.Pages["STEAL"], "FAILED FRUIT COOLDOWN", 3, 20, S.stealFailCooldown, 0, function(v) S.stealFailCooldown = v; saveConfig(true) end)
