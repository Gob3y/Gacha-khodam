...
-- // ============================================================ \ --
-- //              LOADING SCREEN SKIPPER (MANUAL SAFE)            \ --
-- // ============================================================ \ --
local function HaimiyachSkipLoadingOnce()
    pcall(function()
        if LocalPlayer then
    ...
    pcall(function()
        local blur = Lighting and Lighting:FindFirstChild("Blur")
    ...
    pcall(function()
        ...
        if pg then
            local hub = pg:FindFirstChild("Haimiyach_GAG2_Custom_UI")
            if hub and hub:IsA("ScreenGui") then hub.Enabled = true end
            local show = pg:FindFirstChild("Haimiyach_GAG2_Show_Button")
            if show and show:IsA("ScreenGui") then show.Enabled = true end
        end
    end)
end

local function HaimiyachSkipLoadingBurst(seconds)
    seconds = tonumber(seconds) or 4
    task.spawn(function()
        local started = os.clock()
        repeat
            HaimiyachSkipLoadingOnce()
            task.wait(0.15)
        until os.clock() - started >= seconds or S.killed
    end)
end
...
addButton(UI.Pages["VISUALS"], "SKIP LOADING SCREEN NOW", function()
    HaimiyachSkipLoadingBurst(5)
    notify("Loading Screen", "Skip requested.", 3)
end)

task.defer(function()
    task.wait(0.5)
    local active = false
    pcall(function() active = LocalPlayer:GetAttribute("LoadingScreenActive") == true end)
    if active or workspace:FindFirstChild("LoadingScreenMenu") then
        HaimiyachSkipLoadingBurst(5)
    end
end)
