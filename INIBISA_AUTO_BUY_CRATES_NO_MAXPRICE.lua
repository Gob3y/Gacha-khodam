...
function HaimiyachGAG2_BuyCrateViaGui(crateName)
    ...
    local okFinal, result = pcall(function()
        ...
        if not frame then return false, "crate frame not found" end
...
-- CRATE SHOP
addSection(UI.Pages["PETS_CRATES"], "CRATE SHOP")
addMiniNote(UI.Pages["PETS_CRATES"], "No max price limit. Uses stock + enough Sheckles check only.")
UI.crateBuyDrop = addDropdown(UI.Pages["PETS_CRATES"], "CRATES TO BUY", HaimiyachGAG2_CrateNames(), S.buyCrates, true, function(sel)
    if type(S.buyCrates) ~= "table" then S.buyCrates = {} end
    pickMulti(sel, S.buyCrates)
    saveConfig(true)
end)
addButton(UI.Pages["PETS_CRATES"], "REFRESH CRATE LIST", function()
    if UI.crateBuyDrop and UI.crateBuyDrop.SetOptions then UI.crateBuyDrop.SetOptions(HaimiyachGAG2_CrateNames()) end
end)
addToggle(UI.Pages["PETS_CRATES"], "AUTO BUY SELECTED CRATES", S.autoBuyCrates, function(v)
    S.autoBuyCrates = v
    saveConfig(true)
end)
addButton(UI.Pages["PETS_CRATES"], "BUY SELECTED CRATES NOW", function()
    HaimiyachGAG2_ManualCrateBuyNow = true
    HaimiyachGAG2_BuySelectedCratesOnce()
    HaimiyachGAG2_ManualCrateBuyNow = false
end)
addSlider(UI.Pages["PETS_CRATES"], "CRATE BUY DELAY (SEC)", 2, 60, S.crateBuyDelay or 5, 0, function(v)
    S.crateBuyDelay = v
    saveConfig(true)
end)
