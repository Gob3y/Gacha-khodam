...
local S -- forward state for helpers defined before STATE table
...
local function modelWorldPosition(obj)
...
local function gardenCenterPosition()
    ...
    for _, area in ipairs(areas) do sum = sum + area.Position end
    return sum / #areas
end

local function snapToPlantArea(pos)
    ...
    params.FilterType = Enum.RaycastFilterType.Include
    params.FilterDescendantsInstances = areas
    local hit = Workspace:Raycast(Vector3.new(pos.X, pos.Y + 80, pos.Z), Vector3.new(0, -180, 0), params)
    if hit then return hit.Position end
    local best, bestD
    for _, area in ipairs(areas) do
        local lp = area.CFrame:PointToObjectSpace(pos)
        local half = area.Size * 0.5
        local clamped = Vector3.new(math.clamp(lp.X, -half.X + 1, half.X - 1), half.Y, math.clamp(lp.Z, -half.Z + 1, half.Z - 1))
        local wp = area.CFrame:PointToWorldSpace(clamped)
        local d = (Vector2.new(wp.X, wp.Z) - Vector2.new(pos.X, pos.Z)).Magnitude
        if not bestD or d < bestD then best, bestD = wp, d end
    end
    return best
end
...
local function existingSprinklerPositions()
    ...
    local function tryAdd(obj)
        ...
        local attr = obj:GetAttribute("Sprinkler")
        if attr ~= nil or string.find(name, "sprinkler", 1, true) then
            local pos = modelWorldPosition(obj)
            if pos then out[#out + 1] = pos end
        end
    end
    local folders = { plot:FindFirstChild("Sprinklers"), plot:FindFirstChild("Props"), plot:FindFirstChild("Objects"), plot:FindFirstChild("PlacedObjects") }
    for _, folder in ipairs(folders) do
        if folder then
            for _, obj in ipairs(folder:GetChildren()) do tryAdd(obj) end
        end
    end
    for _, obj in ipairs(plot:GetDescendants()) do
        if obj:IsA("Model") or obj:IsA("BasePart") then tryAdd(obj) end
    end
    return out
end

local function sprinklerBaseTarget()
    if S.sprinklerTargetMode == "AVATAR POSITION" then
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local p = hrp and snapToPlantArea(hrp.Position)
        if p then return p end
    end
    return snapToPlantArea(gardenCenterPosition())
end

local function chooseSprinklerPosition(existing)
    local base = sprinklerBaseTarget()
    if not base then return nil end
    local grid = plantGrid(4)
    if #grid == 0 then return base end
    local best, bestScore
    for _, pos in ipairs(grid) do
        local tooClose = false
        for _, sp in ipairs(existing or {}) do
            if (Vector2.new(pos.X, pos.Z) - Vector2.new(sp.X, sp.Z)).Magnitude < 5 then tooClose = true break end
        end
        if not tooClose then
            local d = (Vector2.new(pos.X, pos.Z) - Vector2.new(base.X, base.Z)).Magnitude
            if not bestScore or d < bestScore then best, bestScore = pos, d end
        end
    end
    return best or base
end

local function ownPlantModels()
    local out, plot = {}, myPlot()
    local plants = plot and plot:FindFirstChild("Plants")
    if not plants then return out end
    for _, plant in ipairs(plants:GetChildren()) do
        if plant:IsA("Model") then out[#out + 1] = plant end
    end
    return out
end

local WateredAtByPlant = {}
local function plantAgeReadyState(plant)
    local age, maxAge = plant:GetAttribute("Age"), plant:GetAttribute("MaxAge")
    if typeof(age) == "number" and typeof(maxAge) == "number" and maxAge > 0 then
        return age >= maxAge, age, maxAge
    end
    return false, age, maxAge
end

local function scanWaterAttributes(inst)
    if not inst then return nil end
    local attrs = inst:GetAttributes()
    for k, v in pairs(attrs) do
        local key = string.lower(tostring(k or ""))
        if type(v) == "boolean" then
            if (string.find(key, "need", 1, true) and string.find(key, "water", 1, true)) or string.find(key, "dry", 1, true) or string.find(key, "thirst", 1, true) then
                return v == true
            end
            if string.find(key, "watered", 1, true) or string.find(key, "hydrated", 1, true) or string.find(key, "wet", 1, true) or string.find(key, "moist", 1, true) then
                return v == false
            end
        elseif type(v) == "number" then
            if string.find(key, "water", 1, true) or string.find(key, "moist", 1, true) or string.find(key, "hydrat", 1, true) then
                if v <= 1 then return v <= 0.25 end
                return v <= 25
            end
        elseif type(v) == "string" then
            local sv = string.lower(v)
            if string.find(key, "water", 1, true) or string.find(key, "moist", 1, true) or string.find(key, "state", 1, true) then
                if string.find(sv, "dry", 1, true) or string.find(sv, "need", 1, true) or string.find(sv, "thirst", 1, true) then return true end
                if string.find(sv, "wet", 1, true) or string.find(sv, "watered", 1, true) or string.find(sv, "moist", 1, true) then return false end
            end
        end
    end
    return nil
...
local function plantNeedsWater(plant)
    ...
    local direct = scanWaterAttributes(plant)
    if direct ~= nil then return direct end
    local checked = 0
    for _, obj in ipairs(plant:GetDescendants()) do
        checked = checked + 1
        if checked > 80 then break end
        local attrNeed = scanWaterAttributes(obj)
        if attrNeed ~= nil then return attrNeed end
        if obj:IsA("ProximityPrompt") and obj.Enabled then
            local txt = string.lower(tostring(obj.ActionText or "") .. " " .. tostring(obj.ObjectText or "") .. " " .. tostring(obj.Name or ""))
            if string.find(txt, "water", 1, true) then return true end
        end
    end
    local ready, age, maxAge = plantAgeReadyState(plant)
    if ready then return false end
    if typeof(age) == "number" and typeof(maxAge) == "number" then
        local key = tostring(plant:GetAttribute("PlantId") or plant:GetFullName())
        local last = WateredAtByPlant[key] or 0
        return os.clock() - last >= 45
    end
    return false
end

local function plantsNeedingWater()
    local out = {}
    for _, plant in ipairs(ownPlantModels()) do
        if plantNeedsWater(plant) then
            local pos = modelWorldPosition(plant)
            if pos then out[#out + 1] = { plant = plant, pos = pos } end
        end
    end
    return out
end

local function markPlantWatered(plant)
    if not plant then return end
    local key = tostring(plant:GetAttribute("PlantId") or plant:GetFullName())
    WateredAtByPlant[key] = os.clock()
end
...
S = {
    ...
    -- boosts
    autoSprinkler = false, sprinklerInterval = 30, sprinklerTargetMode = "GARDEN CENTER",
...
local CONFIG_KEYS = {
    ...
    "autoSprinkler", "sprinklerInterval", "sprinklerTargetMode", "autoWater", "waterInterval", "autoSkill", "skillStats",
...
-- Migrate old single-seed config into the new multi-select Auto Plant table.
if type(S.plantSeeds) ~= "table" then S.plantSeeds = {} end
local __hasPlantSeedSelection = false
for _ in pairs(S.plantSeeds) do __hasPlantSeedSelection = true break end
if not __hasPlantSeedSelection then
    if type(S.plantSeed) == "string" and S.plantSeed ~= "" and S.plantSeed ~= "Best owned" and S.plantSeed ~= "Selected" then
...
if S.sprinklerTargetMode ~= "AVATAR POSITION" then S.sprinklerTargetMode = "GARDEN CENTER" end
...
local PlantSeedCursor = 0
local function seedCatalogPrice(name)
    for _, s in ipairs(CATALOG) do if s.name == name then return tonumber(s.price) or 0 end end
    return 0
end

local function selectedPlantSeeds()
    local selected = {}
    if type(S.plantSeeds) == "table" then
        for name, enabled in pairs(S.plantSeeds) do
            if enabled == true and name ~= "BEST OWNED" and name ~= "Best owned" then selected[name] = true end
        end
    end
    if not picked(selected) and type(S.plantSeed) == "string" and S.plantSeed ~= "" and S.plantSeed ~= "Best owned" and S.plantSeed ~= "Selected" then
        selected[S.plantSeed] = true
    end
    return selected
end

local function plantToolList()
    local selected = selectedPlantSeeds()
    local tools = {}
    if picked(selected) then
        for seedName in pairs(selected) do
            for _, t in ipairs(toolsByAttr("SeedTool", seedName)) do tools[#tools + 1] = t end
        end
        table.sort(tools, function(a, b)
            local an, bn = tostring(a:GetAttribute("SeedTool") or a.Name), tostring(b:GetAttribute("SeedTool") or b.Name)
            local ap, bp = seedCatalogPrice(an), seedCatalogPrice(bn)
    ...
    for _, t in ipairs(toolsByAttr("SeedTool")) do tools[#tools + 1] = t end
    table.sort(tools, function(a, b)
        local an, bn = tostring(a:GetAttribute("SeedTool") or a.Name), tostring(b:GetAttribute("SeedTool") or b.Name)
        local ap, bp = seedCatalogPrice(an), seedCatalogPrice(bn)
        if ap == bp then return an < bn end
    ...
    if #tools > 1 then return { tools[1] } end
    return tools
end

local function pickPlantTool()
    local tools = plantToolList()
    if #tools == 0 then return nil end
    PlantSeedCursor = (PlantSeedCursor % #tools) + 1
    return tools[PlantSeedCursor]
end
...
local function stepPlant()
    ...
    for _, pos in ipairs(grid) do
        ...
        if clear then
            ...
            if seedAttr then
                local okPlant = fire("Plant.PlantSeed", pos, seedAttr, tool)
                if okPlant then
...
-- //                       BOOSTS (passive)                      \\ --
-- // ============================================================ \\ --
-- AUTO SPRINKLER: place owned sprinkler tools at selected target mode.
-- GARDEN CENTER = middle of own PlantArea. AVATAR POSITION = nearest valid PlantArea point around the player.
loopOn(function() return S.autoSprinkler end, function() return S.sprinklerInterval end, function()
    ...
    for _, t in ipairs(toolsByAttr("Sprinkler")) do
        ...
        local pos = chooseSprinklerPosition(placed)
        if not pos then break end
        hum:EquipTool(t); task.wait(0.25)
        local cur = heldToolByAttr("Sprinkler"); if not cur then break end
        local spr = cur:GetAttribute("Sprinkler")
        local okPlace = fire("Place.PlaceSprinkler", pos, spr, cur, pid)
        if okPlace then
...
-- AUTO WATER: smart watering. Only plants detected as dry/need-water are watered;
-- if the game does not expose water attributes, growing plants are watered with a safe per-plant cooldown.
loopOn(function() return S.autoWater end, function() return S.waterInterval end, function()
    local need = plantsNeedingWater()
    ...
    for _, data in ipairs(need) do
        ...
        local pos = data.pos
        local okWater = fire("WateringCan.UseWateringCan", pos - Vector3.new(0, 0.3, 0), name, t)
        if okWater then
            markPlantWatered(data.plant)
...
addDropdown(UI.Pages["FARM"], "SEEDS TO PLANT", plantOptions, S.plantSeeds, true, function(sel)
    ...
    if type(S.plantSeeds) ~= "table" then S.plantSeeds = { ["BEST OWNED"] = true } end
...
addDropdown(UI.Pages["BOOSTS"], "SPRINKLER TARGET", { "GARDEN CENTER", "AVATAR POSITION" }, S.sprinklerTargetMode, false, function(v) S.sprinklerTargetMode = v or "GARDEN CENTER" end)
