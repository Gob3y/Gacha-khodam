...
local S = {
    ...
    autoFavoriteFruits = false, favoriteFruitFilter = "", favoriteMinKg = 0, favoriteMaxKg = 0, favoriteMutationFilter = "", unfavoriteNotMatching = false, favoriteDelay = 3,
    -- boosts
...
local CONFIG_KEYS = {
    ...
    "autoFavoriteFruits", "favoriteFruitFilter", "favoriteMinKg", "favoriteMaxKg", "favoriteMutationFilter", "unfavoriteNotMatching", "favoriteDelay",
...
function HaimiyachGAG2_ShouldFavoriteFruit(tool)
    ...
    if not HaimiyachGAG2_TextListMatch(fruit, S.favoriteFruitFilter) then return false end
...
addSection(MiscPage, "INVENTORY FAVORITE")
addToggle(MiscPage, "AUTO FAVORITE FRUITS", S.autoFavoriteFruits, function(v) S.autoFavoriteFruits = v end)
addInput(MiscPage, "FAVORITE FRUIT FILTER", "empty = all / Apple, Mango", S.favoriteFruitFilter, function(v) S.favoriteFruitFilter = tostring(v or "") end)
addInput(MiscPage, "FAVORITE MUTATION FILTER", "empty = any / Gold, Rainbow", S.favoriteMutationFilter, function(v) S.favoriteMutationFilter = tostring(v or "") end)
addSlider(MiscPage, "FAVORITE MIN KG", 0, 100, S.favoriteMinKg, 1, function(v) S.favoriteMinKg = v end)
addSlider(MiscPage, "FAVORITE MAX KG", 0, 100, S.favoriteMaxKg, 1, function(v) S.favoriteMaxKg = v end)
addToggle(MiscPage, "UNFAVORITE NOT MATCHING", S.unfavoriteNotMatching, function(v) S.unfavoriteNotMatching = v end)
addSlider(MiscPage, "FAVORITE DELAY", 1, 30, S.favoriteDelay, 0, function(v) S.favoriteDelay = v end)
addButton(MiscPage, "FAVORITE INVENTORY NOW", function()
