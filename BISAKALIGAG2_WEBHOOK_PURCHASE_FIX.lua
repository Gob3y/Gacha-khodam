...
local Stats = { bought = 0, planted = 0, harvested = 0, sold = 0, earned = 0,
    gearBought = 0, sprinklerBought = 0,
    seedPurchases = {}, gearPurchases = {}, sprinklerPurchases = {},
...
local function reportAddName(bucket, name)
    if type(bucket) ~= "table" then return end
    name = tostring(name or "Unknown")
    if name == "" then name = "Unknown" end
    bucket[name] = (tonumber(bucket[name]) or 0) + 1
end

local function reportMapText(bucket, limit)
    if type(bucket) ~= "table" then return "NONE" end
    local items = {}
    for name, count in pairs(bucket) do
        if (tonumber(count) or 0) > 0 then
            items[#items + 1] = { name = tostring(name), count = tonumber(count) or 0 }
        end
    end
    ...
    for i, item in ipairs(items) do
        if i > limit then
            out[#out + 1] = "+" .. tostring(#items - limit) .. " more"
            break
        end
        out[#out + 1] = item.name .. " x" .. tostring(item.count)
    end
    return table.concat(out, ", ")
end

local function recordSeedPurchase(seedName)
    Stats.bought = (tonumber(Stats.bought) or 0) + 1
    reportAddName(Stats.seedPurchases, seedName)
end

local function recordGearPurchase(gearName)
    gearName = tostring(gearName or "Unknown")
    Stats.gearBought = (tonumber(Stats.gearBought) or 0) + 1
    reportAddName(Stats.gearPurchases, gearName)

    local lowerName = string.lower(gearName)
    if string.find(lowerName, "sprinkler", 1, true) then
        Stats.sprinklerBought = (tonumber(Stats.sprinklerBought) or 0) + 1
        reportAddName(Stats.sprinklerPurchases, gearName)
    end
end
    ...
    for _, s in ipairs(CATALOG) do
        ...
        if S.autoBuyAllSeeds or S.buySeeds[s.name] then
            ...
            while bought < S.buyPerTick do
                ...
                recordSeedPurchase(s.name); bought = bought + 1
    ...
    if S.autoBuyAllGear then
        for _, name in ipairs(GEAR_NAMES) do
            ...
            if stock == nil or stock > 0 then
                if fire("GearShop.PurchaseGear", name) then recordGearPurchase(name) end
...
local function webhookFieldText(text, maxLen)
    text = HaimiyachGAG2_ReportClean(text)
    if text == "" then text = "NONE" end
    maxLen = tonumber(maxLen) or 950
    if #text > maxLen then
...
local function sendWebhook(isTest)
    ...
    local payload = {
        ...
        embeds = { {
            ...
            color = 5763719,
            fields = {
                { name = "Account", value = webhookFieldText("Player: " .. tostring(LocalPlayer.Name) .. "\nGarden: " .. gardenName), inline = false },
                { name = "Balance", value = webhookFieldText("Sheckles: " .. fmt(getSheckles()) .. "\nTokens: " .. fmt(getTokens())), inline = false },
                { name = "Farm Stats", value = webhookFieldText(string.format("Seeds Bought: %d\nGear Bought: %d\nSprinklers Bought: %d\nPlanted: %d\nHarvested: %d\nSold: %d\nEarned: %s", Stats.bought, Stats.gearBought or 0, Stats.sprinklerBought or 0, Stats.planted, Stats.harvested, Stats.sold, fmt(Stats.earned))), inline = false },
                { name = "Purchases", value = webhookFieldText("Seeds: " .. reportMapText(Stats.seedPurchases, 12) .. "\nGear: " .. reportMapText(Stats.gearPurchases, 12) .. "\nSprinklers: " .. reportMapText(Stats.sprinklerPurchases, 12), 1000), inline = false },
                { name = "Activity", value = webhookFieldText(string.format("Sprinklers Placed: %d\nWatered: %d\nTamed: %d\nOpened: %d\nStolen: %d", Stats.sprinklers, Stats.watered, Stats.tamed, Stats.opened, Stats.stolen)), inline = false },
                { name = "Seed Event", value = webhookFieldText("Available: " .. HaimiyachGAG2_ReportClean(getSeedEventAvailableText()) .. "\nClaimed: " .. HaimiyachGAG2_ReportClean(getSeedEventClaimText()) .. "\nLast: " .. HaimiyachGAG2_ReportClean(getSeedEventLastText())), inline = false },
                { name = "Runtime", value = webhookFieldText("Uptime: " .. hms(os.clock() - Stats.startAt) .. "\nFPS: " .. tostring(currentFps) .. "\nPing: " .. getPingText()), inline = false },
            },
    ...
    if not jsonOk then
        if isTest then notify("Webhook", "JSON encode failed", 4) end
        return false
    end
    ...
    local statusLooksOk = (codeNum and codeNum >= 200 and codeNum < 300) or string.match(codeText, "^2%d%d")
    local good = ok and ((res and res.Success == true) or code == nil or statusLooksOk)
    if isTest then
        notify("Webhook", good and "Test sent" or ("Failed (" .. tostring(code or "request") .. ")"), 4)
    end
    return good
end
