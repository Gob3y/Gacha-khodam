...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    -- Harvested fruits can enter PlayerStateClient a bit after the collect remote succeeds.
    -- Poll briefly before saying no item, otherwise auto mail becomes manual/buggy.
    if #items == 0 and type(S.mailFruitTargets) == "table" and HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        local waitStart = os.clock()
        while #items == 0 and os.clock() - waitStart < 3 do
            task.wait(0.35)
    ...
    if #items == 0 then
        if showNotify ~= false then
            ...
            if selectedText ~= "" then
                notify("Mailbox", "No giftable fruit matched: " .. selectedText, 4)
            else
