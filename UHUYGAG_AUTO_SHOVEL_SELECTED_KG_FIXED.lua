...
local S = {
    ...
    autoShovelPlants = false, autoShovelFruits = false,
    shovelPlantTargets = { ANY = true }, shovelFruitTargets = { ANY = true },
    shovelMaxKg = 4, shovelDelay = 2,
    -- boosts
...
local CONFIG_KEYS = {
    "autoFarm", "autoBuy", "buySeeds", "buyInterval", "buyPerTick",
    ...
    "autoClaimSeedEvent", "claimUseFly", "autoShovelPlants", "autoShovelFruits",
    "shovelPlantTargets", "shovelFruitTargets", "shovelMaxKg", "shovelDelay",
    ...
    "autoGear", "gearBuy", "gearInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
...
-- // ============================================================ \ --
-- //            AUTO SHOVEL TARGETS (Selected + KG filter)        \ --
-- // ============================================================ \ --
local SH = {}
function SH.cleanName(x)
    x = tostring(x or "")
    x = x:gsub("_", " "):gsub("%-", " ")
...
function SH.key(x)
    return string.lower(SH.cleanName(x)):gsub("%s+", "")
end
function SH.anySelected(t)
    if type(t) ~= "table" then return false end
    if t.ANY or t.Any or t.any then return true end
    for k, v in pairs(t) do
        if v == true and SH.key(k) == "any" then return true end
        if type(v) == "string" and SH.key(v) == "any" then return true end
    end
    return false
end
function SH.hasPick(t)
    if type(t) ~= "table" then return false end
    for _, v in pairs(t) do if v == true or type(v) == "string" then return true end end
    return false
end
function SH.matchTarget(name, picks)
    if SH.anySelected(picks) then return true end
    if not SH.hasPick(picks) then return false end
    local n = SH.key(name)
    for k, v in pairs(picks) do
        local s = nil
        if v == true then s = k elseif type(v) == "string" then s = v end
        if s and SH.key(s) ~= "any" then
            local sk = SH.key(s)
            if n == sk or string.find(n, sk, 1, true) or string.find(sk, n, 1, true) then return true end
        end
    end
    return false
end
function SH.numberFromText(x, allowPlain)
    if x == nil then return nil end
    if type(x) == "number" then return x end
    local s = tostring(x)
    local n = string.match(s, "([%d%.]+)%s*[Kk][Gg]")
...
function SH.attrKg(inst)
    if not inst or not inst.GetAttributes then return nil end
    local names = { "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass", "FruitWeight", "Scale" }
    for _, a in ipairs(names) do
        local n = SH.numberFromText(inst:GetAttribute(a), true)
    ...
    if ok and type(attrs) == "table" then
        for k, v in pairs(attrs) do
            ...
            if string.find(lk, "weight", 1, true) or string.find(lk, "kilogram", 1, true) or lk == "kg" or string.find(lk, "mass", 1, true) then
                local n = SH.numberFromText(v, true)
                if n then return n end
            end
        end
    end
...
function SH.getKg(obj, prompt)
    local list = {}
    if obj then list[#list + 1] = obj end
    if prompt then list[#list + 1] = prompt end
    local p = obj and obj.Parent
    local steps = 0
    while p and p ~= Workspace and steps < 4 do
        list[#list + 1] = p
        p = p.Parent
        steps = steps + 1
    end
    for _, inst in ipairs(list) do
        local n = SH.attrKg(inst)
        if n then return n end
        if inst and inst.Name then
            n = SH.numberFromText(inst.Name)
            if n then return n end
        end
        if inst and inst:IsA("ProximityPrompt") then
            n = SH.numberFromText(inst.ObjectText) or SH.numberFromText(inst.ActionText)
            if n then return n end
        end
    end
    return nil
end
function SH.kgAllowed(kg)
    local maxKg = tonumber(S.shovelMaxKg) or 0
    if maxKg <= 0 then return true end
    return kg ~= nil and kg <= maxKg
end
function SH.pos(obj)
...
function SH.use(obj, plantId, fruitId)
    local tool = SH.equip()
    if not tool then return false end
    local shovelName = tool:GetAttribute("Shovel") or tool.Name
    local pos = SH.pos(obj)
    if pos then
        atPosition(pos, function()
            fire("Shovel.UseShovel", tostring(plantId or ""), tostring(fruitId or ""), shovelName, tool)
        end)
    else
        fire("Shovel.UseShovel", tostring(plantId or ""), tostring(fruitId or ""), shovelName, tool)
    end
...
function SH.plants()
    ...
    for _, obj in ipairs(plants:GetChildren()) do
        ...
        local nm = SH.cleanName(obj.Name)
        local kg = SH.getKg(obj, nil)
        if pid and SH.matchTarget(nm, S.shovelPlantTargets) and SH.kgAllowed(kg) then
            out[#out + 1] = { obj = obj, plantId = tostring(pid), fruitId = "", kg = kg, name = nm }
        end
    end
    return out
end
function SH.fruits()
    ...
    for _, pr in ipairs(CollectionService:GetTagged("HarvestPrompt")) do
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            local obj = promptCarrier(pr)
            local pid = obj and obj:GetAttribute("PlantId")
            local uid = obj and tonumber(obj:GetAttribute("UserId"))
            if pid and (uid == nil or uid == LocalPlayer.UserId) then
                local fid = tostring(obj:GetAttribute("FruitId") or "")
                local nm = SH.cleanName(obj.Name)
                local kg = SH.getKg(obj, pr)
                if SH.matchTarget(nm, S.shovelFruitTargets) and SH.kgAllowed(kg) then
                    out[#out + 1] = { obj = obj, plantId = tostring(pid), fruitId = fid, kg = kg, name = nm }
                end
            end
        end
    end
...
function SH.step()
    if S.autoShovelFruits then
        for _, d in ipairs(SH.fruits()) do
            if not S.autoShovelFruits or S.killed then break end
            SH.use(d.obj, d.plantId, d.fruitId)
            task.wait(math.max(0.4, tonumber(S.shovelDelay) or 2))
        end
    end
    if S.autoShovelPlants then
        for _, d in ipairs(SH.plants()) do
            if not S.autoShovelPlants or S.killed then break end
            SH.use(d.obj, d.plantId, "")
            task.wait(math.max(0.4, tonumber(S.shovelDelay) or 2))
        end
    end
end
...
local MiniBar = new("Frame", {
    ...
    Size = UDim2.fromOffset(math.floor((math.min(baseW * 0.78, 430) * UI.Scale) + 0.5), math.floor((44 * UI.Scale) + 0.5)),
...
corner(MiniBg, 18); stroke(MiniBg, Color3.fromRGB(80,80,80), 1, 0.15)
...
local function setMinimized(v)
    ...
    if UI.Minimized then
        MiniBar.Size = UDim2.fromOffset(math.floor((math.min(baseW * 0.78, 430) * UI.Scale) + 0.5), math.floor((44 * UI.Scale) + 0.5))
...
addSection(FarmPage, "SEED EVENTS")
addToggle(FarmPage, "AUTO CLAIM SEED EVENT", S.autoClaimSeedEvent, function(v) S.autoClaimSeedEvent = v end)
addToggle(FarmPage, "USE FLY TO SEED EVENT", S.claimUseFly, function(v) S.claimUseFly = v end)
...
addSection(FarmPage, "SHOVEL")
do
    local shovelOptions = { "ANY" }
    local seen = { ANY = true }
    local function addOpt(n)
        n = tostring(n or "")
        if n ~= "" and not seen[n] then seen[n] = true; shovelOptions[#shovelOptions + 1] = n end
    end
    for _, n in ipairs(SEED_NAMES) do addOpt(n) end
    local assets = ReplicatedStorage:FindFirstChild("Assets")
    if assets then
        for _, folderName in ipairs({ "Plants", "Fruits" }) do
            local folder = assets:FindFirstChild(folderName)
            if folder then for _, obj in ipairs(folder:GetChildren()) do addOpt(obj.Name) end end
        end
    end
    table.sort(shovelOptions, function(a, b) if a == "ANY" then return true end if b == "ANY" then return false end return tostring(a) < tostring(b) end)
    addDropdown(FarmPage, "SHOVEL PLANT TARGETS", shovelOptions, S.shovelPlantTargets, true, function(sel) pickMulti(sel, S.shovelPlantTargets) end)
    addDropdown(FarmPage, "SHOVEL FRUIT TARGETS", shovelOptions, S.shovelFruitTargets, true, function(sel) pickMulti(sel, S.shovelFruitTargets) end)
end
addToggle(FarmPage, "AUTO SHOVEL PLANTS", S.autoShovelPlants, function(v) S.autoShovelPlants = v end)
addToggle(FarmPage, "AUTO SHOVEL FRUITS", S.autoShovelFruits, function(v) S.autoShovelFruits = v end)
...
addToggle(FarmPage, "AUTO SELL WHEN FULL", S.autoSell, function(v) S.autoSell = v end)
...
addToggle(ShopPage, "AUTO DAILY DEALS", S.autoDaily, function(v) S.autoDaily = v end)
...
-- AUTO SHOVEL loop
loopOn(function() return S.autoShovelPlants or S.autoShovelFruits end, function()
