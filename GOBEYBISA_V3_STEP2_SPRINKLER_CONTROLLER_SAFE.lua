...
do
    local seen = {}
    for _, n in ipairs(GEAR_NAMES) do
        local name = tostring(n or "")
        if string.find(string.lower(name), "sprinkler", 1, true) and not seen[name] then
            seen[name] = true
            SPRINKLER_NAMES[#SPRINKLER_NAMES + 1] = name
        end
    end
    if #SPRINKLER_NAMES == 0 then
...
-- Sprinkler placement helpers based on the game's controller logic:
-- PlaceSprinkler must receive a position on a PlantArea inside the user's plot.
local function HaimiyachGAG2_FlatDistance(a, b)
    if not (a and b) then return math.huge end
    return (Vector2.new(a.X, a.Z) - Vector2.new(b.X, b.Z)).Magnitude
end
local function HaimiyachGAG2_Clamp(v, lo, hi)
    v = tonumber(v) or 0
...
local function HaimiyachGAG2_PlantAreaCenter()
    local areas = myPlantAreas()
    if #areas == 0 then
        local plot = myPlot()
        local ok, pos = pcall(function() return plot and plot:GetPivot().Position end)
    ...
    if yCount == 0 then return nil end
    return Vector3.new((minX + maxX) / 2, ySum / yCount, (minZ + maxZ) / 2)
end
local function HaimiyachGAG2_ProjectToPlantArea(pos)
    ...
    for _, area in ipairs(areas) do
        ...
        local x = HaimiyachGAG2_Clamp(lp.X, -size.X / 2 + 1, size.X / 2 - 1)
        local z = HaimiyachGAG2_Clamp(lp.Z, -size.Z / 2 + 1, size.Z / 2 - 1)
        local top = (cf * CFrame.new(x, size.Y / 2, z)).Position
        local hit = Workspace:Raycast(top + Vector3.new(0, 12, 0), Vector3.new(0, -50, 0), params)
        local p = hit and hit.Position or top
        local d = HaimiyachGAG2_FlatDistance(pos, p)
        if d < bestDist then
            bestDist = d
            best = p
        end
...
local function HaimiyachGAG2_ExistingSprinklerPositions()
    local out, plot = {}, myPlot()
    if not plot then return out end
    for _, obj in ipairs(plot:GetDescendants()) do
        local name = string.lower(tostring(obj.Name or ""))
        local attr = obj:GetAttribute("Sprinkler") or obj:GetAttribute("SprinklerName") or obj:GetAttribute("ItemName")
        local isSprinkler = string.find(name, "sprinkler", 1, true) ~= nil
        if type(attr) == "string" and string.find(string.lower(attr), "sprinkler", 1, true) then
            isSprinkler = true
        end
        if isSprinkler then
            local pos
...
local function HaimiyachGAG2_SprinklerCandidatePoints(mode)
    mode = tostring(mode or S.sprinklerTargetMode or "GARDEN CENTER")
    local out = {}
    local center = HaimiyachGAG2_ProjectToPlantArea(HaimiyachGAG2_PlantAreaCenter())
    if mode == "AVATAR POSITION" then
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local pos = HaimiyachGAG2_ProjectToPlantArea((hrp and hrp.Position) or center)
        if pos then out[#out + 1] = pos end
        local grid = plantGrid(8)
        table.sort(grid, function(a, b) return HaimiyachGAG2_FlatDistance(a, pos or center) < HaimiyachGAG2_FlatDistance(b, pos or center) end)
        for _, p in ipairs(grid) do out[#out + 1] = p end
    elseif mode == "MAPPING" then
        local grid = plantGrid(12)
        table.sort(grid, function(a, b)
            return HaimiyachGAG2_FlatDistance(a, center) < HaimiyachGAG2_FlatDistance(b, center)
        end)
        if center then out[#out + 1] = center end
        for _, p in ipairs(grid) do out[#out + 1] = p end
    else
        if center then out[#out + 1] = center end
        local grid = plantGrid(8)
        table.sort(grid, function(a, b) return HaimiyachGAG2_FlatDistance(a, center) < HaimiyachGAG2_FlatDistance(b, center) end)
        for _, p in ipairs(grid) do out[#out + 1] = p end
    end
    return out
end
local function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    local points = HaimiyachGAG2_SprinklerCandidatePoints(S.sprinklerTargetMode)
    if #points == 0 then return nil end
    placed = type(placed) == "table" and placed or {}
    local minGap = S.sprinklerTargetMode == "MAPPING" and 10 or 7
    for _, p in ipairs(points) do
        local clear = true
        for _, ex in ipairs(placed) do
            if HaimiyachGAG2_FlatDistance(p, ex) < minGap then clear = false break end
        end
        if clear then return p end
    end
    return points[1]
end
local function HaimiyachGAG2_SelectedSprinklerTools()
    local selected = type(S.sprinklerTargets) == "table" and S.sprinklerTargets or {}
    local useSelected = false
    for _ in pairs(selected) do useSelected = true break end
    local out = {}
    for _, tool in ipairs(HaimiyachGAG2_SelectedSprinklerTools()) do
        local attrName = tostring(tool:GetAttribute("Sprinkler") or "")
        local toolName = tostring(tool.Name or "")
        if not useSelected or selected[attrName] or selected[toolName] then
            out[#out + 1] = tool
        end
    end
