...
local S = {
    ...
    espBackpackFruit = false, espInventoryTotalValue = false, espGardenFruit = false, espGardenFruitValue = false,
    -- perf / webhook
...
local CONFIG_KEYS = {
    ...
    "espBackpackFruit", "espInventoryTotalValue", "espGardenFruit", "espGardenFruitValue",
...
local function queueAutoExecute(force)
    -- force=true is used by Webhook/Server Hop so reports keep working after migration,
    -- even when the normal AUTO EXECUTE toggle is off.
...
if not ok then warn("HAIMIYACH HUB auto execute failed: " .. tostring(err)) end
]]
...
-- //                         FRUIT ESP                            \ --
-- // ============================================================ \ --
-- Backpack ESP reads fruit proxy Configurations/Tools from the local Backpack.
-- Garden ESP reads harvestable fruit models from workspace.Gardens and anchors
-- labels to HarvestPart/PrimaryPart. All labels are clear on mobile and PC.
local FruitESP = {
    LastBackpackClear = 0,
    BaseWeightCache = {},
    MaxGardenLabels = 250,
...
pcall(function()
    ...
    local mod = sm and sm:FindFirstChild("FruitValueCalc")
    if mod then FruitValueCalc = require(mod) end
end)
pcall(function()
    local sm = ReplicatedStorage:FindFirstChild("SharedModules")
    local mod = sm and sm:FindFirstChild("SellValueData")
    if mod then SellValueData = require(mod) end
end)

local function safeHasTag(obj, tag)
    if not obj then return false end
    local ok, res = pcall(function()
...
local function parseKgText(text)
    local n = tostring(text or ""):lower():match("([%d%.]+)%s*kg")
    return n and tonumber(n) or nil
end

local function formatKg(kg)
    kg = tonumber(kg)
    if not kg then return "?kg" end
    if kg >= 100 then
...
local function shortCoins(v)
    v = tonumber(v)
    if not v then return "?¢" end
    return fmt(v) .. "¢"
end

local function sizeName(sizeMulti, kg)
    ...
    kg = tonumber(kg)
    if kg then
        if kg >= 6 then return "GIANT" end
        if kg >= 2.5 then return "BIG" end
    end
    return "NORMAL"
end
...
local function getFruitBaseWeight(fruitName)
    ...
    if fruitName == "" then return nil end
    if FruitESP.BaseWeightCache[fruitName] ~= nil then
        return FruitESP.BaseWeightCache[fruitName]
    end

    local weight = nil
    pcall(function()
        local root = ReplicatedStorage:FindFirstChild("PlantGenerationModules")
        root = root and root:FindFirstChild("Fruits")
        local module = root and root:FindFirstChild(fruitName)
        if module and module:IsA("ModuleScript") then
            local data = require(module)
            if type(data) == "table" and type(data.GrowData) == "table" then
                weight = tonumber(data.GrowData.BaseWeight)
            end
        end
    end)

    FruitESP.BaseWeightCache[fruitName] = weight or false
    return weight
end

local function fruitValue(fruitName, sizeMulti, mutation, decayAlpha)
    fruitName = tostring(fruitName or "")
    if fruitName == "" then return nil end
    ...
    if type(FruitValueCalc) == "function" then
        local ok, res = pcall(function()
            return FruitValueCalc(fruitName, tonumber(sizeMulti) or 1, mutation, LocalPlayer, tonumber(decayAlpha))
        end)
    ...
    if not value and type(SellValueData) == "table" then
        local base = tonumber(SellValueData[fruitName])
        if base then
            local sm = tonumber(sizeMulti) or 1
            value = math.floor(base * (sm ^ 2.65))
        end
    end
...
local function getFruitInfo(obj, isGarden)
    if not obj then return nil end
    local fruitName = obj:GetAttribute("FruitName")
        or obj:GetAttribute("Fruit")
        or obj:GetAttribute("CorePartName")
    if not fruitName or tostring(fruitName) == "" then return nil end

    local sizeMulti = obj:GetAttribute("SizeMultiplier") or obj:GetAttribute("SizeMulti") or 1
    local weight = obj:GetAttribute("Weight")
    local mutation = obj:GetAttribute("Mutation")
    local decayAlpha = obj:GetAttribute("DecayAlpha")

    if not weight and isGarden then
        local baseWeight = getFruitBaseWeight(fruitName)
        if baseWeight and tonumber(sizeMulti) then
            weight = baseWeight * tonumber(sizeMulti)
        end
    end

    local value = fruitValue(fruitName, sizeMulti, mutation, decayAlpha)
    return {
        Object = obj,
        ...
        Value = value,
        Id = obj:GetAttribute("Id") or obj:GetAttribute("FruitId"),
        UserId = tonumber(obj:GetAttribute("UserId")),
    }
end

local function isFruitBackpackItem(obj)
    if not obj then return false end
    if not (obj:IsA("Tool") or obj:IsA("Configuration")) then return false end
    return obj:GetAttribute("Fruit") ~= nil
        or obj:GetAttribute("FruitName") ~= nil
        or obj:GetAttribute("FruitProxy") ~= nil
        or obj:GetAttribute("HarvestedFruit") ~= nil
end

local function scanBackpackFruit()
    local list = {}
    local function scan(container)
        if not container then return end
        for _, item in ipairs(container:GetChildren()) do
            if isFruitBackpackItem(item) then
                local info = getFruitInfo(item, false)
                if info then
...
local function backpackGuiRoot()
    local gui = PlayerGui:FindFirstChild("BackpackGui")
    return gui and gui:FindFirstChild("Backpack") or nil
end

local function clearBackpackFruitESP()
    local gui = PlayerGui:FindFirstChild("BackpackGui")
    if gui then
        for _, obj in ipairs(gui:GetDescendants()) do
            if obj.Name == "HaimiyachBackpackFruitESP" or obj.Name == "HaimiyachInventoryTotalValue" then
                pcall(function() obj:Destroy() end)
            end
        end
    end
...
local function clearGardenFruitESP()
    local gardens = Workspace:FindFirstChild("Gardens")
    if gardens then
        for _, obj in ipairs(gardens:GetDescendants()) do
            if obj.Name == "HaimiyachGardenFruitESP" then
                pcall(function() obj:Destroy() end)
            end
        end
    end
...
local function ClearAllFruitESP()
    clearBackpackFruitESP()
    clearGardenFruitESP()
end

local function getBackpackSlots()
    local root = backpackGuiRoot()
    local slots = {}
    if not root then return slots end
    for _, obj in ipairs(root:GetDescendants()) do
        if obj:IsA("TextButton") and obj:FindFirstChild("ToolName") and obj:FindFirstChild("Icon") then
...
local function slotText(slot, childName)
    local obj = slot and slot:FindFirstChild(childName)
...
local function matchFruitToSlot(slot, fruits, used)
    local nameText = string.lower(slotText(slot, "ToolName"))
    local kgText = slotText(slot, "ToolCount")
    local kg = parseKgText(kgText)

    if nameText == "" then return nil end

    local function nameMatches(info)
        local n = string.lower(info.Name or "")
        return n ~= "" and (nameText == n or string.find(nameText, n, 1, true) or string.find(n, nameText, 1, true))
    end

    if kg then
        for i, info in ipairs(fruits) do
            if not used[i] and nameMatches(info) and info.Weight and math.abs(info.Weight - kg) <= 0.06 then
                return i
            end
        end
    end
    ...
    for i, info in ipairs(fruits) do
        if not used[i] and nameMatches(info) then
            return i
        end
    end
...
local function ensureBackpackLabel(slot)
    local label = slot:FindFirstChild("HaimiyachBackpackFruitESP")
    if not label then
        label = Instance.new("TextLabel")
        label.Name = "HaimiyachBackpackFruitESP"
    ...
    label.Size = UDim2.new(1, -4, 0, isMobile and 27 or 29)
    label.Position = UDim2.new(0, 2, 1, isMobile and -29 or -31)
    label.TextSize = isMobile and 8 or 9
    label.Visible = true
    return label
end

local function updateInventoryTotal(total, count)
    local root = backpackGuiRoot()
    if not root then return end

    local invFrame = root:FindFirstChild("Inventory")
    if not invFrame then return end

    local label = invFrame:FindFirstChild("HaimiyachInventoryTotalValue")
    if not label then
        label = Instance.new("TextLabel")
        label.Name = "HaimiyachInventoryTotalValue"
        label.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
        label.BackgroundTransparency = 0.18
        label.BorderSizePixel = 0
        label.TextColor3 = Color3.fromRGB(255, 245, 170)
    ...
    label.Size = UDim2.new(1, -12, 0, isMobile and 20 or 22)
    label.Position = UDim2.fromOffset(6, isMobile and 18 or 17)
    label.TextSize = isMobile and 11 or 12
    label.Text = string.format("TOTAL VALUE: %s  |  FRUITS: %d", shortCoins(total), tonumber(count) or 0)
    label.Visible = S.espInventoryTotalValue == true
end

local function UpdateBackpackFruitESP()
    local fruits = scanBackpackFruit()
    local total = 0
    for _, info in ipairs(fruits) do
        total = total + (tonumber(info.Value) or 0)
    end

    if S.espInventoryTotalValue then
        updateInventoryTotal(total, #fruits)
    else
        local root = backpackGuiRoot()
        local invFrame = root and root:FindFirstChild("Inventory")
        local label = invFrame and invFrame:FindFirstChild("HaimiyachInventoryTotalValue")
        if label then label:Destroy() end
    end

    if not S.espBackpackFruit then
        for _, slot in ipairs(getBackpackSlots()) do
            local old = slot:FindFirstChild("HaimiyachBackpackFruitESP")
            if old then old:Destroy() end
        end
        return
    end
    ...
    local used = {}
    for _, slot in ipairs(getBackpackSlots()) do
        local idx = matchFruitToSlot(slot, fruits, used)
        local old = slot:FindFirstChild("HaimiyachBackpackFruitESP")
        if idx then
            used[idx] = true
            local info = fruits[idx]
            local label = ensureBackpackLabel(slot)
            local sz = sizeName(info.SizeMulti, info.Weight)
            local kg = formatKg(info.Weight)
            local val = shortCoins(info.Value)
...
local function isGardenFruitModel(model)
    if not model or not model:IsA("Model") then return false end
    if not (model:GetAttribute("CorePartName") or model:GetAttribute("FruitName") or model:GetAttribute("Fruit")) then return false end
    if not (safeHasTag(model, "Harvestable") or model:FindFirstChild("HarvestPart") or model:GetAttribute("FruitId")) then return false end
    return true
end

local function gardenFruitAdornee(model)
...
local function ensureGardenBillboard(model, adornee)
    local gui = model:FindFirstChild("HaimiyachGardenFruitESP", true)
    if not gui then
        gui = Instance.new("BillboardGui")
        gui.Name = "HaimiyachGardenFruitESP"
        gui.AlwaysOnTop = true
        gui.LightInfluence = 0
        gui.MaxDistance = isMobile and 450 or 650
        gui.Size = UDim2.fromOffset(isMobile and 168 or 190, isMobile and 48 or 52)
        gui.StudsOffsetWorldSpace = Vector3.new(0, isMobile and 2.2 or 2.6, 0)
        gui.Parent = adornee
...
local function UpdateGardenFruitESP()
    if not S.espGardenFruit then
        clearGardenFruitESP()
        return
    end
    ...
    -- Keep track of labels that are still valid this update.
    -- When a fruit is harvested, some servers destroy the model, but others only
    -- remove Harvestable/HarvestPart first. This cleanup prevents stale ESP labels.
    local activeGuis = {}
    local count = 0

    for _, model in ipairs(gardens:GetDescendants()) do
        if count >= FruitESP.MaxGardenLabels then break end
        if isGardenFruitModel(model) then
            local info = getFruitInfo(model, true)
            local adornee = gardenFruitAdornee(model)
            if info and adornee and adornee:IsDescendantOf(game) then
                count = count + 1
                local gui = ensureGardenBillboard(model, adornee)
                activeGuis[gui] = true

                local label = gui:FindFirstChild("Label")
                if label then
                    local own = (info.UserId ~= nil and info.UserId == LocalPlayer.UserId)
                    local ownerTag = own and "OWN" or "STEAL"
                    local sz = sizeName(info.SizeMulti, info.Weight)
                    local kg = formatKg(info.Weight)
                    local firstLine = string.format("%s | %s | %s", info.Name, sz, ownerTag)
                    local secondLine = kg
                    if S.espGardenFruitValue then
                        secondLine = secondLine .. " | " .. shortCoins(info.Value)
                    end
    ...
    -- Remove labels whose fruit is no longer harvestable / no longer present.
    for _, gui in ipairs(gardens:GetDescendants()) do
        if gui.Name == "HaimiyachGardenFruitESP" and not activeGuis[gui] then
...
task.spawn(function()
    while not S.killed do
        if S.espBackpackFruit or S.espInventoryTotalValue then
            pcall(UpdateBackpackFruitESP)
        end
        if S.espGardenFruit then
            pcall(UpdateGardenFruitESP)
        end
        task.wait(1)
    end
end)
...
addDropdown(UI.Pages["FARM"], "SEED TO PLANT", plantOptions, (S.plantSeed == "Best owned" and "BEST OWNED") or S.plantSeed, false, function(v) S.plantSeed = (v == "BEST OWNED" and "Best owned") or v or "Best owned" end)
...
addDropdown(UI.Pages["FARM"], "HARVEST FRUIT TARGETS", SEED_NAMES, S.harvestFruitTargets, true, function(sel) pickMulti(sel, S.harvestFruitTargets) end)
...
addToggle(UI.Pages["MISC"], "AUTO EXECUTE AFTER HOP", S.autoExecute, function(v) S.autoExecute = v; saveConfig(true) end)
...
addToggle(UI.Pages["VISUALS"], "ESP BACKPACK FRUIT", S.espBackpackFruit, function(v)
    S.espBackpackFruit = v
    if not v then clearBackpackFruitESP() else pcall(UpdateBackpackFruitESP) end
    saveConfig(true)
end)
addToggle(UI.Pages["VISUALS"], "TOTAL INVENTORY VALUE", S.espInventoryTotalValue, function(v)
    S.espInventoryTotalValue = v
    if not v then
        local root = backpackGuiRoot()
        local invFrame = root and root:FindFirstChild("Inventory")
        local label = invFrame and invFrame:FindFirstChild("HaimiyachInventoryTotalValue")
...
addToggle(UI.Pages["VISUALS"], "ESP GARDEN FRUIT", S.espGardenFruit, function(v)
    S.espGardenFruit = v
    if not v then clearGardenFruitESP() else pcall(UpdateGardenFruitESP) end
    saveConfig(true)
end)
addToggle(UI.Pages["VISUALS"], "GARDEN FRUIT VALUE", S.espGardenFruitValue, function(v)
    S.espGardenFruitValue = v
    clearGardenFruitESP()
    if S.espGardenFruit then pcall(UpdateGardenFruitESP) end
    saveConfig(true)
end)
