...
local function crateCatalog()
    ...
    local ok, data = pcall(function()
        return require(ReplicatedStorage.SharedModules.CrateData)
    end)
    if ok and type(data) == "table" and type(data.GetAllCrates) == "function" then
        local ok2, crates = pcall(function()
            return data.GetAllCrates()
        end)
        if ok2 and type(crates) == "table" then
            for _, e in pairs(crates) do
                if type(e) == "table" then
                    addName(e.Name or e.CrateName or e.ItemName)
                end
            end
        end
    end
    ...
    local folders = { assets and assets:FindFirstChild("Crates"), assets and assets:FindFirstChild("GuildCrates") }
    ...
    if #out == 0 then
        for name in pairs(invNames("Crates")) do
            addName(name)
        end
    end
...
local CRATE_NAMES = crateCatalog()

-- Mailbox selected-send lists are static at boot to avoid blocking PlayerStateClient during execute.
local MAIL_SIMPLE_CATEGORIES = { "Crates", "SeedPacks", "Sprinklers", "WateringCans", "Mushrooms", "Gnomes", "Raccoons", "Trowels", "Props" }
local MAIL_FRUIT_NAMES = {}
for _, n in ipairs(SEED_NAMES or {}) do MAIL_FRUIT_NAMES[#MAIL_FRUIT_NAMES + 1] = n end
local MAIL_OTHER_NAMES = {}
do
    local seen = {}
    local function addMailOption(name)
        name = tostring(name or "")
        if name ~= "" and not seen[name] then
            seen[name] = true
            MAIL_OTHER_NAMES[#MAIL_OTHER_NAMES + 1] = name
        end
    end
    for _, n in ipairs(CRATE_NAMES or {}) do addMailOption(n) end
    for _, n in ipairs(GEAR_NAMES or {}) do addMailOption(n) end
    table.sort(MAIL_OTHER_NAMES)
end
...
local S = {
    ...
    autoBuyPets = false, buyWorldPets = {}, maxPetPrice = 25000, petTeleport = true, petBuyInterval = 5,
    sellPets = {}, autoSellPets = false,
    -- eggs / crates / packs
    autoEgg = false, autoCrate = false, crateOpen = {}, autoPack = false, openInterval = 4,
    -- shop
    ...
    -- misc
    autoMail = false, autoAcceptGift = false, autoSendMail = false, mailTargetUsername = "", mailNote = "Here is a gift!", mailSeeds = {}, mailFruits = {}, mailItems = {}, mailCountPerItem = 1, mailSendInterval = 10, autoHop = false, hopInterval = 0,
    codeText = "", autoCodes = false, antiAfk = true, disableCutscene = false,
    -- perf / webhook
    fpsBoost = false, fpsBoostMode = "BALANCED", highGraphics = false,
...
local Stats = { bought = 0, seedBought = 0, gearBought = 0, sprinklerBought = 0, planted = 0, harvested = 0, sold = 0, earned = 0,
    sprinklers = 0, watered = 0, tamed = 0, opened = 0, stolen = 0, mailSent = 0, codes = 0,
...
local CONFIG_KEYS = {
    ...
    "autoEquipPets", "autoPetSlot", "autoBuyPets", "buyWorldPets", "maxPetPrice", "petTeleport", "petBuyInterval",
    "sellPets", "autoSellPets", "autoEgg", "autoCrate", "crateOpen", "autoPack", "openInterval",
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
    "autoMail", "autoAcceptGift", "autoSendMail", "mailTargetUsername", "mailNote", "mailSeeds", "mailFruits", "mailItems", "mailCountPerItem", "mailSendInterval", "autoHop", "hopInterval", "codeText", "autoCodes", "antiAfk", "disableCutscene",
    "fpsBoost", "fpsBoostMode", "highGraphics", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
if type(S.crateOpen) ~= "table" then S.crateOpen = {} end
...
function BackpackESP.update()
    ...
    if S.espBackpackTotal and inv then
        ...
        if total then
            total.Text = "Total Value: " .. BackpackESP.money(data.total) .. "¢"
...
-- AUTO BUY selected world pets: walk up (teleport) to selected unowned wild pet and buy it.
...
loopOn(function() return S.autoBuyPets end, function() return S.petBuyInterval end, function()
    if type(S.buyWorldPets) ~= "table" or not picked(S.buyWorldPets) then return end
    for _, w in ipairs(wildPets()) do
        if not S.autoBuyPets then break end
        local petName = tostring(w.name or "")
        if petName ~= "" and S.buyWorldPets[petName] == true and w.owner == 0 and w.price > 0 and getSheckles() >= w.price then
...
local function openSelected(category, path, selected)
    if type(selected) ~= "table" or not picked(selected) then return end
    local items = invNames(category)
    for nm, count in pairs(items) do
        if S.killed then break end
        if selected[nm] == true then
            for _ = 1, math.min(count, 25) do
                local ok, res = fire(path, nm)
                if not ok then break end
                if type(res) == "table" and res.Success == false then break end
...
loopOn(function() return S.autoCrate end, function() return S.openInterval end, function() openSelected("Crates", "Crate.OpenCrate", S.crateOpen) end)
...
-- // ============================================================ \\ --
local MailTargetCache = { name = "", userId = nil, lastTry = 0 }
local MailLastSendAt = 0
local function resolveMailTargetUserId(name)
    name = tostring(name or "")
    name = name:gsub("^@", ""):gsub("%s+", "")
    if name == "" then return nil, "Enter target username" end
    local numeric = tonumber(name)
    if numeric and numeric > 0 then return math.floor(numeric), nil end
    local livePlayer = Players:FindFirstChild(name)
    if livePlayer then return livePlayer.UserId, nil end
    local key = string.lower(name)
    if MailTargetCache.name == key and MailTargetCache.userId then return MailTargetCache.userId, nil end
    if MailTargetCache.name == key and os.clock() - (MailTargetCache.lastTry or 0) < 10 then return nil, "Username lookup cooldown" end
    MailTargetCache.name = key
    MailTargetCache.lastTry = os.clock()
    local ok, uid = pcall(function() return Players:GetUserIdFromNameAsync(name) end)
    if ok and type(uid) == "number" then
        MailTargetCache.userId = uid
        return uid, nil
    end
    MailTargetCache.userId = nil
    return nil, "Username not found"
end
local function mailCleanName(v)
    v = tostring(v or "")
    v = v:gsub("%s+Seed$", "")
    v = v:gsub("%s+%b[]$", "")
    return string.lower(v)
end
local function mailGiftName(category, key, value)
    if type(value) == "table" then
        if category == "HarvestedFruits" then return tostring(value.FruitName or value.Name or value.ItemName or key or "") end
        if category == "Pets" then return tostring(value.Name or value.PetName or value.ItemName or key or "") end
        return tostring(value.ItemName or value.Name or value.Type or key or "")
    end
    return tostring(key or "")
end
local function mailGiftCount(value)
    if type(value) == "number" then return math.max(0, math.floor(value)) end
    if type(value) == "table" then return math.max(0, math.floor(tonumber(value.Count) or tonumber(value.Amount) or 1)) end
    return 1
end
local function mailSelectedHas(t, name)
    if type(t) ~= "table" then return false end
    if t[name] == true then return true end
    local cleaned = mailCleanName(name)
    for k, v in pairs(t) do
        if v == true and mailCleanName(k) == cleaned then return true end
    end
    return false
end
local function mailBatchAdd(batch, sentCount, category, key, count)
    if #batch >= 20 or sentCount >= 20 then return sentCount end
    count = math.max(1, math.floor(tonumber(count) or 1))
    count = math.min(count, 20 - sentCount)
    table.insert(batch, { Category = category, ItemKey = tostring(key), Count = count })
    return sentCount + count
end
local function buildMailSendBatch()
    local batch, sentCount = {}, 0
    local perItem = math.max(1, math.min(20, math.floor(tonumber(S.mailCountPerItem) or 1)))
    if type(S.mailSeeds) == "table" and picked(S.mailSeeds) then
        for key, value in pairs(inv("Seeds")) do
            if sentCount >= 20 then break end
            local display = mailGiftName("Seeds", key, value)
            if mailSelectedHas(S.mailSeeds, display) or mailSelectedHas(S.mailSeeds, tostring(key)) then
                local available = mailGiftCount(value)
                if available > 0 then sentCount = mailBatchAdd(batch, sentCount, "Seeds", key, math.min(perItem, available)) end
            end
        end
    end
    if type(S.mailFruits) == "table" and picked(S.mailFruits) then
        for key, value in pairs(inv("HarvestedFruits")) do
            if sentCount >= 20 then break end
            local display = mailGiftName("HarvestedFruits", key, value)
            if mailSelectedHas(S.mailFruits, display) or mailSelectedHas(S.mailFruits, tostring(key)) then
                sentCount = mailBatchAdd(batch, sentCount, "HarvestedFruits", key, 1)
            end
        end
    end
    if type(S.mailItems) == "table" and picked(S.mailItems) then
        for _, category in ipairs(MAIL_SIMPLE_CATEGORIES) do
            if sentCount >= 20 then break end
            for key, value in pairs(inv(category)) do
                if sentCount >= 20 then break end
                local display = mailGiftName(category, key, value)
                if mailSelectedHas(S.mailItems, display) or mailSelectedHas(S.mailItems, tostring(key)) then
                    local available = mailGiftCount(value)
                    if available > 0 then sentCount = mailBatchAdd(batch, sentCount, category, key, math.min(perItem, available)) end
                end
            end
        end
        for key, value in pairs(inv("Pets")) do
            if sentCount >= 20 then break end
            local display = mailGiftName("Pets", key, value)
            if mailSelectedHas(S.mailItems, display) or mailSelectedHas(S.mailItems, tostring(key)) then
                sentCount = mailBatchAdd(batch, sentCount, "Pets", key, 1)
            end
        end
    end
    return batch, sentCount
end
local function sendSelectedMailNow(showEmpty)
    local targetName = tostring(S.mailTargetUsername or "")
    local userId, err = resolveMailTargetUserId(targetName)
    if not userId then
        if showEmpty then notify("Mailbox", err or "Invalid username", 4) end
        return false
    end
    local batch, sentCount = buildMailSendBatch()
    if #batch == 0 then
        if showEmpty then notify("Mailbox", "No selected gift item found in inventory.", 4) end
        return false
    end
    local note = tostring(S.mailNote or "")
    if #note > 100 then note = string.sub(note, 1, 100) end
    local ok, res = fire("Mailbox.SendBatch", userId, batch, note)
    if ok and res then
        Stats.mailSent = (tonumber(Stats.mailSent) or 0) + sentCount
        notify("Mailbox", "Gift sent to @" .. targetName .. " (" .. tostring(sentCount) .. " item)", 4)
        return true
    end
    if showEmpty then notify("Mailbox", "Could not send gift: " .. tostring(res), 4) end
    return false
end
loopOn(function() return S.autoSendMail end, function() return math.max(5, tonumber(S.mailSendInterval) or 10) end, function()
    if os.clock() - MailLastSendAt < math.max(5, tonumber(S.mailSendInterval) or 10) then return end
    MailLastSendAt = os.clock()
    sendSelectedMailNow(false)
end)
...
UI.worldPetDrop = addDropdown(UI.Pages["PETS"], "WORLD PETS TO BUY", HaimiyachGAG2_WorldPetNames(), S.buyWorldPets, true, function(sel)
    if type(S.buyWorldPets) ~= "table" then S.buyWorldPets = {} end
    pickMulti(sel, S.buyWorldPets)
end)
addToggle(UI.Pages["PETS"], "AUTO BUY SELECTED WORLD PETS", S.autoBuyPets, function(v) S.autoBuyPets = v end)
...
addDropdown(UI.Pages["OPEN"], "CRATES TO OPEN", CRATE_NAMES, S.crateOpen, true, function(sel)
    if type(S.crateOpen) ~= "table" then S.crateOpen = {} end
    pickMulti(sel, S.crateOpen)
end)
addToggle(UI.Pages["OPEN"], "AUTO OPEN SELECTED CRATES", S.autoCrate, function(v) S.autoCrate = v end)
...
addInput(UI.Pages["MISC"], "MAIL TARGET USERNAME", "username without @", S.mailTargetUsername, function(text) S.mailTargetUsername = tostring(text or "") end)
addInput(UI.Pages["MISC"], "MAIL NOTE", "optional note", S.mailNote, function(text) S.mailNote = tostring(text or "") end)
addDropdown(UI.Pages["MISC"], "MAIL SEEDS TO SEND", SEED_NAMES, S.mailSeeds, true, function(sel)
    if type(S.mailSeeds) ~= "table" then S.mailSeeds = {} end
    pickMulti(sel, S.mailSeeds)
end)
addDropdown(UI.Pages["MISC"], "MAIL FRUITS TO SEND", MAIL_FRUIT_NAMES, S.mailFruits, true, function(sel)
    if type(S.mailFruits) ~= "table" then S.mailFruits = {} end
    pickMulti(sel, S.mailFruits)
end)
addDropdown(UI.Pages["MISC"], "MAIL OTHER ITEMS", MAIL_OTHER_NAMES, S.mailItems, true, function(sel)
    if type(S.mailItems) ~= "table" then S.mailItems = {} end
    pickMulti(sel, S.mailItems)
end)
addSlider(UI.Pages["MISC"], "MAIL COUNT PER ITEM", 1, 20, S.mailCountPerItem, 0, function(v) S.mailCountPerItem = v end)
addSlider(UI.Pages["MISC"], "MAIL SEND DELAY", 5, 120, S.mailSendInterval, 0, function(v) S.mailSendInterval = v end)
addButton(UI.Pages["MISC"], "SEND SELECTED MAIL NOW", function() sendSelectedMailNow(true) end)
addToggle(UI.Pages["MISC"], "AUTO SEND SELECTED MAIL", S.autoSendMail, function(v) S.autoSendMail = v end)
addLabel(UI.Pages["MISC"], "Send selected giftable inventory items to username. Mailbox sends max 20 items per batch.")
...
addToggle(UI.Pages["ESP"], "ESP READY FRUIT", S.espGardenFruit, function(v) GardenESP.set(v); saveConfig(true) end)
addToggle(UI.Pages["ESP"], "ESP GROWING FRUIT", S.espGardenGrowing, function(v) GardenESP.setGrowing(v); saveConfig(true) end)
addLabel(UI.Pages["ESP"], "Note: Turn off FPS BOOST first before using Fruit ESP.")
addToggle(UI.Pages["ESP"], "GARDEN VALUE SHECKLES", S.espGardenValue, function(v) GardenESP.setValue(v); saveConfig(true) end)
...
addToggle(UI.Pages["ESP"], "BACKPACK VALUE SHECKLES", S.espBackpackValue, function(v) BackpackESP.setValue(v); saveConfig(true) end)
addToggle(UI.Pages["ESP"], "TOTAL BACKPACK VALUE", S.espBackpackTotal, function(v) BackpackESP.setTotal(v); saveConfig(true) end)
