...
-- Lua 5.1 / obfuscator-safe vararg packing.
-- Jangan pakai table pack / table unpack karena beberapa obfuscator lama tidak support.
...
local function fire(path, ...)            -- fire-and-forget OR returns value (both via :Fire)
    ...
    pace()
    local args = packArgs(...)
    local ok, res = pcall(a.Fire, a, unpack(args, 1, args.n))
...
function HaimiyachGAG2_BuyCrateViaGui(crateName)
    ...
    if type(result) == "table" then return unpack(result, 1, result.n or #result) end
