...
function GardenESP.isFruit(model)
    ...
    if model:GetAttribute("PlantId") ~= nil then return true end
...
function GardenESP.weightText(model, sizeMulti)
    local w = model and (model:GetAttribute("Weight") or model:GetAttribute("KG") or model:GetAttribute("Kg") or model:GetAttribute("Mass"))
    w = tonumber(w)
    if w and w > 0 then
...
function GardenESP.scan()
    ...
    pcall(function()
        for _, inst in ipairs(CollectionService:GetTagged("Harvestable")) do add(inst) end
        for _, inst in ipairs(CollectionService:GetTagged("Growable")) do add(inst) end
    end)
...
function BackpackESP.value(name, sizeMulti, decayAlpha)
        ...
        for _, item in ipairs(container:GetChildren()) do
            if BackpackESP.isFruitItem(item) then
                ...
                if name and name ~= "" then
                    ...
                    local value = BackpackESP.value(name, sm, item:GetAttribute("DecayAlpha"))
                    local rec = { name = name, kg = kg, size = sm, value = value, item = item }
...
function BackpackESP.slotInfo(slot)
    ...
    local name = BackpackESP.cleanName(nameLabel.Text)
    if name == "" or GardenESP.base[name] == nil then return nil end
    local kg = weightLabel and BackpackESP.parseKg(weightLabel.Text) or nil
...
function BackpackESP.update()
    ...
    for _, pack in ipairs(BackpackESP.slots()) do
        ...
        local value = rec and rec.value or BackpackESP.value(info.name, 1, nil)
        local kgText = info.kg and (string.format("%.2fkg", info.kg)) or "Fruit"
        local text = kgText
