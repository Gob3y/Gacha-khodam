...
local function HaimiyachGAG2_SafeFruitValue(obj)
    ...
    local kg = 0
    pcall(function()
        kg = tonumber(obj:GetAttribute("Weight") or obj:GetAttribute("KG") or obj:GetAttribute("Kg") or obj:GetAttribute("Size")) or 0
    end)
    if kg > 0 then return kg * 1000 end
    return 0
end

local function HaimiyachGAG2_GardenKeyFromObject(obj, owner)
    local gardens = Workspace:FindFirstChild("Gardens")
    local cur = obj
        ...
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            ...
            if pid then
                ...
                if owner ~= 0 and owner ~= LocalPlayer.UserId then
                    ...
                    local value = HaimiyachGAG2_SafeFruitValue(m)
                    local gardenKey = HaimiyachGAG2_GardenKeyFromObject(m, owner)
                    ...
                    out[#out + 1] = {
                        ...
                        value = value,
                        gardenKey = gardenKey,
                        name = tostring(m:GetAttribute("Seed") or m:GetAttribute("FruitName") or m.Name or "Fruit"),
                    }
                end
            end
        end
...
local function HaimiyachGAG2_FindWildPetVisualSafe(part, petName)
    local map = Workspace:FindFirstChild("Map")
    local spawns = map and map:FindFirstChild("WildPetSpawns")
    if not spawns then return nil end
    local wanted = tostring(petName or (part and part:GetAttribute("PetName")) or (part and part.Name) or "")
    local partName = tostring(part and part.Name or "")
    local best = nil
    for _, obj in ipairs(spawns:GetChildren()) do
        if obj:IsA("Model") then
            local n = tostring(obj.Name or "")
            local pn = tostring(obj:GetAttribute("PetName") or "")
            if (wanted ~= "" and (pn == wanted or string.find(n, wanted, 1, true))) or (partName ~= "" and string.find(n, partName, 1, true)) then
                best = obj
                break
            end
        end
...
local function HaimiyachGAG2_GetWildPetPointSafe(part, visual)
    if visual then
        local bp = visual.PrimaryPart or visual:FindFirstChild("RootPart") or visual:FindFirstChild("HumanoidRootPart") or visual:FindFirstChildWhichIsA("BasePart")
...
local function HaimiyachGAG2_TriggerPromptSafe(container)
    if not container then return false end
    if container:IsA("ProximityPrompt") then
        if not container.Enabled then return false end
        if type(fireproximityprompt) == "function" then
            local ok = pcall(function() fireproximityprompt(container) end)
            if ok then return true end
        end
        local ok2 = pcall(function()
            container:InputHoldBegin()
            task.wait(math.max(0.15, tonumber(container.HoldDuration) or 0.15) + 0.05)
            container:InputHoldEnd()
        end)
        return ok2 == true
    end
    for _, d in ipairs(container:GetDescendants()) do
        if d:IsA("ProximityPrompt") and d.Enabled then
            if HaimiyachGAG2_TriggerPromptSafe(d) then return true end
        end
    end
    return false
end

local function HaimiyachGAG2_TriggerWildPetPrompt(target)
    if not target then return false end
    if type(target) == "table" then
        if target.prompt and HaimiyachGAG2_TriggerPromptSafe(target.prompt) then return true end
        if target.visual and HaimiyachGAG2_TriggerPromptSafe(target.visual) then return true end
        if target.part and HaimiyachGAG2_TriggerPromptSafe(target.part) then return true end
        return false
    end
    return HaimiyachGAG2_TriggerPromptSafe(target)
end
...
local function HaimiyachGAG2_TameWildPet(w)
    ...
    local petName = tostring(w.name or "?")
    w.visual = w.visual or HaimiyachGAG2_FindWildPetVisualSafe(w.part, petName)
    w.pos = HaimiyachGAG2_GetWildPetPointSafe(w.part, w.visual) or w.pos
...
-- //                     STEAL (PvP, night)                      \\ --
-- // ============================================================ \\ --
-- Safe steal: pick highest-value garden/fruit, return only after carry is confirmed.
local function hrpNow() local c = LocalPlayer.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local HaimiyachGAG2_StealFailedUntil = {}
local function HaimiyachGAG2_StealKey(f)
    return tostring(f.owner or "") .. ":" .. tostring(f.plantId or "") .. ":" .. tostring(f.fruitId or "")
end
local function HaimiyachGAG2_IsCarryingStolenFruit()
    return LocalPlayer:GetAttribute("CarryingStolenFruit") or LocalPlayer:GetAttribute("CarryingFruit")
end
local function HaimiyachGAG2_WaitStealCarry(timeout)
    local t0 = os.clock()
    while os.clock() - t0 < (tonumber(timeout) or 2.5) and S.autoSteal do
        if HaimiyachGAG2_IsCarryingStolenFruit() then return true end
        task.wait(0.1)
    end
    return false
end
local function HaimiyachGAG2_OwnerNearFruit(f)
    if not (f and f.pos) then return false end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.UserId == tonumber(f.owner) then
            local c = plr.Character
            local r = c and c:FindFirstChild("HumanoidRootPart")
            if r and (r.Position - f.pos).Magnitude <= 45 then return true end
        end
    end
    return false
end
loopOn(function() return S.autoSteal end, 1.5, function()
    ...
    for _, f in ipairs(stealable()) do
        ...
        if (HaimiyachGAG2_StealFailedUntil[key] or 0) <= now and not HaimiyachGAG2_OwnerNearFruit(f) then
            ...
            if f.prompt then HaimiyachGAG2_TriggerPromptSafe(f.prompt) end
            task.wait(0.1)
            fire("Steal.CompleteSteal")

            if HaimiyachGAG2_WaitStealCarry(2.75) then
                Stats.stolen = (tonumber(Stats.stolen) or 0) + 1
                if Stats.SetLastAction then Stats.SetLastAction("STEAL", tostring(f.name or "Fruit")) end
                if S.stealReturnBase then
                    ...
                    if base and hrp then
                        ...
                        Stats.lastReturnGardenAt = os.clock()
                        local t0 = os.clock()
                        while HaimiyachGAG2_IsCarryingStolenFruit() and os.clock() - t0 < 3.5 and S.autoSteal do task.wait(0.15) end
                    end
                end
                break
            else
                HaimiyachGAG2_StealFailedUntil[key] = os.clock() + 8
