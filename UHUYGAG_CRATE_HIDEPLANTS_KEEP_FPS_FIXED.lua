...
local function crateCatalog()
    ...
    -- Prefer live stock names when the game exposes a crate shop stock folder.
    local stockValues = ReplicatedStorage:FindFirstChild("StockValues")
    if stockValues then
        for _, shopName in ipairs({ "CrateShop", "CratesShop", "GuildCrateShop", "GuildCrates", "Crates" }) do
            local shop = stockValues:FindFirstChild(shopName)
            local items = shop and shop:FindFirstChild("Items") or shop
            if items then
    ...
    -- Fallback to crate asset models so the dropdown is still populated.
    local assets = ReplicatedStorage:FindFirstChild("Assets")
    if assets then
        for _, folderName in ipairs({ "GuildCrates", "Crates" }) do
...
local S = {
    ...
    autoBuyCrates = false, autoBuyAllCrates = false, crateBuy = {}, crateBuyInterval = 10,
    -- steal
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoBuyCrates", "autoBuyAllCrates", "crateBuy", "crateBuyInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
...
local function crateStockOf(name)
    ...
    for _, shopName in ipairs({ "CrateShop", "CratesShop", "GuildCrateShop", "GuildCrates", "Crates" }) do
        local shop = stockValues:FindFirstChild(shopName)
        local items = shop and shop:FindFirstChild("Items") or shop
        local item = items and items:FindFirstChild(tostring(name))
        if item then
...
local function refreshCrateNames()
    local names, seen = {}, {}
    local function add(name)
        name = tostring(name or "")
        if name ~= "" and not seen[name] then
    ...
    for _, name in ipairs(crateCatalog()) do add(name) end
    for name in pairs(invNames("Crates")) do add(name) end
    table.sort(names)
    return names
end

local CRATE_BUY_ACTIONS = {
    ...
    "CrateShop.BuyCrate",
    "CratesShop.BuyCrate",
    "GuildCrateShop.BuyCrate",
    "Crate.BuyCrate",
    "Crates.BuyCrate",
}
local CRATE_BUY_GENERIC = {
    { "Shop.PurchaseItem", "CrateShop" },
    { "Shop.BuyItem", "CrateShop" },
    { "Shop.Purchase", "CrateShop" },
    { "Shop.Buy", "CrateShop" },
}

local function buyCrateOnce(name)
    name = tostring(name or "")
    if name == "" then return false end

    for _, path in ipairs(CRATE_BUY_ACTIONS) do
        if action(path) then
            local ok, res = fire(path, name)
            if ok and not (type(res) == "table" and res.Success == false) then
                return true, res
            end
        end
    end

    for _, item in ipairs(CRATE_BUY_GENERIC) do
        local path, shopName = item[1], item[2]
        if action(path) then
            local ok, res = fire(path, shopName, name)
            if ok and not (type(res) == "table" and res.Success == false) then
                return true, res
            end
        end
...
loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return S.crateBuyInterval end, function()
    ...
    if S.autoBuyAllCrates then
        CRATE_NAMES = refreshCrateNames()
        for _, name in ipairs(CRATE_NAMES) do
            list[#list + 1] = name
        end
    else
        if not picked(S.crateBuy) then return end
    ...
    for _, name in ipairs(list) do
        ...
        if stock == nil or stock > 0 then
            local ok = buyCrateOnce(name)
            if ok then Stats.bought = Stats.bought + 1 end
            task.wait(jitter(0.25, 0.45))
        end
    end
end)
...
local function IsPlantVisualObject(obj)
    ...
    while parent and parent ~= Workspace do
        if parent.Name == "Plants" and parent:IsDescendantOf(gardens) then
            return true
        end
...
local function ApplyHidePlantsToObject(obj)
    if not IsPlantVisualObject(obj) then return end
    if obj:IsA("BasePart") then
        if HidePlantsBackup.Parts[obj] == nil then
            pcall(function() HidePlantsBackup.Parts[obj] = obj.LocalTransparencyModifier end)
        end
        pcall(function() obj.LocalTransparencyModifier = 1 end)
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        if HidePlantsBackup.Decals[obj] == nil then
            pcall(function() HidePlantsBackup.Decals[obj] = obj.Transparency end)
        end
    ...
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
        if HidePlantsBackup.Effects[obj] == nil then
            pcall(function() HidePlantsBackup.Effects[obj] = obj.Enabled end)
        end
...
local function RestoreHiddenPlants()
    for obj, value in pairs(HidePlantsBackup.Parts) do
        if obj and obj.Parent then pcall(function() obj.LocalTransparencyModifier = value end) end
    end
    for obj, value in pairs(HidePlantsBackup.Decals) do
        if obj and obj.Parent then pcall(function() obj.Transparency = value end) end
    end
    for obj, value in pairs(HidePlantsBackup.Effects) do
        if obj and obj.Parent then pcall(function() obj.Enabled = value end) end
    end
    HidePlantsBackup.Parts = {}
    HidePlantsBackup.Decals = {}
    HidePlantsBackup.Effects = {}
end

local function SetHidePlantsEnabled(state)
    S.hidePlants = state and true or false
    HidePlantsActive = S.hidePlants
    ...
    if gardens then
        task.spawn(function()
            local all = {}
            pcall(function() all = gardens:GetDescendants() end)
            local processed = 0
            for _, obj in ipairs(all) do
                if not HidePlantsActive or not S.hidePlants then break end
                ApplyHidePlantsToObject(obj)
                processed = processed + 1
    ...
    HidePlantsConnection = Workspace.DescendantAdded:Connect(function(obj)
        if HidePlantsActive and S.hidePlants then
            task.defer(function()
                if HidePlantsActive and S.hidePlants then
                    ApplyHidePlantsToObject(obj)
                end
...
local crateDrop = addDropdown(ShopPage, "CRATES TO BUY", CRATE_NAMES, S.crateBuy, true, function(sel) pickMulti(sel, S.crateBuy) end)
