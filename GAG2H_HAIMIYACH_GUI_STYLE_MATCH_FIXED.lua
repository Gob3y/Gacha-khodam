...
local _hgBackup = nil
local function applyHighGraphics(on)
    S.highGraphics = on and true or false
    if on then
        if not _hgBackup then
            _hgBackup = {
        ...
        local function ensureEffect(className, name, props)
            ...
            if not obj then obj = Instance.new(className); obj.Name = name; obj.Parent = Lighting end
            for k, v in pairs(props or {}) do pcall(function() obj[k] = v end) end
            return obj
        end
        ensureEffect("BloomEffect", "Haimiyach_High_Bloom", { Enabled = true, Intensity = 0.22, Size = 18, Threshold = 1.05 })
        ensureEffect("ColorCorrectionEffect", "Haimiyach_High_Color", { Enabled = true, Brightness = 0.03, Contrast = 0.08, Saturation = 0.08 })
        ensureEffect("SunRaysEffect", "Haimiyach_High_SunRays", { Enabled = true, Intensity = 0.08, Spread = 0.65 })
        ensureEffect("Atmosphere", "Haimiyach_High_Atmosphere", { Density = 0.22, Offset = 0.15, Glare = 0.08, Haze = 0.6 })
    else
        pcall(function()
            if _hgBackup then
                Lighting.GlobalShadows = _hgBackup.GlobalShadows
                Lighting.Brightness = _hgBackup.Brightness
                Lighting.ClockTime = _hgBackup.ClockTime
                Lighting.FogEnd = _hgBackup.FogEnd
                Lighting.Ambient = _hgBackup.Ambient
                Lighting.OutdoorAmbient = _hgBackup.OutdoorAmbient
            end
            for _, name in ipairs({"Haimiyach_High_Bloom", "Haimiyach_High_Color", "Haimiyach_High_SunRays", "Haimiyach_High_Atmosphere"}) do
                local obj = Lighting:FindFirstChild(name)
...
local baseW = isMobile and math.min(760, math.floor(ScreenSize.X * 0.78)) or 690
local baseH = isMobile and math.min(430, math.floor(ScreenSize.Y * 0.72)) or 430
baseW = math.max(isMobile and 650 or 600, baseW)
baseH = math.max(isMobile and 390 or 360, baseH)
...
local function setMinimized(v)
    ...
    if UI.Minimized then
        Main.Size = UDim2.fromOffset(math.min(baseW, 520), 54)
...
trackConnection(MinBtn.Activated:Connect(function() setMinimized(not UI.Minimized) end))
...
local BoostPage = createTab("BOOSTS", 3)
local PetsPage = createTab("PETS", 4)
local OpenPage = createTab("OPEN", 5)
local ShopPage = createTab("SHOP", 6)
local StealPage = createTab("STEAL", 7)
local MiscPage = createTab("MISC", 8)
local VisualsPage = createTab("VISUALS", 9)
...
addToggle(FarmPage, "AUTO FARM", false, function(v) S.autoFarm = v end)
addToggle(FarmPage, "AUTO EXPAND", false, function(v) S.autoExpand = v end)
...
addSlider(FarmPage, "BUY INTERVAL", 1, 30, S.buyInterval, 0, function(v) S.buyInterval = v end)
addSlider(FarmPage, "MAX BUYS PER SEED", 1, 50, S.buyPerTick, 0, function(v) S.buyPerTick = v end)
...
addToggle(FarmPage, "AUTO PLANT", false, function(v) S.autoPlant = v end)
addSlider(FarmPage, "PLANT SPACING", 2, 10, S.plantSpacing, 0, function(v) S.plantSpacing = v end)
addToggle(FarmPage, "AUTO HARVEST", false, function(v) S.autoHarvest = v end)
addSlider(FarmPage, "HARVEST PACE", 0, 0.2, S.harvestDelay, 3, function(v) S.harvestDelay = v end)
addToggle(FarmPage, "AUTO SELL", false, function(v) S.autoSell = v end)
addSlider(FarmPage, "SELL INTERVAL", 3, 120, S.sellInterval, 0, function(v) S.sellInterval = v end)
addToggle(FarmPage, "AUTO POT PLANTS", false, function(v) S.autoPot = v end)

-- BOOSTS
addSection(BoostPage, "SPRINKLER WATER")
addToggle(BoostPage, "AUTO PLACE SPRINKLERS", false, function(v) S.autoSprinkler = v end)
addSlider(BoostPage, "SPRINKLER INTERVAL", 10, 120, S.sprinklerInterval, 0, function(v) S.sprinklerInterval = v end)
addToggle(BoostPage, "AUTO WATERING CAN", false, function(v) S.autoWater = v end)
addSlider(BoostPage, "WATER INTERVAL", 2, 60, S.waterInterval, 0, function(v) S.waterInterval = v end)
...
addToggle(PetsPage, "AUTO EQUIP PETS", false, function(v) S.autoEquipPets = v end)
...
addSlider(PetsPage, "PET BUY INTERVAL", 2, 60, S.petBuyInterval, 0, function(v) S.petBuyInterval = v end)
...
addSlider(OpenPage, "OPEN INTERVAL", 1, 30, S.openInterval, 0, function(v) S.openInterval = v end)
...
addSlider(ShopPage, "GEAR BUY INTERVAL", 2, 60, S.gearInterval, 0, function(v) S.gearInterval = v end)

-- STEAL
addSection(StealPage, "STEAL")
addToggle(StealPage, "AUTO STEAL RIPE FRUIT", false, function(v) S.autoSteal = v end)
addToggle(StealPage, "TELEPORT TO FRUIT", true, function(v) S.stealTeleport = v end)
addToggle(StealPage, "RETURN TO BASE", true, function(v) S.stealReturnBase = v end)
addSlider(StealPage, "STEAL DELAY", 0, 1, S.stealDelay, 2, function(v) S.stealDelay = v end)
...
addSlider(MiscPage, "HOP INTERVAL", 0, 120, S.hopInterval / 60, 0, function(v) S.hopInterval = v * 60 end)
...
addToggle(VisualsPage, "FPS BOOST", false, function(v)
    S.fpsBoost = v
    applyFpsBoost(v)
end)
addToggle(VisualsPage, "HIGH GRAPHICS", false, function(v)
    applyHighGraphics(v)
end)
...
addSlider(SettingsPage, "REPORT INTERVAL", 1, 60, S.webhookInterval / 60, 0, function(v) S.webhookInterval = v * 60 end)
...
print("[HAIMIYACH HUB] Grow a Garden 2 loaded.")
