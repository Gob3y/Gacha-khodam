...
-- HIDE PLANTS
-- Roblox-safe visual hide: only uses LocalTransparencyModifier on BasePart/Decal/Texture.
-- It does not save to config, does not auto-run, and only runs when the toggle is clicked.
local HidePlantsEnabled = false
local function SetHidePlantsEnabled(state)
    HidePlantsEnabled = (state == true)

    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then return end

    local modifier = HidePlantsEnabled and 1 or 0
    local processed = 0
