...
local S = {
    ...
    espGardenFruit = false, espGardenValue = false,
...
local function ESPGardenValueLite(name, sizeMulti)
    local base = FruitBaseValueLite[tostring(name or "")]
...
local function ESPIsGardenFruitLite(model)
    ...
    if not model:GetAttribute("FruitId") and not model:GetAttribute("PlantId") then return false end
        ...
        if ESPIsGardenFruitLite(model) then
            ...
            if hp and hp.Parent then
                ...
                if not (gui and gui.Parent) then
                    ...
                    gui.Size = UDim2.fromOffset(isMobile and 210 or 230, isMobile and 44 or 46)
                    gui.Parent = hp
                    ...
                    limit.MinTextSize = 9
                    limit.MaxTextSize = isMobile and 13 or 15
                    limit.Parent = label
                ...
                if S.espGardenValue then
                    ...
                    if value then
                        text = text .. " | " .. ESPFormatMoneyLite(value) .. "¢"
                    end
                end
