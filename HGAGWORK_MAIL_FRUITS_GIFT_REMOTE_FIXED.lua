...
-- Direct fruit gift fallback based on the real GiftingController:
-- Gifting.Send:Fire(targetUserId, "HarvestedFruits", fruitUuid).
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    local fruitSent = 0
    local fruitErr = nil
    if type(S.mailFruitTargets) == "table" and HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        -- Fruit uses the real Gifting remote because GiftingController sends HarvestedFruits by uuid.
        -- This fixes cases where Mailbox.SendBatch refuses fruit even when seed/gear mail works.
        fruitSent, fruitErr = HaimiyachGAG2_SendSelectedFruitsViaGifting(userId, S.mailFruitTargets, maxEach)
    end

    local items = HaimiyachGAG2_RemoveFruitMailItems(HaimiyachGAG2_BuildMailItems())
    if #items == 0 and fruitSent <= 0 and type(S.mailFruitTargets) == "table" and HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        local waitStart = os.clock()
        while fruitSent <= 0 and os.clock() - waitStart < 3 do
            task.wait(0.35)
            fruitSent, fruitErr = HaimiyachGAG2_SendSelectedFruitsViaGifting(userId, S.mailFruitTargets, maxEach)
        end
    end

    if #items == 0 and fruitSent <= 0 then
        if showNotify ~= false then
            ...
            if selectedText ~= "" then
                notify("Mailbox", tostring(fruitErr or ("No giftable fruit matched: " .. selectedText)), 4)
            else
    ...
    if #items > 0 then
        ...
        if not (send and send.Fire) then
            ...
            mailOk = ok and success == true
            mailMessage = tostring(message or success or "Could not send mail")
        end
    end

    if fruitSent > 0 or mailOk then
        if showNotify ~= false then
            local total = tonumber(fruitSent) or 0
            if #items > 0 and mailOk then total = total + #items end
            local mailText = "Gift sent to @" .. tostring(targetName) .. " (" .. tostring(total) .. " item)"
            if fruitSent > 0 and #items == 0 then
                mailText = "Fruit gift sent to @" .. tostring(targetName) .. " (" .. tostring(fruitSent) .. " fruit)"
            end
    ...
    if showNotify ~= false then notify("Mailbox", tostring(mailMessage or fruitErr or "Could not send gift"), 4) end
    return false
end
