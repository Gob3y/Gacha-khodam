...
-- FPS BOOST and HIGH GRAPHICS are mutually exclusive and restorable.
-- LIGHT    : effects/particles/post effects OFF, visual still normal.
-- BALANCED : LIGHT + part shadows OFF + terrain/water low + decals/textures lighter.
-- ULTRA    : BALANCED + SmoothPlastic + decals/textures hidden + garden/decor visual hidden client-side.
...
local function CapturePartVisualBackup(obj)
    if not obj or not obj:IsA("BasePart") then return end
    if GraphicsBackup.PartVisuals[obj] == nil then
        pcall(function()
            GraphicsBackup.PartVisuals[obj] = {
...
local function CaptureDecalTextureBackup(obj)
    ...
    if obj:IsA("Decal") or obj:IsA("Texture") then
        if GraphicsBackup.DecalTextures[obj] == nil then
            pcall(function()
                GraphicsBackup.DecalTextures[obj] = obj.Transparency
            end)
        end
    end
end

local function IsCharacterVisual(obj)
    if not obj then return false end
    local current = obj
...
local function HasVisualName(obj)
    local n = string.lower(tostring(obj and obj.Name or ""))
    return string.find(n, "visual", 1, true)
        or string.find(n, "decor", 1, true)
        or string.find(n, "plant", 1, true)
        or string.find(n, "fruit", 1, true)
        or string.find(n, "sprinkler", 1, true)
        or string.find(n, "wateringcanfx", 1, true)
        or string.find(n, "rake", 1, true)
        or string.find(n, "fakeplot", 1, true)
        or string.find(n, "preview", 1, true)
        or string.find(n, "radius", 1, true)
        or string.find(n, "flower", 1, true)
        or string.find(n, "leaf", 1, true)
        or string.find(n, "leaves", 1, true)
        or string.find(n, "tree", 1, true)
        or string.find(n, "bush", 1, true)
end

local function IsGardenOrDecorVisual(obj)
    if not obj or IsCharacterVisual(obj) then return false end
...
local function ApplyFPSBoostObject(obj, fpsMode)
    ...
    if IsCharacterVisual(obj) then
        return
    end

    if obj:IsA("BasePart") then
        CapturePartVisualBackup(obj)
    ...
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        ...
        if fpsMode == "ULTRA" then
            ...
            pcall(function()
                ...
                if currentTransparency < 0.55 then
...
local function RestoreFPSBoostObjects()
    for obj, data in pairs(GraphicsBackup.PartVisuals) do
        if obj and obj.Parent and data then
            if data.CastShadow ~= nil then SafeSetProperty(obj, "CastShadow", data.CastShadow) end
            if data.Material ~= nil then SafeSetProperty(obj, "Material", data.Material) end
            if data.Transparency ~= nil then SafeSetProperty(obj, "Transparency", data.Transparency) end
            if data.Reflectance ~= nil then SafeSetProperty(obj, "Reflectance", data.Reflectance) end
        end
    end

    for obj, transparency in pairs(GraphicsBackup.DecalTextures) do
        if obj and obj.Parent and transparency ~= nil then
