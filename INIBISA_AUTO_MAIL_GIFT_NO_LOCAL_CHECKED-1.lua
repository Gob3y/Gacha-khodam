...
HaimiyachGAG2_MailOtherCategoryOptions = {
    "Crates",
    "SeedPacks",
    "Sprinklers",
    "WateringCans",
...
function HaimiyachGAG2_BuildMailItems()
    ...
    if HaimiyachGAG2_HasSelection(S.mailSeedTargets) then
        for key, value in pairs(inv("Seeds")) do
            ...
            if HaimiyachGAG2_SelectedTextMatch(S.mailSeedTargets, HaimiyachGAG2_MailEntryName("Seeds", key, value)) then
                HaimiyachGAG2_AddMailItem(items, "Seeds", key, HaimiyachGAG2_MailStackCount(value), maxEach)
            end
        end
    end
    ...
    if HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        for key, value in pairs(inv("HarvestedFruits")) do
            if #items >= 20 then break end
            if HaimiyachGAG2_SelectedTextMatch(S.mailFruitTargets, HaimiyachGAG2_MailEntryName("HarvestedFruits", key, value)) then
                HaimiyachGAG2_AddMailItem(items, "HarvestedFruits", key, 1, 1)
            end
        end
    end

    if HaimiyachGAG2_HasSelection(S.mailOtherCategories) then
        for _, category in ipairs(HaimiyachGAG2_MailOtherCategoryOptions) do
            if #items >= 20 then break end
            if S.mailOtherCategories[category] == true then
                for key, value in pairs(inv(category)) do
                    if #items >= 20 then break end
                    HaimiyachGAG2_AddMailItem(items, category, key, category == "Pets" and 1 or HaimiyachGAG2_MailStackCount(value), maxEach)
                end
            end
        end
    end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    if ok and success then
        if showNotify ~= false then notify("Mailbox", tostring((message and message ~= "" and message) or "Gift sent!"), 4) end
        return true
    end
...
addDropdown(UI.Pages["MISC"], "MAIL OTHER TYPES", HaimiyachGAG2_MailOtherCategoryOptions, S.mailOtherCategories, true, function(sel)
