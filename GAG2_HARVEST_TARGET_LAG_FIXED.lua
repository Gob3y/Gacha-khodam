...
-- PERF: cache prompt -> fruit model. Auto Harvest Target used to climb
-- ancestry for every prompt every 0.05s, which caused heavy lag on big gardens.
local HaimiyachGAG2_PromptCarrierCache = setmetatable({}, { __mode = "k" })
local function promptCarrier(prompt)
    if not prompt then return nil end

    local cached = HaimiyachGAG2_PromptCarrierCache[prompt]
    if cached and cached.Parent and cached.IsDescendantOf and cached:IsDescendantOf(Workspace) then
        return cached
    end

    local function save(found)
        if found then HaimiyachGAG2_PromptCarrierCache[prompt] = found end
        return found
    end

    -- IMPORTANT: HarvestPrompt can be inside HarvestPart. HarvestPart often has
    -- PlantId, while the real fruit model above it has FruitId + the correct KG.
    -- Prefer the nearest ancestor that has BOTH PlantId and FruitId so weight
    -- filters do not read the parent plant weight.
    local node = prompt.Parent
    local guard = 0
    while node and node ~= Workspace and guard < 12 do
        guard = guard + 1
        if node:GetAttribute("PlantId") ~= nil and node:GetAttribute("FruitId") ~= nil then
            return save(node)
        end
        node = node.Parent
    end
    ...
    return save(prompt:FindFirstAncestorWhichIsA("Model"))
end
...
local HaimiyachGAG2_HarvestPromptCache = { at = 0, prompts = nil }
local function HaimiyachGAG2_GetHarvestPromptsCached()
    local now = os.clock()
    if HaimiyachGAG2_HarvestPromptCache.prompts and (now - HaimiyachGAG2_HarvestPromptCache.at) < 0.30 then
        return HaimiyachGAG2_HarvestPromptCache.prompts
    end
    HaimiyachGAG2_HarvestPromptCache.prompts = CollectionService:GetTagged("HarvestPrompt")
    HaimiyachGAG2_HarvestPromptCache.at = now
    return HaimiyachGAG2_HarvestPromptCache.prompts
end

local function ripeHarvests(selectedTargets)       -- own ripe fruit (tag "HarvestPrompt")
    local out = {}
    local selectedTarget = HaimiyachGAG2_HasSelection(selectedTargets)
    local prompts = HaimiyachGAG2_GetHarvestPromptsCached()
        ...
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            ...
            if pid then
                ...
                if uid == nil or uid == LocalPlayer.UserId then
                    -- Cheap target prune before the main harvest loop. This keeps the
                    -- returned list small when AUTO HARVEST ALL is OFF and only a few
                    -- fruits are selected.
                    if (not selectedTarget) or HaimiyachGAG2_SelectedNameOk(m, selectedTargets) then
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    -- Workspace fruit/plant objects need GardenESP weight first. The old path
    -- read parent PlantId weight before fruit weight, so ABOVE/BELOW KG could be
    -- wrong and small fruits could pass because the plant was heavy.
    if obj.IsDescendantOf and obj:IsDescendantOf(Workspace) then
        ...
        if fruitNode then
            ...
            -- Last fruit-only fallback: scan the fruit descendants, but do not
            -- climb to the parent plant.
            local checked = 0
    ...
    return HaimiyachGAG2_RawObjectKg and HaimiyachGAG2_RawObjectKg(obj) or nil
end

-- PERF: weight calculation can call GardenESP.calculateWeight and scan descendants.
-- Cache for a short time so harvest/sell/shovel/favorite filters do not recompute
-- KG for the same fruit many times per second.
local HaimiyachGAG2_ObjectKg_NoCache = HaimiyachGAG2_ObjectKg
local HaimiyachGAG2_ObjectKgCache = setmetatable({}, { __mode = "k" })
function HaimiyachGAG2_ObjectKg(obj)
    if not obj then return nil end
    local now = os.clock()
    local cached = HaimiyachGAG2_ObjectKgCache[obj]
    if cached and cached.expire and cached.expire > now then
        return cached.kg
    end
    local kg = HaimiyachGAG2_ObjectKg_NoCache(obj)
    HaimiyachGAG2_ObjectKgCache[obj] = { kg = kg, expire = now + 0.75 }
    return kg
end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    -- Inventory fruit weight is usually stored directly on Tool.Weight or in the
    -- tool name like "Apple [12.34kg]". Read those first before any expensive scan.
    ...
    -- Controller-consistent fallback for harvested fruit tools:
    -- Weight = BaseWeight * SizeMultiplier. GardenESP is already defined here.
    local fruitName = tool:GetAttribute("FruitName") or tool:GetAttribute("Fruit") or HaimiyachGAG2_ObjectName(tool)
    local sizeMulti = tonumber(tool:GetAttribute("SizeMultiplier") or tool:GetAttribute("SizeMulti") or tool:GetAttribute("FruitSizeMultiplier"))
    if fruitName and sizeMulti and GardenESP and GardenESP.fruitBaseWeight then
        local base = tonumber(GardenESP.fruitBaseWeight[tostring(fruitName)] or GardenESP.plantBaseWeight[tostring(fruitName)])
        if base and base > 0 and sizeMulti > 0 then
...
function HaimiyachGAG2_GetFruitToolId(tool)
    if not tool then return nil end
    for _, attr in ipairs({ "Id", "FruitId", "ID", "UUID", "Guid", "GUID" }) do
...
function HaimiyachGAG2_ShouldKeepFruitFromSell(tool)
    ...
    if HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue) then
        local kg = HaimiyachGAG2_GetFruitToolKg(tool)
        -- Safety rule: when keep-weight filter is active and weight cannot be
        -- read, do NOT sell it. This prevents rare/heavy fruit from being lost
        -- just because the client had not loaded its KG text/attribute yet.
        if not kg then return true end
        if HaimiyachGAG2_KgValueOk(kg, S.sellKeepKgMode, S.sellKeepKgValue) then
            return true
        end
    end
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    if protected > 0 then task.wait(0.45) end
    return protected
end
...
function HaimiyachGAG2_SellFilteredFruits()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        ...
        if tool and tool.Parent and tool:GetAttribute("IsFavorite") ~= true and not HaimiyachGAG2_ShouldKeepFruitFromSell(tool) then
            local fruitId = HaimiyachGAG2_GetFruitToolId(tool)
            if fruitId then
                local ok, res = fireFast("NPCS.SellFruit", fruitId)
                if ok and type(res) == "table" and res.Success then
                    sold = sold + 1
                    earned = earned + (tonumber(res.SellPrice) or 0)
                    pcall(function() if tool and tool.Parent then tool:Destroy() end end)
                    task.wait(0.08)
                elseif type(res) == "table" and res.Reason == "Favorited" then
...
function HaimiyachGAG2_SeedEventAttrTrue(obj, attr)
    if not obj then return false end
    local v = obj:GetAttribute(attr)
...
function HaimiyachGAG2_SeedEventType(obj)
    if not obj then return nil end
    if HaimiyachGAG2_SeedEventAttrTrue(obj, "RainbowSeed") then return "RAINBOW" end
    if HaimiyachGAG2_SeedEventAttrTrue(obj, "GoldSeed") then return "GOLD" end
    local name = string.lower(tostring(obj.Name or ""))
    if string.find(name, "rainbow", 1, true) then return "RAINBOW" end
    if string.find(name, "gold", 1, true) then return "GOLD" end
    return nil
end
...
local function findSeedEventObjects()
    ...
    local function addObject(obj)
        ...
        local seedType = HaimiyachGAG2_SeedEventType(obj)
    ...
    local function scanFolder(folder)
        ...
        for _, obj in ipairs(folder:GetChildren()) do
            addObject(obj)
            -- Client folder often holds the actual visible prompt/model deeper
            -- than one level, especially for event seeds.
            for _, d in ipairs(obj:GetDescendants()) do
                addObject(d)
            end
        end
    ...
    scanFolder(spawnServer)
    scanFolder(spawnClient)
    return results
end
    ...
    local function scan(folder)
        ...
        for _, prompt in ipairs(folder:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") and prompt.Enabled then
                ...
                if not ppos and parent and parent.Parent then ppos = getSeedEventPosition(parent.Parent) end
                if ppos then
...
local function triggerSeedEventPrompt(obj, pos)
    ...
    local ppos = getSeedEventPosition(prompt.Parent)
    if ppos then
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
...
local function claimSeedEvent()
    ...
    for _, data in ipairs(events) do
        ...
        if obj and obj.Parent and pos then
            ...
            local prompt = obj and obj:FindFirstChildWhichIsA("ProximityPrompt", true)
            if not (prompt and prompt.Enabled) then prompt = findPromptNearPosition(pos) end
            local promptPos = prompt and getSeedEventPosition(prompt.Parent) or nil
            if promptPos then
                moveToSeedEvent(promptPos)
                task.wait(0.2)
            end

            local okClaim = triggerSeedEventPrompt(obj, pos)

            -- Always try the network fallback too. Some executors return success
            -- from fireproximityprompt even when the prompt did not actually claim.
            local okNet, resNet = fire("SeedPack.Claim", obj)
            if okNet and (resNet == nil or resNet == true or (type(resNet) == "table" and resNet.Success)) then
                okClaim = true
            end
            if obj and obj.Name then
                local okName, resName = fire("SeedPack.Claim", obj.Name)
                if okName and (resName == nil or resName == true or (type(resName) == "table" and resName.Success)) then
                    okClaim = true
                end
            end
...
function HaimiyachGAG2_HarvestAllowed(obj)
    -- Cheap checks first. This prevents weight calculation from running on every
    -- fruit in the garden when the user only selected a few target fruits.
...
task.spawn(function()
    while not S.killed do
        if S.autoFarm or S.autoHarvest then
            pcall(stepHarvest)
            -- PERF: when target/weight/mutation filters are enabled, each cycle
            -- needs name/mutation/KG checks. Do not run that scan 20x per second.
            local filteredHarvest = HaimiyachGAG2_HasSelection(S.harvestFruitTargets)
                or HaimiyachGAG2_KgModeActive(S.harvestKgMode, S.harvestKgValue)
                or HaimiyachGAG2_HasSelection(S.harvestKeepMutations)
            task.wait(filteredHarvest and 0.18 or 0.08)
