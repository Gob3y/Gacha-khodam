...
local SPRINKLER_NAMES = (function()
    local out, seen = {}, {}
    ...
    if #out == 0 then
        for _, name in ipairs({ "Basic Sprinkler", "Advanced Sprinkler", "Godly Sprinkler", "Master Sprinkler" }) do
            out[#out + 1] = name
        end
    end
    table.sort(out)
...
function HaimiyachGAG2_SprinklerNameKey(text)
...
function HaimiyachGAG2_SprinklerSelectedOk(tool)
    ...
    local key = HaimiyachGAG2_SprinklerNameKey(raw)
    if key == "" then return false end
    for name, enabled in pairs(S.sprinklerTargets) do
        if enabled == true and HaimiyachGAG2_SprinklerNameKey(name) == key then return true end
    end
    return false
end
...
function HaimiyachGAG2_SelectedSprinklerTools()
    ...
    table.sort(out, function(a, b)
        local an = tostring(a:GetAttribute("Sprinkler") or a.Name or "")
        local bn = tostring(b:GetAttribute("Sprinkler") or b.Name or "")
        return an < bn
    end)
    return out
end
...
function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    ...
    local hrp = root()
    local origin = center
    if mode == "AVATAR POSITION" and hrp then
        origin = hrp.Position
    end
    ...
    for _, p in ipairs(candidates) do
        if not HaimiyachGAG2_TooClose2D(p, placed, minDist) then return p end
    end
    for _, p in ipairs(candidates) do
        if p then return p end
    end
