...
local function mailGiftName(category, key, value)
    if type(value) == "table" then
        if category == "HarvestedFruits" then
            return tostring(value.FruitName or value.Name or value.ItemName or key or "")
        elseif category == "Pets" then
            return tostring(value.Name or value.PetName or value.ItemName or key or "")
        else
...
local function mailAddOption(out, seen, name)
    name = tostring(name or "")
    if name ~= "" and not seen[name] then
        seen[name] = true
        out[#out + 1] = name
...
local function mailOtherCatalog()
    local out, seen = {}, {}
    for _, n in ipairs(CRATE_NAMES or {}) do mailAddOption(out, seen, n) end
    for _, n in ipairs(GEAR_NAMES or {}) do mailAddOption(out, seen, n) end
    for _, category in ipairs(MAIL_SIMPLE_CATEGORIES) do
        for key, value in pairs(inv(category)) do
            mailAddOption(out, seen, mailGiftName(category, key, value))
        end
    end
    table.sort(out)
    return out
end
local MAIL_FRUIT_NAMES = (function()
    local out, seen = {}, {}
    for _, n in ipairs(SEED_NAMES or {}) do mailAddOption(out, seen, n) end
    for key, value in pairs(inv("HarvestedFruits")) do mailAddOption(out, seen, mailGiftName("HarvestedFruits", key, value)) end
    table.sort(out)
    return out
end)()
local MAIL_OTHER_NAMES = mailOtherCatalog()
...
local S = {
    ...
    -- misc
    autoMail = false, autoAcceptGift = false,
    autoSendMail = false, mailTargetUsername = "", mailNote = "Here is a gift!", mailSeeds = {}, mailFruits = {}, mailItems = {}, mailCountPerItem = 1, mailSendInterval = 10,
    autoHop = false, hopInterval = 0,
...
local function resolveMailTargetUserId(name)
    ...
    if MailTargetCache.name == string.lower(name) and MailTargetCache.userId then
        return MailTargetCache.userId, nil
    end
    if MailTargetCache.name == string.lower(name) and (os.clock() - (MailTargetCache.lastTry or 0)) < 10 then
        return nil, "Username lookup cooldown"
    end
    MailTargetCache.name = string.lower(name)
...
local function buildMailSendBatch()
    ...
    local perItem = math.clamp(math.floor(tonumber(S.mailCountPerItem) or 1), 1, 20)
...
addLabel(UI.Pages["MISC"], "Mailbox send uses username/userId and sends selected Seeds, Fruits, Crates, Seed Packs, Sprinklers, Pets, or other giftable items. Max 20 items per batch.")
