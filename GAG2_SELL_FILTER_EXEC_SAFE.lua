...
function HaimiyachGAG2_GetInventoryFruitTools()
    ...
    local function isFruitTool(tool)
        ...
        if tool:GetAttribute("SeedTool") ~= nil then return false end
        local c = tostring(tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
        c = string.lower(c)
        if c == "seeds" or c == "seed" or c == "gear" or c == "gears" or c == "pets" or c == "pet" then return false end
        ...
        if tool:GetAttribute("Id") ~= nil and HaimiyachGAG2_ParseKgText(tool.Name, false) then return true end
        return false
    end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    if kg then return kg end
    for _, attr in ipairs({ "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }) do
        kg = HaimiyachGAG2_ParseKgText(tool:GetAttribute(attr), true)
        if kg then return kg end
    end
    ...
    if handle then
        kg = HaimiyachGAG2_ObjectKg(handle)
        if kg then return kg end
    end
    return nil
end
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        ...
        if hasKgKeep then
            local kg = HaimiyachGAG2_GetFruitToolKg(tool)
            if (not kg) or HaimiyachGAG2_KgValueOk(kg, S.sellKeepKgMode, S.sellKeepKgValue) then
                keep = true
            end
        end
