...
local function sprinklerCatalogSimple()
    local out, seen = {}, {}
    for _, name in ipairs(GEAR_NAMES or {}) do
        local low = string.lower(tostring(name or ""))
        if string.find(low, "sprinkler", 1, true) and not seen[name] then
            seen[name] = true
            out[#out + 1] = name
        end
    end
    if #out == 0 then
        for _, name in ipairs({"Basic Sprinkler", "Advanced Sprinkler", "Godly Sprinkler"}) do
...
local SPRINKLER_NAMES = sprinklerCatalogSimple()
...
function HaimiyachGAG2_SelectedSprinklerTools()
    ...
    for _, tool in ipairs(HaimiyachGAG2_SelectedSprinklerTools()) do
        if HaimiyachGAG2_SprinklerToolSelected(tool) then
            out[#out + 1] = tool
        end
    end
    table.sort(out, function(a, b)
...
function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    ...
    local function add(p)
        local projected = HaimiyachGAG2_NearestPlantAreaPoint(p, 4)
        if projected then candidates[#candidates + 1] = projected end
    end

    if mode == "MAPPING" then
        local grid = plantGrid(8)
        local ref = center or origin
        table.sort(grid, function(a, b)
            return (Vector2.new(a.X, a.Z) - Vector2.new(ref.X, ref.Z)).Magnitude < (Vector2.new(b.X, b.Z) - Vector2.new(ref.X, ref.Z)).Magnitude
        end)
        for i = 1, math.min(#grid, 120) do add(grid[i]) end
    else
        add(origin)
        for _, r in ipairs({4, 8, 12, 16, 20, 24}) do
            add(origin + Vector3.new(r, 0, 0)); add(origin + Vector3.new(-r, 0, 0))
            add(origin + Vector3.new(0, 0, r)); add(origin + Vector3.new(0, 0, -r))
            add(origin + Vector3.new(r, 0, r)); add(origin + Vector3.new(-r, 0, r))
            add(origin + Vector3.new(r, 0, -r)); add(origin + Vector3.new(-r, 0, -r))
        end
        if mode == "GARDEN CENTER" then
            local grid = plantGrid(8)
            table.sort(grid, function(a, b)
                return (Vector2.new(a.X, a.Z) - Vector2.new(origin.X, origin.Z)).Magnitude < (Vector2.new(b.X, b.Z) - Vector2.new(origin.X, origin.Z)).Magnitude
            end)
            for i = 1, math.min(#grid, 50) do add(grid[i]) end
        end
    end
