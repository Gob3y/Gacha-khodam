...
local function crateCatalog()
    ...
    pcall(function()
        for _, shopName in ipairs({ "CrateShop", "Crates", "Crate", "GuildCrates", "GuildCrate" }) do
            local shop = ReplicatedStorage:FindFirstChild("StockValues")
            shop = shop and shop:FindFirstChild(shopName)
...
local function buyCrateByName(name)
    ...
    return false, "no crate buy remote"
end
...
loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return S.crateInterval end, function()
    ...
    if S.autoBuyAllCrates then
        ...
        for name in pairs(S.crateBuy) do
...
local function ApplyFPSBoostGraphics()
    ...
    -- Ultra-light mobile mode: do NOT scan the whole map on enable.
    -- Full Workspace:GetDescendants() scans can freeze low-end Android devices on this map.
    -- New client effects are still disabled as they appear.
...
local function IsPlantNamed(obj)
    local name = tostring(obj and obj.Name or "")
    return name == "Plants" or string.sub(name, 1, 6) == "plant_"
end

local function IsInsidePlantObject(obj)
    local cur = obj
    while cur and cur ~= Workspace do
        if IsPlantNamed(cur) then return true end
        cur = cur.Parent
    end
    return false
end
        ...
        for _, plot in ipairs(gardens:GetChildren()) do
            ...
            if plants then
                ...
                for _, child in ipairs(plot:GetChildren()) do
                    if IsPlantNamed(child) then addRoot(child) end
                end
            end
        end
    end
...
addButton(ShopPage, "REFRESH CRATE LIST", function()
    ...
    if crateDropdown and crateDropdown.SetOptions then
        crateDropdown.SetOptions(CRATE_NAMES)
    end
