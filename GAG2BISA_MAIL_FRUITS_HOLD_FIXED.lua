...
function HaimiyachGAG2_NormalFruitMailName(v)
    v = string.lower(HaimiyachGAG2_TrimText(v))
    v = v:gsub("%b[]", " ")
    v = v:gsub("%b()", " ")
    v = v:gsub("%d+%.?%d*%s*[kK][gG]", " ")
    v = v:gsub("%s+[xX]%d+$", " ")
    v = v:gsub("[_%-%|]", " ")
    v = v:gsub("%f[%a]harvested%f[%A]", " ")
    v = v:gsub("%s+", " ")
    v = v:gsub("^%s+", ""):gsub("%s+$", "")
    return v
end

function HaimiyachGAG2_SelectedFruitTextMatch(selected, fruitName)
    if type(selected) ~= "table" then return false end
    local n = HaimiyachGAG2_NormalFruitMailName(fruitName)
    if n == "" then return false end
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local s = HaimiyachGAG2_NormalFruitMailName(sel)
            if s == "all fruits" or s == "all fruit" then
                return true
            end
            if s ~= "" and (s == n or string.find(n, s, 1, true) == 1) then
                return true
            end
        end
    end
...
function HaimiyachGAG2_FruitEntryName(key, value)
    if type(value) == "table" then
        return value.FruitName or value.Name or value.ItemName or value.SeedName or value.Fruit or value.PlantName or tostring(key)
    end
    return tostring(key)
end

function HaimiyachGAG2_IsSelectedFruitEntry(key, value)
    if type(S.mailFruitTargets) ~= "table" or not HaimiyachGAG2_HasSelection(S.mailFruitTargets) then return false end
    return HaimiyachGAG2_SelectedFruitTextMatch(S.mailFruitTargets, HaimiyachGAG2_FruitEntryName(key, value))
end

function HaimiyachGAG2_IsMailFruitTool(tool)
    ...
    if mainCat == "Seed" then return false end
    if mainCat == "Fruit" then return true end
...
function HaimiyachGAG2_MailFruitToolName(tool)
    ...
    local a = tool:GetAttribute("Fruit")
    if type(a) == "string" and a ~= "" then return a end
    a = tool:GetAttribute("FruitName")
    if type(a) == "string" and a ~= "" then return a end
    a = tool:GetAttribute("SeedName")
    if type(a) == "string" and a ~= "" then return a end
    return tostring(tool.Name or "")
end

function HaimiyachGAG2_GetHeldFruitTool()
    local char = LocalPlayer.Character
    if not char then return nil end
    for _, obj in ipairs(char:GetChildren()) do
        if HaimiyachGAG2_IsMailFruitTool(obj) then
            return obj
        end
    end
    return nil
...
function HaimiyachGAG2_MailFruitToolMatchesSelected(tool)
    if not HaimiyachGAG2_IsMailFruitTool(tool) then return false end
    return HaimiyachGAG2_SelectedFruitTextMatch(S.mailFruitTargets, HaimiyachGAG2_MailFruitToolName(tool))
end

function HaimiyachGAG2_FindMailFruitKeyById(fruitId)
    if fruitId == nil then return nil, nil end
    fruitId = tostring(fruitId)
    for key, value in pairs(inv("HarvestedFruits")) do
        if tostring(key) == fruitId then
            return key, value
        end
        if type(value) == "table" and tostring(value.Id or "") == fruitId then
            return key, value
        end
    end
    return nil, nil
...
function HaimiyachGAG2_FindSelectedMailFruitEntry()
    if type(S.mailFruitTargets) ~= "table" or not HaimiyachGAG2_HasSelection(S.mailFruitTargets) then return nil, nil, nil end
    local held = HaimiyachGAG2_GetHeldFruitTool()
    if HaimiyachGAG2_MailFruitToolMatchesSelected(held) then
        local heldId = held:GetAttribute("Id")
        local key, value = HaimiyachGAG2_FindMailFruitKeyById(heldId)
        if key ~= nil then
            return key, value, held
        end
        if heldId ~= nil then
            return tostring(heldId), { Id = heldId, FruitName = HaimiyachGAG2_MailFruitToolName(held) }, held
        end
    end

    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" and value.Id ~= nil and HaimiyachGAG2_IsSelectedFruitEntry(key, value) then
...
function HaimiyachGAG2_FindFruitToolByIdOrSelected(fruitId)
    local char = LocalPlayer.Character
    ...
    local function scan(parent)
        ...
        for _, obj in ipairs(parent:GetChildren()) do
            if HaimiyachGAG2_IsMailFruitTool(obj) then
                ...
                if fruitId ~= nil and tostring(id or "") == tostring(fruitId) then
                    return obj
                end
                if fruitId == nil and HaimiyachGAG2_MailFruitToolMatchesSelected(obj) then
                    return obj
                end
            end
        end
...
function HaimiyachGAG2_RequestPromoteFruit(fruitId)
    if fruitId == nil then return false end
    local ok, fpu = pcall(function()
        return require(ReplicatedStorage.SharedModules.FruitProxyUtil)
    end)
    if ok and fpu then
        pcall(function()
            if type(fpu.Pending) ~= "table" then fpu.Pending = {} end
            fpu.Pending.Equip = tostring(fruitId)
        end)
        if fpu.RequestPromote then
            local ok2 = pcall(function()
                fpu.RequestPromote(tostring(fruitId))
            end)
...
function HaimiyachGAG2_WaitForMailFruitTool(fruitId, timeout)
    timeout = tonumber(timeout) or 3
    local deadline = os.clock() + timeout
    local tool = HaimiyachGAG2_FindFruitToolByIdOrSelected(fruitId)
    while not tool and os.clock() < deadline do
        task.wait(0.10)
        tool = HaimiyachGAG2_FindFruitToolByIdOrSelected(fruitId)
    end
    return tool
end

function HaimiyachGAG2_HoldMailFruitTool(tool, timeout)
    if not HaimiyachGAG2_IsMailFruitTool(tool) then return false, "fruit tool not found" end
    ...
    return false, "could not hold fruit"
end

function HaimiyachGAG2_PrepareSelectedFruitForMail(showNotify)
    if type(S.mailFruitTargets) ~= "table" or not HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        return nil
    end

    local key, value, held = HaimiyachGAG2_FindSelectedMailFruitEntry()
    if key == nil then
        if showNotify ~= false then notify("Mailbox", "Selected fruit not found in Fruit Inventory.", 3) end
        return nil
    end

    local fruitId = nil
    if type(value) == "table" then fruitId = value.Id end
    if fruitId == nil and held then fruitId = held:GetAttribute("Id") end

    local tool = held
    if not HaimiyachGAG2_MailFruitToolMatchesSelected(tool) then
        tool = HaimiyachGAG2_FindFruitToolByIdOrSelected(fruitId)
    end

    if not tool and fruitId ~= nil then
        HaimiyachGAG2_RequestPromoteFruit(fruitId)
        tool = HaimiyachGAG2_WaitForMailFruitTool(fruitId, 4)
    end

    if not tool then
        tool = HaimiyachGAG2_WaitForMailFruitTool(nil, 1)
    end

    if not HaimiyachGAG2_MailFruitToolMatchesSelected(tool) then
        if showNotify ~= false then notify("Mailbox", "Could not hold selected fruit from backpack.", 4) end
        return nil
    end

    local okHold, holdErr = HaimiyachGAG2_HoldMailFruitTool(tool, 2)
    if not okHold then
        if showNotify ~= false then notify("Mailbox", tostring(holdErr or "Could not hold fruit"), 4) end
        return nil
    end

    -- If the held tool has a newer Id than the replica entry, rematch the exact inventory key.
    local heldId = tool:GetAttribute("Id")
    local exactKey, exactValue = HaimiyachGAG2_FindMailFruitKeyById(heldId)
    if exactKey ~= nil then
        key = exactKey
        value = exactValue
    elseif heldId ~= nil and key == nil then
        key = tostring(heldId)
    end
    ...
    return {
        ...
        __Tool = tool,
        __Id = heldId,
        __Name = HaimiyachGAG2_MailFruitToolName(tool)
    }
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    -- Send normal mail items first. Fruits are handled separately because the game expects the fruit to be held/equipped first.
    ...
    if #items > 0 then
        ...
        if ok and success then
            sentCount = sentCount + #items
        else
            firstError = tostring(message or success or "Could not send gift")
        end
        task.wait(0.25)
    end

    local fruitItem = HaimiyachGAG2_PrepareSelectedFruitForMail(showNotify)
    if fruitItem then
        -- Keep fruit in hand at the exact moment SendBatch fires.
        if fruitItem.__Tool then
            HaimiyachGAG2_HoldMailFruitTool(fruitItem.__Tool, 1)
            task.wait(0.15)
        end
        local cleanFruit = {
            Category = "HarvestedFruits",
            ItemKey = tostring(fruitItem.ItemKey),
            Count = 1
        }
        local ok, success, message = pcall(function()
            return send:Fire(userId, { cleanFruit }, note)
        end)
        if ok and success then
            sentCount = sentCount + 1
        else
            firstError = firstError or tostring(message or success or "Could not send fruit")
        end
    end
    ...
    if showNotify ~= false then
        notify("Mailbox", firstError or "No selected gift item found.", 4)
    end
    return false
end
