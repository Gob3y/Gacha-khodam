...
local function sprinklerCatalog()
    local out, seen = {}, {}
    local function add(name)
        name = tostring(name or "")
        if name == "" then return end
        local key = string.lower(name)
        if not string.find(key, "sprinkler", 1, true) then return end
        if not seen[name] then
            seen[name] = true
            out[#out + 1] = name
        end
    end
    for _, name in ipairs(GEAR_NAMES) do add(name) end
    local ok, data = pcall(function() return require(ReplicatedStorage.SharedModules.SprinklerData) end)
    if ok and type(data) == "table" then
        for _, entry in pairs(data) do
            if type(entry) == "table" then
                add(entry.SprinklerName or entry.Name or entry.ItemName)
            end
        end
    end
    ...
    if assets then
        for _, obj in ipairs(assets:GetChildren()) do add(obj.Name) end
    end
    if #out == 0 then
        for _, name in ipairs({"Basic Sprinkler", "Advanced Sprinkler", "Godly Sprinkler"}) do add(name) end
    end
    table.sort(out)
    return out
end
local SPRINKLER_NAMES = sprinklerCatalog()
...
local function HaimiyachGAG2_PivotPosition(inst)
...
local function HaimiyachGAG2_InAreaXZ(area, pos)
    if not (area and area:IsA("BasePart") and pos) then return false end
    local lp = area.CFrame:PointToObjectSpace(pos)
    return math.abs(lp.X) <= area.Size.X * 0.5 and math.abs(lp.Z) <= area.Size.Z * 0.5
end

local function HaimiyachGAG2_ProjectToPlantArea(pos)
    local areas = myPlantAreas()
    if not pos or #areas == 0 then return nil end
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Include
    params.FilterDescendantsInstances = areas
    local hit = Workspace:Raycast(Vector3.new(pos.X, pos.Y + 180, pos.Z), Vector3.new(0, -360, 0), params)
    if hit and hit.Instance and CollectionService:HasTag(hit.Instance, "PlantArea") then
        return hit.Position
    end
    return nil
end

local function HaimiyachGAG2_PlotCenterPosition()
    local plot = myPlot()
    if not plot then return nil end
    local ref = plot:FindFirstChild("PlotSizeReference")
    if ref and ref:IsA("BasePart") then
        local p = HaimiyachGAG2_ProjectToPlantArea(ref.Position)
        if p then return p end
    end
    local areas = myPlantAreas()
    if #areas == 0 then return nil end
    local minX, maxX, minZ, maxZ, y = math.huge, -math.huge, math.huge, -math.huge, 0
    local n = 0
    for _, area in ipairs(areas) do
        n = n + 1
        y = y + area.Position.Y
        local sx, sz = area.Size.X * 0.5, area.Size.Z * 0.5
        for _, dx in ipairs({-sx, sx}) do
            for _, dz in ipairs({-sz, sz}) do
                local w = (area.CFrame * CFrame.new(dx, 0, dz)).Position
                minX = math.min(minX, w.X); maxX = math.max(maxX, w.X)
                minZ = math.min(minZ, w.Z); maxZ = math.max(maxZ, w.Z)
            end
        end
    end
    local raw = Vector3.new((minX + maxX) * 0.5, y / math.max(n, 1), (minZ + maxZ) * 0.5)
    return HaimiyachGAG2_ProjectToPlantArea(raw) or raw
end

local function HaimiyachGAG2_NearestPlantAreaPoint(pos, spacing)
    if not pos then return nil end
    local direct = HaimiyachGAG2_ProjectToPlantArea(pos)
    if direct then return direct end
    local grid = plantGrid(spacing or 4)
    local best, bestD = nil, math.huge
    for _, p in ipairs(grid) do
        local d = (Vector2.new(p.X, p.Z) - Vector2.new(pos.X, pos.Z)).Magnitude
        if d < bestD then best, bestD = p, d end
    end
    return best
end

local function HaimiyachGAG2_TooClose2D(pos, list, dist, ignore)
    dist = dist or 3
    if not pos then return true end
    for _, p in ipairs(list or {}) do
        if p ~= ignore and (Vector2.new(pos.X, pos.Z) - Vector2.new(p.X, p.Z)).Magnitude < dist then return true end
    end
    return false
end
...
function HaimiyachGAG2_ObjectName(obj)
    ...
    for _, attr in ipairs({ "Fruit", "FruitName", "CorePartName", "Plant", "PlantName", "Seed", "SeedName", "Crop", "CropName", "ItemName", "ToolName" }) do
...
local CONFIG_KEYS = {
    ...
    "autoClaimSeedEvent", "claimUseFly", "seedEventDelay", "autoShovelPlants", "autoShovelFruits", "shovelPlantTargets", "shovelFruitTargets", "shovelKeepMutations", "shovelMaxKg", "shovelKgMode", "shovelKgValue", "shovelNameFilter", "shovelDelay", "autoTrowelPlants", "trowelPlantTargets", "trowelPositionMode", "trowelDelay",
    ...
    "autoSprinkler", "sprinklerInterval", "sprinklerTargetMode", "sprinklerTargets", "autoWater", "waterInterval", "autoSkill", "skillStats",
...
if S.sprinklerTargetMode ~= "AVATAR POSITION" and S.sprinklerTargetMode ~= "MAPPING" then S.sprinklerTargetMode = "GARDEN CENTER" end
...
-- // ============================================================ \\ --
-- //             SMART SPRINKLER / WATER / TROWEL HELPERS          \\ --
-- // ============================================================ \\ --
function HaimiyachGAG2_ExistingSprinklerPositions()
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        if not (plants and obj:IsDescendantOf(plants)) then
            local isSprinkler = false
            if obj.GetAttribute then
                isSprinkler = obj:GetAttribute("Sprinkler") ~= nil or obj:GetAttribute("SprinklerName") ~= nil
            end
            if not isSprinkler then
                local nm = string.lower(tostring(obj.Name or ""))
                isSprinkler = string.find(nm, "sprinkler", 1, true) ~= nil
            end
            if isSprinkler then
                local pos = HaimiyachGAG2_PivotPosition(obj)
...
function HaimiyachGAG2_SprinklerToolSelected(tool)
    if not picked(S.sprinklerTargets) then return true end
    if not tool then return false end
    local attrName = tool:GetAttribute("Sprinkler")
    local toolName = tostring(tool.Name or "")
    if HaimiyachGAG2_SelectedNameMatch(attrName, S.sprinklerTargets) then return true end
    if HaimiyachGAG2_SelectedNameMatch(toolName, S.sprinklerTargets) then return true end
    return false
end
...
function HaimiyachGAG2_SelectedSprinklerTools()
    ...
    for _, tool in ipairs(toolsByAttr("Sprinkler")) do
        if HaimiyachGAG2_SprinklerToolSelected(tool) then out[#out + 1] = tool end
    end
    table.sort(out, function(a, b)
        local aa = tostring((a and a:GetAttribute("Sprinkler")) or (a and a.Name) or "")
        local bb = tostring((b and b:GetAttribute("Sprinkler")) or (b and b.Name) or "")
        return string.lower(aa) < string.lower(bb)
    end)
    return out
end

function HaimiyachGAG2_AddSprinklerCandidate(candidates, point)
    if not point then return end
    local projected = HaimiyachGAG2_NearestPlantAreaPoint(point, 4)
    if projected then candidates[#candidates + 1] = projected end
end

function HaimiyachGAG2_AddSprinklerRing(candidates, origin, radius)
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(radius, 0, 0))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(-radius, 0, 0))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(0, 0, radius))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(0, 0, -radius))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(radius, 0, radius))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(-radius, 0, radius))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(radius, 0, -radius))
    HaimiyachGAG2_AddSprinklerCandidate(candidates, origin + Vector3.new(-radius, 0, -radius))
end

function HaimiyachGAG2_SprinklerMappingCandidates(origin)
    local candidates = {}
    local grid = plantGrid(8)
    table.sort(grid, function(a, b)
        local da = (Vector2.new(a.X, a.Z) - Vector2.new(origin.X, origin.Z)).Magnitude
        local db = (Vector2.new(b.X, b.Z) - Vector2.new(origin.X, origin.Z)).Magnitude
        return da < db
    end)
    for i = 1, math.min(#grid, 120) do
        HaimiyachGAG2_AddSprinklerCandidate(candidates, grid[i])
    end
    return candidates
end
...
function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    ...
    local center = HaimiyachGAG2_PlotCenterPosition()
    local origin = center
    if mode == "AVATAR POSITION" then
        local hrp = root()
        origin = hrp and hrp.Position or center
    end
    if not origin then return nil end

    local candidates = {}
    if mode == "MAPPING" then
        candidates = HaimiyachGAG2_SprinklerMappingCandidates(center or origin)
    else
        HaimiyachGAG2_AddSprinklerCandidate(candidates, origin)
        for _, r in ipairs({4, 8, 12, 16, 20, 24}) do
            HaimiyachGAG2_AddSprinklerRing(candidates, origin, r)
        end
        if mode == "GARDEN CENTER" then
            local grid = plantGrid(8)
            table.sort(grid, function(a, b)
                local da = (Vector2.new(a.X, a.Z) - Vector2.new(origin.X, origin.Z)).Magnitude
                local db = (Vector2.new(b.X, b.Z) - Vector2.new(origin.X, origin.Z)).Magnitude
    ...
    for _, p in ipairs(candidates) do
        local k = string.format("%d:%d", math.floor(p.X * 10), math.floor(p.Z * 10))
        if not seen[k] then
            seen[k] = true
            if not HaimiyachGAG2_TooClose2D(p, placed, 10) then return p end
        end
    end
    for _, p in ipairs(candidates) do
        if p then return p end
...
HaimiyachGAG2_WateredCache = HaimiyachGAG2_WateredCache or {}
function HaimiyachGAG2_MarkWatered(plant)
    if plant then HaimiyachGAG2_WateredCache[plant] = os.clock() end
end

function HaimiyachGAG2_PlantNeedsWater(plant)
    if not plant or not plant.Parent then return false end
    for _, attr in ipairs({"NeedsWater", "NeedWater", "NeedWatter", "Dry", "IsDry", "CanWater", "CanBeWatered"}) do
        local v = plant:GetAttribute(attr)
        if v == true then return true end
    end
    for _, attr in ipairs({"Water", "WaterLevel", "Moisture", "MoistureLevel", "Hydration", "HydrationLevel"}) do
        local v = plant:GetAttribute(attr)
        if type(v) == "number" and v <= 0.25 then return true end
    end
    local age, maxAge = plant:GetAttribute("Age"), plant:GetAttribute("MaxAge")
    local growing = type(age) == "number" and type(maxAge) == "number" and age < maxAge
    if not growing then return false end
    local last = HaimiyachGAG2_WateredCache[plant]
    return (not last) or (os.clock() - last >= math.max(20, tonumber(S.waterInterval) or 8))
end

function HaimiyachGAG2_PlantsNeedingWater()
    local out, plot = {}, myPlot()
    local plants = plot and plot:FindFirstChild("Plants")
    if not plants then return out end
    for _, plant in ipairs(plants:GetChildren()) do
        if HaimiyachGAG2_PlantNeedsWater(plant) then
            local pos = HaimiyachGAG2_PivotPosition(plant)
            if pos then
                local target = HaimiyachGAG2_NearestPlantAreaPoint(pos, 4) or pos
                out[#out + 1] = { plant = plant, pos = target }
            end
        end
    end
    table.sort(out, function(a, b)
        local hrp = root(); if not hrp then return false end
        return (a.pos - hrp.Position).Magnitude < (b.pos - hrp.Position).Magnitude
    end)
    return out
end

function HaimiyachGAG2_TrowelPlantNameOk(plant)
...
function HaimiyachGAG2_TrowelTargets()
    local out, plot = {}, myPlot()
    local plants = plot and plot:FindFirstChild("Plants")
    if not plants or not picked(S.trowelPlantTargets) then return out end
    for _, plant in ipairs(plants:GetChildren()) do
        if HaimiyachGAG2_TrowelPlantNameOk(plant) then
            local key = tostring(plant.Name or plant:GetAttribute("PlantId") or "")
            local pos = HaimiyachGAG2_PivotPosition(plant)
            if key ~= "" and pos then out[#out + 1] = { model = plant, key = key, pos = pos } end
        end
    end
    return out
end

function HaimiyachGAG2_TrowelMappingPositions(targets)
    ...
    for _, p in ipairs(existingPlantPositions()) do
        ...
        for _, data in ipairs(targets or {}) do
            if data.pos and (Vector2.new(p.X, p.Z) - Vector2.new(data.pos.X, data.pos.Z)).Magnitude <= 2 then
                isTargetPos = true
                break
            end
        end
        if not isTargetPos then occupied[#occupied + 1] = p end
    end
    local center = HaimiyachGAG2_PlotCenterPosition() or (targets[1] and targets[1].pos)
    table.sort(grid, function(a, b)
        if not center then return a.X < b.X end
        local da = (Vector2.new(a.X, a.Z) - Vector2.new(center.X, center.Z)).Magnitude
        local db = (Vector2.new(b.X, b.Z) - Vector2.new(center.X, center.Z)).Magnitude
        return da < db
    end)
    local out = {}
    for _, p in ipairs(grid) do
        if not HaimiyachGAG2_TooClose2D(p, occupied, math.max(2, tonumber(S.plantSpacing) or 4) * 0.75) then
            out[#out + 1] = p
            occupied[#occupied + 1] = p
            if #out >= #targets then break end
        end
    end
    return out
end

function HaimiyachGAG2_TrowelAvatarPosition(index)
    local hrp = root()
    if not hrp then return nil end
    local offsets = {
        Vector3.new(0,0,0), Vector3.new(4,0,0), Vector3.new(-4,0,0), Vector3.new(0,0,4), Vector3.new(0,0,-4),
        Vector3.new(4,0,4), Vector3.new(-4,0,4), Vector3.new(4,0,-4), Vector3.new(-4,0,-4)
    }
    return HaimiyachGAG2_NearestPlantAreaPoint(hrp.Position + offsets[((index - 1) % #offsets) + 1], 4)
end

function HaimiyachGAG2_AutoTrowelStep()
    if not picked(S.trowelPlantTargets) then return 0 end
    ...
    if S.trowelPositionMode == "AVATAR POSITION" then
        for i = 1, #targets do positions[i] = HaimiyachGAG2_TrowelAvatarPosition(i) end
    else
        positions = HaimiyachGAG2_TrowelMappingPositions(targets)
    end
    for i, data in ipairs(targets) do
        ...
        if pos and (Vector2.new(pos.X, pos.Z) - Vector2.new(data.pos.X, data.pos.Z)).Magnitude > 1.5 then
            ...
            task.wait(math.max(0.4, tonumber(S.trowelDelay) or 3))
        end
    end
    return moved
end
...
S.shovelObjectName = function(obj)
    if not obj then return "" end
    local attrs = { "Fruit", "FruitName", "CorePartName", "Plant", "PlantName", "Seed", "SeedName", "Crop", "CropName", "ItemName", "ToolName" }
    for _, attr in ipairs(attrs) do
        local v = obj:GetAttribute(attr)
        if type(v) == "string" and v ~= "" then return v end
    end
...
addDropdown(UI.Pages["BOOSTS"], "SELECTED SPRINKLERS", SPRINKLER_NAMES, S.sprinklerTargets, true, function(sel) pickMulti(sel, S.sprinklerTargets) end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        catalog = CATALOG, gearNames = GEAR_NAMES, sprinklerNames = SPRINKLER_NAMES, myPlot = myPlot, replica = replica,
