                ...
                pcall(function()
                    if obj:IsA("BasePart") then
                        ...
                        if fpsMode == "ULTRA" then
                            ...
                            local hideVisual = string.find(lowerName, "decor") or string.find(lowerName, "leaf") or string.find(lowerName, "leaves") or string.find(lowerName, "effect") or string.find(lowerName, "vfx") or string.find(lowerName, "fruit") or string.find(lowerName, "crop")
                            ...
                            while cur and cur ~= Workspace do
                                ...
                                -- Hide all plant/fruit visuals in own garden and other gardens.
                                -- This targets workspace.Gardens.PlotX.Plants and Fruits without destroying prompts/tools/controllers.
                                if curName == "plants" or curName == "plant" or curName == "fruits" or curName == "fruit" then
                                    hideVisual = true
                                    break
                                end
                                if (string.find(curName, "plant") and curName ~= "plantarea" and curName ~= "plantareapart" and curName ~= "plantareaparts") or string.find(curName, "fruit") or string.find(curName, "crop") or string.find(curName, "leaf") or string.find(curName, "leaves") or string.find(curName, "stem") then
                                    hideVisual = true
                                    break
                                end
                        ...
                        pcall(function()
                            if obj:IsA("BasePart") then
                                ...
                                if liveMode == "ULTRA" then
                                    ...
                                    obj.Reflectance = 0
                                    local lowerName = string.lower(tostring(obj.Name or ""))
                                    local hideNewVisual = string.find(lowerName, "decor") or string.find(lowerName, "leaf") or string.find(lowerName, "leaves") or string.find(lowerName, "effect") or string.find(lowerName, "vfx") or string.find(lowerName, "fruit") or string.find(lowerName, "crop")
                                    local cur = obj
                                    while cur and cur ~= Workspace do
                                        local curName = string.lower(tostring(cur.Name or ""))
                                        if curName == "plants" or curName == "plant" or curName == "fruits" or curName == "fruit" then
                                        ...
                                        if (string.find(curName, "plant") and curName ~= "plantarea" and curName ~= "plantareapart" and curName ~= "plantareaparts") or string.find(curName, "fruit") or string.find(curName, "crop") or string.find(curName, "leaf") or string.find(curName, "leaves") or string.find(curName, "stem") or string.find(curName, "decor") or string.find(curName, "cosmetic") or string.find(curName, "effect") or string.find(curName, "vfx") then
                                            hideNewVisual = true
                                            break
                                        end
                                        cur = cur.Parent
                                    end
                                    if hideNewVisual then
                                        obj.LocalTransparencyModifier = 1
                                    end
                                end
                            elseif obj:IsA("Decal") or obj:IsA("Texture") then
