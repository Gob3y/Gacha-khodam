...
local function RestoreAfterLoadingScreenBlock()
    local now = os.clock()
    if now - (LoadingScreenBlocker.lastRestore or 0) < 0.35 then return end
...
local function LoadingTextReady(root)
    ...
    pcall(function()
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                ...
                if string.find(text, "tap anywhere")
                    or string.find(text, "press any key")
                    or string.find(text, "click to skip")
                    or string.find(text, "fully loaded")
                    or string.find(text, "ready! 100") then
                    ready = true
                    break
                end
            end
...
local function GetLoadingScreenRoot()
    local root = nil

    pcall(function()
        local direct = Workspace:FindFirstChild("LoadingScreenMenu")
        if direct then
            root = direct
            return
        end
        ...
        for _, child in ipairs(Workspace:GetChildren()) do
            if child.Name == "LoadingScreenMenu" or child:FindFirstChild("LoadingGui", true) then
                if LoadingTextReady(child) then
                    root = child
                    return
                end
            end
    ...
    pcall(function()
        ...
        if pg then
            for _, gui in ipairs(pg:GetChildren()) do
                if gui.Name == "LoadingGui" or gui:FindFirstChild("PressAnyTxt", true) or gui:FindFirstChild("CounterTxt", true) then
                    root = gui
                    return
                end
            end
...
local function DisableActiveLoadingScreen()
    local root = GetLoadingScreenRoot()
    if not root then return false end

    if LoadingTextReady(root) then
        SendLoadingScreenInput()
        RestoreAfterLoadingScreenBlock()
        task.delay(1.2, function()
            if S.disableLoadingScreen and root and root.Parent then
                pcall(function()
                    if root:IsA("ScreenGui") or root:IsA("SurfaceGui") or root:IsA("BillboardGui") then
                        root.Enabled = false
                    end
                end)
...
loopOn(function() return S.disableLoadingScreen end, 0.5, function()
    DisableActiveLoadingScreen()
end)
