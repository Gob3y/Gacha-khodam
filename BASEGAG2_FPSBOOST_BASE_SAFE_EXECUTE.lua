...
-- Safe execute guard: do not auto-run FPS BOOST from old config while the GUI is still loading.
-- Turn FPS BOOST ON manually after the menu appears.
S.fpsBoost = false
...
-- LIGHT    : effect/particle/post effect OFF, visual masih normal.
-- BALANCED : LIGHT + shadow part OFF + terrain/water low + decal/texture diringankan.
-- ULTRA    : BALANCED + SmoothPlastic + texture/decal hidden + garden/decor visual hidden client-side.
...
local function BackupLightingObject(obj)
    if not obj then return end
...
local function CapturePartBackup(obj)
    ...
    if GraphicsBackup.Parts[obj] == nil then
        pcall(function()
            GraphicsBackup.Parts[obj] = {
...
local function CaptureDecalTextureBackup(obj)
    if not obj then return end
    if obj:IsA("Decal") or obj:IsA("Texture") then
        if GraphicsBackup.DecalsTextures[obj] == nil then
            pcall(function()
                GraphicsBackup.DecalsTextures[obj] = { Transparency = obj.Transparency }
            end)
        end
    end
end
...
local function CaptureSurfaceAppearanceBackup(obj)
    ...
    if GraphicsBackup.SurfaceAppearances[obj] == nil then
        pcall(function()
            GraphicsBackup.SurfaceAppearances[obj] = {
...
local function CaptureMeshTextureBackup(obj)
    if not obj then return end
    if obj:IsA("SpecialMesh") then
        if GraphicsBackup.MeshTextures[obj] == nil then
            pcall(function() GraphicsBackup.MeshTextures[obj] = { TextureId = obj.TextureId } end)
        end
...
local function HasNameWord(obj, words)
...
local function IsGardenOrDecorVisual(obj)
    if not obj or IsCharacterObject(obj) then return false end

    local gardens = Workspace:FindFirstChild("Gardens")
    if gardens and obj:IsDescendantOf(gardens) then
        return true
    end

    local temporary = Workspace:FindFirstChild("Temporary")
    if temporary and obj:IsDescendantOf(temporary) then
        return true
    end

    return HasNameWord(obj, {
        "visual",
        "decor",
        "decoration",
        "plant",
...
local function ApplyLightingObjectFPS(obj, fpsMode)
    if not obj then return end
    BackupLightingObject(obj)
    if obj:IsA("PostEffect") then
        SafeSetProperty(obj, "Enabled", false)
...
local function ApplyFPSBoostObject(obj, fpsMode)
    if not obj then return end
    ...
    if obj:IsA("BasePart") then
        ...
        if fpsMode == "ULTRA" then
            ...
            if IsGardenOrDecorVisual(obj) then
                SafeSetProperty(obj, "Transparency", 1)
            end
            CaptureMeshTextureBackup(obj)
            if obj:IsA("MeshPart") then
                SafeSetProperty(obj, "TextureID", "")
            end
        end
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        CaptureDecalTextureBackup(obj)
        if fpsMode == "ULTRA" then
            SafeSetProperty(obj, "Transparency", 1)
        else
            pcall(function()
                local oldValue = tonumber(obj.Transparency) or 0
                obj.Transparency = math.max(oldValue, 0.55)
            end)
        end
    elseif obj:IsA("SurfaceAppearance") then
        CaptureSurfaceAppearanceBackup(obj)
        if fpsMode == "ULTRA" then
            SafeSetProperty(obj, "ColorMap", "")
            SafeSetProperty(obj, "NormalMap", "")
            SafeSetProperty(obj, "MetalnessMap", "")
            SafeSetProperty(obj, "RoughnessMap", "")
        end
...
local function RestoreFPSBoostObjects()
    for obj, values in pairs(GraphicsBackup.Parts) do
        if obj and obj.Parent and values then
            if values.CastShadow ~= nil then SafeSetProperty(obj, "CastShadow", values.CastShadow) end
            if values.Material ~= nil then SafeSetProperty(obj, "Material", values.Material) end
            if values.Transparency ~= nil then SafeSetProperty(obj, "Transparency", values.Transparency) end
            if values.Reflectance ~= nil then SafeSetProperty(obj, "Reflectance", values.Reflectance) end
        end
    end

    for obj, values in pairs(GraphicsBackup.DecalsTextures) do
        if obj and obj.Parent and values and values.Transparency ~= nil then
            SafeSetProperty(obj, "Transparency", values.Transparency)
        end
    end

    for obj, values in pairs(GraphicsBackup.SurfaceAppearances) do
        if obj and obj.Parent and values then
            if values.ColorMap ~= nil then SafeSetProperty(obj, "ColorMap", values.ColorMap) end
            if values.NormalMap ~= nil then SafeSetProperty(obj, "NormalMap", values.NormalMap) end
            if values.MetalnessMap ~= nil then SafeSetProperty(obj, "MetalnessMap", values.MetalnessMap) end
            if values.RoughnessMap ~= nil then SafeSetProperty(obj, "RoughnessMap", values.RoughnessMap) end
        end
    end

    for obj, values in pairs(GraphicsBackup.MeshTextures) do
        if obj and obj.Parent and values then
            if values.TextureId ~= nil then SafeSetProperty(obj, "TextureId", values.TextureId) end
            if values.TextureID ~= nil then SafeSetProperty(obj, "TextureID", values.TextureID) end
        end
    end
end
...
local function ApplyFPSBoostGraphics()
    ...
    for _, obj in ipairs(Lighting:GetDescendants()) do
        ApplyLightingObjectFPS(obj, fpsMode)
    end
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(all) do
            ...
            ApplyFPSBoostObject(obj, fpsMode)
            processed = processed + 1
            if processed % 60 == 0 then
                task.wait()
            end
        end
    end)
    ...
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.2, function()
                if FPSBoostActive and S.fpsBoost then
                    ApplyFPSBoostObject(obj, NormalizeFPSBoostMode(S.fpsBoostMode))
                end
            end)
        end
    end)
    ...
    FPSBoostLightingConnection = Lighting.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.05, function()
                if FPSBoostActive and S.fpsBoost then
                    ApplyLightingObjectFPS(obj, NormalizeFPSBoostMode(S.fpsBoostMode))
                end
            end)
        end
    end)
