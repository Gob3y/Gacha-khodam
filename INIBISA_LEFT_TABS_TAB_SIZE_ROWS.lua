...
local baseW = isMobile and 730 or 820
...
local function createTab(name, order)
    ...
    local btn = new("TextButton", {
        ...
        TextColor3 = T.Text,
        Font = Enum.Font.GothamBold,
        TextSize = (#display > 12) and 9 or 10,
        BorderSizePixel = 0,
...
local function addLabel(page, text)
    page = safePage(page)
    local row = new("Frame", { Size = UDim2.new(1,0,0,30), BackgroundColor3 = T.Row, BorderSizePixel = 0 }, page)
    corner(row, 9)
    local lbl = new("TextLabel", {
...
local function addToggle(page, label, default, callback)
    ...
    corner(hit, 10); stroke(hit, Color3.fromRGB(80,80,80), 1, 0.35)
...
local function addSlider(page, label, min, max, default, precision, callback)
    ...
    local row = new("Frame", { Size = UDim2.new(1,0,0,40), BackgroundColor3 = T.Row, BorderSizePixel = 0 }, page)
    corner(row, 9)
    local txt = new("TextLabel", {
        Size = UDim2.new(0.56, -14, 0, 18),
    ...
    local valLbl = new("TextLabel", {
        Size = UDim2.new(0.44, -14, 0, 18),
        Position = UDim2.new(0.56, 0, 0, 1),
        BackgroundTransparency = 1,
    ...
    corner(fill, 10)
    local knob = new("Frame", { Size = UDim2.fromOffset(14,14), AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(0,0,0.5,0), BackgroundColor3 = T.White, BorderSizePixel = 0 }, bar)
    corner(knob, 8)
...
local function addInput(page, label, placeholder, default, callback)
    page = safePage(page)
    local row = new("Frame", { Size = UDim2.new(1,0,0,34), BackgroundColor3 = T.Row, BorderSizePixel = 0 }, page)
    corner(row, 9)
    new("TextLabel", {
        Size = UDim2.new(0.40, -14, 1, 0),
        Position = UDim2.fromOffset(10, 0),
        BackgroundTransparency = 1,
        Text = cleanUiText(label),
        TextColor3 = T.Text,
    ...
    local box = new("TextBox", {
        Size = UDim2.new(0.60, -16, 0, 24),
        Position = UDim2.new(0.40, 6, 0.5, -12),
...
local function addDropdown(page, label, options, default, multi, callback)
    ...
    local row = new("Frame", { Size = UDim2.new(1,0,0,34), BackgroundColor3 = T.Row, BorderSizePixel = 0, ClipsDescendants = false, ZIndex = dropdownZ }, page)
    corner(row, 9)
    new("TextLabel", {
        Size = UDim2.new(0.40, -14, 0, 34),
        Position = UDim2.fromOffset(10, 0),
        BackgroundTransparency = 1,
        Text = cleanUiText(label),
        TextColor3 = T.Text,
    ...
    new("TextLabel", { Size = UDim2.fromOffset(22, 24), Position = UDim2.new(1, -25, 0, 0), BackgroundTransparency = 1, Text = "v", TextColor3 = T.Muted, Font = Enum.Font.GothamBold, TextSize = 12 }, btn)
    local list = new("ScrollingFrame", {
        Size = UDim2.new(0.60, -16, 0, 0),
        Position = UDim2.new(0.40, 6, 0, 32),
    ...
    local function rebuild()
        ...
        task.defer(function()
            local h = math.min(112, layout.AbsoluteContentSize.Y + 8)
...
UI.worldPetDrop = addDropdown(UI.Pages["PETS_CRATES"], "WORLD PETS TO BUY", HaimiyachGAG2_WorldPetNames(), S.buyWorldPets, true, function(sel)
    if type(S.buyWorldPets) ~= "table" then S.buyWorldPets = {} end
    pickMulti(sel, S.buyWorldPets)
end)
addToggle(UI.Pages["PETS_CRATES"], "AUTO BUY SELECTED WORLD PETS", S.autoBuyPets, function(v) S.autoBuyPets = v end)
