...
-- FPS BOOST keeps gameplay objects intact. ULTRA hides heavy garden visuals client-side only.
...
local function IsUltraHiddenGardenVisual(obj)
    if not obj then return false end
    ...
    if gardens then
        ...
        while cur and cur ~= Workspace do
            if cur.Parent == gardens then
                local scan = obj
                while scan and scan ~= cur.Parent do
                    local n = string.lower(tostring(scan.Name or ""))

                    -- Main heavy folders/models from the visual controllers:
                    -- workspace.Gardens.PlotX.Plants.<PlantModel>.Fruits.<FruitModel>
                    if n == "plants" or n == "plant" or n == "fruits" or n == "fruit" then
                        return true
                    end

                    -- Other garden visuals that are cosmetic/heavy on mobile.
                    if n == "props" or n == "prop" or n == "decor" or n == "decors" or n == "decorations" or n == "cosmetics" then
                        return true
                    end
                    if n == "sprinklers" or n == "sprinkler" or n == "rakes" or n == "rake" then
                        return true
                    end

                    -- Extra safety for generated plant/fruit visual parts/models.
                    if string.find(n, "fruit", 1, true) or string.find(n, "crop", 1, true) then
                        return true
                    end
                    if string.find(n, "plant", 1, true) and n ~= "plantarea" and n ~= "plantareaparts" and n ~= "plantareapart" then
                        return true
                    end
                    if string.find(n, "leaf", 1, true) or string.find(n, "leaves", 1, true) or string.find(n, "stem", 1, true) then
                        return true
                    end
                    if string.find(n, "decor", 1, true) or string.find(n, "cosmetic", 1, true) or string.find(n, "effect", 1, true) or string.find(n, "vfx", 1, true) then
                        return true
                    end
    ...
    if string.find(n, "wateringcanfx", 1, true) or string.find(n, "sprinklerpreview", 1, true) or string.find(n, "sprinklerradius", 1, true) then
        return true
    end
    if string.find(n, "fakeplot", 1, true) or string.find(n, "preview", 1, true) or string.find(n, "radius", 1, true) then
        return true
    end
    if string.find(n, "decor", 1, true) or string.find(n, "cosmetic", 1, true) or string.find(n, "effect", 1, true) or string.find(n, "vfx", 1, true) then
        return true
...
local function ApplyFPSBoostGraphics()
    ...
    -- ULTRA    = BALANCED + smooth plastic + hidden decals + hide all plant/fruit/garden decor visuals client-side.
                ...
                pcall(function()
                    if obj:IsA("BasePart") then
                        ...
                        if fpsMode == "ULTRA" then
                            ...
                            if IsUltraHiddenGardenVisual(obj) then
                    ...
                    if liveMode ~= "LIGHT" then
                        pcall(function()
                            ...
                            elseif obj:IsA("Decal") or obj:IsA("Texture") then
                                if liveMode == "ULTRA" then
                                    obj.Transparency = 1
                                elseif (tonumber(obj.Transparency) or 0) < 0.6 then
                                    obj.Transparency = 0.6
                                end
                            end
                        end)
