...
local S = {
    ...
    hidePlants = false, hidePlantsOwnOnly = false,
...
local CONFIG_KEYS = {
    ...
    "fpsBoost", "highGraphics", "hidePlants", "hidePlantsOwnOnly", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
loadConfig(true)
-- Keep Hide Plants manual-only on startup so saved configs never run plant scans during execute.
S.hidePlants = false
...
-- //                       HIDE PLANTS                            \ --
-- // ============================================================ \ --
-- Roblox-safe visual hide:
-- - Only runs after the HIDE PLANTS toggle is clicked.
-- - Does not delete plants, does not fire remotes, and does not change plant growth.
-- - Uses LocalTransparencyModifier on BasePart, so it stays client-side.
local HidePlantsCache = {}
local HidePlantsConnection = nil

local function HidePlantsGetContainers()
    local out = {}
    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then return out end

    if S.hidePlantsOwnOnly then
        local plot = myPlot()
        local plants = plot and plot:FindFirstChild("Plants")
        if plants then out[#out + 1] = plants end
        return out
    end
...
local function HidePlantsIsInsidePlantsFolder(obj)
    local node = obj
    while node and node ~= Workspace do
        if node.Name == "Plants" then
            return true
        end
        node = node.Parent
    end
...
local function HidePlantsCanTouch(obj)
    if not obj then return false end
    if S.hidePlantsOwnOnly then
        local plot = myPlot()
        return plot and obj:IsDescendantOf(plot)
    end
    local gardens = Workspace:FindFirstChild("Gardens")
    return gardens and obj:IsDescendantOf(gardens)
end

local function HidePlantsApplyPart(part, hide)
    if not part or not part:IsA("BasePart") then return end

    if hide then
        if HidePlantsCache[part] == nil then
            local ok, oldValue = pcall(function()
                return part.LocalTransparencyModifier
            end)
            HidePlantsCache[part] = ok and oldValue or 0
        end
        SafeSetProperty(part, "LocalTransparencyModifier", 1)
    else
        local oldValue = HidePlantsCache[part]
        if oldValue == nil then oldValue = 0 end
        SafeSetProperty(part, "LocalTransparencyModifier", oldValue)
        HidePlantsCache[part] = nil
    end
end

local function HidePlantsApplyObject(obj, hide)
    if not obj then return end

    if obj:IsA("BasePart") then
        HidePlantsApplyPart(obj, hide)
        return
    end
    ...
    if obj:IsA("Model") or obj:IsA("Folder") then
        for _, desc in ipairs(obj:GetDescendants()) do
            if desc:IsA("BasePart") then
                HidePlantsApplyPart(desc, hide)
            end
        end
    end
end

local function HidePlantsRestoreAll()
    for part, oldValue in pairs(HidePlantsCache) do
        if part and part.Parent then
            SafeSetProperty(part, "LocalTransparencyModifier", oldValue or 0)
        end
        HidePlantsCache[part] = nil
    end
end

local function HidePlantsApplyCurrent()
    if not S.hidePlants then
        HidePlantsRestoreAll()
        return
    end

    for _, plants in ipairs(HidePlantsGetContainers()) do
        for _, plant in ipairs(plants:GetChildren()) do
            HidePlantsApplyObject(plant, true)
        end
    end
end

local function HidePlantsDisconnectListener()
...
local function HidePlantsSetupListener()
    HidePlantsDisconnectListener()

    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then return end

    HidePlantsConnection = gardens.DescendantAdded:Connect(function(obj)
        if not S.hidePlants then return end
        if not HidePlantsCanTouch(obj) then return end
        if not HidePlantsIsInsidePlantsFolder(obj) then return end

        task.defer(function()
            if S.hidePlants and obj and obj.Parent then
                HidePlantsApplyObject(obj, true)
            end
        end)
    end)
end
...
local function SetHidePlantsEnabled(state)
    ...
    if S.hidePlants then
        HidePlantsApplyCurrent()
        HidePlantsSetupListener()
    else
        HidePlantsDisconnectListener()
        HidePlantsRestoreAll()
    end
...
local function RefreshHidePlants()
    HidePlantsRestoreAll()
    if S.hidePlants then
        HidePlantsApplyCurrent()
        HidePlantsSetupListener()
...
addToggle(VisualsPage, "HIDE OWN PLANT ONLY", S.hidePlantsOwnOnly, function(v)
    S.hidePlantsOwnOnly = v and true or false
    RefreshHidePlants()
end)
addButton(VisualsPage, "REFRESH HIDE PLANTS", function()
    RefreshHidePlants()
    notify("Hide Plants", "Visibility refreshed", 3)
end)
...
addButton(SettingsPage, "UNLOAD HUB", function()
    S.killed = true
    UI.Unloaded = true
    pcall(function() SetHidePlantsEnabled(false) end)
