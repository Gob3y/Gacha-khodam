...
-- // ============================================================ \\ --
-- //        MAILBOX FRUITS FIX - USE BACKPACK ESP FRUIT LOGIC     \\ --
-- // ============================================================ \\ --
-- Fruit mail must follow the same source used by BACKPACK ESP:
-- actual fruit tools/proxies in Backpack + Character, matched with
-- Inventory.HarvestedFruits by Id, then held before SendBatch.

HaimiyachGAG2_OriginalBuildMailItems = HaimiyachGAG2_OriginalBuildMailItems or HaimiyachGAG2_BuildMailItems

function HaimiyachGAG2_MailFruitSelectedAny()
    if type(S.mailFruitTargets) ~= "table" then return false end
    for name, enabled in pairs(S.mailFruitTargets) do
        if enabled == true then
            local n = HaimiyachGAG2_NormalMailName(name)
            if n == "all fruits" or n == "all fruit" or n == "fruits" or n == "fruit" then
                return true
            end
        end
    end
...
function HaimiyachGAG2_MailFruitSelectedOk(name)
    if type(S.mailFruitTargets) ~= "table" or not HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        return false
    end
    if HaimiyachGAG2_MailFruitSelectedAny() then return true end
    return HaimiyachGAG2_SelectedTextMatch(S.mailFruitTargets, name)
end

function HaimiyachGAG2_MailSelectedFruitText()
    if type(S.mailFruitTargets) ~= "table" then return "NONE" end
    local out = {}
    for name, enabled in pairs(S.mailFruitTargets) do
        if enabled == true then out[#out + 1] = tostring(name) end
    end
    table.sort(out)
    return if #out > 0 then table.concat(out, ", ") else "NONE"
end

function HaimiyachGAG2_MailIsFruitTool(item)
    if not item then return false end
    if item.GetAttribute == nil then return false end
    if item:GetAttribute("SeedTool") ~= nil or item:GetAttribute("Seed") ~= nil then return false end
    if tostring(item.Name or ""):lower():find("seed") and item:GetAttribute("Fruit") == nil and item:GetAttribute("HarvestedFruit") == nil then
        return false
    end
    if BackpackESP and BackpackESP.isFruitItem and BackpackESP.isFruitItem(item) then return true end
    if item:GetAttribute("HarvestedFruit") ~= nil then return true end
    if item:GetAttribute("Fruit") ~= nil then return true end
    if item:GetAttribute("FruitName") ~= nil then return true end
    if item:GetAttribute("FruitProxy") ~= nil then return true end
...
function HaimiyachGAG2_MailFruitToolName(item)
    if not item then return nil end
    if BackpackESP and BackpackESP.itemName then
        local ok, name = pcall(function() return BackpackESP.itemName(item) end)
        if ok and type(name) == "string" and name ~= "" then return name end
    end
    for _, attr in ipairs({ "FruitName", "Fruit", "CorePartName", "SeedName", "Name" }) do
        local v = item:GetAttribute(attr)
        if type(v) == "string" and v ~= "" then return v end
    end
    local parsed = tostring(item.Name or ""):match("^([^%[]+)") or tostring(item.Name or "")
    parsed = HaimiyachGAG2_TrimText(parsed:gsub("%d+%.?%d*%s*[kK][gG]", ""))
    if parsed ~= "" then return parsed end
    return nil
end

function HaimiyachGAG2_MailFruitToolKg(item)
    if not item then return nil end
    if BackpackESP and BackpackESP.itemKg then
        local ok, kg = pcall(function() return BackpackESP.itemKg(item) end)
        if ok and tonumber(kg) then return tonumber(kg) end
    end
    return HaimiyachGAG2_ObjectKg(item)
end

function HaimiyachGAG2_MailFruitToolId(item)
    if not (item and item.GetAttribute) then return nil end
    for _, attr in ipairs({ "Id", "UUID", "Uuid", "FruitId", "InventoryId", "ItemId", "HarvestedFruit" }) do
        local v = item:GetAttribute(attr)
        if type(v) == "string" and v ~= "" then return v end
    end
    return nil
...
function HaimiyachGAG2_MailFindHeldFruit()
    local char = LocalPlayer and LocalPlayer.Character
    if not char then return nil end
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") and HaimiyachGAG2_MailIsFruitTool(item) then
            local name = HaimiyachGAG2_MailFruitToolName(item)
            if name and HaimiyachGAG2_MailFruitSelectedOk(name) then
                return item
            end
        end
    end
...
function HaimiyachGAG2_MailInventoryFruitName(key, value)
    if type(value) == "table" then
        return value.FruitName or value.Name or value.ItemName or value.SeedName or tostring(key)
    end
    return tostring(key)
end

function HaimiyachGAG2_MailFindInventoryFruitById(id, fallbackName)
    local fruits = inv("HarvestedFruits")
    id = if id ~= nil then tostring(id) else nil
    local fallbackKey, fallbackValue = nil, nil
    for key, value in pairs(fruits) do
        if type(value) == "table" and value.Id ~= nil then
            local valueId = tostring(value.Id)
            local name = HaimiyachGAG2_MailInventoryFruitName(key, value)
            if id and (valueId == id or tostring(key) == id) then
                return key, value
            end
            if fallbackName and HaimiyachGAG2_SelectedTextMatch({ [fallbackName] = true }, name) then
                fallbackKey, fallbackValue = key, value
            end
        end
    end
...
function HaimiyachGAG2_MailFindFruitToolById(id)
    if id == nil then return nil end
    id = tostring(id)
    local function scan(container)
        if not container then return nil end
        for _, item in ipairs(container:GetChildren()) do
            if HaimiyachGAG2_MailIsFruitTool(item) then
                local itemId = HaimiyachGAG2_MailFruitToolId(item)
                if itemId and tostring(itemId) == id then return item end
            end
        end
        return nil
    end
    return scan(LocalPlayer and LocalPlayer.Character) or scan(LocalPlayer and LocalPlayer:FindFirstChildOfClass("Backpack"))
end

function HaimiyachGAG2_MailFindFruitToolByName(name, kg)
    local best, bestDiff = nil, math.huge
    local function scan(container)
        if not container then return end
        for _, item in ipairs(container:GetChildren()) do
            if HaimiyachGAG2_MailIsFruitTool(item) then
                local itemName = HaimiyachGAG2_MailFruitToolName(item)
                if itemName and HaimiyachGAG2_SelectedTextMatch({ [name] = true }, itemName) then
                    local diff = 0
                    local itemKg = HaimiyachGAG2_MailFruitToolKg(item)
                    if kg and itemKg then diff = math.abs(itemKg - kg) end
                    if not kg or not itemKg or diff < bestDiff then
                        best = item
                        bestDiff = diff
                    end
                end
...
function HaimiyachGAG2_MailBackpackFruitCandidates()
    local out = {}
    local seen = {}

    -- Use EXACT same fruit source as Backpack ESP value: collectItems() from Backpack + Character.
    if BackpackESP and BackpackESP.collectItems then
        local ok, data = pcall(function() return BackpackESP.collectItems() end)
        if ok and type(data) == "table" and type(data.list) == "table" then
            for _, rec in ipairs(data.list) do
                if rec and rec.item and HaimiyachGAG2_MailIsFruitTool(rec.item) then
                    local name = rec.name or HaimiyachGAG2_MailFruitToolName(rec.item)
                    if name and HaimiyachGAG2_MailFruitSelectedOk(name) then
                        local id = HaimiyachGAG2_MailFruitToolId(rec.item)
                        local key, value = HaimiyachGAG2_MailFindInventoryFruitById(id, name)
                        local unique = tostring(id or key or tostring(rec.item))
                        if not seen[unique] then
                            seen[unique] = true
                            out[#out + 1] = { name = name, id = id, key = key, value = value, tool = rec.item, kg = rec.kg, source = "BackpackESP" }
                        end
                    end
                end
            end
    ...
    -- Replica fallback: same logic as MailboxController rebuildInventory for HarvestedFruits.
    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" and value.Id ~= nil then
            local name = HaimiyachGAG2_MailInventoryFruitName(key, value)
            if HaimiyachGAG2_MailFruitSelectedOk(name) then
                local id = tostring(value.Id)
                local unique = tostring(id or key)
                if not seen[unique] then
                    seen[unique] = true
                    out[#out + 1] = { name = name, id = id, key = key, value = value, tool = HaimiyachGAG2_MailFindFruitToolById(id), source = "Inventory" }
                end
            end
        end
    end
...
function HaimiyachGAG2_MailPromoteFruit(id)
    if not id then return false end
    local ok = pcall(function()
        local sm = ReplicatedStorage and ReplicatedStorage:FindFirstChild("SharedModules")
        local fpModule = sm and sm:FindFirstChild("FruitProxyUtil")
        if not fpModule then return end
        local fp = require(fpModule)
        if type(fp) ~= "table" then return end
        if type(fp.Pending) == "table" then
            fp.Pending.Equip = tostring(id)
        end
        if type(fp.RequestPromote) == "function" then
            fp.RequestPromote(tostring(id))
        end
    end)
    return ok
end

function HaimiyachGAG2_MailHoldFruit(candidate)
    if type(candidate) ~= "table" then return nil, "bad fruit candidate" end
    local char = LocalPlayer and LocalPlayer.Character
    local backpack = LocalPlayer and LocalPlayer:FindFirstChildOfClass("Backpack")
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if not (char and backpack and humanoid) then return nil, "character/backpack not ready" end

    local tool = candidate.tool
    if not tool and candidate.id then tool = HaimiyachGAG2_MailFindFruitToolById(candidate.id) end
    if not tool and candidate.name then tool = HaimiyachGAG2_MailFindFruitToolByName(candidate.name, candidate.kg) end

    if tool and tool.Parent == char and HaimiyachGAG2_MailIsFruitTool(tool) then
        return tool, nil
    end
    ...
    if tool and tool:IsA("Tool") and tool.Parent == backpack then
        ...
        repeat
            if tool.Parent == char then return tool, nil end
            task.wait(0.08)
    ...
    if candidate.id then
        pcall(function() humanoid:UnequipTools() end)
        HaimiyachGAG2_MailPromoteFruit(candidate.id)
        local untilTime = os.clock() + 4
        repeat
            local t = HaimiyachGAG2_MailFindFruitToolById(candidate.id)
            if t and t:IsA("Tool") then
                if t.Parent == backpack then
                    pcall(function() humanoid:EquipTool(t) end)
                elseif t.Parent ~= char then
                    pcall(function() t.Parent = char end)
                end
                task.wait(0.08)
                if t.Parent == char then return t, nil end
            end
            task.wait(0.1)
        until os.clock() > untilTime
    end

    return nil, "could not hold fruit: " .. tostring(candidate.name or candidate.id or "?")
end

function HaimiyachGAG2_BuildMailItemsNoFruit()
    local old = HaimiyachGAG2_OriginalBuildMailItems or HaimiyachGAG2_BuildMailItems
    local raw = old()
    local out = {}
    for _, item in ipairs(raw) do
        if tostring(item.Category) ~= "HarvestedFruits" then
            out[#out + 1] = item
        end
    end
    return out
...
function HaimiyachGAG2_SendFruitMailOne(userId, candidate, note, showNotify)
    if type(candidate) ~= "table" then return false, "bad candidate" end

    -- If player already holds the selected fruit manually, use that first.
    local held = HaimiyachGAG2_MailFindHeldFruit()
    if held then
        local heldName = HaimiyachGAG2_MailFruitToolName(held)
        local heldId = HaimiyachGAG2_MailFruitToolId(held)
        if heldName and HaimiyachGAG2_MailFruitSelectedOk(heldName) then
            candidate.tool = held
            candidate.name = heldName
            candidate.id = heldId or candidate.id
        end
    end

    local tool, holdErr = HaimiyachGAG2_MailHoldFruit(candidate)
    if not tool then return false, holdErr or "fruit hold failed" end

    local heldName = HaimiyachGAG2_MailFruitToolName(tool) or candidate.name
    local heldId = HaimiyachGAG2_MailFruitToolId(tool) or candidate.id
    local key, value = HaimiyachGAG2_MailFindInventoryFruitById(heldId, heldName)
    key = key or candidate.key or heldId

    if not key then
        return false, "fruit ItemKey not found: " .. tostring(heldName or candidate.name or "?")
    end

    local send = action("Mailbox.SendBatch")
    if not (send and send.Fire) then return false, "SendBatch remote not found" end
    ...
    local lastMsg = nil
    for _, itemKey in ipairs(tryKeys) do
        local payload = { { Category = "HarvestedFruits", ItemKey = itemKey, Count = 1 } }
        local ok, success, message = pcall(function()
            return send:Fire(userId, payload, note)
        end)
        if ok and success then
            return true, message or ("Fruit sent: " .. tostring(heldName or itemKey))
        end
        lastMsg = tostring(message or success or "send failed") .. " | key=" .. tostring(itemKey)
        task.wait(0.25)
    end

    return false, lastMsg or "fruit send failed"
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    local normalItems = HaimiyachGAG2_BuildMailItemsNoFruit()
    if #normalItems > 0 then
        local send = action("Mailbox.SendBatch")
        if not (send and send.Fire) then
            lastError = "SendBatch remote not found"
        else
            local ok, success, message = pcall(function()
                return send:Fire(userId, normalItems, note)
            end)
            if ok and success then
                sentCount = sentCount + #normalItems
            else
                lastError = tostring(message or success or "Could not send gift")
            end
        end
    end

    if type(S.mailFruitTargets) == "table" and HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        local candidates = HaimiyachGAG2_MailBackpackFruitCandidates()
        if #candidates == 0 then
            lastError = "No fruit matched from BackpackESP/Inventory: " .. HaimiyachGAG2_MailSelectedFruitText()
        else
            local limit = math.floor(tonumber(S.mailSendCount) or 1)
            if limit < 1 then limit = 1 end
            if limit > 20 then limit = 20 end
            for i, candidate in ipairs(candidates) do
                if i > limit then break end
                local ok, msg = HaimiyachGAG2_SendFruitMailOne(userId, candidate, note, showNotify)
                if ok then
    ...
    if sentCount > 0 then
        if showNotify ~= false then
            local mailText = "Mail sent to @" .. tostring(targetName) .. " (" .. tostring(sentCount) .. " item)"
            notify("Mailbox", mailText, 4)
            pcall(function()
                local nc = require(LocalPlayer.PlayerScripts.Controllers.NotificationController)
                if nc and nc.CreateNotification then nc:CreateNotification(mailText) end
    ...
    if showNotify ~= false then notify("Mailbox", tostring(lastError or "No selected gift item found."), 4) end
    return false
end
