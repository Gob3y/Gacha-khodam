...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    if ok and success then
        if showNotify ~= false then
            notify("Mailbox", "Mail sent to @" .. tostring(targetName) .. " (" .. tostring(#items) .. " item)", 4)
        end
        return true
    end
...
function HaimiyachGAG2_CrateNames()
    local list, seen = {}, {}
    pcall(function()
        local m = ReplicatedStorage.SharedModules:FindFirstChild("CrateData")
        local data = m and require(m)
        if type(data) == "table" and type(data.GetAllCrates) == "function" then
            for _, e in pairs(data.GetAllCrates()) do
                if type(e) == "table" then
                    HaimiyachGAG2_AddUniqueName(list, seen, e.Name or e.CrateName or e.ItemName)
                elseif type(e) == "string" then
                    HaimiyachGAG2_AddUniqueName(list, seen, e)
                end
            end
        end
    end)
...
function HaimiyachGAG2_SeedPackNames()
    local list, seen = {}, {}
    pcall(function()
        local m = ReplicatedStorage.SharedModules:FindFirstChild("SeedPackData")
        local data = m and require(m)
        if type(data) == "table" then
            for key, e in pairs(data) do
                if type(e) == "table" then
                    HaimiyachGAG2_AddUniqueName(list, seen, e.Name or e.SeedPackName or e.ItemName or key)
                elseif type(key) == "string" then
                    HaimiyachGAG2_AddUniqueName(list, seen, key)
                end
            end
        end
    end)
...
local AutoReconnectBusy = false
local LastReconnectReason = ""
local LastReconnectAt = 0

local function triggerAutoReconnect(reason, detail)
    if S.killed or not S.autoReconnect or AutoReconnectBusy then return false end
    local now = os.clock()
    local reasonText = tostring(reason or "Disconnect detected")
    if LastReconnectReason == reasonText and (now - LastReconnectAt) < 10 then return false end
    LastReconnectReason = reasonText
    LastReconnectAt = now
        ...
        if GuiService and GuiService.ErrorMessageChanged then
            GuiService.ErrorMessageChanged:Connect(function(message)
                ...
                if text ~= "" then
                    ...
                    triggerAutoReconnect("ROBLOX ERROR MESSAGE", text)
                end
            end)
        end
    end)
        ...
        if TeleportService and TeleportService.TeleportInitFailed then
            TeleportService.TeleportInitFailed:Connect(function(player, result, errorMessage)
                ...
                if player == LocalPlayer then
                    ...
                    triggerAutoReconnect("TELEPORT FAILED", detail)
                end
            end)
        end
    end)
    ...
    task.spawn(function()
        ...
        local function inspect(inst)
            ...
            if text ~= "" and looksLikeDisconnect(text) then
                ...
                triggerAutoReconnect("ROBLOX ERROR PROMPT", text)
            end
        end
        pcall(function()
...
addDropdown(UI.Pages["MAILBOX"], "PROPS TO SEND", HaimiyachGAG2_InventoryOptionNames("Props", nil), S.mailPropTargets, true, function(sel)
