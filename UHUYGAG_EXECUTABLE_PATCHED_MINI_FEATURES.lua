...
local CONFIG_KEYS = {
    ...
    "autoClaimSeedEvent", "claimUseFly", "autoShovelPlants", "autoShovelFruits", "shovelNameFilter", "shovelDelay",
    ...
    "autoGear", "gearBuy", "gearInterval", "autoBuyAllSeeds", "autoBuyAllGear", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
...
-- // ============================================================ \\ --
-- //                       SHOVEL (plants / fruits)              \\ --
-- // ============================================================ \\ --
local function shovelFilterOk(obj)
    local f = string.lower(tostring(S.shovelNameFilter or ""))
    if f == "" then return true end
    if not obj then return false end
    local n = string.lower(tostring(obj.Name or ""))
    if string.find(n, f, 1, true) then return true end
    for _, a in ipairs({ "Seed", "SeedTool", "Fruit", "Plant", "Name" }) do
        local v = obj:GetAttribute(a)
        if v and string.find(string.lower(tostring(v)), f, 1, true) then return true end
    end
    return false
end

local function shovelTool()
    return equipByAttr("Shovel")
end

local function shovelUse(plantId, fruitId)
    local tool = shovelTool()
    if not (tool and plantId) then return false end
    local shovelName = tool:GetAttribute("Shovel") or tool.Name
    local ok = fire("Shovel.UseShovel", tostring(plantId), tostring(fruitId or ""), shovelName, tool)
    if ok then task.wait(0.25) end
    return ok
end
...
local function shovelPlants()
    ...
    for _, plant in ipairs(plants:GetChildren()) do
        if shovelFilterOk(plant) then
            local pid = plant:GetAttribute("PlantId")
            if not pid then
                local n = tostring(plant.Name or "")
                pid = n:match("^([^_]+)")
            end
            if pid and tostring(pid) ~= "" then
                out[#out + 1] = { plantId = tostring(pid), fruitId = "" }
            end
        end
    end
    return out
...
local function shovelFruits()
    ...
    for _, pr in ipairs(CollectionService:GetTagged("HarvestPrompt")) do
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            ...
            if pid then
                local uid = tonumber(m:GetAttribute("UserId"))
                if (uid == nil or uid == LocalPlayer.UserId) and shovelFilterOk(m) then
                    out[#out + 1] = { plantId = tostring(pid), fruitId = tostring(m:GetAttribute("FruitId") or "") }
                end
            end
        end
...
loopOn(function() return S.autoShovelPlants or S.autoShovelFruits end, function() return math.max(1, tonumber(S.shovelDelay) or 2) end, function()
    if not shovelTool() then return end
    if S.autoShovelFruits then
        for _, item in ipairs(shovelFruits()) do
            if not S.autoShovelFruits then break end
            shovelUse(item.plantId, item.fruitId)
            task.wait(math.max(0.5, tonumber(S.shovelDelay) or 2))
        end
    end
    if S.autoShovelPlants then
        for _, item in ipairs(shovelPlants()) do
            if not S.autoShovelPlants then break end
            shovelUse(item.plantId, "")
            task.wait(math.max(0.5, tonumber(S.shovelDelay) or 2))
        end
    end
end)
...
loopOn(function() return S.autoBuyAllSeeds end, function() return math.max(1, tonumber(S.buyInterval) or 5) end, function()
    for _, s in ipairs(CATALOG) do
        if not S.autoBuyAllSeeds then break end
        local stock = stockOf("SeedShop", s.name)
        local limit = stock
        if limit == nil then limit = tonumber(S.buyPerTick) or 8 end
        limit = math.max(0, math.min(limit, tonumber(S.buyPerTick) or 8))
        for i = 1, limit do
            if not S.autoBuyAllSeeds then break end
...
loopOn(function() return S.autoBuyAllGear end, function() return math.max(2, tonumber(S.gearInterval) or 10) end, function()
    for _, name in ipairs(GEAR_NAMES) do
        if not S.autoBuyAllGear then break end
        local stock = stockOf("GearShop", name)
        local limit = stock
...
local MiniBar = new("Frame", {
    ...
    Size = UDim2.fromOffset(math.floor((math.min(baseW * 0.62, 420) * UI.Scale) + 0.5), math.floor((42 * UI.Scale) + 0.5)),
...
local function setMinimized(v)
    ...
    if UI.Minimized then
        MiniBar.Size = UDim2.fromOffset(math.floor((math.min(baseW * 0.62, 420) * UI.Scale) + 0.5), math.floor((42 * UI.Scale) + 0.5))
...
addInput(FarmPage, "SHOVEL FILTER", "Apple / Strawberry / empty = all", S.shovelNameFilter, function(v) S.shovelNameFilter = tostring(v or "") end)
...
addSlider(SettingsPage, "UI SCALE", 0.6, 1.4, UI.Scale, 2, function(v)
    ...
    UIScaleObj.Scale = v
    if UI.Minimized then
        MiniBar.Size = UDim2.fromOffset(math.floor((baseW * UI.Scale) + 0.5), math.floor((54 * UI.Scale) + 0.5))
    end
end)
