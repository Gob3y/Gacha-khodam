...
local S = {
    ...
    codeText = "", autoCodes = false, antiAfk = true, disableCutscene = false, disableLoading = false,
    -- perf / webhook
...
local CONFIG_KEYS = {
    ...
    "autoMail", "autoAcceptGift", "autoHop", "hopInterval", "codeText", "autoCodes", "antiAfk", "disableCutscene", "disableLoading",
...
local function LoadingSaveAndSet(inst, prop, value)
    if not inst then return end
    pcall(function()
        if LoadingBlocker.states[inst] == nil then
            LoadingBlocker.states[inst] = { prop = prop, value = inst[prop] }
        end
        inst[prop] = value
    end)
end

local function LoadingTextMatch(txt)
    txt = string.lower(tostring(txt or ""))
    if txt == "" then return false end
    if string.find(txt, "spawning garden", 1, true) then return true end
    if string.find(txt, "loading garden", 1, true) then return true end
    if string.find(txt, "loading", 1, true) and string.find(txt, "garden", 1, true) then return true end
    if string.find(txt, "gardenloading", 1, true) then return true end
    return false
end

local function LoadingNameMatch(name)
    name = string.lower(tostring(name or ""))
    if name == "" then return false end
    if string.find(name, "loadingscreen", 1, true) then return true end
    if string.find(name, "loading_screen", 1, true) then return true end
    if string.find(name, "gardenloading", 1, true) then return true end
    if string.find(name, "loadingbar", 1, true) then return true end
    if string.find(name, "progressbar", 1, true) then return true end
    return false
end

local function GetScreenGuiAncestor(obj)
    local cur = obj
...
local function HideLoadingObject(obj)
    if not obj then return end
    local sg = GetScreenGuiAncestor(obj)
    if sg and sg.Name ~= "HAIMIYACH_HUB_GUI" and sg.Name ~= "HAIMIYACH_SHOW_BUTTON_GUI" then
        LoadingSaveAndSet(sg, "Enabled", false)
        return
    end
    if obj:IsA("ScreenGui") then
        LoadingSaveAndSet(obj, "Enabled", false)
    elseif obj:IsA("GuiObject") then
        LoadingSaveAndSet(obj, "Visible", false)
    end
end

local function IsLikelyLoadingGui(gui)
    if not gui then return false end
    if LoadingNameMatch(gui.Name) then return true end
    local strong = false
    local markers = 0
    pcall(function()
        for _, d in ipairs(gui:GetDescendants()) do
            if LoadingNameMatch(d.Name) then
                markers = markers + 1
            end
            if (d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox")) and LoadingTextMatch(d.Text) then
                strong = true
                markers = markers + 2
            end
            if markers >= 2 or strong then
                break
            end
        end
    end)
    return strong or markers >= 2
end

local function HideLoadingScreenNow()
    pcall(function()
    ...
    pcall(function()
        for _, gui in ipairs(pg:GetChildren()) do
            if gui:IsA("ScreenGui") and IsLikelyLoadingGui(gui) then
                HideLoadingObject(gui)
            end
        end
        for _, d in ipairs(pg:GetDescendants()) do
            if LoadingNameMatch(d.Name) then
                HideLoadingObject(d)
            elseif (d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox")) and LoadingTextMatch(d.Text) then
                HideLoadingObject(d)
            end
        end
    end)
...
local function RestoreLoadingScreen()
    for inst, data in pairs(LoadingBlocker.states) do
        if inst and inst.Parent and type(data) == "table" then
            pcall(function()
                inst[data.prop] = data.value
            end)
        end
        LoadingBlocker.states[inst] = nil
    end
end

local function SetLoadingScreenDisabled(enabled, silent)
    S.disableLoading = enabled and true or false
    if S.disableLoading then
        HideLoadingScreenNow()
        if not silent then notify("Loading Screen", "Loading screen disabled.", 3) end
    else
        RestoreLoadingScreen()
        if not silent then notify("Loading Screen", "Loading screen enabled.", 3) end
    end
end

loopOn(function() return S.disableLoading end, 0.25, function()
    HideLoadingScreenNow()
end)
...
local function ApplyFPSBoostGraphics()
    ...
    -- ULTRA    = BALANCED + smooth plastic + hidden decals + hide other gardens/decor visuals.
    ...
    local fpsOwnPlot = nil
    pcall(function()
        local gardens = Workspace:FindFirstChild("Gardens")
        local plotId = LocalPlayer and LocalPlayer:GetAttribute("PlotId")
        if gardens and plotId then
            fpsOwnPlot = gardens:FindFirstChild("Plot" .. tostring(plotId))
        end
    end)
            ...
            if fpsMode ~= "LIGHT" then
                pcall(function()
                    if obj:IsA("BasePart") then
                        ...
                        if fpsMode == "ULTRA" then
                            obj.Material = Enum.Material.SmoothPlastic
                            obj.Reflectance = 0

                            local lowerName = string.lower(tostring(obj.Name or ""))
                            local hideVisual = string.find(lowerName, "decor") or string.find(lowerName, "leaf") or string.find(lowerName, "leaves") or string.find(lowerName, "effect") or string.find(lowerName, "vfx")

                            local cur = obj
                            local gardens = Workspace:FindFirstChild("Gardens")
                            while cur and cur ~= Workspace do
                                local curName = string.lower(tostring(cur.Name or ""))
                                if string.find(curName, "decor") or string.find(curName, "cosmetic") or string.find(curName, "effect") or string.find(curName, "vfx") then
                                    hideVisual = true
                                    break
                                end
                                if gardens and cur.Parent == gardens then
                                    if fpsOwnPlot and cur ~= fpsOwnPlot then
                                        hideVisual = true
                                    end
                                    break
                                end
                    ...
                    elseif obj:IsA("Decal") or obj:IsA("Texture") then
                        if fpsMode == "ULTRA" then
                            ...
                            if currentTransparency < 0.45 then
    ...
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost then
                    DisableClientVisualEffect(obj)
                    local liveMode = NormalizeFPSBoostMode(S.fpsBoostMode)
                    if liveMode ~= "LIGHT" then
                        pcall(function()
                            if obj:IsA("BasePart") then
                                obj.CastShadow = false
                                if liveMode == "ULTRA" then
                                    obj.Material = Enum.Material.SmoothPlastic
                                    obj.Reflectance = 0
                                    local lowerName = string.lower(tostring(obj.Name or ""))
                                    if string.find(lowerName, "decor") or string.find(lowerName, "leaf") or string.find(lowerName, "leaves") or string.find(lowerName, "effect") or string.find(lowerName, "vfx") then
                                        obj.LocalTransparencyModifier = 1
                                    end
                                end
                            elseif obj:IsA("Decal") or obj:IsA("Texture") then
                                if liveMode == "ULTRA" then
                                    obj.Transparency = 1
                                elseif (tonumber(obj.Transparency) or 0) < 0.45 then
                                    obj.Transparency = 0.45
                                end
                            end
                        end)
...
addToggle(UI.Pages["VISUALS"], "DISABLE LOADING SCREEN", S.disableLoading, function(v) SetLoadingScreenDisabled(v); saveConfig(true) end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        claimSeedEvent = claimSeedEvent, flyToPosition = flyToPosition, SetCutsceneDisabled = SetCutsceneDisabled, SetLoadingScreenDisabled = SetLoadingScreenDisabled,
