...
function HaimiyachGAG2_MailFruitToolName(tool)
    ...
    for _, attr in ipairs({ "FruitName", "Fruit", "CorePartName", "SeedName", "PlantName", "CropName", "ItemName", "Name" }) do
...
function HaimiyachGAG2_AddMailHarvestedFruitToolFallback(items, selected, addedKeys)
    ...
    local added = 0
    local fruitTools = HaimiyachGAG2_GetMailboxFruitToolCandidates and HaimiyachGAG2_GetMailboxFruitToolCandidates() or (HaimiyachGAG2_GetInventoryFruitTools and HaimiyachGAG2_GetInventoryFruitTools() or {})
    for _, tool in ipairs(fruitTools) do
        if #items >= 20 then break end
        if tool and tool.Parent and ((HaimiyachGAG2_MailSelectedMatchesHeldFruitTool and HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, tool)) or HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool)) then
            local itemKey = HaimiyachGAG2_MailFruitToolItemKey(tool)
            if itemKey and not (addedKeys and addedKeys[itemKey]) then
                HaimiyachGAG2_AddMailItem(items, "HarvestedFruits", itemKey, 1, 1)
                if addedKeys then addedKeys[itemKey] = true end
                added = added + 1
            end
        end
    end
...
function HaimiyachGAG2_AddMailHarvestedFruits(items, selected)
    ...
    for key, value in pairs(inv("HarvestedFruits")) do
        if #items >= 20 then break end
        -- Match the game MailboxController: HarvestedFruits are giftable only when the entry value has Id.
    ...
    -- Fallback for cases where the fruit exists as a Backpack tool first and replica is late.
...
function HaimiyachGAG2_FindFruitToolByGiftUuid(uuid)
    ...
    uuid = tostring(uuid)
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools and HaimiyachGAG2_GetInventoryFruitTools() or {}) do
...
-- Exact fix: the working SendBatch fruit path requires the fruit Tool to be held first.
-- This only auto-holds the selected fruit before HaimiyachGAG2_BuildMailItems() runs.
function HaimiyachGAG2_MailIsSeedOrGearTool(tool)
    ...
    if tool:GetAttribute("Pet") ~= nil or tool:GetAttribute("PetId") ~= nil then return true end
    for _, attr in ipairs({
        "Sprinkler", "WateringCan", "Shovel", "Trowel", "Crowbar", "Build", "Mushroom", "Gnome", "Raccoon",
        "SeedPack", "Egg", "Crate", "Teleporter", "PowerHose", "Wheelbarrow", "Ladder", "FreezeRay",
        "Flashbang", "MagicDice", "WeatherStaff", "StrawberrySniper", "Bird", "Rake", "PlayerMagnet",
        "FruitMagnet", "PetTeleporter"
    }) do
        if tool:GetAttribute(attr) ~= nil then return true end
    end
    local cat = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
    cat = string.lower(cat)
    if cat == "seed" or cat == "seeds" or cat == "gear" or cat == "gears" or cat == "egg" or cat == "pets" or cat == "pet" then return true end
    return false
end

function HaimiyachGAG2_MailIsFruitToolStrict(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if HaimiyachGAG2_MailIsSeedOrGearTool(tool) then return false end
    if tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("HarvestedFruit") ~= nil or tool:GetAttribute("FruitName") ~= nil then return true end
    local cat = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
    cat = string.lower(cat)
    if cat == "fruit" or cat == "fruits" or cat == "harvestedfruits" or cat == "harvestedfruit" then return true end
    local n = string.lower(tostring(tool.Name or "") .. " " .. tostring(tool.ToolTip or ""))
    if string.find(n, "seed", 1, true) then return false end
    if HaimiyachGAG2_ParseKgText and (HaimiyachGAG2_ParseKgText(tool.Name, false) or HaimiyachGAG2_ParseKgText(tool.ToolTip, false)) then
        return true
    end
    return false
end
...
function HaimiyachGAG2_MailFruitToolCandidates(tool)
    local arr = {}
    if not tool then return arr end
    for _, attr in ipairs({ "Fruit", "HarvestedFruit", "FruitName", "CorePartName", "SeedName", "PlantName", "CropName", "ItemName" }) do
        local v = tool:GetAttribute(attr)
        if type(v) == "string" and v ~= "" then arr[#arr + 1] = v end
    end
    arr[#arr + 1] = tostring(tool.Name or "")
    arr[#arr + 1] = tostring(tool.ToolTip or "")
    return arr
end

function HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, tool)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    if not HaimiyachGAG2_MailIsFruitToolStrict(tool) then return false end
    if HaimiyachGAG2_MailSelectedAllFruits and HaimiyachGAG2_MailSelectedAllFruits(selected) then return true end
    local candidates = HaimiyachGAG2_MailFruitToolCandidates(tool)
    for _, candidate in ipairs(candidates) do
        if candidate and tostring(candidate) ~= "" then
            if HaimiyachGAG2_SelectedTextMatch(selected, candidate) then return true end
...
function HaimiyachGAG2_GetHeldMailFruitTool()
    local char = LocalPlayer and LocalPlayer.Character
    if not char then return nil end
    for _, child in ipairs(char:GetChildren()) do
        if HaimiyachGAG2_MailIsFruitToolStrict(child) then
            return child
        end
    end
    return nil
...
function HaimiyachGAG2_GetMailboxFruitToolCandidates()
    local out, seen = {}, {}
    local function add(tool)
        if tool and tool:IsA("Tool") and HaimiyachGAG2_MailIsFruitToolStrict(tool) and not seen[tool] then
            seen[tool] = true
            out[#out + 1] = tool
        end
    end
    local char = LocalPlayer and LocalPlayer.Character
    if char then
        for _, tool in ipairs(char:GetChildren()) do add(tool) end
    end
    local bp = LocalPlayer and (LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
    if bp then
        for _, tool in ipairs(bp:GetChildren()) do add(tool) end
    end
    if HaimiyachGAG2_GetInventoryFruitTools then
        for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do add(tool) end
    end
    return out
end

function HaimiyachGAG2_HoldSelectedFruitForMailbox(selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false, "No fruit selected" end
    local char = LocalPlayer and LocalPlayer.Character
    if not char then return false, "Character not found" end

    local held = HaimiyachGAG2_GetHeldMailFruitTool()
    if held and HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, held) then
        return true, held
    end

    local target = nil
    for _, tool in ipairs(HaimiyachGAG2_GetMailboxFruitToolCandidates()) do
        if HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, tool) then
            target = tool
            break
        end
    end
    ...
    if not target then
        return false, "Selected fruit is not in Backpack/Character"
    end
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    pcall(function()
        if hum and hum.UnequipTools then hum:UnequipTools() end
    end)
    task.wait(0.08)
    pcall(function()
        if hum and hum.EquipTool then
    ...
    while os.clock() - start < 2 do
        local nowHeld = HaimiyachGAG2_GetHeldMailFruitTool()
        if nowHeld and HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, nowHeld) then
            return true, nowHeld
        end
        task.wait(0.08)
    end
    ...
    local nowHeld = HaimiyachGAG2_GetHeldMailFruitTool()
    if nowHeld then
        return false, "Holding different item: " .. tostring(nowHeld.Name)
    end
    return false, "Could not hold selected fruit"
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    if hasFruitSelection then
        local heldOk, heldInfo = HaimiyachGAG2_HoldSelectedFruitForMailbox(S.mailFruitTargets)
        if heldOk then
            task.wait(0.25)
        else
            Stats.SetWarning("MAIL", tostring(heldInfo or "Could not hold selected fruit"))
        end
    end
    ...
    if #items == 0 and hasFruitSelection then
        ...
        while #items == 0 and os.clock() - waitStart < 4 do
            HaimiyachGAG2_HoldSelectedFruitForMailbox(S.mailFruitTargets)
            task.wait(0.35)
    ...
    local mailOk = ok and success == true
    local mailMessage = tostring(message or success or "Could not send mail")

    if mailOk then
        if showNotify ~= false then
            local mailText = "Gift sent to @" .. tostring(targetName) .. " (" .. tostring(#items) .. " item)"
    ...
    if showNotify ~= false then notify("Mailbox", mailMessage, 4) end
    return false
end
...
function HaimiyachGAG2_GetInventoryFruitTools()
    ...
    local function isFruitTool(tool)
        ...
        if tool:GetAttribute("Fruit") or tool:GetAttribute("FruitName") or tool:GetAttribute("HarvestedFruit") == true then return true end
        if HaimiyachGAG2_ParseKgText(tool.Name, false) then return true end
        local c = tostring(tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
        c = string.lower(c)
        if c == "harvestedfruits" or c == "fruits" or c == "fruit" then return true end
        return false
    end
...
-- // ============================================================ \\ --
-- // MAIL FRUITS SELECTED AUTO-HOLD FIX                         \\ --
-- // Uses the working SendBatch path, only improves selected     \\ --
-- // fruit matching + auto equip before sending.                 \\ --
-- // ============================================================ \\ --
function HaimiyachGAG2_MailFruitCleanNameStrict(v)
    v = tostring(v or "")
    v = v:gsub("%c", " ")
    v = v:gsub("%b[]", " ")
    v = v:gsub("%b()", " ")
    ...
    v = v:gsub("%s+x%d+%s*$", "")
    v = v:gsub("%s+%d+%.?%d*%s*kg%s*$", "")
    v = v:gsub("%s+%d+%.?%d*%s*g%s*$", "")
    v = v:gsub("%s+kg%s*$", "")
    v = v:gsub("%s+seed%s*$", "")
    v = v:gsub("^%s+", ""):gsub("%s+$", "")
    v = v:lower():gsub("%s+", " ")
    -- strip common mutation prefixes so selected Bamboo can match Rainbow Bamboo, Gold Bamboo, etc.
    local changed = true
    while changed do
        changed = false
        for _, mut in ipairs({
            "rainbow", "gold", "golden", "shiny", "wet", "frozen", "chilled", "burning", "burnt", "silver",
            "giant", "big", "normal", "mutated", "mutation", "bloodlit", "moonlit", "zombified", "disco", "plasma"
        }) do
            local prefix = mut .. " "
            if string.sub(v, 1, #prefix) == prefix then
                v = string.sub(v, #prefix + 1)
                v = v:gsub("^%s+", ""):gsub("%s+$", "")
                changed = true
            end
        end
...
function HaimiyachGAG2_MailSelectedFruitWantSet(selected)
    local set = {}
    local all = false
    if type(selected) == "table" then
        for name, enabled in pairs(selected) do
            if enabled == true then
                local clean = HaimiyachGAG2_MailFruitCleanNameStrict(name)
                if clean == "all fruits" or clean == "all fruit" then
                    all = true
                elseif clean ~= "" then
                    set[clean] = true
                end
...
function HaimiyachGAG2_MailFruitToolAliasList(tool)
    local aliases, seen = {}, {}
    local function add(v)
        v = HaimiyachGAG2_MailFruitCleanNameStrict(v)
    ...
    if tool then
        for _, attr in ipairs({ "FruitName", "Fruit", "CorePartName", "CropName", "PlantName", "ItemName" }) do
            add(tool:GetAttribute(attr))
        end
        -- Some harvested fruit tools only expose SeedName. This is safe because seed tools are blocked by strict fruit checks.
        add(tool:GetAttribute("SeedName"))
        add(tool.Name)
        add(tool.ToolTip)
    end
    return aliases
...
function HaimiyachGAG2_MailEntryAliasList(key, value)
    local aliases, seen = {}, {}
    local function add(v)
        v = HaimiyachGAG2_MailFruitCleanNameStrict(v)
        if v ~= "" and not seen[v] then
    ...
    add(key)
    if type(value) == "table" then
        for _, k in ipairs({ "FruitName", "Name", "ItemName", "SeedName", "Type", "CorePartName", "PlantName", "CropName" }) do
            add(value[k])
        end
    end
    local ok, display = pcall(function()
        return HaimiyachGAG2_MailFruitDisplayName(key, value)
    end)
    if ok then add(display) end
    return aliases
end

function HaimiyachGAG2_MailSelectedMatchesFruitAliases(selected, aliases)
    local want, all = HaimiyachGAG2_MailSelectedFruitWantSet(selected)
    if all then return true end
    if type(aliases) ~= "table" then return false end
    for _, alias in ipairs(aliases) do
        local clean = HaimiyachGAG2_MailFruitCleanNameStrict(alias)
        if clean ~= "" and want[clean] == true then
            return true
        end
    end
    return false
...
function HaimiyachGAG2_MailIsFruitToolStrict(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    -- hard block seeds/gears/pets so Bamboo Seed never counts as Bamboo fruit
    ...
    for _, attr in ipairs({
        ...
        if tool:GetAttribute(attr) ~= nil then return false end
    end
    local cat = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
    cat = string.lower(cat)
    if cat == "seed" or cat == "seeds" or cat == "gear" or cat == "gears" or cat == "egg" or cat == "pets" or cat == "pet" then return false end
    ...
    local text = string.lower(tostring(tool.Name or "") .. " " .. tostring(tool.ToolTip or ""))
    if string.find(text, "seed", 1, true) then return false end
    if HaimiyachGAG2_ParseKgText and (HaimiyachGAG2_ParseKgText(tool.Name, false) or HaimiyachGAG2_ParseKgText(tool.ToolTip, false)) then return true end
    return false
end
...
function HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    if not HaimiyachGAG2_MailIsFruitToolStrict(tool) then return false end
    return HaimiyachGAG2_MailSelectedMatchesFruitAliases(selected, HaimiyachGAG2_MailFruitToolAliasList(tool))
end

function HaimiyachGAG2_MailSelectedMatchesHeldFruitTool(selected, tool)
    return HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool)
end

function HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    return HaimiyachGAG2_MailSelectedMatchesFruitAliases(selected, HaimiyachGAG2_MailEntryAliasList(key, value))
end
...
function HaimiyachGAG2_MailFindSelectedFruitTool(selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return nil end
    local held = HaimiyachGAG2_GetHeldMailFruitTool()
    if held and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, held) then
        return held, "held"
    end
    for _, tool in ipairs(HaimiyachGAG2_GetMailboxFruitToolCandidates()) do
        if tool ~= held and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool) then
...
function HaimiyachGAG2_HoldSelectedFruitForMailbox(selected)
    ...
    local target, source = HaimiyachGAG2_MailFindSelectedFruitTool(selected)
    if not target then
        return false, "Selected fruit is not in Backpack/Character"
    end
    if target.Parent == char then
