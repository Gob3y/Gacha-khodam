...
local function getMiniBarSize()
    local sc = tonumber(UI.Scale) or 1
    local w = math.floor((baseW * sc) + 0.5)
    local h = math.floor((54 * sc) + 0.5)
    if w < 320 then w = 320 end
    if h < 46 then h = 46 end
    return w, h
end

local miniW, miniH = getMiniBarSize()
local MiniBar = new("Frame", {
    Name = "MinimizedWindow",
    Size = UDim2.fromOffset(miniW, miniH),
...
local function syncMiniBarSize()
    local w, h = getMiniBarSize()
    MiniBar.Size = UDim2.fromOffset(w, h)
end
...
addSlider(SettingsPage, "UI SCALE", 0.6, 1.4, UI.Scale, 2, function(v)
    ...
    S.uiScale = v
    UIScaleObj.Scale = v
    if UI.Minimized then syncMiniBarSize() end
end)
