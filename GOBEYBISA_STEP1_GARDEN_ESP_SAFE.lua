...
-- //                    VISUALS ESP LITE                         \ --
-- // ============================================================ \ --
local VisualESPLite = { gardenLooping = false, gardenLabels = {} }

local FruitBaseValueLite = {
...
local function ESPIsMobileLite()
    local ok, uis = pcall(function() return game:GetService("UserInputService") end)
    return ok and uis and uis.TouchEnabled and not uis.KeyboardEnabled
end

local function ESPFormatMoneyLite(n)
...
local function ESPGardenHasTagLite(inst, tag)
    local ok, res = pcall(function() return CollectionService:HasTag(inst, tag) end)
    return ok and res == true
end

local function ESPGardenFruitNameLite(model)
...
local function ESPGardenSizeLite(sizeMulti)
...
local function ESPGardenValueLite(name, sizeMulti)
    name = tostring(name or "")
    local base = FruitBaseValueLite[name]
...
local function ESPFindHarvestPartLite(model)
    if not model then return nil end
...
local function ESPIsGardenFruitLite(model)
    if not (model and model:IsA("Model")) then return false end
    if not ESPGardenFruitNameLite(model) then return false end
    if model:GetAttribute("FruitId") == nil and model:GetAttribute("PlantId") == nil then return false end
    if ESPGardenHasTagLite(model, "Harvestable") then return true end
...
local function ClearGardenFruitESP()
    for model, gui in pairs(VisualESPLite.gardenLabels) do
        pcall(function() if gui then gui:Destroy() end end)
        VisualESPLite.gardenLabels[model] = nil
    end
    local gardens = Workspace:FindFirstChild("Gardens")
    if gardens then
        pcall(function()
...
local function UpdateGardenFruitESP()
    if not S.espGardenFruit then return end
    local seen, list = {}, {}
    pcall(function()
    ...
    if #list == 0 then
        ...
        if gardens then
            for _, inst in ipairs(gardens:GetDescendants()) do
                if inst:IsA("Model") and inst:GetAttribute("FruitId") ~= nil then
                    list[#list + 1] = inst
                end
            end
        end
    ...
    for _, model in ipairs(list) do
        ...
        if ESPIsGardenFruitLite(model) then
            local hp = ESPFindHarvestPartLite(model)
            if hp and hp.Parent then
                shown = shown + 1
                seen[model] = true
                local gui = VisualESPLite.gardenLabels[model]
                if not (gui and gui.Parent) then
                    local mobile = ESPIsMobileLite()
                    ...
                    gui.Size = UDim2.fromOffset(mobile and 210 or 230, mobile and 44 or 46)
                    gui.Parent = hp
                    ...
                    limit.MinTextSize = 9
                    limit.MaxTextSize = mobile and 13 or 15
                    limit.Parent = label

                    VisualESPLite.gardenLabels[model] = gui
                elseif gui.Parent ~= hp then
                    gui.Parent = hp
                end

                local name = ESPGardenFruitNameLite(model) or "Fruit"
                local sm = tonumber(model:GetAttribute("SizeMulti") or model:GetAttribute("SizeMultiplier") or 1) or 1
                local text = tostring(name) .. " | " .. ESPGardenSizeLite(sm) .. " | x" .. string.format("%.2f", sm)
                if S.espGardenValue then
                    local value = ESPGardenValueLite(name, sm)
                    if value then text = text .. " | " .. ESPFormatMoneyLite(value) .. "¢" end
                end
...
local function StartGardenFruitESPLoop()
    if VisualESPLite.gardenLooping then return end
    VisualESPLite.gardenLooping = true
    task.spawn(function()
        while VisualESPLite.gardenLooping and not S.killed do
        ...
        pcall(ClearGardenFruitESP)
    end)
end

local function SetGardenFruitESP(state)
    S.espGardenFruit = state and true or false
    if S.espGardenFruit then
        StartGardenFruitESPLoop()
        pcall(UpdateGardenFruitESP)
    else
        pcall(ClearGardenFruitESP)
    end
...
local function SetGardenFruitValueESP(state)
...
addToggle(UI.Pages["VISUALS"], "ESP GARDEN FRUIT", S.espGardenFruit, function(v) SetGardenFruitESP(v) end)
addToggle(UI.Pages["VISUALS"], "GARDEN VALUE SHECKLES", S.espGardenValue, function(v) SetGardenFruitValueESP(v) end)
