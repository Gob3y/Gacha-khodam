...
local S = {
    ...
    -- misc
    autoMail = false, autoAcceptGift = false, autoSendMail = false, mailTargetUsername = "", mailNote = "Here is a gift!", mailSeedTargets = {}, mailFruitTargets = {}, mailOtherCategories = {}, mailSendCount = 1, mailSendDelay = 30, autoHop = false, hopInterval = 0,
...
-- // Mailbox send helper (built from MailboxController SendBatch flow)
local MAIL_OTHER_CATEGORY_OPTIONS = {
    "Crates",
    "SeedPacks",
    "Sprinklers",
    "WateringCans",
...
local function HaimiyachGAG2_TrimText(v)
    v = tostring(v or "")
    v = v:gsub("%c", " ")
    v = v:gsub("^%s+", ""):gsub("%s+$", "")
    return v
...
local function HaimiyachGAG2_NormalMailName(v)
...
local function HaimiyachGAG2_SelectedTextMatch(selected, name)
    if type(selected) ~= "table" then return false end
    local target = HaimiyachGAG2_NormalMailName(name)
    if target == "" then return false end
    for sel, enabled in pairs(selected) do
        if enabled == true and HaimiyachGAG2_NormalMailName(sel) == target then
            return true
        end
    end
    return false
...
local function HaimiyachGAG2_MailEntryName(category, key, value)
    if type(value) == "table" then
        return value.FruitName or value.Name or value.ItemName or value.Type or value.SeedName or tostring(key)
    end
    return tostring(key)
end

local function HaimiyachGAG2_MailStackCount(value)
...
local function HaimiyachGAG2_ResolveMailUserId(username)
...
local function HaimiyachGAG2_BuildMailItems()
    local items = {}
    ...
    local totalCount = 0
    local function addItem(category, key, count)
        if totalCount >= 20 then return end
        count = math.floor(tonumber(count) or 1)
        if count < 1 then return end
        local remain = 20 - totalCount
        if count > maxEach then count = maxEach end
        if count > remain then count = remain end
        table.insert(items, {
            Category = category,
    ...
    if HaimiyachGAG2_HasSelection(S.mailSeedTargets) then
        for key, value in pairs(inv("Seeds")) do
            local name = HaimiyachGAG2_MailEntryName("Seeds", key, value)
            if HaimiyachGAG2_SelectedTextMatch(S.mailSeedTargets, name) then
                addItem("Seeds", key, HaimiyachGAG2_MailStackCount(value))
            end
        end
    end

    if HaimiyachGAG2_HasSelection(S.mailFruitTargets) then
        for key, value in pairs(inv("HarvestedFruits")) do
            local name = HaimiyachGAG2_MailEntryName("HarvestedFruits", key, value)
            if HaimiyachGAG2_SelectedTextMatch(S.mailFruitTargets, name) then
                addItem("HarvestedFruits", key, 1)
            end
        end
    end

    if HaimiyachGAG2_HasSelection(S.mailOtherCategories) then
        for _, category in ipairs(MAIL_OTHER_CATEGORY_OPTIONS) do
            if S.mailOtherCategories[category] == true then
                local bucket = inv(category)
                if type(bucket) == "table" then
                    for key, value in pairs(bucket) do
                        local count = (category == "Pets") and 1 or HaimiyachGAG2_MailStackCount(value)
                        addItem(category, key, count)
                        if totalCount >= 20 then break end
                    end
                end
            end
            if totalCount >= 20 then break end
...
local function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    local userId, userErr = HaimiyachGAG2_ResolveMailUserId(S.mailTargetUsername)
    if not userId then
    ...
    if ok and success then
        if showNotify ~= false then notify("Mailbox", tostring(message and message ~= "" and message or ("Gift sent: " .. tostring(#items) .. " item(s)")), 4) end
        return true
    else
...
local CONFIG_KEYS = {
    ...
    "autoMail", "autoAcceptGift", "autoSendMail", "mailTargetUsername", "mailNote", "mailSeedTargets", "mailFruitTargets", "mailOtherCategories", "mailSendCount", "mailSendDelay", "autoHop", "hopInterval", "codeText", "autoCodes", "antiAfk", "disableCutscene",
...
loopOn(function() return S.autoSendMail end, function()
    local d = tonumber(S.mailSendDelay) or 30
    if d < 5 then d = 5 end
    return d
end, function()
    HaimiyachGAG2_SendSelectedMailNow(false)
end)
...
addInput(UI.Pages["MISC"], "MAIL TARGET USERNAME", "username", S.mailTargetUsername or "", function(v)
    S.mailTargetUsername = tostring(v or "")
    saveConfig(true)
end)
addInput(UI.Pages["MISC"], "MAIL NOTE", "optional note", S.mailNote or "", function(v)
    S.mailNote = tostring(v or "")
    saveConfig(true)
end)
addDropdown(UI.Pages["MISC"], "MAIL SEEDS TO SEND", SEED_NAMES, S.mailSeedTargets, true, function(sel)
    if type(S.mailSeedTargets) ~= "table" then S.mailSeedTargets = {} end
    pickMulti(sel, S.mailSeedTargets)
    saveConfig(true)
end)
addDropdown(UI.Pages["MISC"], "MAIL FRUITS TO SEND", SEED_NAMES, S.mailFruitTargets, true, function(sel)
    if type(S.mailFruitTargets) ~= "table" then S.mailFruitTargets = {} end
    pickMulti(sel, S.mailFruitTargets)
    saveConfig(true)
end)
addDropdown(UI.Pages["MISC"], "MAIL OTHER TYPES", MAIL_OTHER_CATEGORY_OPTIONS, S.mailOtherCategories, true, function(sel)
    if type(S.mailOtherCategories) ~= "table" then S.mailOtherCategories = {} end
    pickMulti(sel, S.mailOtherCategories)
    saveConfig(true)
end)
addSlider(UI.Pages["MISC"], "MAIL COUNT PER ITEM", 1, 20, S.mailSendCount, 0, function(v)
    S.mailSendCount = v
    saveConfig(true)
end)
addSlider(UI.Pages["MISC"], "MAIL SEND DELAY", 5, 300, S.mailSendDelay, 0, function(v)
    S.mailSendDelay = v
    saveConfig(true)
end)
addButton(UI.Pages["MISC"], "SEND SELECTED MAIL NOW", function()
    HaimiyachGAG2_SendSelectedMailNow(true)
end)
addToggle(UI.Pages["MISC"], "AUTO SEND SELECTED MAIL", S.autoSendMail, function(v)
    S.autoSendMail = v
    saveConfig(true)
end)
