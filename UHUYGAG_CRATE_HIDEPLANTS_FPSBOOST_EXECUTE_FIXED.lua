...
local function crateCatalog()
    ...
    local function scanFolder(folder)
        ...
        for _, obj in ipairs(folder:GetChildren()) do
            -- Do not use abstract class checks here; some executors error on invalid class aliases.
            addName(obj.Name)
        end
    end
    pcall(function()
...
local function buyCrate(name)
    ...
    -- Different server builds can name the purchase remote differently.
    -- Every call is protected so a missing/refused remote cannot stop the whole hub.
    for _, path in ipairs({
        "CrateShop.PurchaseCrate",
        "CrateShop.BuyCrate",
        "CratesShop.PurchaseCrate",
        "CratesShop.BuyCrate",
        "GuildCrateShop.PurchaseCrate",
        "GuildCrates.PurchaseCrate",
        ...
        local ok, res = pcall(function()
            local sent, result = fire(path, name)
            return sent, result
        end)
...
loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return tonumber(S.crateBuyInterval) or 10 end, function()
    local selected = type(S.buyCrates) == "table" and S.buyCrates or {}
    if not (S.autoBuyAllCrates or picked(selected)) then return end
    local list = CRATE_NAMES or {}
    if S.autoBuyAllCrates then
        for _, name in ipairs(list) do
            if not (S.autoBuyCrates or S.autoBuyAllCrates) then break end
            local stock = crateStockOf(name)
            if stock == nil or stock > 0 then
                if buyCrate(name) then Stats.bought = Stats.bought + 1 end
...
-- FPS BOOST stays mobile-safe: no mass material edits, no GUI/character changes.
-- HIDE PLANTS is a separate toggle under FPS BOOST.
...
local function IsLightObject(obj)
    return obj and (obj:IsA("PointLight") or obj:IsA("SpotLight") or obj:IsA("SurfaceLight"))
end
...
local function IsPlantVisualObject(obj)
    ...
    while node and node ~= Workspace do
        ...
        if n == "plants" then
            local gardens = Workspace:FindFirstChild("Gardens")
            if gardens and node:IsDescendantOf(gardens) then return true end
        end
        node = node.Parent
    end
    return false
...
local function HidePlantVisual(obj)
    ...
    elseif obj:IsA("BillboardGui") or obj:IsA("SurfaceGui") then
        ...
        SafeSetProperty(obj, "Enabled", false)
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") or IsLightObject(obj) then
        CaptureHiddenPlantBackup(obj, { "Enabled" })
        SafeSetProperty(obj, "Enabled", false)
    end
end
...
local function ApplyHidePlants()
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(descendants) do
            ...
            pcall(HidePlantVisual, obj)
            processed = processed + 1
            if processed % 120 == 0 then task.wait() end
        end
    end)
    ...
    HidePlantsConnection = Workspace.DescendantAdded:Connect(function(obj)
        if S.hidePlants then
            task.delay(0.2, function()
                if S.hidePlants and not S.killed then pcall(HidePlantVisual, obj) end
            end)
        end
    end)
end
...
local function ApplyFPSBoostGraphics()
    ...
    -- Mobile ultra-light graphics: avoids mass material edits that can freeze Android executors.
    -- Removes expensive lighting, water, particles and local light objects only.
    ...
    -- BasePart, Texture, Decal, GUI and characters are left alone; use HIDE PLANTS separately if needed.
    task.spawn(function()
        local descendants = {}
        pcall(function()
        ...
        for _, obj in ipairs(descendants) do
            ...
            pcall(DisableClientVisualEffect, obj)
            processed = processed + 1
...
addButton(ShopPage, "REFRESH CRATE LIST", function()
    ...
    if crateDrop and crateDrop.SetOptions then crateDrop.SetOptions(CRATE_NAMES) end
end)
