...
-- FPS BOOST modes are tiered: LIGHT keeps visuals normal, BALANCED is lighter for mobile, ULTRA is potato mode client-side.
...
local FPSBoostPlayerScriptsConnection = nil
local FPSBoostLightingConnection = nil
...
local function CaptureWorkspaceEffectBackup(obj)
    if not obj then return end
    if obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") or obj:IsA("Clouds") then
...
local function CaptureVisualPartBackup(part)
    if not part or not part:IsA("BasePart") then return end
    if GraphicsBackup.VisualParts[part] ~= nil then return end
    local data = {}
    pcall(function() data.CastShadow = part.CastShadow end)
    pcall(function() data.Material = part.Material end)
    pcall(function() data.MaterialVariant = part.MaterialVariant end)
    pcall(function() data.Reflectance = part.Reflectance end)
    pcall(function() data.LocalTransparencyModifier = part.LocalTransparencyModifier end)
    pcall(function() data.Transparency = part.Transparency end)
    GraphicsBackup.VisualParts[part] = data
end

local function CaptureTextureLikeBackup(obj)
    if not obj or not (obj:IsA("Decal") or obj:IsA("Texture")) then return end
    if GraphicsBackup.TextureLike[obj] ~= nil then return end
    local data = {}
    pcall(function() data.Transparency = obj.Transparency end)
    pcall(function() data.Color3 = obj.Color3 end)
    GraphicsBackup.TextureLike[obj] = data
end

local function CaptureSurfaceAppearanceBackup(obj)
    if not obj or not obj:IsA("SurfaceAppearance") then return end
    if GraphicsBackup.SurfaceAppearances[obj] ~= nil then return end
    GraphicsBackup.SurfaceAppearances[obj] = { Parent = obj.Parent }
end
...
local function IsGardenDescendant(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
    return gardens and obj and obj:IsDescendantOf(gardens)
end

local function IsGardenVisualCandidate(obj)
    if not IsGardenDescendant(obj) then return false end

    local objName = string.lower(tostring(obj.Name or ""))
    if objName == "plantarea" or objName == "spawnpoint" or objName == "baseplate" or objName == "ground" or objName == "floor" or objName == "path" or objName == "border" or objName == "boundary" then
        return false
    end
    if obj:IsA("BasePart") then
        local taggedPlantArea = false
        pcall(function() taggedPlantArea = CollectionService:HasTag(obj, "PlantArea") end)
        if taggedPlantArea then return false end
    end

    local cur = obj
    while cur and cur ~= Workspace do
        local name = string.lower(tostring(cur.Name or ""))
        if name == "plants" or name == "props" or name == "decor" or name == "decorations" or name == "sprinklers" or name == "rakes" then
            return true
        end
        if name ~= "plantarea" and (string.find(name, "plant", 1, true) or string.find(name, "fruit", 1, true) or string.find(name, "prop", 1, true) or string.find(name, "decor", 1, true) or string.find(name, "sprinkler", 1, true) or string.find(name, "rake", 1, true)) then
            return true
        end
        if cur:GetAttribute("SeedName") ~= nil or cur:GetAttribute("PlantId") ~= nil or cur:GetAttribute("FruitId") ~= nil or cur:GetAttribute("Sprinkler") ~= nil or cur:GetAttribute("Rake") ~= nil then
            return true
        end
        cur = cur.Parent
    end
...
-- From the in-game Dex screenshot: visuals are controlled by Bird/Fruit/Garden/Pet/Plant/Prop/Rake visual controllers.
-- Do not destroy these controllers; only cut their spawned client visuals so execute stays safe.
local VisualControllerNameMap = {
    BirdVisualController = true,
    FruitVisualizerController = true,
    GardenVisualController = true,
    MegaphoneVisualizerController = true,
    PetVisualController = true,
    PlantVisualizerController = true,
    PropVisualizerController = true,
    RakeVisualizerController = true,
    SeedInspectController = true,
}

local function HasNameKeyword(name, keywords)
    name = string.lower(tostring(name or ""))
...
local function IsTemporaryVisualCandidate(obj)
    local temporary = Workspace:FindFirstChild("Temporary")
    if temporary and obj and obj:IsDescendantOf(temporary) then
        return true
    end
    local name = string.lower(tostring(obj and obj.Name or ""))
    if name == "fakeplot" or name == "potvisual" or name == "wateringcanfx" or name == "sprinklerpreview" or name == "sprinklerradius" then
        return true
    end
    if HasNameKeyword(name, {"preview", "visual", "vfx", "fx", "effect", "particle", "radius", "cosmetic"}) then
        return true
    end
    return false
end

local function IsMapDecorVisualCandidate(obj)
    local map = Workspace:FindFirstChild("Map")
    if not (map and obj and obj:IsDescendantOf(map)) then return false end
    local cur = obj
    while cur and cur ~= Workspace do
        local name = string.lower(tostring(cur.Name or ""))
        if name == "decor" or name == "decors" or name == "decorations" or name == "cosmetic" or name == "cosmetics" or name == "props" or name == "visual" or name == "visuals" or name == "vfx" or name == "effects" then
            return true
        end
        cur = cur.Parent
    end
...
local function IsUltraVisualCandidate(obj)
    if not obj or IsPlayerCharacterDescendant(obj) then return false end
    if IsGardenVisualCandidate(obj) then return true end
    if IsTemporaryVisualCandidate(obj) then return true end
    if IsMapDecorVisualCandidate(obj) then return true end
    return false
end

local function IsVisualControllerDescendant(obj)
    ...
    if not (controllers and obj and obj:IsDescendantOf(controllers)) then return false end
    local cur = obj
    while cur and cur ~= controllers.Parent do
        if VisualControllerNameMap[tostring(cur.Name or "")] then
            return true
        end
        if HasNameKeyword(cur.Name, {"visualcontroller", "visualizercontroller"}) then
            return true
        end
        cur = cur.Parent
    end
...
local function ApplyVisualControllerUltraState(obj)
    if not obj or not IsVisualControllerDescendant(obj) then return end
    -- Some dumps show these as ModuleScripts, some clients can show LocalScripts.
    -- Only Script/LocalScript have Disabled; ModuleScripts are left alone to avoid nil-property errors.
    if obj:IsA("LocalScript") or obj:IsA("Script") then
        if GraphicsBackup.DisabledVisualControllers[obj] == nil then
            pcall(function() GraphicsBackup.DisabledVisualControllers[obj] = obj.Disabled end)
        end
        SafeSetProperty(obj, "Disabled", true)
    end
end

local function ApplyVisualControllersUltra()
    local playerScripts = LocalPlayer and LocalPlayer:FindFirstChild("PlayerScripts")
    local controllers = playerScripts and playerScripts:FindFirstChild("Controllers")
    if not controllers then return end
    for _, obj in ipairs(controllers:GetDescendants()) do
        ApplyVisualControllerUltraState(obj)
    end
end
...
local function RestoreClientVisualBackups()
    for part, data in pairs(GraphicsBackup.VisualParts) do
        if part and part.Parent then
            if data.CastShadow ~= nil then SafeSetProperty(part, "CastShadow", data.CastShadow) end
            if data.Material ~= nil then SafeSetProperty(part, "Material", data.Material) end
            if data.MaterialVariant ~= nil then SafeSetProperty(part, "MaterialVariant", data.MaterialVariant) end
            if data.Reflectance ~= nil then SafeSetProperty(part, "Reflectance", data.Reflectance) end
            if data.LocalTransparencyModifier ~= nil then SafeSetProperty(part, "LocalTransparencyModifier", data.LocalTransparencyModifier) end
            if data.Transparency ~= nil then SafeSetProperty(part, "Transparency", data.Transparency) end
        end
    end

    for obj, data in pairs(GraphicsBackup.TextureLike) do
        if obj and obj.Parent then
            if data.Transparency ~= nil then SafeSetProperty(obj, "Transparency", data.Transparency) end
            if data.Color3 ~= nil then SafeSetProperty(obj, "Color3", data.Color3) end
        end
    end

    for obj, data in pairs(GraphicsBackup.SurfaceAppearances) do
        if obj and not obj.Parent and data.Parent then
            pcall(function() obj.Parent = data.Parent end)
        end
    end
end

local function RestoreDisabledVisualControllers()
    for obj, disabled in pairs(GraphicsBackup.DisabledVisualControllers) do
        if obj and obj.Parent then
            SafeSetProperty(obj, "Disabled", disabled)
        end
    end
end
...
local function ApplyBalancedVisualObject(obj, hideTextures)
    if not obj or IsPlayerCharacterDescendant(obj) then return end

    if obj:IsA("BasePart") then
        CaptureVisualPartBackup(obj)
        SafeSetProperty(obj, "CastShadow", false)
        if hideTextures then
            SafeSetProperty(obj, "Material", Enum.Material.SmoothPlastic)
            SafeSetProperty(obj, "MaterialVariant", "")
            SafeSetProperty(obj, "Reflectance", 0)
            if IsUltraVisualCandidate(obj) then
                SafeSetProperty(obj, "LocalTransparencyModifier", 1)
                SafeSetProperty(obj, "Transparency", 1)
            end
        end
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        CaptureTextureLikeBackup(obj)
        if hideTextures then
            SafeSetProperty(obj, "Transparency", 1)
        else
            pcall(function()
                obj.Transparency = math.max(tonumber(obj.Transparency) or 0, 0.65)
            end)
        end
    elseif obj:IsA("SurfaceAppearance") and hideTextures then
        CaptureSurfaceAppearanceBackup(obj)
...
local function RestoreDefaultGraphics()
    ...
    if FPSBoostPlayerScriptsConnection then
        pcall(function() FPSBoostPlayerScriptsConnection:Disconnect() end)
        FPSBoostPlayerScriptsConnection = nil
    end
    if FPSBoostLightingConnection then
        pcall(function() FPSBoostLightingConnection:Disconnect() end)
        FPSBoostLightingConnection = nil
    end
    ...
    RestoreClientVisualBackups()
    RestoreDisabledVisualControllers()
...
local function CaptureLightingVisualBackup(obj)
    if not obj then return end
    if obj:IsA("PostEffect") then
        if GraphicsBackup.Effects[obj] == nil then
            pcall(function() GraphicsBackup.Effects[obj] = obj.Enabled end)
        end
    elseif obj:IsA("Atmosphere") then
        if GraphicsBackup.Atmosphere[obj] == nil then
            pcall(function()
                GraphicsBackup.Atmosphere[obj] = {
                    Density = obj.Density,
                    Offset = obj.Offset,
...
local function ApplyFPSLightingObject(obj, isBalancedOrUltra)
    if not obj then return end
    CaptureLightingVisualBackup(obj)
    if obj:IsA("PostEffect") then
        SafeSetProperty(obj, "Enabled", false)
    elseif obj:IsA("Atmosphere") and isBalancedOrUltra then
...
local function ApplyFPSBoostGraphics()
    ...
    local isBalancedOrUltra = fpsMode ~= "LIGHT"
    local isUltra = fpsMode == "ULTRA"
    ...
    -- LIGHT   : safe normal look; only particles/trails/beams/smoke/fire/sparkles + post effects OFF.
    -- BALANCED: mobile-friendly; shadows OFF, terrain decoration OFF, water effects OFF, decals/textures lightened.
    -- ULTRA   : potato mode; BALANCED + SmoothPlastic, hidden textures/decals, hidden garden/decor visuals client-side.
    ...
    for _, obj in ipairs(Lighting:GetDescendants()) do
        ApplyFPSLightingObject(obj, isBalancedOrUltra)
    end
    FPSBoostLightingConnection = Lighting.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.1, function()
                if FPSBoostActive and S.fpsBoost then
                    ApplyFPSLightingObject(obj, NormalizeFPSBoostMode(S.fpsBoostMode) ~= "LIGHT")
                end
            end)
        end
    end)
    ...
    local terrain = Workspace:FindFirstChildOfClass("Terrain")
    if terrain and isBalancedOrUltra then
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(descendants) do
            ...
            if isBalancedOrUltra then
                ApplyBalancedVisualObject(obj, isUltra)
            end
            processed = processed + 1

            if processed % 65 == 0 then
                task.wait()
            end
        end
    end)
    ...
    if isUltra then
        ApplyVisualControllersUltra()
        local playerScripts = LocalPlayer and LocalPlayer:FindFirstChild("PlayerScripts")
        local controllers = playerScripts and playerScripts:FindFirstChild("Controllers")
        if controllers then
            FPSBoostPlayerScriptsConnection = controllers.DescendantAdded:Connect(function(obj)
                if FPSBoostActive and S.fpsBoost and NormalizeFPSBoostMode(S.fpsBoostMode) == "ULTRA" then
            ...
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost then
                    ...
                    if isBalancedOrUltra then
                        ApplyBalancedVisualObject(obj, isUltra)
                        if isUltra then
                            local ok, list = pcall(function() return obj:GetDescendants() end)
                            if ok and type(list) == "table" then
                                local n = 0
                                for _, child in ipairs(list) do
                                    DisableClientVisualEffect(child)
                                    ApplyBalancedVisualObject(child, true)
                                    n = n + 1
