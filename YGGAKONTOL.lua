...
local LEGACY_CONFIG_FILE = CONFIG_DIR .. "/GAG2H_Config.json"
local CONFIG_SLOT_FILE = CONFIG_DIR .. "/GAG2H_ActiveConfig.txt"
local CONFIG_SLOT_OPTIONS = {
    "CUSTOM 1",
    "CUSTOM 2",
    "CUSTOM 3",
    "CUSTOM 4",
...
local ActiveConfigName = "CUSTOM 1"
...
local function normalizeConfigName(name)
    name = tostring(name or "CUSTOM 1")
    name = name:gsub("^%s+", ""):gsub("%s+$", "")
    if name == "" then name = "CUSTOM 1" end
    name = name:upper()
    local allowed = {}
    for _, opt in ipairs(CONFIG_SLOT_OPTIONS) do allowed[opt] = true end
    if not allowed[name] then name = "CUSTOM 1" end
    return name
end
...
local function getConfigFile(name)
    return CONFIG_DIR .. "/GAG2H_Config_" .. configSafeName(name or ActiveConfigName) .. ".json"
end

local function saveActiveConfigName()
    if not fsSupported() then return false end
    ensureConfigDir()
    pcall(function() writefile(CONFIG_SLOT_FILE, ActiveConfigName) end)
    return true
end

local function loadActiveConfigName()
    if fsSupported() then
        local ok, exists = pcall(function() return isfile(CONFIG_SLOT_FILE) end)
        if ok and exists then
            local ok2, raw = pcall(function() return readfile(CONFIG_SLOT_FILE) end)
            if ok2 and type(raw) == "string" then
                ActiveConfigName = normalizeConfigName(raw)
            end
        end
    end
...
local function saveConfig(silent)
    ...
    local file = getConfigFile(ActiveConfigName)
    local ok2, err = pcall(function() writefile(file, encoded) end)
    if not ok2 then
        if not silent then notify("HAIMIYACH HUB", "Failed to save config: " .. tostring(err), 4) end
        return false
    end
    if not silent then notify("HAIMIYACH HUB", "Config saved: " .. ActiveConfigName, 3) end
    return true
end
local function loadConfig(silent)
    ...
    if not exists and ActiveConfigName == "CUSTOM 1" then
        pcall(function() exists = isfile(LEGACY_CONFIG_FILE) end)
        if exists then file = LEGACY_CONFIG_FILE end
    end
    if not exists then
        if not silent then notify("HAIMIYACH HUB", "No saved config found for: " .. ActiveConfigName, 3) end
        return false
    end
    local ok, raw = pcall(function() return readfile(file) end)
    ...
    if not silent then notify("HAIMIYACH HUB", "Config loaded: " .. ActiveConfigName .. ". Re-open or re-execute to refresh UI controls.", 4) end
    return true
end
local function setActiveConfigName(name, shouldLoad)
    ActiveConfigName = normalizeConfigName(name)
    saveActiveConfigName()
    if shouldLoad then
...
-- Auto-load selected config before loops and UI are created.
loadActiveConfigName()
loadConfig(true)
...
addDropdown(SettingsPage, "SELECT CONFIG", CONFIG_SLOT_OPTIONS, ActiveConfigName, false, function(v)
    setActiveConfigName(v, false)
    notify("HAIMIYACH HUB", "Selected config: " .. ActiveConfigName, 2)
end)
