...
function HaimiyachGAG2_GardenCalcKg(obj)
    if not obj then return nil end
    local esp = rawget(_G, "HaimiyachGAG2_GardenESP")
    if esp and type(esp.calculateWeight) == "function" then
        local sizeMulti = HaimiyachGAG2_ObjectSizeMultiplier(obj)
        local ok, weight = pcall(function()
            return esp.calculateWeight(obj, sizeMulti)
        end)
...
_G.HaimiyachGAG2_GardenESP = GardenESP
...
function GardenESP.initModules()
    ...
    pcall(function()
        ...
        -- Executor-safe: do not require FruitVisualizerController at startup.
        -- Some mobile executors fail when client controller modules are required directly.
        -- KG calculation below still follows the controller formula:
        -- GrowData.BaseWeight * SizeMultiplier * OvertimeGrowth.

        if not GardenESP.gardenSync then
            local sync = controllers and controllers:FindFirstChild("GardenSyncController")
            if sync and sync:IsA("ModuleScript") then
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    if esp and type(esp.calculateWeight) == "function" then
        local ok, weight = pcall(function()
            return esp.calculateWeight(tool, sizeMulti)
        end)
        weight = tonumber(weight)
        if ok and weight and weight > 0 then return weight end
    end

    if esp and type(esp.initModules) == "function" then
        pcall(esp.initModules)
        local name = HaimiyachGAG2_ObjectName(tool)
        local baseWeight = (esp.fruitBaseWeight and esp.fruitBaseWeight[name])
            or (esp.plantBaseWeight and esp.plantBaseWeight[name])
