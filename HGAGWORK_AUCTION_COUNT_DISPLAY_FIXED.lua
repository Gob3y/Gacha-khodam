...
function HaimiyachGAG2_AuctionParseRowsFromGameGui()
    ...
    for i, row in ipairs(rows) do
        ...
        if row:FindFirstChild("OUT_OF_STOCK", true) and row:FindFirstChild("OUT_OF_STOCK", true).Visible then stockNum = 0 end
    ...
    if #guiLots > 0 then
        local state = HaimiyachGAG2_AuctionState
        local liveLots = state and state.lots or {}

        -- If real snapshot lots already exist, do NOT replace them with GUI-only lots.
        -- Merge GUI row/stock/timer/button into the real lot so price + purchase lotId
        -- still follow AuctioneerController exactly.
        if type(liveLots) == "table" and #liveLots > 0 then
            local used = {}
            for i, g in ipairs(guiLots) do
                local gkey = HaimiyachGAG2_NameKey(HaimiyachGAG2_AuctionDisplayName(g))
                local target = nil
                for _, lot in ipairs(liveLots) do
                    local lkey = HaimiyachGAG2_NameKey(HaimiyachGAG2_AuctionDisplayName(lot))
                    local raw = HaimiyachGAG2_NameKey(lot.item or lot.Name or lot.name or "")
                    if not used[lot] and (lkey == gkey or raw == gkey or string.find(lkey, gkey, 1, true) or string.find(gkey, lkey, 1, true)) then
                        target = lot
                        break
                    end
                end
                if not target and liveLots[i] and not used[liveLots[i]] then target = liveLots[i] end
                if target then
                    used[target] = true
            ...
            pcall(HaimiyachGAG2_AuctionRememberItems, liveLots)
            state.optionVersion = (tonumber(state.optionVersion) or 0) + 1
            state.status = "SYNCED GAME GUI WITH SNAPSHOT + LIVE STOCK"
            return true
        end

        for _, lot in ipairs(guiLots) do pcall(HaimiyachGAG2_AuctionSyncLotStockFromLive, lot) end
        HaimiyachGAG2_AuctionState.lots = guiLots
        pcall(HaimiyachGAG2_AuctionRememberItems, guiLots)
        HaimiyachGAG2_AuctionState.optionVersion = (tonumber(HaimiyachGAG2_AuctionState.optionVersion) or 0) + 1
        HaimiyachGAG2_AuctionState.status = "LOADED " .. tostring(#guiLots) .. " LOTS FROM GAME AUCTION GUI"
        return true
    end
...
function HaimiyachGAG2_AuctionRefresh(force)
    ...
    -- Request the authoritative snapshot first. The GUI is only used to sync
    -- visible stock/timer/button, because GUI-only rows can have wrong price.
    ...
    if shouldRequest then
        ...
        if ok and result == nil then
            state.status = "REQUEST SENT, WAITING SNAPSHOT EVENT..."
        elseif not ok then
        ...
        if force then
            ...
            if state.lots and #state.lots > 0 then
                pcall(HaimiyachGAG2_AuctionParseRowsFromGameGui)
                return true
            end
    ...
    -- Fast GUI sync while the game's Auction window is visible, so stock/timer
    -- follows the changing cards without replacing real snapshot lots.
    if force or not state.lots or #state.lots == 0 or now - (tonumber(state.lastGuiScan) or 0) >= 0.45 then
        local pg = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
        local auctionGui = pg and pg:FindFirstChild("Auction")
        if auctionGui and (not auctionGui:IsA("ScreenGui") or auctionGui.Enabled) then
    ...
    if force then state.status = "NO AUCTION SNAPSHOT. OPEN AUCTION NPC ONCE, THEN REFRESH." end
    return false
end
...
function HaimiyachGAG2_AuctionListText()
    ...
    for _, lot in ipairs(lots) do
        ...
        local tag = HaimiyachGAG2_AuctionIsActive(lot) and "ACTIVE" or "EXPIRED"
