...
-- //                    HIDE PLANTS                              \ --
-- // ============================================================ \ --
-- Executor-friendly version: uses normal Transparency only.
-- It does not run during script load; it only works after the toggle is clicked.
local HidePlantsCache = {}
local HidePlantsConnections = {}

local function HidePlantsSafeSet(obj, prop, value)
    pcall(function()
        if obj then obj[prop] = value end
    end)
end

local function HidePlantsIsVisualObject(obj)
    return obj and (obj:IsA("BasePart") or obj:IsA("Decal") or obj:IsA("Texture"))
end

local function HidePlantsApplyVisualObject(obj, hide)
    if not HidePlantsIsVisualObject(obj) then return end

    if hide then
        if HidePlantsCache[obj] == nil then
            local oldTransparency = 0
            pcall(function() oldTransparency = obj.Transparency end)
            HidePlantsCache[obj] = oldTransparency
        end
        HidePlantsSafeSet(obj, "Transparency", 1)
    else
        local oldTransparency = HidePlantsCache[obj]
        if oldTransparency ~= nil then
            HidePlantsSafeSet(obj, "Transparency", oldTransparency)
            HidePlantsCache[obj] = nil
        end
    end
end
...
local function HidePlantsApplyObject(obj, hide)
    ...
    if HidePlantsIsVisualObject(obj) then
        HidePlantsApplyVisualObject(obj, hide)
        return
    end
    ...
    for _, child in ipairs(descendants) do
        if HidePlantsIsVisualObject(child) then
            HidePlantsApplyVisualObject(child, hide)
        end
    end
end

local function HidePlantsRestoreAll()
    for obj, oldTransparency in pairs(HidePlantsCache) do
        if obj then
            HidePlantsSafeSet(obj, "Transparency", oldTransparency)
        end
        HidePlantsCache[obj] = nil
...
local function HidePlantsStopListeners()
    for _, connection in ipairs(HidePlantsConnections) do
        pcall(function()
            if connection then connection:Disconnect() end
        end)
    end
    for i = #HidePlantsConnections, 1, -1 do
        HidePlantsConnections[i] = nil
    end
end

local function HidePlantsStartListeners()
    HidePlantsStopListeners()

    for _, plants in ipairs(HidePlantsGetContainers()) do
        local ok, connection = pcall(function()
            return plants.ChildAdded:Connect(function(plant)
        ...
        if ok and connection then
            HidePlantsConnections[#HidePlantsConnections + 1] = connection
        end
    end
end

local function HidePlantsRefresh()
    HidePlantsStopListeners()
    HidePlantsRestoreAll()

    if S.hidePlants then
        HidePlantsApplyCurrent()
        HidePlantsStartListeners()
    end
end

local function HidePlantsSetEnabled(state)
...
addToggle(VisualsPage, "HIDE PLANTS", false, function(v)
    HidePlantsSetEnabled(v)
end)
addToggle(VisualsPage, "HIDE OWN PLANT ONLY", false, function(v)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        unload = function()
            S.killed = true
            UI.Unloaded = true
            pcall(function() HidePlantsSetEnabled(false) end)
            pcall(RestoreDefaultGraphics)
            if fpsConnection then pcall(function() fpsConnection:Disconnect() end) end
            disconnectUiConnections()
            cleanContainer(PlayerGui)
