...
function HaimiyachGAG2_SprinklerSelectedOk(tool)
    if not tool then return false end
    if type(S.sprinklerTargets) ~= "table" or not picked(S.sprinklerTargets) then return true end
    local raw = tool:GetAttribute("Sprinkler") or tool:GetAttribute("SprinklerName") or tool.Name
    local key = HaimiyachGAG2_NameKey(raw)
...
function HaimiyachGAG2_SelectedSprinklerTools()
    ...
    for _, tool in ipairs(toolsByAttr("Sprinkler")) do
        if HaimiyachGAG2_SprinklerSelectedOk(tool) then
            out[#out + 1] = tool
        end
    end
    return out
...
function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    ...
    local hrp = root()
    local origin = center
    if mode == "AVATAR POSITION" and hrp then origin = hrp.Position end
    ...
    local function add(p)
        ...
        local k = string.format("%d:%d", math.floor(projected.X * 10), math.floor(projected.Z * 10))
        if not seen[k] then
            seen[k] = true
            candidates[#candidates + 1] = projected
        end
    end
    ...
    if mode == "MAPPING" then
        ...
        table.sort(grid, function(a, b)
            local c = center or origin
            local da = (Vector2.new(a.X, a.Z) - Vector2.new(c.X, c.Z)).Magnitude
            local db = (Vector2.new(b.X, b.Z) - Vector2.new(c.X, c.Z)).Magnitude
            return da < db
        end)
        for _, p in ipairs(grid) do add(p) end
    else
        add(origin)
        local ring = {4, 8, 12, 16, 20, 24}
        ...
        if mode == "GARDEN CENTER" then
            ...
            for i = 1, math.min(#grid, 60) do add(grid[i]) end
        end
    end

    local minDist = mode == "MAPPING" and 12 or 10
    for _, p in ipairs(candidates) do
        if not HaimiyachGAG2_TooClose2D(p, placed or {}, minDist) then return p end
    end
    for _, p in ipairs(candidates) do if p then return p end end
    return nil
end
...
function HaimiyachGAG2_TrowelMappingPositions(targets)
    ...
    local occupied = {}
    for _, op in ipairs(existingPlantPositions()) do
        local isTargetPos = false
        for _, target in ipairs(targets or {}) do
            if target.pos and (Vector2.new(op.X, op.Z) - Vector2.new(target.pos.X, target.pos.Z)).Magnitude < 1.5 then
                isTargetPos = true
                break
            end
        end
        if not isTargetPos then occupied[#occupied + 1] = op end
    end
...
local sprinklerOptions = {}
for _, name in ipairs(GEAR_NAMES) do
    if string.find(string.lower(tostring(name or "")), "sprinkler", 1, true) then
        sprinklerOptions[#sprinklerOptions + 1] = name
    end
end
if #sprinklerOptions == 0 then sprinklerOptions = { "Basic Sprinkler", "Advanced Sprinkler", "Godly Sprinkler", "Master Sprinkler" } end
addDropdown(UI.Pages["BOOSTS"], "SELECTED SPRINKLERS", sprinklerOptions, S.sprinklerTargets, true, function(sel) pickMulti(sel, S.sprinklerTargets) end)
