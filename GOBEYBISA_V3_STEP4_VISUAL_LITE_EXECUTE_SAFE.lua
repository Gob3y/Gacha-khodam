...
local function IsLocalOrOtherCharacterPart(obj)
    if not (obj and obj:IsA("BasePart")) then return false end
...
local function ApplyUltraFPSPart(obj)
    if not (obj and obj:IsA("BasePart")) then return end
    if IsLocalOrOtherCharacterPart(obj) then return end
...
local function ApplyFPSBoostGraphics()
    ...
    -- LIGHT = safest effects-only boost. BALANCED = original low graphics. ULTRA = balanced + part shadows off.
...
-- // ----------------------- HIDE VISUALS -----------------------
-- Client-side only. Restore-safe and not auto-enabled on startup.
local HideOwnPlantsConnection = nil
local HideOwnPlantsBackup = {}

local function HideBackupSet(obj, prop, value)
    if not obj then return end
    if HideOwnPlantsBackup[obj] == nil then HideOwnPlantsBackup[obj] = {} end
    if HideOwnPlantsBackup[obj][prop] == nil then
        pcall(function() HideOwnPlantsBackup[obj][prop] = obj[prop] end)
    end
    SafeSetProperty(obj, prop, value)
end

local function HideOwnPlantObject(obj)
    if not obj then return end
    if obj:IsA("BasePart") then
        HideBackupSet(obj, "LocalTransparencyModifier", 1)
        HideBackupSet(obj, "CastShadow", false)
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        HideBackupSet(obj, "Transparency", 1)
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
        HideBackupSet(obj, "Enabled", false)
    end
end

local function RestoreHideOwnPlants()
    for obj, props in pairs(HideOwnPlantsBackup) do
...
local function SetHideOwnPlantsEnabled(state)
    ...
    task.spawn(function()
        ...
        if plants and S.hideOwnPlants then
            HideOwnPlantsConnection = plants.DescendantAdded:Connect(function(obj)
                if not S.hideOwnPlants then return end
                task.defer(function()
                    if S.hideOwnPlants then HideOwnPlantObject(obj) end
                end)
            end)
        end
    end)
...
addToggle(UI.Pages["VISUALS"], "HIDE OWN PLANTS", false, function(v) SetHideOwnPlantsEnabled(v) end)
