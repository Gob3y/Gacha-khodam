...
function HaimiyachGAG2_MailFruitItemKey(key, value)
    ...
    if type(value) == "table" then
        return value.Id or value.UUID or value.Uuid or value.Guid or value.ItemId or key
    end
    return key
end
...
function HaimiyachGAG2_AddMailHarvestedFruits(items, selected)
    ...
    for key, value in pairs(inv("HarvestedFruits")) do
        if #items >= 20 then break end
        if HaimiyachGAG2_IsGiftableMailEntry("HarvestedFruits", key, value) and HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value) then
...
function HaimiyachGAG2_MailGardenFruitSelected(obj, selected)
    ...
    return HaimiyachGAG2_SelectedNameOk(obj, selected)
end
