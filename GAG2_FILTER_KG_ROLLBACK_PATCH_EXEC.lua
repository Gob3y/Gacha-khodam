    ...
    while node and node ~= Workspace and guard < 8 do
        ...
        if plantId ~= nil or fruitId ~= nil then
            if esp and type(esp.calculateWeight) == "function" then
                local sizeMulti = tonumber(node:GetAttribute("SizeMultiplier") or node:GetAttribute("SizeMulti") or node:GetAttribute("Size"))
                local ok, weight = pcall(function()
                    return esp.calculateWeight(node, sizeMulti)
                end)
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    local isGardenObj = false
    if obj.GetAttribute then
        isGardenObj = (obj:GetAttribute("PlantId") ~= nil) or (obj:GetAttribute("FruitId") ~= nil)
    end

    local kg = HaimiyachGAG2_InstanceKg(obj)
    if not kg and not isGardenObj then
        local node = obj.Parent
        local guard = 0
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    if not (tool and GardenESP) then return nil end
    if not GardenESP.modulesReady and type(GardenESP.initModules) == "function" then
        pcall(GardenESP.initModules)
    end

    local sizeMulti = tonumber(tool:GetAttribute("SizeMultiplier") or tool:GetAttribute("SizeMulti") or tool:GetAttribute("Size")) or 1
    local overtimeGrowth = tonumber(tool:GetAttribute("OvertimeGrowth")) or 1
    local name = HaimiyachGAG2_ObjectName(tool)
    local baseWeight = (GardenESP.fruitBaseWeight and GardenESP.fruitBaseWeight[name]) or (GardenESP.plantBaseWeight and GardenESP.plantBaseWeight[name])
    baseWeight = tonumber(baseWeight)
    if baseWeight and baseWeight > 0 then
        return baseWeight * sizeMulti * overtimeGrowth
    end
