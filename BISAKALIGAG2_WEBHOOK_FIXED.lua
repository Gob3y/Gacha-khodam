...
local function cleanPurchaseName(name)
...
local function isSprinklerGearName(name)
    name = string.lower(tostring(name or ""))
    return string.find(name, "sprinkler", 1, true) ~= nil
end

local function addPurchaseCounter(counter, name, amount)
    if type(counter) ~= "table" then return end
    name = cleanPurchaseName(name)
    amount = math.floor(tonumber(amount) or 1)
    if amount < 1 then amount = 1 end
    counter[name] = (tonumber(counter[name]) or 0) + amount
end

local function trackPurchase(kind, name, amount)
    amount = math.floor(tonumber(amount) or 1)
    if amount < 1 then amount = 1 end
    name = cleanPurchaseName(name)
    ...
    if kind == "Seed" then
        ...
        addPurchaseCounter(Stats.purchaseSeeds, name, amount)
    elseif kind == "Gear" then
        Stats.gearBought = (tonumber(Stats.gearBought) or 0) + amount
        Stats.lastGearBuy = name
        addPurchaseCounter(Stats.purchaseGear, name, amount)
        if isSprinklerGearName(name) then
            Stats.sprinklerBought = (tonumber(Stats.sprinklerBought) or 0) + amount
            Stats.lastSprinklerBuy = name
            addPurchaseCounter(Stats.purchaseSprinklers, name, amount)
        end
    end
end
    ...
    for _, s in ipairs(CATALOG) do
        ...
        if S.autoBuyAllSeeds or S.buySeeds[s.name] then
            ...
            while bought < S.buyPerTick do
                ...
                trackPurchase("Seed", s.name, 1); bought = bought + 1
...
local function webhookFieldValue(text, maxLen)
...
local function purchaseCounterSummary(counter, limit)
    local rows = {}
    if type(counter) == "table" then
        for name, count in pairs(counter) do
            count = tonumber(count) or 0
            if count > 0 then
                rows[#rows + 1] = { name = cleanPurchaseName(name), count = count }
            end
        end
    end
...
local function purchaseSummary(total, counter, last)
    total = tonumber(total) or 0
    last = cleanPurchaseName(last or "NONE")
    return webhookFieldValue("Total: " .. tostring(total) .. "\nLast: " .. last .. "\nItems:\n" .. purchaseCounterSummary(counter, 10), 1000)
end

local function webhookStatusCode(res)
    if type(res) ~= "table" then return nil end
    return tonumber(res.StatusCode or res.Status or res.status_code or res.status)
end

local function webhookRequestGood(ok, res)
    if not ok then return false, "request" end
    local code = webhookStatusCode(res)
    if code == nil then
...
local function sendWebhook(isTest)
    if not httpRequest then notify("Webhook", "Executor exposes no HTTP request fn", 4); return false end
    ...
    local payload = {
        ...
        embeds = { {
            ...
            color = 5763719,
            fields = {
                { name = "Account", value = webhookFieldValue("Player: " .. tostring(LocalPlayer.Name) .. "\nGarden: " .. gardenName), inline = false },
                { name = "Balance", value = webhookFieldValue("Sheckles: " .. fmt(getSheckles()) .. "\nTokens: " .. fmt(getTokens())), inline = false },
                { name = "Farm Stats", value = webhookFieldValue(string.format("Bought: %d\nSeeds Bought: %d\nGear Bought: %d\nSprinklers Bought: %d\nPlanted: %d\nHarvested: %d\nSold: %d\nEarned: %s", tonumber(Stats.bought) or 0, tonumber(Stats.seedBought) or 0, tonumber(Stats.gearBought) or 0, tonumber(Stats.sprinklerBought) or 0, tonumber(Stats.planted) or 0, tonumber(Stats.harvested) or 0, tonumber(Stats.sold) or 0, fmt(Stats.earned))), inline = false },
                { name = "Seed Purchases", value = purchaseSummary(Stats.seedBought, Stats.purchaseSeeds, Stats.lastSeedBuy), inline = false },
                { name = "Gear Purchases", value = purchaseSummary(Stats.gearBought, Stats.purchaseGear, Stats.lastGearBuy), inline = false },
                { name = "Sprinkler Purchases", value = purchaseSummary(Stats.sprinklerBought, Stats.purchaseSprinklers, Stats.lastSprinklerBuy), inline = false },
                { name = "Activity", value = webhookFieldValue(string.format("Sprinklers Placed: %d\nWatered: %d\nTamed: %d\nOpened: %d\nStolen: %d", tonumber(Stats.sprinklers) or 0, tonumber(Stats.watered) or 0, tonumber(Stats.tamed) or 0, tonumber(Stats.opened) or 0, tonumber(Stats.stolen) or 0)), inline = false },
                { name = "Seed Event", value = webhookFieldValue("Available: " .. HaimiyachGAG2_ReportClean(getSeedEventAvailableText()) .. "\nClaimed: " .. HaimiyachGAG2_ReportClean(getSeedEventClaimText()) .. "\nLast: " .. HaimiyachGAG2_ReportClean(getSeedEventLastText())), inline = false },
                { name = "Runtime", value = webhookFieldValue("Uptime: " .. hms(os.clock() - Stats.startAt) .. "\nFPS: " .. tostring(currentFps) .. "\nPing: " .. getPingText()), inline = false },
            },
    ...
    local good, code = webhookRequestGood(ok, res)
    ...
    local payload = {
        ...
        embeds = { {
            ...
            fields = {
                ...
                { name = "Reason", value = webhookFieldValue(tostring(reason or "Disconnect detected")), inline = false },
                { name = "Detail", value = webhookFieldValue(tostring(detail or "No extra detail")), inline = false },
                { name = "Server", value = webhookFieldValue("PlaceId: " .. tostring(game.PlaceId) .. "\nJobId: " .. tostring(game.JobId ~= "" and game.JobId or "Unknown")), inline = false },
                { name = "Balance", value = webhookFieldValue("Sheckles: " .. fmt(getSheckles()) .. "\nTokens: " .. fmt(getTokens())), inline = false },
                { name = "Seed Purchases", value = purchaseSummary(Stats.seedBought, Stats.purchaseSeeds, Stats.lastSeedBuy), inline = false },
                { name = "Gear Purchases", value = purchaseSummary(Stats.gearBought, Stats.purchaseGear, Stats.lastGearBuy), inline = false },
                { name = "Sprinkler Purchases", value = purchaseSummary(Stats.sprinklerBought, Stats.purchaseSprinklers, Stats.lastSprinklerBuy), inline = false },
