...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    local function parse(v)
        ...
        local n = string.match(text, "([%d%.]+)%s*[Kk][Gg]")
        if n then return tonumber(n) end
        n = string.match(text, "([%d%.]+)%s*[Gg]")
        if n then return (tonumber(n) or 0) / 1000 end
    ...
    local function scan(inst)
        ...
        for _, attr in ipairs(attrs) do
            local kg = parse(inst:GetAttribute(attr))
            if kg then return kg end
        end
        return parse(inst.Name)
    end
    local kg = scan(obj)
    if kg then return kg end
    local node = obj.Parent
