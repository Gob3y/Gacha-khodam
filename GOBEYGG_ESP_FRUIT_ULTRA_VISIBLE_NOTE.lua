...
local function worldPetNames()
    local seen, names = {}, {}
    for _, w in ipairs(wildPets()) do
        local nm = tostring(w.name or "")
...
-- AUTO BUY selected world pets: walk up (teleport) to selected affordable unowned wild pet and buy/tame it.
...
loopOn(function() return S.autoBuyPets end, function() return S.petBuyInterval end, function()
    if not picked(S.buyWorldPets) then return end
    for _, w in ipairs(wildPets()) do
        if not S.autoBuyPets then break end
        local petName = tostring(w.name or "")
        if S.buyWorldPets[petName] and w.owner == 0 and w.price > 0 and w.price <= S.maxPetPrice and getSheckles() >= w.price then
...
local function ApplyFPSBoostGraphics()
    ...
    local function isHaimiyachESPObject(obj)
        local cur = obj
        while cur and cur ~= Workspace do
            local n = tostring(cur.Name or "")
            if n == "HaimiyachGardenFruitESP" or n == "HaimiyachBackpackESP" or n == "HaimiyachBackpackTotalESP" then
                return true
            end
            cur = cur.Parent
        end
    ...
    local function shouldHideUltraVisual(obj)
        if not obj or isCharacterOrTool(obj) or isHaimiyachESPObject(obj) then
            return false
        end
    ...
    local function applyFPSObject(obj, mode)
        ...
        -- Keep custom ESP labels visible even when FPS BOOST ULTRA hides garden visuals.
        -- Without this, ULTRA can disable the BillboardGui because it is parented under plants/fruits.
        if isHaimiyachESPObject(obj) then
            pcall(function()
...
UI.worldPetDrop = addDropdown(UI.Pages["PETS"], "WORLD PETS TO BUY", worldPetNames(), S.buyWorldPets, true, function(sel) pickMulti(sel, S.buyWorldPets) end)
addButton(UI.Pages["PETS"], "REFRESH WORLD PET LIST", function() UI.worldPetDrop.SetOptions(worldPetNames()) end)
...
new("TextLabel", {
    ...
    BackgroundTransparency = 1,
    Text = "Note: Turn off FPS BOOST first before using Fruit ESP.",
    TextColor3 = T.Muted,
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        toolsByAttr = toolsByAttr, plantGrid = plantGrid, ownedPetNames = ownedPetNames, worldPetNames = worldPetNames, myBasePos = myBasePos,
