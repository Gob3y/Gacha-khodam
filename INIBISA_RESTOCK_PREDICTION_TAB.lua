...
local S = {
    ...
    autoBuyPets = false, buyWorldPets = {}, wildPetRarities = {}, wildPetMaxPrice = 5000000, wildPetOnlyUnowned = true, maxPetPrice = 25000, petTeleport = true, petBuyInterval = 5,
...
local CONFIG_KEYS = {
    ...
    "autoEquipPets", "autoPetSlot", "bestPetMode", "autoBuyPets", "buyWorldPets", "wildPetRarities", "wildPetMaxPrice", "wildPetOnlyUnowned", "maxPetPrice", "petTeleport", "petBuyInterval",
    ...
    "espGardenFruit", "espGardenGrowing", "espGardenValue", "espBackpackFruit", "espBackpackValue", "espBackpackTotal", "espWildPet", "espWildPetDetails", "notifyRareWeather", "rareWeatherTargets", "autoExecute", "autoExecuteUrl", "uiScale", "uiKeybindName"
}
local CONFIG_KEYSET = {}
...
HaimiyachGAG2_RestockModeOptions = { "OFF", "LOCAL HISTORY", "LIVE STOCK ONLY", "HISTORY + LIVE STOCK" }
...
function HaimiyachGAG2_FormatClock(sec)
    ...
    if sec >= 60 then return string.format("%dm %02ds", sec // 60, sec % 60) end
...
function HaimiyachGAG2_WildPetMatchesAuto(w)
    ...
    local maxPrice = tonumber(S.wildPetMaxPrice or S.maxPetPrice) or 0
    if maxPrice > 0 and (tonumber(w.price) or 0) > maxPrice then return false end
...
function HaimiyachGAG2_RestockPredictionRows(limit)
    ...
    for _, data in ipairs(CATALOG or {}) do
        ...
        if name ~= "" and HaimiyachGAG2_RestockSeedPassFilter(name) then
            ...
            if mode ~= "LIVE STOCK ONLY" or stock > 0 then
                ...
                local rarityScore = HaimiyachGAG2_SeedRarityScore(name)
                local price = tonumber(data.price) or 0
                local score = (stock > 0 and 100000000 or 0) + rarityScore * 1000000 + math.min(since, 86400) + (price / 100)
                if mode == "LOCAL HISTORY" and lastSeen <= 0 then
                    score = score - 500000
                end
                rows[#rows + 1] = {
                    name = name,
                    rarity = HaimiyachGAG2_SeedRarity(name),
                    price = price,
                    stock = stock,
                    lastSeen = lastSeen,
                    score = score,
...
function HaimiyachGAG2_RestockPredictionText()
    local rows = HaimiyachGAG2_RestockPredictionRows(3)
    if #rows == 0 then return "NONE" end
    local out = {}
    for _, row in ipairs(rows) do
        local status = row.stock > 0 and ("IN STOCK x" .. tostring(row.stock)) or HaimiyachGAG2_RestockAgoText(row.lastSeen)
        out[#out + 1] = tostring(row.name) .. " · " .. tostring(row.rarity) .. " · " .. status
    end
    return table.concat(out, "\n")
end

function HaimiyachGAG2_RestockTargetText()
    local rows = HaimiyachGAG2_RestockPredictionRows(6)
    local watched = HaimiyachGAG2_RestockSelectedSeedCount()
    local inStock, total = 0, 0
    for _, row in ipairs(rows) do
        if HaimiyachGAG2_RestockSeedIsTarget(row.name) then
            total = total + 1
            if row.stock > 0 then inStock = inStock + 1 end
        end
    end
    if watched > 0 then
        return tostring(inStock) .. "/" .. tostring(watched) .. " watched in stock"
    end
    return tostring(inStock) .. "/" .. tostring(total) .. " filtered in stock"
end
...
function HaimiyachGAG2_RestockNotifyStep()
    ...
    HaimiyachGAG2_RestockScan()
    for _, row in ipairs(HaimiyachGAG2_RestockPredictionRows(10)) do
        if row.stock > 0 and HaimiyachGAG2_RestockSeedIsTarget(row.name) and not HaimiyachGAG2_RestockNotified[row.name] then
            HaimiyachGAG2_RestockNotified[row.name] = true
            notify("Seed Restock", tostring(row.name) .. " is in stock x" .. tostring(row.stock), 5)
        end
    end
end
...
function HaimiyachGAG2_UpdateWildPetESP()
    ...
    for _, w in ipairs(wildPets()) do
        ...
        if part and part.Parent then
            ...
            if type(S.espWildPetDetails) ~= "table" or S.espWildPetDetails.PRICE then lines[#lines + 1] = fmt(w.price or 0) .. "¢" end
...
UI.restockDashLabel = addLabel(UI.Pages["DASHBOARD"], "SEED RESTOCK: ?")
...
-- RESTOCK
addSection(UI.Pages["RESTOCK"], "SEED RESTOCK")
UI.restockNextLabel = addLabel(UI.Pages["RESTOCK"], "NEXT RESTOCK: ?")
UI.restockPredictionLabel = addLabel(UI.Pages["RESTOCK"], "PREDICTION: NONE")
UI.restockTargetLabel = addLabel(UI.Pages["RESTOCK"], "TARGET STATUS: NONE")
UI.restockRecentLabel = addLabel(UI.Pages["RESTOCK"], "RECENT RARE SEEDS: NONE")
addSection(UI.Pages["RESTOCK"], "SETTINGS")
addDropdown(UI.Pages["RESTOCK"], "PREDICTION MODE", HaimiyachGAG2_RestockModeOptions, S.restockPredictionMode, false, function(v)
    S.restockPredictionMode = tostring(v or "HISTORY + LIVE STOCK")
    saveConfig(true)
end)
addDropdown(UI.Pages["RESTOCK"], "SEED RARITY FILTER", HaimiyachGAG2_RestockRarityOptions, S.restockRarityFilter, false, function(v)
    S.restockRarityFilter = tostring(v or "ALL")
    saveConfig(true)
end)
addDropdown(UI.Pages["RESTOCK"], "WATCHED SEEDS", SEED_NAMES, S.restockWatchedSeeds, true, function(sel)
    if type(S.restockWatchedSeeds) ~= "table" then S.restockWatchedSeeds = {} end
    pickMulti(sel, S.restockWatchedSeeds)
    saveConfig(true)
end)
addToggle(UI.Pages["RESTOCK"], "NOTIFY WATCHED SEED", S.restockNotifyWatched, function(v) S.restockNotifyWatched = v; saveConfig(true) end)
addToggle(UI.Pages["RESTOCK"], "AUTO BUY WATCHED SEEDS", S.restockAutoBuyWatched, function(v) S.restockAutoBuyWatched = v; saveConfig(true) end)
addToggle(UI.Pages["RESTOCK"], "AUTO HOP IF TARGET NOT FOUND", S.restockAutoHopTarget, function(v) S.restockAutoHopTarget = v; saveConfig(true) end)
...
addSlider(UI.Pages["PETS_CRATES"], "MAX WILD PET PRICE", 0, 10000000, S.wildPetMaxPrice or S.maxPetPrice or 0, 0, function(v)
    S.wildPetMaxPrice = v
    S.maxPetPrice = v
    saveConfig(true)
end)
...
task.spawn(function()
    while not S.killed do
        ...
        UI.SafeSetText(UI.restockDashLabel, "SEED RESTOCK: " .. HaimiyachGAG2_SeedRestockText())
        if UI.restockNextLabel then UI.SafeSetText(UI.restockNextLabel, "NEXT RESTOCK: " .. HaimiyachGAG2_SeedRestockText()) end
        if UI.restockPredictionLabel then UI.SafeSetText(UI.restockPredictionLabel, "PREDICTION:\n" .. HaimiyachGAG2_RestockPredictionText()) end
