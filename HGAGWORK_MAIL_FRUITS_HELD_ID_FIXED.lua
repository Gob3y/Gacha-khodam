...
local function promptCarrier(prompt)
    ...
    local fallback = nil
    local guard = 0
    while node and node ~= Workspace and guard < 12 do
        guard = guard + 1
        if node:GetAttribute("PlantId") ~= nil then
            if not fallback then fallback = node end
            if node:GetAttribute("FruitId") ~= nil then
                return node
            end
        end
        node = node.Parent
...
function HaimiyachGAG2_ParseKgText(value, allowPlainNumber)
    ...
    local n = string.match(text, "([%d%.]+)%s*kg")
    if n then
        local v = tonumber(n)
        if v and v > 0 then return v end
    end
    n = string.match(text, "([%d%.]+)%s*g")
    if n then
        local v = tonumber(n)
        if v and v > 0 then return v / 1000 end
    end
    if allowPlainNumber then
        n = string.match(text, "^%s*([%d%.]+)%s*$")
        if n then
            local v = tonumber(n)
            if v and v > 0 then return v end
        end
...
function HaimiyachGAG2_IsWeightFieldName(name)
    name = string.lower(tostring(name or ""))
    return string.find(name, "weight", 1, true) or string.find(name, "kg", 1, true) or string.find(name, "kilogram", 1, true) or string.find(name, "mass", 1, true)
end

function HaimiyachGAG2_InstanceKg(inst)
    if not inst then return nil end
    local attrs = { "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }
    for _, attr in ipairs(attrs) do
        local kg = HaimiyachGAG2_ParseKgText(inst:GetAttribute(attr), true)
        if kg then return kg end
    end
    if inst:IsA("NumberValue") and HaimiyachGAG2_IsWeightFieldName(inst.Name) then
        local kg = HaimiyachGAG2_ParseKgText(inst.Value, true)
        if kg then return kg end
    elseif inst:IsA("StringValue") and HaimiyachGAG2_IsWeightFieldName(inst.Name) then
        local kg = HaimiyachGAG2_ParseKgText(inst.Value, true)
        if kg then return kg end
    elseif inst:IsA("TextLabel") or inst:IsA("TextButton") then
        local kg = HaimiyachGAG2_ParseKgText(inst.Text, false)
        if kg then return kg end
    end
    return HaimiyachGAG2_ParseKgText(inst.Name, false)
end

function HaimiyachGAG2_ObjectKg(obj)
    if not obj then return nil end
    local kg = HaimiyachGAG2_InstanceKg(obj)
    if kg then return kg end
    local node = obj.Parent
    local guard = 0
    while node and node ~= Workspace and guard < 10 do
        guard = guard + 1
        kg = HaimiyachGAG2_InstanceKg(node)
        if kg then return kg end
        node = node.Parent
    end
    local checked = 0
...
function HaimiyachGAG2_ObjectMutation(obj)
    if not obj then return nil end
    local attrs = { "Mutation", "Mutations", "Variant", "FruitMutation", "SeedMutation", "MutationName" }
    local function clean(v)
        if type(v) ~= "string" then return nil end
        if v == "" or v == "None" or v == "Normal" then return nil end
        return v
    end
    local function scan(inst)
        if not inst then return nil end
        for _, attr in ipairs(attrs) do
            local v = clean(inst:GetAttribute(attr))
...
function HaimiyachGAG2_SelectedMutationOk(obj, selected)
    ...
    local got = HaimiyachGAG2_NameKey(mutation)
    if got == "" then return false end
    for name, enabled in pairs(selected) do
        if enabled == true then
            local want = HaimiyachGAG2_NameKey(name)
            if want ~= "" and (got == want or string.find(got, want, 1, true) or string.find(want, got, 1, true)) then
                return true
            end
        end
    end
...
local function ripeHarvests(selectedTargets)       -- own ripe fruit (tag "HarvestPrompt")
    local out = {}
    local hasTarget = HaimiyachGAG2_HasSelection(selectedTargets)
        ...
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            ...
            if pid then
                ...
                if uid == nil or uid == LocalPlayer.UserId then
                    if (not hasTarget) or HaimiyachGAG2_SelectedNameOk(m, selectedTargets) then
...
local function wildPets()
    ...
    if ref then for _, p in ipairs(ref:GetChildren()) do
        if p:IsA("BasePart") then
            ...
            out[#out + 1] = {
                part = p,
                name = p:GetAttribute("PetName") or p:GetAttribute("Name") or p.Name,
...
local S = {
    ...
    autoSell = false, autoSellFruits = false, sellInterval = 15, sellKeepMutations = {}, sellKeepKgMode = "OFF", sellKeepKgValue = 0,
    ...
    -- auction
    autoBuyAuction = false, auctionBuyItems = {}, auctionBuyDelay = 3, auctionShowActive = true, auctionNameFilter = "",
    -- steal
    autoSteal = false, stealTeleport = true, stealReturnBase = true, stealDelay = 0.05,
    -- misc
    autoMail = false, autoAcceptGift = false, autoSendMail = false, mailTargetUsername = "", mailNote = "Here is a gift!", mailSeedTargets = {}, mailFruitTargets = {}, mailHarvestGarden = true, mailGearTargets = {}, mailPetTargets = {}, mailCrateTargets = {}, mailSeedPackTargets = {}, mailSprinklerTargets = {}, mailWateringCanTargets = {}, mailMushroomTargets = {}, mailGnomeTargets = {}, mailRaccoonTargets = {}, mailTrowelTargets = {}, mailPropTargets = {}, mailEmptyPotTargets = {}, mailOtherCategories = {}, mailSendCount = 1, mailSendDelay = 30, autoHop = false, hopInterval = 0, hopConditions = {}, autoReconnect = false, reconnectDelay = 5,
    -- protect
...
function HaimiyachGAG2_MailLooseName(v)
    v = HaimiyachGAG2_NormalMailName(v)
    v = v:gsub("%b[]", " ")
    v = v:gsub("%b()", " ")
    v = v:gsub("%s*x%d+%s*$", "")
    v = v:gsub("%s+%d+%.?%d*%s*kg%s*$", "")
    v = v:gsub("%s+kg%s*$", "")
    v = v:gsub("%s+", " ")
    v = v:gsub("^%s+", ""):gsub("%s+$", "")
    return v
...
function HaimiyachGAG2_SelectedTextMatch(selected, name)
    ...
    local looseName = HaimiyachGAG2_MailLooseName(name)
    if name == "" and looseName == "" then return false end
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local selName = HaimiyachGAG2_NormalMailName(sel)
            local looseSel = HaimiyachGAG2_MailLooseName(sel)
            if selName == name or (looseSel ~= "" and looseSel == looseName) then
                return true
            end
        end
    end
...
function HaimiyachGAG2_AddMailFromInventory(items, category, selected, maxEach, uniqueOnly)
    ...
    for key, value in pairs(inv(category)) do
        ...
        if HaimiyachGAG2_SelectedTextMatch(selected, HaimiyachGAG2_MailEntryName(category, key, value)) and HaimiyachGAG2_IsGiftableMailEntry(category, key, value) then
...
function HaimiyachGAG2_IsGiftableMailEntry(category, key, value)
    if category == "HarvestedFruits" then
        if type(value) ~= "table" then return false end
        if value.Locked == true or value.Favorite == true or value.IsFavorite == true then return false end
        return value.Id ~= nil or value.UUID ~= nil or value.Uuid ~= nil or value.Guid ~= nil or value.ItemId ~= nil or (type(key) == "string" and key ~= "")
    end
    if category == "Pets" then
        return type(value) == "table" and value.Id ~= nil and value.Equipped ~= true
    end
    if type(value) == "number" then
        return value > 0
    end
    if type(value) == "table" then
        return HaimiyachGAG2_MailStackCount(value) > 0
    end
    return false
end

function HaimiyachGAG2_MailCatalogDisplayName(category, key, value)
    local display = nil
    pcall(function()
        local controllers = LocalPlayer and LocalPlayer:FindFirstChild("PlayerScripts") and LocalPlayer.PlayerScripts:FindFirstChild("Controllers")
        local mb = controllers and controllers:FindFirstChild("MailboxController")
        local cat = mb and mb:FindFirstChild("MailboxItemCatalog")
        if cat then
            local catalog = require(cat)
            if catalog and catalog.Resolve then
                local d = catalog.Resolve(category, key, value)
                if type(d) == "string" and d ~= "" then display = d end
            end
        end
    end)
    return display
...
function HaimiyachGAG2_MailFruitBaseName(key, value)
    if type(value) == "table" then
        return value.FruitName or value.Name or value.ItemName or value.SeedName or value.Type or tostring(key)
    end
    return tostring(key)
end

function HaimiyachGAG2_MailFruitDisplayName(key, value)
    local display = HaimiyachGAG2_MailCatalogDisplayName("HarvestedFruits", key, value)
    if type(display) == "string" and display ~= "" then return display end
    local base = HaimiyachGAG2_MailFruitBaseName(key, value)
    if type(value) == "table" and type(value.Mutation) == "string" and value.Mutation ~= "" then
        return tostring(base) .. " [" .. tostring(value.Mutation) .. "]"
    end
    return tostring(base)
end

function HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    for sel, enabled in pairs(selected) do
        if enabled == true and HaimiyachGAG2_MailLooseName(sel) == "all fruits" then
            return true
        end
    end
    local base = HaimiyachGAG2_MailFruitBaseName(key, value)
    local display = HaimiyachGAG2_MailFruitDisplayName(key, value)
    if HaimiyachGAG2_SelectedTextMatch(selected, base) or HaimiyachGAG2_SelectedTextMatch(selected, display) then
        return true
    end
    if type(value) == "table" then
        if HaimiyachGAG2_SelectedTextMatch(selected, value.FruitName or "") then return true end
        if HaimiyachGAG2_SelectedTextMatch(selected, value.Name or "") then return true end
    end
    return false
end

function HaimiyachGAG2_MailFruitItemKey(key, value)
    -- MailboxController uses the inventory table key as ItemKey, while the entry value must have Id.
    -- Keep key priority for replica entries; use Id fallback only for tool/backpack fallback.
    if type(key) == "string" and key ~= "" then
        return key
    end
    if type(value) == "table" then
        return value.Id or value.UUID or value.Uuid or value.Guid or value.ItemId or value.HarvestedFruitId or key
    end
    return key
end

function HaimiyachGAG2_MailSelectedTextList(selected)
    local arr = {}
    if type(selected) == "table" then
        for name, enabled in pairs(selected) do
            if enabled == true then arr[#arr + 1] = tostring(name) end
        end
    end
    table.sort(arr)
...
function HaimiyachGAG2_IsMailHarvestedFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if tool:GetAttribute("PetId") ~= nil or tool:GetAttribute("Pet") ~= nil then return false end

    local hasHarvested = tool:GetAttribute("HarvestedFruit") ~= nil or tool:GetAttribute("HarvestedFruitId") ~= nil
    local hasFruit = tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("FruitName") ~= nil or tool:GetAttribute("FruitProxy") ~= nil
    local c = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
    c = string.lower(c)
    local categoryFruit = c == "fruit" or c == "fruits" or c == "harvestedfruits" or c == "harvestedfruit"

    local isSeedTool = tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil
    local lowerName = string.lower(tostring(tool.Name or ""))
    local nameLooksSeed = string.find(lowerName, "seed", 1, true) ~= nil and not string.find(lowerName, "seed pack", 1, true)

    if (isSeedTool or nameLooksSeed) and not (hasHarvested or hasFruit or categoryFruit) then
        return false
    end

    if tool:GetAttribute("Sprinkler") ~= nil or tool:GetAttribute("WateringCan") ~= nil or tool:GetAttribute("Trowel") ~= nil or tool:GetAttribute("Shovel") ~= nil or tool:GetAttribute("Crate") ~= nil or tool:GetAttribute("Egg") ~= nil or tool:GetAttribute("Mushroom") ~= nil or tool:GetAttribute("Gnome") ~= nil then
        return false
    end

    if hasHarvested or hasFruit or categoryFruit then return true end

    if HaimiyachGAG2_ParseKgText and HaimiyachGAG2_ParseKgText(tool.Name, false) and not nameLooksSeed then
        return true
    end
    return false
end

function HaimiyachGAG2_GetMailFruitTools()
    local out = {}
    local function scan(container)
        if not container then return end
        for _, tool in ipairs(container:GetChildren()) do
            if HaimiyachGAG2_IsMailHarvestedFruitTool(tool) then
                out[#out + 1] = tool
            end
        end
    end
    scan(LocalPlayer.Character)
    scan(LocalPlayer:FindFirstChildOfClass("Backpack") or LocalPlayer:FindFirstChild("Backpack"))
    return out
end

function HaimiyachGAG2_MailFruitToolName(tool)
    if not tool then return "" end
    for _, attr in ipairs({ "FruitName", "Fruit", "CorePartName", "PlantName", "CropName", "ItemName" }) do
    ...
    n = n:gsub("%b[]", " ")
    n = n:gsub("%b()", " ")
    n = n:gsub("%s+%d+%.?%d*%s*kg%s*$", "")
    n = n:gsub("%s+x%d+%s*$", "")
    n = n:gsub("%s+", " "):gsub("^%s+", ""):gsub("%s+$", "")
    return n
end

function HaimiyachGAG2_MailFruitToolItemKey(tool)
    if not tool then return nil end
    -- For fruit tools, the most reliable value is usually Id.
    -- InventoryKey/ItemKey are kept first only if the game exposes them directly.
    for _, attr in ipairs({ "InventoryKey", "ItemKey", "Id", "ID", "HarvestedFruit", "FruitId", "HarvestedFruitId", "UID", "UUID", "Uuid", "Guid", "ItemId" }) do
        local v = tool:GetAttribute(attr)
...
function HaimiyachGAG2_MailFruitToolCandidates(tool)
    local candidates = {}
    if not tool then return candidates end
    for _, attr in ipairs({ "InventoryKey", "ItemKey", "Id", "ID", "FruitId", "HarvestedFruitId", "UID", "UUID", "Uuid", "Guid", "ItemId", "HarvestedFruit" }) do
        local v = tool:GetAttribute(attr)
        if v ~= nil and tostring(v) ~= "" and tostring(v) ~= "true" and tostring(v) ~= "false" then
            candidates[tostring(v)] = true
        end
    end
    return candidates
end

function HaimiyachGAG2_FindMailFruitReplicaKeyForTool(tool)
    if not tool then return nil, nil end
    local candidates = HaimiyachGAG2_MailFruitToolCandidates(tool)

    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" and HaimiyachGAG2_IsGiftableMailEntry("HarvestedFruits", key, value) then
            if candidates[tostring(key)] then return tostring(key), value end
            for _, field in ipairs({ "Id", "ID", "UUID", "Uuid", "Guid", "ItemId", "HarvestedFruitId", "FruitId", "UID" }) do
                local v = value[field]
                if v ~= nil and candidates[tostring(v)] then
    ...
    local toolLoose = HaimiyachGAG2_MailLooseName(HaimiyachGAG2_MailFruitToolName(tool))
    if toolLoose ~= "" then
        for key, value in pairs(inv("HarvestedFruits")) do
            if type(value) == "table" and HaimiyachGAG2_IsGiftableMailEntry("HarvestedFruits", key, value) then
                local base = HaimiyachGAG2_MailLooseName(HaimiyachGAG2_MailFruitBaseName(key, value))
                local display = HaimiyachGAG2_MailLooseName(HaimiyachGAG2_MailFruitDisplayName(key, value))
                if base == toolLoose or display == toolLoose or (base ~= "" and string.find(toolLoose, base, 1, true)) then
                    return tostring(key), value
                end
            end
        end
    ...
    return HaimiyachGAG2_MailFruitToolItemKey(tool), nil
end

function HaimiyachGAG2_EquipMailToolNow(tool)
    if not (tool and tool:IsA("Tool")) then return false, "fruit tool not found" end
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(2)
    if not char then return false, "character not found" end
    local hum = char:FindFirstChildOfClass("Humanoid")

    if tool.Parent ~= char then
        pcall(function() if hum and hum.UnequipTools then hum:UnequipTools() end end)
        task.wait(0.15)
        pcall(function()
            if hum and hum.EquipTool then
                hum:EquipTool(tool)
            else
                tool.Parent = char
            end
    ...
    while os.clock() - start < 2.5 do
        ...
        pcall(function()
            ...
            if h and h.EquipTool then h:EquipTool(tool) else tool.Parent = char end
        end)
    end
    ...
    return false, "failed to hold fruit"
end

function HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return nil end
    local char = LocalPlayer.Character
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if tool and tool:IsA("Tool") and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool) then
                return tool
            end
        end
    end
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetMailFruitTools and HaimiyachGAG2_GetMailFruitTools() or {}) do
        if tool and tool.Parent and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool) then
            return tool
        end
    end
    return nil
...
function HaimiyachGAG2_EquipSelectedMailFruit(selected)
    local tool = HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    if not tool then return nil, "selected fruit tool not found" end
    local ok, err = HaimiyachGAG2_EquipMailToolNow(tool)
    if not ok then return nil, err end
    return tool, nil
end

function HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    if not HaimiyachGAG2_IsMailHarvestedFruitTool(tool) then return false end
    if HaimiyachGAG2_MailSelectedAllFruits and HaimiyachGAG2_MailSelectedAllFruits(selected) then return true end
    local nm = HaimiyachGAG2_MailFruitToolName(tool)
    if HaimiyachGAG2_SelectedTextMatch(selected, nm) then return true end
    local loose = HaimiyachGAG2_MailLooseName(nm)
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local want = HaimiyachGAG2_MailLooseName(sel)
            if want ~= "" and loose ~= "" then
                if want == loose then return true end
                -- safe contains fallback for names like "Rainbow Bamboo" or "Bamboo 12kg"
                if string.find(loose, want, 1, true) and not (want == "dragon fruit" and string.find(loose, "dragon breath", 1, true)) then
                    return true
                end
            end
        end
...
function HaimiyachGAG2_AddMailHarvestedFruitToolFallback(items, selected, addedKeys)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return 0 end
    local added = 0
    for _, tool in ipairs(HaimiyachGAG2_GetMailFruitTools and HaimiyachGAG2_GetMailFruitTools() or {}) do
        if #items >= 20 then break end
        if tool and tool.Parent and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool) then
            local itemKey, entryValue = HaimiyachGAG2_FindMailFruitReplicaKeyForTool(tool)
            if itemKey and tostring(itemKey) ~= "" and not (addedKeys and addedKeys[tostring(itemKey)]) then
                table.insert(items, {
                    Category = "HarvestedFruits",
                    ItemKey = tostring(itemKey),
                    Count = 1,
                    __Tool = tool,
                    __EntryValue = entryValue
                })
                if addedKeys then addedKeys[tostring(itemKey)] = true end
                added = added + 1
            end
        end
    end
...
function HaimiyachGAG2_AddMailHarvestedFruits(items, selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return 0 end
    local added = 0
    local addedKeys = {}

    -- Priority 1: use the fruit tool currently held / selected, because the game only accepts
    -- fruit mail reliably after the fruit is actually held in the character.
    local selectedTool = HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    if selectedTool and #items < 20 then
        local itemKey, entryValue = HaimiyachGAG2_FindMailFruitReplicaKeyForTool(selectedTool)
        if itemKey and tostring(itemKey) ~= "" then
            table.insert(items, {
                Category = "HarvestedFruits",
                ItemKey = tostring(itemKey),
                Count = 1,
            ...
            addedKeys[tostring(itemKey)] = true
            added = added + 1
        end
    end

    -- Priority 2: replica inventory entries, paired back to their Tool when possible.
    for key, value in pairs(inv("HarvestedFruits")) do
        if #items >= 20 then break end
        if type(value) == "table" and value.Id ~= nil and HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value) then
            local itemKey = HaimiyachGAG2_MailFruitItemKey(key, value)
            if itemKey ~= nil and tostring(itemKey) ~= "" and not addedKeys[tostring(itemKey)] then
                local matchedTool = HaimiyachGAG2_FindFruitToolForMailItem and HaimiyachGAG2_FindFruitToolForMailItem({ Category = "HarvestedFruits", ItemKey = tostring(itemKey) }) or nil
                table.insert(items, {
                    Category = "HarvestedFruits",
                    ItemKey = tostring(itemKey),
                    Count = 1,
    ...
    added = added + HaimiyachGAG2_AddMailHarvestedFruitToolFallback(items, selected, addedKeys)
    return added
end


-- Fruit fallback helpers kept for reading equipped/backpack fruit ids.
-- Mail sending itself uses Mailbox.SendBatch, matching MailboxController.
function HaimiyachGAG2_MailFruitGiftUuid(key, value)
    if type(value) == "table" then
        local v = value.Id or value.UUID or value.Uuid or value.Guid or value.ItemId or value.HarvestedFruitId
        if v ~= nil and tostring(v) ~= "" then return tostring(v) end
    end
    return nil
end

function HaimiyachGAG2_FindFruitToolByGiftUuid(uuid)
    if uuid == nil or tostring(uuid) == "" then return nil end
    uuid = tostring(uuid)
    for _, tool in ipairs(HaimiyachGAG2_GetMailFruitTools and HaimiyachGAG2_GetMailFruitTools() or {}) do
        if tool and tool.Parent then
            for _, attr in ipairs({ "Id", "ID", "FruitId", "HarvestedFruitId", "UID", "UUID", "Uuid", "Guid", "ItemId" }) do
                local v = tool:GetAttribute(attr)
                if v ~= nil and tostring(v) == uuid then return tool end
            end
        end
    end
    return nil
...
function HaimiyachGAG2_EquipFruitToolForGift(uuid)
    local tool = HaimiyachGAG2_FindFruitToolByGiftUuid(uuid)
...
function HaimiyachGAG2_CollectSelectedFruitGiftList(selected, maxCount)
    local out = {}
    local seen = {}
    maxCount = math.floor(tonumber(maxCount) or 1)
    if maxCount < 1 then maxCount = 1 end
    if maxCount > 20 then maxCount = 20 end

    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return out end

    for key, value in pairs(inv("HarvestedFruits")) do
        if #out >= maxCount then break end
        if type(value) == "table" and HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value) then
            if not (value.Locked == true or value.Favorite == true or value.IsFavorite == true) then
                local uuid = HaimiyachGAG2_MailFruitGiftUuid(key, value)
                if uuid and not seen[uuid] then
                    seen[uuid] = true
                    out[#out + 1] = {
                        uuid = uuid,
                        name = HaimiyachGAG2_MailFruitDisplayName(key, value),
                        key = tostring(key)
                    }
                end
            end
    ...
    -- Backpack/Character fallback for fruit that exists as Tool before replica finishes updating.
    for _, tool in ipairs(HaimiyachGAG2_GetMailFruitTools and HaimiyachGAG2_GetMailFruitTools() or {}) do
        if #out >= maxCount then break end
        if tool and tool.Parent and HaimiyachGAG2_SelectedFruitToolMailMatch(selected, tool) then
            local uuid = HaimiyachGAG2_MailFruitToolItemKey(tool)
            if uuid and tostring(uuid) ~= "" and not seen[tostring(uuid)] then
                seen[tostring(uuid)] = true
                out[#out + 1] = {
                    uuid = tostring(uuid),
                    name = HaimiyachGAG2_MailFruitToolName(tool),
                    tool = tool
                }
            end
        end
...
function HaimiyachGAG2_SendSelectedFruitsViaGifting(userId, selected, maxCount)
    if not userId then return 0, "Invalid receiver" end
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return 0, "No fruit selected" end

    local send = action("Gifting.Send")
    if not (send and send.Fire) then return 0, "Gifting.Send remote not found" end

    local fruits = HaimiyachGAG2_CollectSelectedFruitGiftList(selected, maxCount)
    if #fruits == 0 then return 0, "No giftable fruit matched" end

    local sent = 0
    local lastErr = nil
    for _, fruit in ipairs(fruits) do
        if sent >= (math.floor(tonumber(maxCount) or 1)) then break end
        HaimiyachGAG2_EquipFruitToolForGift(fruit.uuid)
        local ok, result = pcall(function()
            return send:Fire(userId, "HarvestedFruits", fruit.uuid)
        end)
        if ok then
            sent = sent + 1
            task.wait(0.45)
...
function HaimiyachGAG2_RemoveFruitMailItems(items)
    local out = {}
    if type(items) ~= "table" then return out end
    for _, it in ipairs(items) do
        if type(it) == "table" and tostring(it.Category or "") ~= "HarvestedFruits" then
            out[#out + 1] = it
        end
    end
    return out
...
function HaimiyachGAG2_IsFruitMailItem(item)
    return type(item) == "table" and tostring(item.Category or "") == "HarvestedFruits"
end

function HaimiyachGAG2_SplitFruitMailItems(items)
    local fruits = {}
    local others = {}
    if type(items) ~= "table" then return fruits, others end
    for _, item in ipairs(items) do
        if HaimiyachGAG2_IsFruitMailItem(item) then
            fruits[#fruits + 1] = item
        else
...
function HaimiyachGAG2_FruitToolMatchesMailItem(tool, itemKey)
    if not HaimiyachGAG2_IsMailHarvestedFruitTool(tool) then return false end
    ...
    local fruitEntry = inv("HarvestedFruits")[itemKey]
    if type(fruitEntry) == "table" then
        for _, attr in ipairs({ "Id", "ID", "UUID", "Uuid", "Guid", "ItemId", "HarvestedFruitId", "FruitId", "UID" }) do
            local v = fruitEntry[attr]
            if v ~= nil and tostring(v) ~= "" then wanted[tostring(v)] = true end
        end
    end

    for _, attr in ipairs({ "InventoryKey", "ItemKey", "Id", "ID", "FruitId", "HarvestedFruitId", "UID", "UUID", "Uuid", "Guid", "ItemId" }) do
        local v = tool:GetAttribute(attr)
        if v ~= nil and wanted[tostring(v)] then return true end
    end

    if type(fruitEntry) == "table" then
        local entryName = HaimiyachGAG2_MailFruitBaseName(itemKey, fruitEntry)
        local toolName = HaimiyachGAG2_MailFruitToolName(tool)
        local a = HaimiyachGAG2_MailLooseName(entryName)
        local b = HaimiyachGAG2_MailLooseName(toolName)
        if a ~= "" and b ~= "" and (a == b or string.find(b, a, 1, true)) then
            return true
        end
    end
...
function HaimiyachGAG2_FindFruitToolForMailItem(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return nil end
    if type(item.__Tool) == "userdata" and item.__Tool.Parent and item.__Tool:IsA("Tool") then
        return item.__Tool
    end
    local itemKey = tostring(item.ItemKey or "")
    if itemKey == "" then return nil end

    local char = LocalPlayer.Character
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if tool and tool:IsA("Tool") and HaimiyachGAG2_FruitToolMatchesMailItem(tool, itemKey) then
                return tool
            end
        end
    end
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetMailFruitTools and HaimiyachGAG2_GetMailFruitTools() or {}) do
        if tool and tool.Parent and HaimiyachGAG2_FruitToolMatchesMailItem(tool, itemKey) then
            return tool
        end
    end
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    local tool = HaimiyachGAG2_FindFruitToolForMailItem(item)
    if not tool then return false, "fruit tool not found" end
    return HaimiyachGAG2_EquipMailToolNow(tool)
end


-- FINAL FIX: mailbox fruit must be promoted/equipped (held) before SendBatch.
-- This follows the game's inventory controller flow: fruit proxies are promoted by Id,
-- then the resulting Tool is equipped before the mailbox payload is fired.
function HaimiyachGAG2_MailFruitEntryIdForItemKey(itemKey)
    itemKey = tostring(itemKey or "")
    local value = inv("HarvestedFruits")[itemKey]
    if type(value) == "table" then
        for _, field in ipairs({ "Id", "ID", "UUID", "Uuid", "Guid", "ItemId", "HarvestedFruitId", "FruitId", "UID" }) do
            local v = value[field]
...
function HaimiyachGAG2_StrictMailboxFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end

    -- Never treat seeds/gears as fruits, even when their name contains the selected fruit name.
    if tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil then return false end
    if tool:GetAttribute("Sprinkler") ~= nil or tool:GetAttribute("WateringCan") ~= nil or tool:GetAttribute("Trowel") ~= nil or tool:GetAttribute("Shovel") ~= nil then return false end
    if tool:GetAttribute("Crate") ~= nil or tool:GetAttribute("Egg") ~= nil or tool:GetAttribute("Mushroom") ~= nil or tool:GetAttribute("Gnome") ~= nil then return false end
    if tool:GetAttribute("Pet") ~= nil or tool:GetAttribute("PetId") ~= nil then return false end

    local lowerName = string.lower(tostring(tool.Name or ""))
    if string.find(lowerName, "seed", 1, true) and not string.find(lowerName, "seed pack", 1, true) then return false end

    if tool:GetAttribute("HarvestedFruit") ~= nil or tool:GetAttribute("HarvestedFruitId") ~= nil then return true end
    if tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("FruitName") ~= nil or tool:GetAttribute("FruitProxy") ~= nil then return true end

    local c = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or "")
    c = string.lower(c)
    if c == "fruit" or c == "fruits" or c == "harvestedfruit" or c == "harvestedfruits" then return true end

    if HaimiyachGAG2_ParseKgText and HaimiyachGAG2_ParseKgText(tool.Name, false) then
        return true
    end
...
function HaimiyachGAG2_MailFruitToolHasId(tool, fruitId)
    if not HaimiyachGAG2_StrictMailboxFruitTool(tool) then return false end
    fruitId = tostring(fruitId or "")
    if fruitId == "" then return false end
    for _, attr in ipairs({ "Id", "ID", "InventoryKey", "ItemKey", "HarvestedFruit", "HarvestedFruitId", "FruitId", "UID", "UUID", "Uuid", "Guid", "ItemId" }) do
        local v = tool:GetAttribute(attr)
        if v ~= nil and tostring(v) == fruitId then
            return true
        end
    end
    return false
...
function HaimiyachGAG2_MailFruitToolNameMatchesSelected(tool, selected)
    if not HaimiyachGAG2_StrictMailboxFruitTool(tool) then return false end
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    if HaimiyachGAG2_MailSelectedAllFruits and HaimiyachGAG2_MailSelectedAllFruits(selected) then return true end

    local nm = HaimiyachGAG2_MailFruitToolName and HaimiyachGAG2_MailFruitToolName(tool) or tostring(tool.Name or "")
    local loose = HaimiyachGAG2_MailLooseName(nm)
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local want = HaimiyachGAG2_MailLooseName(sel)
            if want ~= "" and loose ~= "" then
                if loose == want then return true end
                if string.find(loose, want, 1, true) and not (want == "dragon fruit" and string.find(loose, "dragon breath", 1, true)) then
                    return true
                end
            end
...
function HaimiyachGAG2_FindStrictMailFruitToolById(fruitId)
    fruitId = tostring(fruitId or "")
    ...
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if HaimiyachGAG2_MailFruitToolHasId(tool, fruitId) then return tool end
        end
    end
...
function HaimiyachGAG2_GetMailFruitProxyUtil()
    local ok, result = pcall(function()
        local rs = ReplicatedStorage or game:GetService("ReplicatedStorage")
        local sm = rs and rs:FindFirstChild("SharedModules")
...
function HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
    fruitId = tostring(fruitId or "")
    if fruitId == "" then return false end

    local fpu = HaimiyachGAG2_GetMailFruitProxyUtil()
    if not fpu then return false end

    pcall(function()
        if type(fpu.Pending) == "table" then
            fpu.Pending.Equip = fruitId
            if type(fpu.Pending.Slots) == "table" and fpu.Pending.Slots[fruitId] == nil then
                fpu.Pending.Slots[fruitId] = 1
            end
        end
    end)

    local ok = pcall(function()
        if fpu.RequestPromote then
            fpu.RequestPromote(fruitId)
        elseif fpu.Promote then
            fpu.Promote(fruitId)
        end
    end)
...
function HaimiyachGAG2_WaitMailFruitToolById(fruitId, timeoutSec)
    fruitId = tostring(fruitId or "")
    timeoutSec = tonumber(timeoutSec) or 3
    local deadline = os.clock() + timeoutSec

    while os.clock() < deadline do
        local tool = HaimiyachGAG2_FindStrictMailFruitToolById(fruitId)
        if tool then return tool end
        task.wait(0.08)
    end

    return HaimiyachGAG2_FindStrictMailFruitToolById(fruitId)
end

function HaimiyachGAG2_HoldMailboxFruitById(fruitId)
    fruitId = tostring(fruitId or "")
    if fruitId == "" then return false, "fruit id missing" end

    local tool = HaimiyachGAG2_FindStrictMailFruitToolById(fruitId)
    if not tool then
        HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
        tool = HaimiyachGAG2_WaitMailFruitToolById(fruitId, 3.5)
    end

    if not tool then return false, "fruit tool not promoted" end
    if not HaimiyachGAG2_StrictMailboxFruitTool(tool) then return false, "matched tool is not fruit" end

    local ok, err = HaimiyachGAG2_EquipMailToolNow(tool)
    if not ok then return false, err or "failed to hold fruit" end

    local char = LocalPlayer.Character
    local held = char and char:FindFirstChildWhichIsA("Tool")
    if not HaimiyachGAG2_StrictMailboxFruitTool(held) then
        return false, "held item is not fruit"
    end

    if not HaimiyachGAG2_MailFruitToolHasId(held, fruitId) then
        -- If Id attributes differ, still require that the held tool is a fruit;
        -- this avoids holding Bamboo Seed when the selected target is Bamboo fruit.
        local heldId = HaimiyachGAG2_MailFruitToolItemKey and HaimiyachGAG2_MailFruitToolItemKey(held) or ""
        if tostring(heldId or "") == "" then
            return false, "held fruit id missing"
        end
    end
...
function HaimiyachGAG2_FindSelectedMailFruitEntry(selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return nil, nil end
    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" and HaimiyachGAG2_IsGiftableMailEntry("HarvestedFruits", key, value) and HaimiyachGAG2_SelectedFruitMailMatch(selected, key, value) then
            return tostring(key), value
        end
    end
    return nil, nil
...
function HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    ...
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if HaimiyachGAG2_MailFruitToolNameMatchesSelected(tool, selected) then return tool end
        end
    end
...
function HaimiyachGAG2_EquipSelectedMailFruit(selected)
    ...
    if tool then
        local ok, err = HaimiyachGAG2_EquipMailToolNow(tool)
        if ok then return tool, nil end
    end

    local key, value = HaimiyachGAG2_FindSelectedMailFruitEntry(selected)
    local fruitId = nil
    if key then
        fruitId = HaimiyachGAG2_MailFruitEntryIdForItemKey(key)
    end
    if not fruitId then return nil, "selected fruit id not found" end

    local ok, err = HaimiyachGAG2_HoldMailboxFruitById(fruitId)
    if not ok then return nil, err end

    return HaimiyachGAG2_FindStrictMailFruitToolById(fruitId), nil
end

function HaimiyachGAG2_FindFruitToolForMailItem(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return nil end

    if type(item.__Tool) == "userdata" and item.__Tool.Parent and HaimiyachGAG2_StrictMailboxFruitTool(item.__Tool) then
        return item.__Tool
    end

    local itemKey = tostring(item.ItemKey or "")
    local fruitId = HaimiyachGAG2_MailFruitEntryIdForItemKey(itemKey)
    if fruitId then
        local tool = HaimiyachGAG2_FindStrictMailFruitToolById(fruitId)
        if tool then return tool end
    end
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return false, "not fruit item" end

    local itemKey = tostring(item.ItemKey or "")
    local fruitId = HaimiyachGAG2_MailFruitEntryIdForItemKey(itemKey)

    if not fruitId and type(item.__Tool) == "userdata" and item.__Tool.Parent and HaimiyachGAG2_StrictMailboxFruitTool(item.__Tool) then
        fruitId = HaimiyachGAG2_MailFruitToolItemKey(item.__Tool)
    end

    if not fruitId then return false, "fruit id not found" end
...
function HaimiyachGAG2_MailSelectedAllFruits(selected)
    if type(selected) ~= "table" then return false end
    for sel, enabled in pairs(selected) do
        if enabled == true and HaimiyachGAG2_MailLooseName(sel) == "all fruits" then
            return true
...
function HaimiyachGAG2_MailGardenFruitSelected(obj, selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
    if HaimiyachGAG2_MailSelectedAllFruits(selected) then return true end
    if HaimiyachGAG2_SelectedNameOk(obj, selected) then return true end
    local candidates = HaimiyachGAG2_ObjectNameCandidates and HaimiyachGAG2_ObjectNameCandidates(obj) or { HaimiyachGAG2_ObjectName(obj) }
    for _, candidate in ipairs(candidates) do
        local looseCandidate = HaimiyachGAG2_MailLooseName(candidate)
        for sel, enabled in pairs(selected) do
            if enabled == true then
                local looseSel = HaimiyachGAG2_MailLooseName(sel)
                if looseSel ~= "" and looseCandidate ~= "" then
                    if looseSel == looseCandidate then return true end
                    if string.find(looseCandidate, looseSel, 1, true) and not (looseSel == "dragon fruit" and string.find(looseCandidate, "dragon breath", 1, true)) then
                        return true
                    end
                end
            end
...
function HaimiyachGAG2_HarvestMailFruitsFromGarden()
    if S.mailHarvestGarden ~= true then return 0 end
    if type(S.mailFruitTargets) ~= "table" or not HaimiyachGAG2_HasSelection(S.mailFruitTargets) then return 0 end

    local cap = maxFruitCap and maxFruitCap() or (tonumber(LocalPlayer:GetAttribute("MaxFruitCapacity")) or 100)
    local count = fruitCount and fruitCount() or (tonumber(LocalPlayer:GetAttribute("FruitCount")) or 0)
    if count >= cap - 1 then return 0 end

    local want = math.floor(tonumber(S.mailSendCount) or 1)
    if want < 1 then want = 1 end
    if want > 20 then want = 20 end

    local harvested = 0
    for _, pr in ipairs(CollectionService:GetTagged("HarvestPrompt")) do
        if harvested >= want then break end
        if not (pr and pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace)) then
            -- skip invalid prompt
        else
            local m = promptCarrier(pr)
            local pid = m and m:GetAttribute("PlantId")
            if pid then
                local uid = tonumber(m:GetAttribute("UserId"))
                if (uid == nil or uid == LocalPlayer.UserId) and HaimiyachGAG2_MailGardenFruitSelected(m, S.mailFruitTargets) then
                    count = fruitCount and fruitCount() or (tonumber(LocalPlayer:GetAttribute("FruitCount")) or count)
                    if count >= cap - 1 then break end
                    local fid = tostring(m:GetAttribute("FruitId") or "")
                    local ok = fireFast("Garden.CollectFruit", tostring(pid), fid)
                    if ok then
                        harvested = harvested + 1
                        Stats.harvested = (tonumber(Stats.harvested) or 0) + 1
                        task.wait(math.max(0.05, tonumber(S.harvestDelay) or 0.05))
                    end
                end
            end
        end
...
function HaimiyachGAG2_FruitMailOptionNames()
    local list, seen = {}, {}
    HaimiyachGAG2_AddUniqueName(list, seen, "ALL FRUITS")

    -- Auto mode: dropdown is based on fruit names/catalog, not on current inventory IDs.
    -- The actual fruit UUID/key is resolved every time auto-send runs.
    if type(SEED_NAMES) == "table" then
        for _, nm in ipairs(SEED_NAMES) do
            HaimiyachGAG2_AddUniqueName(list, seen, nm)
        end
    end

    -- Extra safety: also add any live harvested fruit names if inventory already exists.
    for key, value in pairs(inv("HarvestedFruits")) do
        if HaimiyachGAG2_IsGiftableMailEntry("HarvestedFruits", key, value) then
            HaimiyachGAG2_AddUniqueName(list, seen, HaimiyachGAG2_MailFruitBaseName(key, value))
        end
    end
...
function HaimiyachGAG2_CleanMailBatch(batch)
    local clean = {}
    if type(batch) ~= "table" then return clean end
    for _, it in ipairs(batch) do
        if type(it) == "table" and it.Category and it.ItemKey and it.Count then
            clean[#clean + 1] = {
                Category = tostring(it.Category),
                ItemKey = tostring(it.ItemKey),
...
function HaimiyachGAG2_BuildMailItems()
    ...
    HaimiyachGAG2_AddMailHarvestedFruits(items, S.mailFruitTargets)
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    local harvestedForMail = HaimiyachGAG2_HarvestMailFruitsFromGarden()
    if harvestedForMail and harvestedForMail > 0 then
        -- MailboxController reads HarvestedFruits from the PlayerState replica.
        -- After harvest, wait a bit so the fruit appears in Inventory.HarvestedFruits before SendBatch.
        task.wait(0.8)
    end

    local hasFruitSelection = type(S.mailFruitTargets) == "table" and HaimiyachGAG2_HasSelection(S.mailFruitTargets)

    -- Critical: fruit mail only works when the selected fruit is actually held first.
    -- Equip before building/sending so Mailbox sees the fruit like manual gameplay.
    local preHeldFruit = nil
    if hasFruitSelection then
        preHeldFruit = HaimiyachGAG2_EquipSelectedMailFruit(S.mailFruitTargets)
        task.wait(0.35)
    end

    local items = HaimiyachGAG2_BuildMailItems()

    if #items == 0 and hasFruitSelection then
        local waitStart = os.clock()
        while #items == 0 and os.clock() - waitStart < 4 do
            task.wait(0.35)
            HaimiyachGAG2_EquipSelectedMailFruit(S.mailFruitTargets)
            task.wait(0.15)
            items = HaimiyachGAG2_BuildMailItems()
        end
    end

    if #items == 0 then
        if showNotify ~= false then
            local selectedText = HaimiyachGAG2_MailSelectedTextList and HaimiyachGAG2_MailSelectedTextList(S.mailFruitTargets) or ""
            if selectedText ~= "" then
                notify("Mailbox", "No giftable mailbox item matched: " .. selectedText, 4)
            else
                notify("Mailbox", "No selected gift item found.", 3)
            end
        end
        return false
    end
    ...
    local fruitItems, otherItems = HaimiyachGAG2_SplitFruitMailItems(items)
    local sentItems = 0
    local lastMessage = nil

    local function sendMailBatch(batch)
        if type(batch) ~= "table" or #batch == 0 then return true, "OK" end
        local ok, success, message = pcall(function()
            return send:Fire(userId, HaimiyachGAG2_CleanMailBatch(batch), note)
        end)
        if ok and success == true then
            sentItems = sentItems + #batch
            return true, tostring(message or "OK")
        end
        return false, tostring(message or success or "Could not send mail")
    end

    -- Fruits must be held/equipped before SendBatch. Send fruit one-by-one so
    -- the equipped fruit always matches the HarvestedFruits ItemKey being mailed.
    for _, fruitItem in ipairs(fruitItems) do
        local equipped, equipErr = HaimiyachGAG2_EquipFruitForMailboxSend(fruitItem)
        if not equipped then
            if showNotify ~= false then notify("Mailbox", "Hold fruit failed: " .. tostring(equipErr or fruitItem.ItemKey), 4) end
            return false
        end
        local okFruit, msgFruit = sendMailBatch({ fruitItem })
        lastMessage = msgFruit
        if not okFruit then
            if showNotify ~= false then notify("Mailbox", msgFruit, 4) end
            return false
        end
        task.wait(0.45)
    end

    local okOther, msgOther = sendMailBatch(otherItems)
    lastMessage = msgOther or lastMessage
    if not okOther then
        if showNotify ~= false then notify("Mailbox", tostring(lastMessage or "Could not send mail"), 4) end
        return false
    end

    if sentItems > 0 then
        if showNotify ~= false then
            local mailText = "Gift sent to @" .. tostring(targetName) .. " (" .. tostring(sentItems) .. " item)"
...
local CONFIG_KEYS = {
    ...
    "autoSell", "autoSellFruits", "sellInterval", "sellKeepMutations", "sellKeepKgMode", "sellKeepKgValue", "autoExpand", "autoPot", "autoDaily", "dailyDelay",
...
-- // ============================================================ \ --
-- //                    AUCTION FINDER / AUTO BUY                 \ --
-- Robust Auction snapshot reader. It first asks the game's Auctioneer request,
-- then waits for the Snapshot event. If the request returns nil, the UI status
-- will say so instead of staying empty forever.
HaimiyachGAG2_AuctionState = HaimiyachGAG2_AuctionState or {
    lots = {},
    stock = {},
    lastFetch = 0,
    lastBuy = 0,
    optionVersion = 0,
    status = "WAITING SNAPSHOT",
    requestId = 0,
}

function HaimiyachGAG2_AuctionServerNow()
    local ok, result = pcall(function() return Workspace:GetServerTimeNow() end)
    if ok and type(result) == "number" then return result end
    return os.time()
end

function HaimiyachGAG2_AuctionLoadModules()
    if HaimiyachGAG2_AuctionModulesTried then return end
    HaimiyachGAG2_AuctionModulesTried = true
    pcall(function()
        local sm = ReplicatedStorage:FindFirstChild("SharedModules")
        local mod = sm and sm:FindFirstChild("Auctioneer")
        if mod then HaimiyachGAG2_AuctioneerModule = require(mod) end
    end)
    pcall(function()
        local sm = ReplicatedStorage:FindFirstChild("SharedModules")
        local mod = sm and sm:FindFirstChild("NumberUtils")
        if mod then HaimiyachGAG2_AuctionNumberUtils = require(mod) end
    end)
end

function HaimiyachGAG2_AuctionFormatPrice(value)
    value = tonumber(value) or 0
    HaimiyachGAG2_AuctionLoadModules()
    if HaimiyachGAG2_AuctionNumberUtils and HaimiyachGAG2_AuctionNumberUtils.Abbreviate then
        local ok, txt = pcall(function() return HaimiyachGAG2_AuctionNumberUtils.Abbreviate(math.floor(value)) end)
        if ok and txt then return tostring(txt) end
    end
    return fmt(value)
end

function HaimiyachGAG2_AuctionTextToNumber(text)
    text = tostring(text or ""):gsub(",", ""):gsub("¢", ""):gsub("%s+", "")
    local raw, suffix = text:match("([%d%.]+)([KkMmBbTt]?)")
    ...
    if suffix == "T" then n = n * 1000000000000 end
    return n
end


function HaimiyachGAG2_AuctionParseStockNumber(text)
    text = tostring(text or "")
    local low = string.lower(text)
    if string.find(low, "sold", 1, true) or string.find(low, "out of stock", 1, true) then return 0 end
    if string.find(low, "unlimited", 1, true) then return nil end
    local n = text:match("x%s*([%d,]+)") or text:match("([%d,]+)%s*in%s*stock") or text:match("([%d,]+)")
    if n then return tonumber((tostring(n):gsub(",", ""))) end
    return nil
end

function HaimiyachGAG2_AuctionGuiPriceFromButton(button, row)
    local candidates = {}
    local function add(txt, score)
        txt = tostring(txt or "")
        if txt ~= "" then candidates[#candidates + 1] = { text = txt, score = score or 0 } end
    end

    if button then
        if button:IsA("TextButton") or button:IsA("TextLabel") then add(button.Text, 30) end
        for _, d in ipairs(button:GetDescendants()) do
            if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                local n = string.lower(tostring(d.Name or ""))
                local p = d.Parent and string.lower(tostring(d.Parent.Name or "")) or ""
                local score = 40
                if string.find(n, "price", 1, true) or string.find(p, "price", 1, true) then score = score + 20 end
                if string.find(n, "text", 1, true) or string.find(p, "buy", 1, true) then score = score + 10 end
                add(d.Text, score)
            end
        end
    end
    ...
    if row then
        for _, d in ipairs(row:GetDescendants()) do
            if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                ...
                local objName = string.lower(tostring(d.Name or ""))
                local parentName = d.Parent and string.lower(tostring(d.Parent.Name or "")) or ""
                if string.find(txt, "¢", 1, true) or string.find(objName, "price", 1, true) or string.find(parentName, "price", 1, true) or string.find(parentName, "buy", 1, true) then
                    if not string.find(low, "stock", 1, true) and not string.find(low, "refresh", 1, true) then
                        add(txt, 10)
                    end
                end
            end
    ...
    for _, c in ipairs(candidates) do
        local n = HaimiyachGAG2_AuctionTextToNumber(c.text)
        if n and n > 0 then return n, c.text end
    end
    return 0, ""
end

function HaimiyachGAG2_AuctionTimeText(sec)
    ...
    if m > 0 then return tostring(m) .. "m " .. tostring(s2) .. "s" end
    return tostring(s2) .. "s"
end

function HaimiyachGAG2_AuctionParseTimeText(text)
    text = tostring(text or ""):lower()
    local total = 0
    local d = tonumber(text:match("(%d+)%s*d")) or tonumber(text:match("(%d+)%s*day")) or 0
    local h = tonumber(text:match("(%d+)%s*h")) or tonumber(text:match("(%d+)%s*hour")) or 0
    local m = tonumber(text:match("(%d+)%s*m")) or tonumber(text:match("(%d+)%s*minute")) or 0
    local s2 = tonumber(text:match("(%d+)%s*s")) or tonumber(text:match("(%d+)%s*second")) or 0
    total = d * 86400 + h * 3600 + m * 60 + s2
    return total
end

function HaimiyachGAG2_AuctionParseCountText(text)
    text = tostring(text or "")
    local n = text:match("[xX×]%s*([%d,]+)") or text:match("%(%s*[xX×]%s*([%d,]+)%s*%)")
    if not n then return nil end
    n = tonumber((tostring(n):gsub(",", "")))
    if n and n == n and n > 0 then return math.floor(n) end
    return nil
end

function HaimiyachGAG2_AuctionGuiAmountText(row)
    if not row then return "" end
    local obj = row:FindFirstChild("Amount", true)
    if obj and (obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox")) then
        local txt = tostring(obj.Text or "")
        if HaimiyachGAG2_AuctionParseCountText(txt) then return txt end
    end
    for _, d in ipairs(row:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            local txt = tostring(d.Text or "")
            local lowName = string.lower(tostring(d.Name or ""))
            local lowParent = d.Parent and string.lower(tostring(d.Parent.Name or "")) or ""
            if HaimiyachGAG2_AuctionParseCountText(txt) and not string.find(lowName, "stock", 1, true) and not string.find(lowParent, "stock", 1, true) then
                return txt
            end
        end
    end
...
function HaimiyachGAG2_AuctionLotCount(lot)
    if type(lot) ~= "table" then return nil end
    local direct = lot.count or lot.Count or lot.itemCount or lot.ItemCount or lot.stackCount or lot.StackCount
    local n = tonumber(direct)
    if n and n == n and n > 0 then return math.floor(n) end
    n = HaimiyachGAG2_AuctionParseCountText(lot.guiAmountText or lot.guiDisplayName or lot.DisplayName or "")
    if n and n > 0 then return n end
    return nil
end

function HaimiyachGAG2_AuctionBaseDisplayName(lot)
    if type(lot) ~= "table" then return "Unknown" end
    local name = nil
    if lot.guiDisplayName and tostring(lot.guiDisplayName) ~= "" then name = tostring(lot.guiDisplayName) end
    if not name then
        HaimiyachGAG2_AuctionLoadModules()
        if HaimiyachGAG2_AuctioneerModule and HaimiyachGAG2_AuctioneerModule.DisplayName then
            local ok, txt = pcall(function() return HaimiyachGAG2_AuctioneerModule.DisplayName(lot) end)
            if ok and txt and tostring(txt) ~= "" then name = tostring(txt) end
        end
    end
    if not name then
        name = tostring(lot.item or lot.Name or lot.FruitName or lot.name or lot.DisplayName or lot.displayName or "Unknown")
        if lot.mutation and tostring(lot.mutation) ~= "" then name = tostring(lot.mutation) .. " " .. name end
    end
    name = tostring(name):gsub("%s*%(%s*[xX×]%s*[%d,]+%s*%)%s*$", ""):gsub("%s+[xX×]%s*[%d,]+%s*$", "")
    return name
end

function HaimiyachGAG2_AuctionDisplayName(lot)
    local name = HaimiyachGAG2_AuctionBaseDisplayName(lot)
    local count = HaimiyachGAG2_AuctionLotCount(lot)
    if count and count > 1 and not string.find(name, "%(x") then
        name = name .. " (x" .. tostring(count) .. ")"
    end
    return name
end

function HaimiyachGAG2_AuctionCurrentPrice(lot)
    if type(lot) ~= "table" then return 0 end

    -- Match the game's AuctioneerController: price is calculated every frame
    -- with Auctioneer.CurrentPrice(lot, serverNow()). GUI text is only fallback.
    HaimiyachGAG2_AuctionLoadModules()
    if HaimiyachGAG2_AuctioneerModule and HaimiyachGAG2_AuctioneerModule.CurrentPrice and not lot.fromGuiOnly then
        local ok, price = pcall(function()
            return HaimiyachGAG2_AuctioneerModule.CurrentPrice(lot, HaimiyachGAG2_AuctionServerNow())
        end)
    ...
    local direct = tonumber(lot.currentPrice or lot.price or lot.startPrice or lot.startingPrice or lot.buyPrice or lot.PurchasePrice or lot.cost)
    if direct and direct == direct and direct > 0 then return direct end

    -- Only use GUI price when it was refreshed recently. Old GUI text is stale.
    if tonumber(lot.guiCurrentPrice) and (os.clock() - (tonumber(lot.guiScanAt) or 0) < 2.5) then
        return tonumber(lot.guiCurrentPrice)
    end

    return tonumber(lot.guiCurrentPrice) or 0
end

function HaimiyachGAG2_AuctionNormalizeLot(lot, fallbackId)
    if type(lot) ~= "table" then return nil end
    local id = lot.lotId or lot.LotId or lot.LotID or lot.id or lot.Id or lot.ID or fallbackId
    if id == nil then return nil end
    lot.lotId = tostring(id)
    return lot
end

function HaimiyachGAG2_AuctionExtractLots(snapshot)
    local out = {}
    if type(snapshot) ~= "table" then return out end
    local candidates = {}
    candidates[#candidates + 1] = snapshot.lots
    candidates[#candidates + 1] = snapshot.Lots
    candidates[#candidates + 1] = snapshot.activeLots
    candidates[#candidates + 1] = snapshot.ActiveLots
    if type(snapshot.manifest) == "table" then
        candidates[#candidates + 1] = snapshot.manifest.lots
        candidates[#candidates + 1] = snapshot.manifest.Lots
        candidates[#candidates + 1] = snapshot.manifest
    end
    if type(snapshot.Manifest) == "table" then
        candidates[#candidates + 1] = snapshot.Manifest.lots
        candidates[#candidates + 1] = snapshot.Manifest.Lots
    ...
    candidates[#candidates + 1] = snapshot

    local seen = {}
    for _, list in ipairs(candidates) do
        if type(list) == "table" then
            for k, lot in pairs(list) do
                if type(lot) == "table" then
                    local normalized = HaimiyachGAG2_AuctionNormalizeLot(lot, k)
                    if normalized and not seen[tostring(normalized.lotId)] then
                        seen[tostring(normalized.lotId)] = true
...
function HaimiyachGAG2_AuctionRememberItems(lots)
    HaimiyachGAG2_AuctionState.knownItems = HaimiyachGAG2_AuctionState.knownItems or {}
    if type(lots) ~= "table" then return end
    for _, lot in ipairs(lots) do
        local name = HaimiyachGAG2_AuctionDisplayName(lot)
        if name and tostring(name) ~= "" and tostring(name) ~= "Unknown" then
            HaimiyachGAG2_AuctionState.knownItems[tostring(name)] = true
        end
    end
end

function HaimiyachGAG2_AuctionLiveStock(lot)
    if type(lot) ~= "table" then return nil end
    local stockTable = HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.stock
    if type(stockTable) ~= "table" then return nil end
    local id = lot.lotId or lot.LotId or lot.LotID or lot.id or lot.Id or lot.ID
    if id == nil then return nil end
    local v = stockTable[id]
    if v == nil then v = stockTable[tostring(id)] end
    if v == nil and tonumber(id) then v = stockTable[tonumber(id)] end
...
function HaimiyachGAG2_AuctionSyncLotStockFromLive(lot)
    local live = HaimiyachGAG2_AuctionLiveStock(lot)
    if live ~= nil and type(lot) == "table" then
        lot.stockQuantity = live
        lot.StockQuantity = live
        lot.stock = live
        lot.guiStockText = nil
    end
    return live
end

function HaimiyachGAG2_AuctionApplySnapshot(snapshot)
    if type(snapshot) ~= "table" then return false end
    local state = HaimiyachGAG2_AuctionState
    local lots = HaimiyachGAG2_AuctionExtractLots(snapshot)
    state.lots = lots
    state.stock = type(snapshot.stock) == "table" and snapshot.stock or type(snapshot.Stock) == "table" and snapshot.Stock or state.stock or {}
    state.rollIntervalSeconds = tonumber(snapshot.rollIntervalSeconds or snapshot.RollIntervalSeconds) or state.rollIntervalSeconds or 0
    state.rollWindowUnix = tonumber(snapshot.rollWindowUnix or snapshot.RollWindowUnix) or state.rollWindowUnix or 0
    state.timerShiftSeconds = tonumber(snapshot.timerShiftSeconds or snapshot.TimerShiftSeconds) or 0
    state.lastSnapshotAt = os.clock()
    pcall(HaimiyachGAG2_AuctionRememberItems, state.lots)
    for _, lot in ipairs(state.lots or {}) do pcall(HaimiyachGAG2_AuctionSyncLotStockFromLive, lot) end
    state.optionVersion = (tonumber(state.optionVersion) or 0) + 1
    state.status = #state.lots > 0 and ("LOADED " .. tostring(#state.lots) .. " LOTS + LIVE STOCK") or "SNAPSHOT RETURNED BUT NO LOTS"
    return #state.lots > 0
end

function HaimiyachGAG2_AuctionBindEvents()
    if HaimiyachGAG2_AuctionEventsBound then return end
    HaimiyachGAG2_AuctionEventsBound = true
    pcall(function()
        local ev = action("Auctioneer.Snapshot")
        if ev and ev.OnClientEvent then
            ev.OnClientEvent:Connect(function(snapshot)
                pcall(HaimiyachGAG2_AuctionApplySnapshot, snapshot)
            end)
        end
    end)
    pcall(function()
        local ev = action("Auctioneer.StockUpdate")
        if ev and ev.OnClientEvent then
            ev.OnClientEvent:Connect(function(data)
                if type(data) == "table" and (type(data.stock) == "table" or type(data.Stock) == "table") then
                    HaimiyachGAG2_AuctionState.stock = data.stock or data.Stock
                    HaimiyachGAG2_AuctionState.stockUpdatedAt = os.clock()
                    for _, lot in ipairs(HaimiyachGAG2_AuctionState.lots or {}) do
                        pcall(HaimiyachGAG2_AuctionSyncLotStockFromLive, lot)
                    end
                    HaimiyachGAG2_AuctionState.status = "LIVE STOCK UPDATED"
                    HaimiyachGAG2_AuctionState.optionVersion = (tonumber(HaimiyachGAG2_AuctionState.optionVersion) or 0) + 1
                end
            end)
        end
    end)
...
function HaimiyachGAG2_AuctionReadTextFrom(root, name)
...
function HaimiyachGAG2_AuctionFirstText(root, names)
    if not root then return "" end
    for _, n in ipairs(names or {}) do
        local txt = HaimiyachGAG2_AuctionReadTextFrom(root, n)
        if txt and tostring(txt) ~= "" then return tostring(txt) end
    end
    return ""
end

function HaimiyachGAG2_AuctionTextPattern(root, mode)
    if not root then return "" end
    local best = ""
    for _, d in ipairs(root:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            ...
            local low = string.lower(txt)
            if mode == "stock" then
                if string.find(low, "stock", 1, true) or string.find(low, "sold out", 1, true) then return txt end
            elseif mode == "timer" then
                if string.find(low, "refresh", 1, true) then
                    local m = txt:match("%d+%s*m[^%d]*%d+%s*s") or txt:match("%d+%s*h[^%d]*%d+%s*m") or txt:match("%d+%s*s")
                    if m then return m end
                end
                if txt:match("%d+%s*m[^%d]*%d+%s*s") or txt:match("%d+%s*h[^%d]*%d+%s*m") or txt:match("%d+%s*s") then return txt end
            elseif mode == "price" then
                local objName = string.lower(tostring(d.Name or ""))
                local parentName = d.Parent and string.lower(tostring(d.Parent.Name or "")) or ""
                if txt:match("[%d%.]+%s*[KkMmBbTt]?") and (string.find(txt, "¢", 1, true) or string.find(objName, "price", 1, true) or string.find(parentName, "buy", 1, true) or string.find(parentName, "price", 1, true)) then
                    best = txt
                end
            elseif mode == "name" then
                if txt ~= "" and not string.find(low, "stock", 1, true) and not string.find(low, "refresh", 1, true) and not string.find(low, "sold out", 1, true) and not string.find(low, "out of", 1, true) and not string.find(txt, "¢", 1, true) and not txt:match("^%d+%s*[mhs]") then
                    best = txt
                end
            end
        end
...
function HaimiyachGAG2_AuctionRowName(row, fallback)
    if not row then return "Lot_" .. tostring(fallback or 1) end
    local obj = row
    while obj do
        local n = tostring(obj.Name or "")
        if string.sub(n, 1, 4) == "Lot_" then return string.gsub(n, "^Lot_", "") end
        obj = obj.Parent
    end
    return "GUI_" .. tostring(fallback or 1)
end

function HaimiyachGAG2_AuctionCollectGuiRows(root)
    local rows, seen = {}, {}
    if not root then return rows end

    local function addRow(row)
        if not row or not row:IsA("GuiObject") or seen[row] then return end
        local nameText = HaimiyachGAG2_AuctionFirstText(row, { "ItemName", "Title", "Name", "Item_Name" })
        local stockText = HaimiyachGAG2_AuctionFirstText(row, { "Stock_Text", "StockText", "Stock" })
        local timerText = HaimiyachGAG2_AuctionFirstText(row, { "Timer", "RefreshIn", "Time", "TimerText" })
        local buyButton = row:FindFirstChild("BuyButton", true) or row:FindFirstChild("Button", true)
        local outOfStock = row:FindFirstChild("OUT_OF_STOCK", true)
        local expired = row:FindFirstChild("EXPIRED", true)

        if nameText == "" then nameText = HaimiyachGAG2_AuctionTextPattern(row, "name") end
        if stockText == "" then stockText = HaimiyachGAG2_AuctionTextPattern(row, "stock") end
        if timerText == "" then timerText = HaimiyachGAG2_AuctionTextPattern(row, "timer") end

        if nameText ~= "" and (stockText ~= "" or buyButton or outOfStock or expired or string.sub(tostring(row.Name), 1, 4) == "Lot_") then
            seen[row] = true
            rows[#rows + 1] = row
        end
    end
    ...
    for _, d in ipairs(root:GetDescendants()) do
        if d:IsA("GuiObject") then
            ...
            if string.sub(n, 1, 4) == "Lot_" then addRow(d) end
        end
    end

    if #rows == 0 then
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("TextLabel") and tostring(d.Name) == "ItemName" then
                local row = d.Parent
                local depth = 0
                while row and row ~= root and depth < 6 do
                    if row:IsA("GuiObject") and (row:FindFirstChild("BuyButton", true) or row:FindFirstChild("Stock_Text", true) or row:FindFirstChild("RefreshIn", true) or row:FindFirstChild("OUT_OF_STOCK", true)) then
                        addRow(row)
                        break
                    end
                    row = row.Parent
    ...
    if #rows == 0 then
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("TextLabel") or d:IsA("TextButton") then
                local txt = tostring(d.Text or "")
                local low = string.lower(txt)
                if txt ~= "" and not string.find(low, "auction", 1, true) and not string.find(low, "refresh", 1, true) and not string.find(low, "stock", 1, true) and not string.find(low, "out of", 1, true) and not string.find(txt, "¢", 1, true) then
                    local row = d.Parent
                    local depth = 0
                    while row and row ~= root and depth < 5 do
                        if row:IsA("GuiObject") and (row:FindFirstChild("BuyButton", true) or row:FindFirstChild("Stock_Text", true) or row:FindFirstChild("OUT_OF_STOCK", true)) then
                            addRow(row)
                            break
                        end
                        row = row.Parent
    ...
    table.sort(rows, function(a, b)
        ...
        if ao == bo then return tostring(a.Name) < tostring(b.Name) end
        return ao < bo
    end)
    return rows
end

function HaimiyachGAG2_AuctionParseRowsFromGameGui()
    local pg = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
    local auctionGui = pg and pg:FindFirstChild("Auction")
    if not auctionGui then
        if HaimiyachGAG2_AuctionState then HaimiyachGAG2_AuctionState.status = "GAME AUCTION GUI NOT FOUND" end
        return false
    end

    -- Scan the real game Auction GUI even if the ScreenGui is disabled/hidden.
    -- The rows still exist under PlayerGui, and scanning only when Enabled caused EMPTY
    -- after the player closed the NPC window or opened this hub.
    local root = auctionGui:FindFirstChild("Frame") or auctionGui
    local rows = HaimiyachGAG2_AuctionCollectGuiRows(root)
    local guiLots = {}
    local now = HaimiyachGAG2_AuctionServerNow()

    for i, row in ipairs(rows) do
        local itemName = HaimiyachGAG2_AuctionFirstText(row, { "ItemName", "Title", "Name", "Item_Name" })
        if itemName == "" then itemName = HaimiyachGAG2_AuctionTextPattern(row, "name") end
        itemName = tostring(itemName or ""):gsub("^%s+", ""):gsub("%s+$", "")

        local stockText = HaimiyachGAG2_AuctionFirstText(row, { "Stock_Text", "StockText", "Stock" })
        if stockText == "" then stockText = HaimiyachGAG2_AuctionTextPattern(row, "stock") end
        local amountText = HaimiyachGAG2_AuctionGuiAmountText(row)
        local amountCount = HaimiyachGAG2_AuctionParseCountText(amountText)

        local timerText = HaimiyachGAG2_AuctionFirstText(row, { "Timer", "RefreshIn", "Time", "TimerText" })
        if timerText == "" then timerText = HaimiyachGAG2_AuctionTextPattern(row, "timer") end

        local buyButton = row:FindFirstChild("BuyButton", true) or row:FindFirstChild("Button", true)
        local guiPrice, guiPriceText = HaimiyachGAG2_AuctionGuiPriceFromButton(buyButton, row)
        local stockNum = HaimiyachGAG2_AuctionParseStockNumber(stockText)
        local outObj = row:FindFirstChild("OUT_OF_STOCK", true)
        if outObj and outObj:IsA("GuiObject") and outObj.Visible then stockNum = 0 end

        local remain = HaimiyachGAG2_AuctionParseTimeText(timerText)
        local expiredObj = row:FindFirstChild("EXPIRED", true)
        if expiredObj and expiredObj:IsA("GuiObject") and expiredObj.Visible then remain = 0 end

        if itemName ~= "" then
            guiLots[#guiLots + 1] = {
                lotId = HaimiyachGAG2_AuctionRowName(row, i),
                item = itemName,
                guiDisplayName = itemName,
                guiStockText = stockText,
                guiAmountText = amountText,
                count = amountCount,
                guiTimerText = timerText,
                guiCurrentPrice = guiPrice,
                guiPriceText = guiPriceText,
                guiBuyButton = buyButton,
                stockQuantity = stockNum,
                expiresAt = remain > 0 and (now + remain) or 0,
                rolledAt = now,
                guiRow = row,
                guiScanAt = os.clock(),
                fromGuiOnly = true,
    ...
    if #guiLots > 0 then
        local state = HaimiyachGAG2_AuctionState
        local oldLots = state and state.lots or {}
        local merged = {}
        local used = {}

        -- Prefer real snapshot lots only when the item name matches.
        -- Never force-match by index, because after a restock that can attach the
        -- wrong lotId/price to a new visible item.
        for _, g in ipairs(guiLots) do
            local gbase = HaimiyachGAG2_AuctionSelectionKey(g.guiDisplayName or g.item or "")
            local target = nil
            if type(oldLots) == "table" then
                for _, lot in ipairs(oldLots) do
                    if not used[lot] then
                        local lbase = HaimiyachGAG2_AuctionSelectionKey(HaimiyachGAG2_AuctionBaseDisplayName(lot))
                        local ldisp = HaimiyachGAG2_AuctionSelectionKey(HaimiyachGAG2_AuctionDisplayName(lot))
                        local lraw = HaimiyachGAG2_AuctionSelectionKey(lot.item or lot.Name or lot.name or lot.DisplayName or "")
                        if gbase ~= "" and (gbase == lbase or gbase == ldisp or gbase == lraw) then
                            target = lot
                            break
                        end
                    end
            ...
            if target then
                used[target] = true
                target.guiDisplayName = g.guiDisplayName
                target.guiStockText = g.guiStockText
                target.guiAmountText = g.guiAmountText
                if g.count ~= nil then target.count = g.count end
                target.guiTimerText = g.guiTimerText
                target.guiCurrentPrice = g.guiCurrentPrice
                target.guiPriceText = g.guiPriceText
                target.guiBuyButton = g.guiBuyButton
                target.guiRow = g.guiRow
                target.guiScanAt = g.guiScanAt
                target.stockQuantity = g.stockQuantity
                target.stock = g.stockQuantity
                if tonumber(g.expiresAt) and tonumber(g.expiresAt) > 0 then target.expiresAt = g.expiresAt end
                target.fromGuiOnly = false
        ...
        state.lots = merged
        pcall(HaimiyachGAG2_AuctionRememberItems, merged)
        state.optionVersion = (tonumber(state.optionVersion) or 0) + 1
        state.status = "LOADED " .. tostring(#merged) .. " LOTS FROM GAME AUCTION GUI"
        return true
    end

    HaimiyachGAG2_AuctionState.status = "GAME AUCTION GUI FOUND, BUT LOT CARDS NOT READ"
    return false
end
function HaimiyachGAG2_AuctionTryModuleLots()
    HaimiyachGAG2_AuctionLoadModules()
    local mod = HaimiyachGAG2_AuctioneerModule
    if type(mod) ~= "table" then return false end
    local snap = {
        lots = mod.lots or mod.Lots or mod.activeLots or mod.ActiveLots or mod.CurrentLots or mod.Manifest or mod.manifest,
        stock = mod.stock or mod.Stock or mod.CurrentStock,
    }
    if type(snap.lots) == "table" then
        return HaimiyachGAG2_AuctionApplySnapshot(snap)
    end
    return false
end

function HaimiyachGAG2_AuctionRefresh(force)
    HaimiyachGAG2_AuctionBindEvents()
    local state = HaimiyachGAG2_AuctionState
    local now = os.clock()

    -- First try the actual PlayerGui Auction cards. This is the safest fallback
    -- because the auction remote can return nil until the original GUI/controller
    -- has initialized. Scan even when the original Auction GUI is hidden.
    if force or not state.lots or #state.lots == 0 or now - (tonumber(state.lastGuiScan) or 0) >= 0.35 then
        state.lastGuiScan = now
        if HaimiyachGAG2_AuctionParseRowsFromGameGui() then return true end
    end

    local shouldRequest = force or not state.lots or #state.lots == 0 or not state.lastFetch or now - state.lastFetch >= 5
    if shouldRequest then
        state.lastFetch = now
        state.requestId = (tonumber(state.requestId) or 0) + 1
        state.status = "REQUESTING AUCTION SNAPSHOT..."

        local ok, result = fire("Auctioneer.RequestSnapshot")
        if ok and type(result) == "table" then
            HaimiyachGAG2_AuctionApplySnapshot(result)
            state.lastGuiScan = 0
            pcall(HaimiyachGAG2_AuctionParseRowsFromGameGui)
            return state.lots and #state.lots > 0
        end

        if ok and result == nil then
            state.status = "SNAPSHOT NIL; USING GAME GUI SCAN"
        elseif not ok then
            state.status = "SNAPSHOT FAILED: " .. tostring(result)
        else
            state.status = "BAD SNAPSHOT RESPONSE: " .. tostring(typeof and typeof(result) or type(result))
        end

        if force then
            local start = os.clock()
            while os.clock() - start < 1.25 do
                if state.lots and #state.lots > 0 then break end
                pcall(HaimiyachGAG2_AuctionParseRowsFromGameGui)
                if state.lots and #state.lots > 0 then break end
                task.wait(0.15)
            end
            if state.lots and #state.lots > 0 then return true end
            if HaimiyachGAG2_AuctionTryModuleLots() then
                pcall(HaimiyachGAG2_AuctionParseRowsFromGameGui)
                return true
            end
        end
    ...
    if state.lots and #state.lots > 0 then return true end
    if force then state.status = "EMPTY: OPEN AUCTION NPC ONCE, THEN REFRESH." end
    return false
end

function HaimiyachGAG2_AuctionStock(lot)
    if type(lot) ~= "table" then return nil end

    -- If the Auction GUI was scanned recently, use its visible stock first.
    -- It changes live on screen and prevents stale initial snapshot stock.
    if lot.guiStockText and tostring(lot.guiStockText) ~= "" and (os.clock() - (tonumber(lot.guiScanAt) or 0) < 2.5) then
        local guiStock = HaimiyachGAG2_AuctionParseStockNumber(lot.guiStockText)
        if guiStock ~= nil then return guiStock end
    end

    -- Controller auction uses a separate live stock table from StockUpdate.
    local live = HaimiyachGAG2_AuctionLiveStock(lot)
    if live ~= nil then return live end

    local direct = lot.stockQuantity or lot.StockQuantity or lot.stock or lot.Stock or lot.quantity or lot.Quantity
    if direct ~= nil then
        local n = tonumber(direct)
        if n ~= nil and n == n then return math.max(0, math.floor(n)) end
    end

    if lot.guiStockText and tostring(lot.guiStockText) ~= "" then
        return HaimiyachGAG2_AuctionParseStockNumber(lot.guiStockText)
    end

    return nil
end

function HaimiyachGAG2_AuctionStockText(lot)
    local stock = HaimiyachGAG2_AuctionStock(lot)
    if stock ~= nil then
        if stock <= 0 then return "Sold out" end
        return "x" .. tostring(stock) .. " in Stock"
    end
    if type(lot) == "table" and lot.guiStockText and tostring(lot.guiStockText) ~= "" then return tostring(lot.guiStockText) end
    return "Unlimited"
end

function HaimiyachGAG2_AuctionIsActive(lot)
...
function HaimiyachGAG2_AuctionSelectionKey(text)
    text = tostring(text or "")
    text = text:gsub("%s*%(%s*[xX×]%s*[%d,]+%s*%)%s*$", ""):gsub("%s+[xX×]%s*[%d,]+%s*$", "")
    return HaimiyachGAG2_NameKey(text)
end

function HaimiyachGAG2_AuctionMatchesSelection(lot)
    local display = HaimiyachGAG2_NameKey(HaimiyachGAG2_AuctionDisplayName(lot))
    local base = HaimiyachGAG2_AuctionSelectionKey(HaimiyachGAG2_AuctionBaseDisplayName(lot))
    local rawName = HaimiyachGAG2_AuctionSelectionKey(lot and (lot.item or lot.Name or lot.name or lot.DisplayName) or "")
    local filter = HaimiyachGAG2_AuctionSelectionKey(S.auctionNameFilter or "")
    if filter ~= "" and (string.find(display, filter, 1, true) or string.find(base, filter, 1, true) or string.find(rawName, filter, 1, true)) then return true end
    if not HaimiyachGAG2_HasSelection(S.auctionBuyItems) then return false end
    for want, enabled in pairs(S.auctionBuyItems) do
        if enabled == true then
            local key = HaimiyachGAG2_NameKey(want)
            local cleanKey = HaimiyachGAG2_AuctionSelectionKey(want)
            if key == "allauctionitems" or key == "allactiveauctionitems" or cleanKey == "allauctionitems" or cleanKey == "allactiveauctionitems" or key == display or cleanKey == base or cleanKey == rawName then
                return true
            end
        end
    end
...
function HaimiyachGAG2_AuctionItemOptions()
    local out = { "ALL ACTIVE AUCTION ITEMS" }
    local seen = { ["ALL ACTIVE AUCTION ITEMS"] = true }
    local lots = HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.lots or {}
    for _, lot in ipairs(lots) do
        local name = HaimiyachGAG2_AuctionDisplayName(lot)
    ...
    if HaimiyachGAG2_AuctionState and type(HaimiyachGAG2_AuctionState.knownItems) == "table" then
        for name, yes in pairs(HaimiyachGAG2_AuctionState.knownItems) do
            if yes == true and name and not seen[name] then
                seen[name] = true
                out[#out + 1] = name
            end
        end
    end
    if type(S.auctionBuyItems) == "table" then
        for name, enabled in pairs(S.auctionBuyItems) do
            if enabled == true and name and not seen[name] then
                seen[name] = true
                out[#out + 1] = name
            end
        end
...
function HaimiyachGAG2_AuctionListText()
    local lots = HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.lots or {}
    HaimiyachGAG2_AuctionRefresh(#lots == 0)
    lots = HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.lots or {}
    if #lots == 0 then
        return tostring((HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.status) or "WAITING SNAPSHOT") .. "\nOpen Auction NPC once, then tap REFRESH AUCTION SNAPSHOT."
    end
    local now = HaimiyachGAG2_AuctionServerNow()
    local lines = {}
    local count = 0
    for _, lot in ipairs(lots) do
        ...
        local name = HaimiyachGAG2_AuctionDisplayName(lot)
        local price = HaimiyachGAG2_AuctionFormatPrice(HaimiyachGAG2_AuctionCurrentPrice(lot))
        local stock = HaimiyachGAG2_AuctionStockText(lot)
        local remain
        if lot.guiTimerText and tostring(lot.guiTimerText) ~= "" then remain = tostring(lot.guiTimerText) else remain = HaimiyachGAG2_AuctionTimeText((tonumber(lot.expiresAt) or now) - now) end
        local tag = HaimiyachGAG2_AuctionIsActive(lot) and "ACTIVE" or "SOLD OUT / EXPIRED"
        lines[#lines + 1] = tostring(name) .. "\nNow: " .. tostring(price) .. " | " .. tostring(stock) .. " | " .. tostring(remain) .. " [" .. tag .. "]"
    end
    return table.concat(lines, "\n\n")
end

function HaimiyachGAG2_AuctionStatusText()
    local state = HaimiyachGAG2_AuctionState or {}
    local selected = HaimiyachGAG2_HasSelection(S.auctionBuyItems) and "SELECTED" or "NONE"
    if tostring(S.auctionNameFilter or "") ~= "" then selected = "FILTER: " .. tostring(S.auctionNameFilter) end
    return "STATUS: " .. tostring(state.status or "WAITING") .. "\nAUTO BUY: " .. tostring(S.autoBuyAuction and "ON" or "OFF") .. " | TARGET: " .. selected
end

function HaimiyachGAG2_AuctionBuyLot(lot)
    if type(lot) ~= "table" then return false, "bad lot" end
    if not HaimiyachGAG2_AuctionIsActive(lot) then return false, "inactive" end
    local price = HaimiyachGAG2_AuctionCurrentPrice(lot)
    local id = tostring(lot.lotId or "")
    local ok, res = false, "no lot id"

    if id ~= "" and string.sub(id, 1, 4) ~= "GUI_" then
        ok, res = fire("Auctioneer.PurchaseLot", id, price)
    end

    if not ok and lot.guiBuyButton then
        local btn = lot.guiBuyButton
        local clicked = false
        ...
        if not clicked then
            pcall(function()
                if btn.Activate then btn:Activate(); clicked = true end
            end)
        end
        if clicked then ok, res = true, "clicked gui button" end
    end

    if ok then
        HaimiyachGAG2_AuctionState.lastBuy = os.clock()
        HaimiyachGAG2_AuctionState.status = "BUY SENT: " .. HaimiyachGAG2_AuctionDisplayName(lot)
        if Stats and Stats.SetLastAction then Stats.SetLastAction("AUCTION", "Buy " .. HaimiyachGAG2_AuctionDisplayName(lot)) end
        return true, res
    end
    HaimiyachGAG2_AuctionState.status = "BUY FAILED: " .. tostring(res)
    return false, res
end

function HaimiyachGAG2_AuctionAutoBuyStep(forceBuy)
    HaimiyachGAG2_AuctionRefresh(false)
    if not forceBuy and not S.autoBuyAuction then return 0 end
    if not forceBuy and os.clock() - (tonumber(HaimiyachGAG2_AuctionState.lastBuy) or 0) < math.max(1, tonumber(S.auctionBuyDelay) or 3) then return 0 end
    local bought = 0
    for _, lot in ipairs(HaimiyachGAG2_AuctionState.lots or {}) do
        pcall(HaimiyachGAG2_AuctionSyncLotStockFromLive, lot)
        if HaimiyachGAG2_AuctionMatchesSelection(lot) and HaimiyachGAG2_AuctionIsActive(lot) then
            local ok = HaimiyachGAG2_AuctionBuyLot(lot)
            if ok then bought = bought + 1; break end
        end
    end
    return bought
end

loopOn(function() return S.autoBuyAuction end, function()
    return math.max(1, tonumber(S.auctionBuyDelay) or 3)
end, function()
    HaimiyachGAG2_AuctionAutoBuyStep()
end)

task.spawn(function()
    task.wait(4)
    pcall(HaimiyachGAG2_AuctionRefresh, true)
end)
...
local HaimiyachGAG2_RawObjectKg = HaimiyachGAG2_ObjectKg
function HaimiyachGAG2_ObjectKg(obj)
    if not obj then return nil end

    if obj.IsDescendantOf and obj:IsDescendantOf(Workspace) then
        local node = obj
        local fruitNode = nil
        local plantNode = nil
        local guard = 0
        while node and node ~= Workspace and guard < 12 do
            ...
            if not plantNode and node:GetAttribute("PlantId") ~= nil then
                plantNode = node
            end
            node = node.Parent
        end
        ...
        if fruitNode then
            local kg = HaimiyachGAG2_InstanceKg(fruitNode)
            if kg then return kg end
            local ok, weight = pcall(function()
                return GardenESP.calculateWeight(fruitNode)
            end)
            weight = tonumber(weight)
            if ok and weight and weight > 0 then return weight end

            local checked = 0
            for _, d in ipairs(fruitNode:GetDescendants()) do
                checked = checked + 1
                kg = HaimiyachGAG2_InstanceKg(d)
                if kg then return kg end
        ...
        if plantNode then
            local ok, weight = pcall(function()
                return GardenESP.calculateWeight(plantNode)
            end)
            weight = tonumber(weight)
            if ok and weight and weight > 0 then return weight end
        end
    ...
    local kg = HaimiyachGAG2_RawObjectKg and HaimiyachGAG2_RawObjectKg(obj) or nil
    return kg
end
...
function BackpackESP.itemKg(item)
    if not item then return nil end
    return HaimiyachGAG2_ObjectKg(item)
end
...
function HaimiyachGAG2_GetInventoryFruitTools()
    ...
    local function isFruitTool(tool)
        if not (tool and tool:IsA("Tool")) then return false end
        if tool:GetAttribute("PetId") ~= nil then return false end
        if tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil then return false end
        local n = string.lower(tostring(tool.Name or ""))
        if string.find(n, "seed", 1, true) and not string.find(n, "seed pack", 1, true) then return false end
        if tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("FruitName") ~= nil or tool:GetAttribute("FruitProxy") ~= nil or tool:GetAttribute("HarvestedFruit") ~= nil then return true end
        if HaimiyachGAG2_ParseKgText(tool.Name, false) then return true end
        local c = tostring(tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or tool:GetAttribute("MainCategory") or "")
        c = string.lower(c)
        if c == "harvestedfruits" or c == "harvestedfruit" or c == "fruits" or c == "fruit" then return true end
        return false
    end
    local function scan(container)
        if not container then return end
        for _, tool in ipairs(container:GetChildren()) do
            if isFruitTool(tool) then out[#out + 1] = tool end
        end
    end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    if not tool then return nil end
    local kg = HaimiyachGAG2_InstanceKg(tool)
    if kg then return kg end
    kg = HaimiyachGAG2_ParseKgText(tool.Name, false)
    if kg then return kg end
    kg = HaimiyachGAG2_ParseKgText(tool.ToolTip, false)
    if kg then return kg end
    kg = HaimiyachGAG2_ObjectKg(tool)
    if kg then return kg end
    return nil
end
...
function HaimiyachGAG2_ShouldFavoriteFruit(tool)
    ...
    local fruit = HaimiyachGAG2_ObjectName(tool)
    if fruit == "" then return false end

    if picked(S.favoriteFruitTargets) then
        if not HaimiyachGAG2_SelectedNameOk(tool, S.favoriteFruitTargets) then return false end
    elseif not HaimiyachGAG2_TextListMatch(fruit, S.favoriteFruitFilter) then
        return false
    end

    if picked(S.favoriteMutations) then
        if not HaimiyachGAG2_SelectedMutationOk(tool, S.favoriteMutations) then return false end
    else
        local mutFilter = tostring(S.favoriteMutationFilter or "")
        if mutFilter ~= "" and mutFilter ~= "all" then
            local mutation = tostring(HaimiyachGAG2_ObjectMutation(tool) or "")
...
function HaimiyachGAG2_SetFruitFavorite(tool, enabled)
    if not tool then return false end
    local fruitId = HaimiyachGAG2_GetFruitToolId and HaimiyachGAG2_GetFruitToolId(tool) or tool:GetAttribute("Id")
    if not fruitId then return false end
...
function HaimiyachGAG2_GetFruitToolId(tool)
    if not tool then return nil end
    return tool:GetAttribute("Id") or tool:GetAttribute("FruitId") or tool:GetAttribute("UID") or tool:GetAttribute("UUID")
end

function HaimiyachGAG2_ShouldKeepFruitFromSell(tool)
    if not tool then return false end
    if picked(S.sellKeepMutations) and HaimiyachGAG2_SelectedMutationOk(tool, S.sellKeepMutations) then
        return true
    end
    if HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue) then
        if HaimiyachGAG2_KgValueOk(HaimiyachGAG2_GetFruitToolKg(tool), S.sellKeepKgMode, S.sellKeepKgValue) then
            return true
        end
    end
    return false
...
function HaimiyachGAG2_ProtectSellMutations()
    if not picked(S.sellKeepMutations) and not HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue) then return 0 end
    local protected = 0
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        if HaimiyachGAG2_ShouldKeepFruitFromSell(tool) and tool:GetAttribute("IsFavorite") ~= true then
...
function HaimiyachGAG2_SellFilteredFruits()
    local sold = 0
    local earned = 0
    HaimiyachGAG2_ProtectSellMutations()
    task.wait(0.15)
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        if not (S.autoSell or S.autoSellFruits) then break end
        if tool and tool.Parent and not HaimiyachGAG2_ShouldKeepFruitFromSell(tool) then
            local fruitId = HaimiyachGAG2_GetFruitToolId(tool)
            if fruitId then
                local ok, res = fireFast("NPCS.SellFruit", fruitId)
                if ok and (res == true or type(res) ~= "table" or res.Success) then
                    sold = sold + 1
                    if type(res) == "table" then earned = earned + (tonumber(res.SellPrice) or 0) end
                    task.wait(0.06)
                else
                    task.wait(0.05)
                end
    ...
    if sold > 0 or earned > 0 then
        Stats.sold = (Stats.sold or 0) + sold
        Stats.earned = (Stats.earned or 0) + earned
        Stats.lastSellValue = earned
        Stats.lastSellAt = os.clock()
        if Stats.SetLastAction then Stats.SetLastAction("SELL", tostring(sold) .. " filtered fruits for " .. fmt(earned)) end
    end
    return sold
end
...
local function findSeedEventObjects()
    local results = {}
    local spawnServer, spawnClient = getSeedSpawnFolders()
    local function yes(v)
        return v == true or tostring(v) == "true" or tostring(v) == "1"
    end
    local function add(obj)
        if not obj then return end
        local isRainbow = yes(obj:GetAttribute("RainbowSeed"))
        local isGold = yes(obj:GetAttribute("GoldSeed"))
        if not isRainbow and not isGold then return end
        local pos = getSeedEventPosition(obj) or (obj.Parent and getSeedEventPosition(obj.Parent))
        if pos then
            results[#results + 1] = {
                object = obj,
                position = pos,
    ...
    local function scan(folder)
        ...
        for _, prompt in ipairs(folder:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") and prompt.Enabled then
                ...
                local ppos = getSeedEventPosition(parent) or (parent and parent.Parent and getSeedEventPosition(parent.Parent))
                if ppos then
                    local dist = (ppos - pos).Magnitude
                    if dist < bestDist and dist <= 55 then
                        bestDist = dist
                        bestPrompt = prompt
                    end
                end
...
local function triggerSeedEventPrompt(obj, pos)
    ...
    local ppos = getSeedEventPosition(prompt.Parent) or (prompt.Parent and prompt.Parent.Parent and getSeedEventPosition(prompt.Parent.Parent))
    if ppos then
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then pcall(function() hrp.CFrame = CFrame.new(ppos + Vector3.new(0, 3, 0)) end) end
        task.wait(0.12)
    end
    ...
    if type(fireproximityprompt) == "function" then
        ...
        if not ok then
            ok = pcall(function() fireproximityprompt(prompt, math.max(0.2, tonumber(prompt.HoldDuration) or 0.2)) end)
        end
    end
    if not ok then
        ok = pcall(function()
            prompt:InputHoldBegin()
            task.wait(math.max(0.25, tonumber(prompt.HoldDuration) or 0.25))
            prompt:InputHoldEnd()
        end)
    end
    return ok == true
...
local function claimSeedEvent()
    ...
    for _, data in ipairs(events) do
        ...
        if obj and obj.Parent and pos then
            ...
            local okNet, resNet = fire("SeedPack.Claim", obj)
            if okNet and (resNet == nil or resNet == true or type(resNet) ~= "table" or resNet.Success) then
                okClaim = true
            end
            if not okClaim and obj and obj.Name then
                local okName, resName = fire("SeedPack.Claim", obj.Name)
                if okName and (resName == nil or resName == true or type(resName) ~= "table" or resName.Success) then
                    okClaim = true
                end
            end
...
function HaimiyachGAG2_HarvestAllowed(obj)
    if HaimiyachGAG2_HasSelection(S.harvestFruitTargets) then
        if not HaimiyachGAG2_SelectedNameOk(obj, S.harvestFruitTargets) then return false end
    elseif not S.harvestAll then
        return false
    end
...
local function sellAllNow()
    if picked(S.sellKeepMutations) or HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue) then
        return HaimiyachGAG2_SellFilteredFruits()
    end
...
local function stepHarvest()
    local sellFull = S.autoSell
    local sellByDelay = S.autoSellFruits
    local list = ripeHarvests(S.harvestFruitTargets)
    local cap = maxFruitCap()
    local count = fruitCount()
    if sellByDelay and count > 0 and due("sell", math.max(3, tonumber(S.sellInterval) or 15)) then
        sellAllNow()
        task.wait(0.2)
        count = fruitCount()
    end
    ...
    -- collect until inventory is near full. Sell only when one of the sell toggles is enabled.
    ...
    count = fruitCount()
    if sellFull and count >= cap - 1 then
        sellAllNow()
    elseif sellByDelay and count > 0 and due("sell", math.max(3, tonumber(S.sellInterval) or 15)) then
        sellAllNow()
    end
end
...
local function stepSell()
    ...
    if S.autoSell and count >= maxFruitCap() - 1 then
        if due("sellFull", 1.5) then
            local n = sellAllNow()
            if n > 0 then notify("Sold", n .. " items", 3) end
        end
        return
    end
    if S.autoSellFruits then
        if not due("sell", math.max(3, tonumber(S.sellInterval) or 15)) then return end
        local n = sellAllNow()
        if n > 0 then notify("Sold", n .. " items", 3) end
    end
end
...
task.spawn(function()
    while not S.killed do
        if S.autoFarm or S.autoHarvest then
            pcall(stepHarvest)
            if HaimiyachGAG2_HasSelection(S.harvestFruitTargets) or HaimiyachGAG2_KgModeActive(S.harvestKgMode, S.harvestKgValue) or HaimiyachGAG2_HasSelection(S.harvestKeepMutations) then
                task.wait(0.16)
            else
                task.wait(0.08)
            end
        elseif S.autoSell or S.autoSellFruits then
            pcall(stepSell)
            task.wait(0.3)
        else
            task.wait(0.4)
...
S.shovelObjectName = function(obj)
    return HaimiyachGAG2_ObjectName(obj)
end
...
S.shovelKgOf = function(obj)
    return HaimiyachGAG2_ObjectKg(obj)
end
...
local function HaimiyachGAG2_FindWildPetPrompt(w)
    if not w then return nil, nil end
    local ref = w.part
    local wanted = tostring(w.name or "")
    ...
    local function scan(root)
        if not root then return end
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("ProximityPrompt") and d.Name == "BuyPrompt" and d.Enabled then
                local carrier = d.Parent
                local model = carrier and carrier:FindFirstAncestorOfClass("Model") or nil
                local name = model and (model:GetAttribute("PetName") or model.Name) or ""
                if wanted == "" or tostring(name) == wanted then
                    local pos = partPos(carrier) or partPos(model)
                    if pos then
                        local dist = 0
                        if ref and ref:IsA("BasePart") then dist = (pos - ref.Position).Magnitude end
    ...
    scan(ref)
    local map = Workspace:FindFirstChild("Map")
    if map then scan(map:FindFirstChild("WildPetSpawns")) end
    return bestPrompt, bestPart
end

local function HaimiyachGAG2_UseWildPetPrompt(prompt)
    ...
    return pcall(function()
        prompt:InputHoldBegin()
        task.wait(math.max(0.2, tonumber(prompt.HoldDuration) or 0.2) + 0.05)
        prompt:InputHoldEnd()
    end)
end

HaimiyachGAG2_WildPetTameResults = HaimiyachGAG2_WildPetTameResults or {}
if not HaimiyachGAG2_WildPetTameResultConnected then
    HaimiyachGAG2_WildPetTameResultConnected = true
    pcall(function()
        local ev = action("Pets.WildPetTameResult")
        if ev and ev.OnClientEvent then
            ev.OnClientEvent:Connect(function(refPart, ownerUserId)
                if refPart and tonumber(ownerUserId) == LocalPlayer.UserId then
                    HaimiyachGAG2_WildPetTameResults[refPart] = os.clock()
                end
            end)
        end
    end)
...
local function HaimiyachGAG2_WildPetConfirmed(w, prompt)
    local ref = w and w.part
    local deadline = os.clock() + 4
    while os.clock() < deadline do
        if ref and (not ref.Parent or not ref:IsDescendantOf(Workspace)) then return true end
        if ref then
            local owner = tonumber(ref:GetAttribute("OwnerUserId")) or tonumber(ref:GetAttribute("Owner")) or 0
            if owner == LocalPlayer.UserId then return true end
            if HaimiyachGAG2_WildPetTameResults and HaimiyachGAG2_WildPetTameResults[ref] then return true end
            local state = string.lower(tostring(ref:GetAttribute("State") or ref:GetAttribute("PetState") or ""))
            if string.find(state, "tame", 1, true) or string.find(state, "own", 1, true) or string.find(state, "sold", 1, true) then return true end
        end
        if prompt and prompt.Parent and prompt.Enabled == false then return true end
        task.wait(0.12)
    end
    return false
end

local function HaimiyachGAG2_ReturnGardenPet(savedCf)
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local pos = myBasePos and myBasePos() or nil
    if pos then
...
local function HaimiyachGAG2_TameWildPet(w)
    ...
    local prompt, promptPart = HaimiyachGAG2_FindWildPetPrompt(w)
    local targetPos = nil

    if promptPart and promptPart:IsA("BasePart") then targetPos = promptPart.Position end
    if not targetPos then targetPos = w.pos end
    ...
    if HaimiyachGAG2_WildPetTameResults then HaimiyachGAG2_WildPetTameResults[w.part] = nil end

    local fired = false
    for attempt = 1, 2 do
        if not S.autoBuyPets then break end
        prompt, promptPart = HaimiyachGAG2_FindWildPetPrompt(w)
        if prompt then fired = HaimiyachGAG2_UseWildPetPrompt(prompt) or fired end
        local ok = fire("Pets.WildPetTame", w.part)
        fired = fired or ok
        if HaimiyachGAG2_WildPetConfirmed(w, prompt) then
            if S.petTeleport then HaimiyachGAG2_ReturnGardenPet(savedCf) end
            return true
        end
        task.wait(0.35)
    end

    if Stats.SetWarning then
        if fired then Stats.SetWarning("PET", "Tame not confirmed: " .. petName)
        else Stats.SetWarning("PET", "Tame failed: " .. petName) end
    end
    return false
end
...
local function HaimiyachGAG2_DashboardFeatureFarmText()
    return "FARM " .. HaimiyachGAG2_OnOff(S.autoFarm) .. " | HARVEST " .. HaimiyachGAG2_OnOff(S.autoHarvest) .. " | PLANT " .. HaimiyachGAG2_OnOff(S.autoPlant) .. "\nSELL " .. HaimiyachGAG2_OnOff(S.autoSell or S.autoSellFruits) .. " | SHOVEL " .. HaimiyachGAG2_OnOff(S.autoShovelPlants or S.autoShovelFruits) .. " | WATER " .. HaimiyachGAG2_OnOff(S.autoWater)
end
...
local WebhookLastAutoAttempt = 0
sendWebhook = function(isTest, force)
    local forceSend = isTest or force
    local now = os.clock()
    if WebhookBusy then return false end
    if not forceSend and now - (Stats.lastWebhookAt or 0) < 10 then return false end
    if not forceSend and now - WebhookLastAutoAttempt < 8 then return false end
    WebhookBusy = true
    WebhookLastAutoAttempt = now
    local req = refreshHttpRequest(isTest and 5 or 8)
    if not req then
        WebhookBusy = false
        notify("Webhook", "Executor exposes no HTTP request fn", 4)
        return false
    end
    if not string.match(S.webhookUrl or "", "^https?://") then
        WebhookBusy = false
        notify("Webhook", "Set a valid webhook URL", 4)
        return false
    end
    ...
    local payload = {
        ...
        embeds = { {
            ...
            fields = {
                ...
                { name = "Farm Stats", value = Stats.WebhookFieldValue(string.format("Bought: %d\nSeeds Bought: %d\nGear Bought: %d\nCrates Bought: %d\nSprinklers Bought: %d\nPlanted: %d\nHarvested: %d\nSold: %d\nEarned: %s", tonumber(Stats.bought) or 0, tonumber(Stats.seedBought) or 0, tonumber(Stats.gearBought) or 0, tonumber(Stats.crateBought) or 0, tonumber(Stats.sprinklerBought) or 0, tonumber(Stats.planted) or 0, tonumber(Stats.harvested) or 0, tonumber(Stats.sold) or 0, fmt(Stats.earned))), inline = false },
                ...
                { name = "Crate Purchases", value = Stats.PurchaseSummary(Stats.crateBought, Stats.purchaseCrates, Stats.lastCrateBuy), inline = false },
                { name = "Collect Stats", value = Stats.WebhookFieldValue(string.format("Seeds: %d\nFruits: %d\nPets: %d", tonumber(Stats.collectedSeeds) or 0, tonumber(Stats.collectedFruits) or 0, tonumber(Stats.collectedPets) or 0)), inline = false },
...
-- Webhook report loop waits the interval first. This prevents instant double reports on auto-execute/rejoin.
task.spawn(function()
    local nextReportAt = nil
    while not S.killed do
        if S.webhookEnabled then
            local interval = math.max(60, tonumber(S.webhookInterval) or 300)
            if not nextReportAt then
                nextReportAt = os.clock() + interval
            elseif os.clock() >= nextReportAt then
                sendWebhook(false)
                nextReportAt = os.clock() + interval
            end
        else
...
local DisconnectWebhookBusy = false
local DisconnectWebhookLastAttempt = 0
local function sendDisconnectWebhook(reason, detail, force)
    if not force and not S.webhookDisconnect then return false end
    if DisconnectWebhookBusy then return false end
    if DisconnectWebhookSent and not force then return false end
    if not force and os.clock() - DisconnectWebhookLastAttempt < 10 then return false end
    DisconnectWebhookBusy = true
    DisconnectWebhookLastAttempt = os.clock()
    local req = refreshHttpRequest(force and 5 or 8)
    if not req then
        DisconnectWebhookBusy = false
    ...
    Stats.lastWebhookAt = os.clock()
    if good and not force then DisconnectWebhookSent = true end
    DisconnectWebhookBusy = false
    if not good and Stats.SetWarning then Stats.SetWarning("WEBHOOK", tostring(code or "request")) end
...
UI.reportBuyLabel = addLabel(UI.Pages["DASHBOARD"], "BUY SEED: 0 | GEAR: 0 | CRATE: 0\nSPRINKLER: 0 | OPENED: 0")
...
-- FARM
addSection(UI.Pages["FARM"], "SMART FARM")
...
addSlider(UI.Pages["FARM"], "HARVEST WEIGHT VALUE", 0, 1000, S.harvestKgValue, 1, function(v) S.harvestKgValue = v end)
...
-- AUTO
addSection(UI.Pages["FARM"], "AUTO SELL FRUITS")
addToggle(UI.Pages["FARM"], "AUTO SELL INVENTORY FULL", S.autoSell, function(v) S.autoSell = v end)
addToggle(UI.Pages["FARM"], "AUTO SELL FRUITS", S.autoSellFruits, function(v) S.autoSellFruits = v end)
...
addSlider(UI.Pages["FARM"], "SELL KEEP WEIGHT VALUE", 0, 1000, S.sellKeepKgValue, 1, function(v) S.sellKeepKgValue = v end)
...
addSlider(UI.Pages["FARM"], "SHOVEL WEIGHT VALUE", 0, 1000, S.shovelKgValue, 1, function(v) S.shovelKgValue = v end)
...
addSlider(UI.Pages["FARM"], "FAVORITE WEIGHT VALUE", 0, 1000, S.favoriteKgValue, 1, function(v) S.favoriteKgValue = v end)
...
-- AUCTION
addSection(UI.Pages["AUCTION"], "AUCTION AUTO BUY")
addMiniNote(UI.Pages["AUCTION"], "Reads the live Auction GUI first, then snapshot fallback. Open the game Auction GUI once if list is empty.")
UI.auctionStatusLabel = addLabel(UI.Pages["AUCTION"], "STATUS: WAITING SNAPSHOT\nAUTO BUY: OFF | TARGET: NONE")
UI.auctionItemDrop = addDropdown(UI.Pages["AUCTION"], "AUCTION ITEMS TO BUY", HaimiyachGAG2_AuctionItemOptions(), S.auctionBuyItems, true, function(sel)
    if type(S.auctionBuyItems) ~= "table" then S.auctionBuyItems = {} end
    pickMulti(sel, S.auctionBuyItems)
    saveConfig(true)
end)
addInput(UI.Pages["AUCTION"], "AUCTION NAME FILTER", "optional exact or partial item name", S.auctionNameFilter or "", function(v)
    S.auctionNameFilter = v or ""
    saveConfig(true)
end)
addToggle(UI.Pages["AUCTION"], "AUTO BUY AUCTION ITEMS", S.autoBuyAuction, function(v)
    S.autoBuyAuction = v
    if v then pcall(HaimiyachGAG2_AuctionRefresh, true) end
    saveConfig(true)
end)
addSlider(UI.Pages["AUCTION"], "AUCTION BUY DELAY (SEC)", 1, 60, S.auctionBuyDelay or 3, 0, function(v)
    S.auctionBuyDelay = v
    saveConfig(true)
end)
addButton(UI.Pages["AUCTION"], "REFRESH AUCTION SNAPSHOT", function()
    HaimiyachGAG2_AuctionRefresh(true)
    if UI.auctionItemDrop and UI.auctionItemDrop.SetOptions then UI.auctionItemDrop.SetOptions(HaimiyachGAG2_AuctionItemOptions()) end
    notify("Auction", "Snapshot refreshed", 3)
end)
addButton(UI.Pages["AUCTION"], "BUY MATCHING AUCTION NOW", function()
    HaimiyachGAG2_AuctionRefresh(true)
    local n = HaimiyachGAG2_AuctionAutoBuyStep(true)
    notify("Auction", n > 0 and "Buy request sent" or "No matching active auction", 3)
end)
addSection(UI.Pages["AUCTION"], "ACTIVE AUCTIONS")
UI.auctionListLabel = addLabel(UI.Pages["AUCTION"], "WAITING SNAPSHOT\nTap refresh if empty.")
if UI.auctionListLabel and UI.auctionListLabel.Parent then
    UI.auctionListLabel.Parent.Size = UDim2.new(1, 0, 0, 260)
end
...
UI.mailFruitDropdown = addDropdown(UI.Pages["MAILBOX"], "FRUITS TO SEND", HaimiyachGAG2_FruitMailOptionNames(), S.mailFruitTargets, true, function(sel)
    if type(S.mailFruitTargets) ~= "table" then S.mailFruitTargets = {} end
    pickMulti(sel, S.mailFruitTargets)
    saveConfig(true)
end)
-- Fruit list is catalog/name based, so auto send does not need manual refresh.
addToggle(UI.Pages["MAILBOX"], "HARVEST SELECTED FRUIT FROM GARDEN", S.mailHarvestGarden == true, function(v)
    S.mailHarvestGarden = v
    saveConfig(true)
end)
...
task.spawn(function()
    while not S.killed do
        ...
        if UI.auctionStatusLabel then UI.SafeSetText(UI.auctionStatusLabel, HaimiyachGAG2_AuctionStatusText()) end
        if UI.auctionListLabel then UI.SafeSetText(UI.auctionListLabel, HaimiyachGAG2_AuctionListText()) end
        if UI.auctionItemDrop and UI.auctionItemDrop.SetOptions and UI.auctionOptionVersion ~= (HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.optionVersion) then
            UI.auctionOptionVersion = HaimiyachGAG2_AuctionState and HaimiyachGAG2_AuctionState.optionVersion
            UI.auctionItemDrop.SetOptions(HaimiyachGAG2_AuctionItemOptions())
        end
...
--[[ MAIL FRUITS EXACT HOLD FIX
     Based on uploaded controllers:
     - MailboxController sends HarvestedFruits using Inventory key via Mailbox.SendBatch.
     - Backpack/Main promotes fruit proxy with FruitProxyUtil.Pending.Equip + RequestPromote(Id).
     - Gift/mail prompt sees real fruit only when held Tool has HarvestedFruit == true and Id.
     This override never equips seed/gears for fruit mail.
]]
function HaimiyachGAG2_RealMailboxFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if tool:GetAttribute("HarvestedFruit") ~= true then return false end
    local id = tool:GetAttribute("Id")
    if id == nil or tostring(id) == "" then return false end
    return true
end

function HaimiyachGAG2_MailFruitIdCandidatesForItemKey(itemKey)
    local out, seen = {}, {}
    local function add(v)
        if v ~= nil then
            v = tostring(v)
...
function HaimiyachGAG2_MailFruitEntryIdForItemKey(itemKey)
    local ids, value = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(itemKey)
    return ids[1], value
end

function HaimiyachGAG2_MailHeldFruitId(tool)
    if not HaimiyachGAG2_RealMailboxFruitTool(tool) then return nil end
    local id = tool:GetAttribute("Id")
    if id == nil then return nil end
    return tostring(id)
end

function HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
    fruitId = tostring(fruitId or "")
    if fruitId == "" then return nil end
    local char = LocalPlayer.Character
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if HaimiyachGAG2_RealMailboxFruitTool(tool) and tostring(tool:GetAttribute("Id")) == fruitId then
                return tool
            end
        end
    end
...
function HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
    ...
    pcall(function()
        if type(fpu.Pending) == "table" then
            fpu.Pending.Equip = fruitId
            if type(fpu.Pending.Slots) == "table" then
                -- Controller Select() fills Pending.Slots[id] before RequestPromote(id).
                -- Slot 1 is enough to make OnChildAdded run the equip path after the tool appears.
                fpu.Pending.Slots[fruitId] = fpu.Pending.Slots[fruitId] or 1
            end
        end
    end)

    local ok = pcall(function()
        if type(fpu.RequestPromote) == "function" then
            fpu.RequestPromote(fruitId)
        elseif type(fpu.Promote) == "function" then
            fpu.Promote(fruitId)
        end
    end)
    return ok
...
function HaimiyachGAG2_WaitRealMailboxFruitToolById(fruitId, timeoutSec)
    fruitId = tostring(fruitId or "")
    timeoutSec = tonumber(timeoutSec) or 5
    local deadline = os.clock() + timeoutSec
    while os.clock() < deadline do
        local tool = HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
        if tool then return tool end
        task.wait(0.06)
    end
    return HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
end

function HaimiyachGAG2_EquipMailToolNow(tool)
    if not HaimiyachGAG2_RealMailboxFruitTool(tool) then return false, "real fruit tool not found" end
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(3)
    if not char then return false, "character not found" end
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    local wantedId = tostring(tool:GetAttribute("Id"))
    local hum = char:FindFirstChildOfClass("Humanoid")

    for _ = 1, 36 do
        if not tool.Parent then return false, "fruit tool removed" end
        if tool.Parent ~= char then
            pcall(function() if hum and hum.UnequipTools then hum:UnequipTools() end end)
            task.wait(0.05)
            pcall(function()
        ...
        task.wait(0.08)
        local held = char:FindFirstChildWhichIsA("Tool")
        if HaimiyachGAG2_RealMailboxFruitTool(held) and tostring(held:GetAttribute("Id")) == wantedId then
            task.wait(0.35)
            return true, nil
        end
    end
    ...
    local held = char:FindFirstChildWhichIsA("Tool")
    if HaimiyachGAG2_RealMailboxFruitTool(held) then
        return false, "holding different fruit: " .. tostring(held.Name)
    end
    return false, "fruit was not held"
end

function HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    if type(ids) ~= "table" then return false, "fruit id missing" end
    local lastErr = "fruit not held"
    for _, fruitId in ipairs(ids) do
        fruitId = tostring(fruitId or "")
        if fruitId ~= "" then
            local tool = HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
            if not tool then
                HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
                tool = HaimiyachGAG2_WaitRealMailboxFruitToolById(fruitId, 5.5)
            end
            if tool then
                local ok, err = HaimiyachGAG2_EquipMailToolNow(tool)
                if ok then return true, nil, tool end
                lastErr = err or lastErr
            else
                lastErr = "fruit tool not promoted: " .. fruitId
            end
        end
    end
...
function HaimiyachGAG2_HoldMailboxFruitById(fruitId)
    return HaimiyachGAG2_HoldMailboxFruitByCandidateIds({ tostring(fruitId or "") })
end
...
function HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    ...
    local function check(container)
        ...
        for _, tool in ipairs(container:GetChildren()) do
            if HaimiyachGAG2_RealMailboxFruitTool(tool) then
                local nm = HaimiyachGAG2_MailFruitToolName and HaimiyachGAG2_MailFruitToolName(tool) or tostring(tool.Name or "")
                local loose = HaimiyachGAG2_MailLooseName(nm)
                if HaimiyachGAG2_MailSelectedAllFruits and HaimiyachGAG2_MailSelectedAllFruits(selected) then return tool end
                for sel, enabled in pairs(selected) do
                    if enabled == true then
                        local want = HaimiyachGAG2_MailLooseName(sel)
                        if want ~= "" and loose ~= "" and (loose == want or string.find(loose, want, 1, true)) then
                            if not (want == "dragon fruit" and string.find(loose, "dragon breath", 1, true)) then
                                return tool
                            end
                        end
                    end
    ...
    return check(LocalPlayer.Character) or check(LocalPlayer:FindFirstChild("Backpack"))
end
...
function HaimiyachGAG2_EquipSelectedMailFruit(selected)
    ...
    local key, value = HaimiyachGAG2_FindSelectedMailFruitEntry(selected)
    if not key then return nil, "selected fruit id not found" end
    local ids = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(key)
    local ok, err, heldTool = HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    if not ok then return nil, err end
    return heldTool or HaimiyachGAG2_FindSelectedMailFruitTool(selected), nil
end

function HaimiyachGAG2_FindFruitToolForMailItem(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return nil end
    if type(item.__Tool) == "userdata" and item.__Tool.Parent and HaimiyachGAG2_RealMailboxFruitTool(item.__Tool) then
        return item.__Tool
    end
    local ids = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(tostring(item.ItemKey or ""))
    for _, id in ipairs(ids) do
        local tool = HaimiyachGAG2_FindRealMailboxFruitToolById(id)
        if tool then return tool end
    end
    return nil
end
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    ...
    local ok, err, heldTool = HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    if not ok then return false, err end
    item.__Tool = heldTool or item.__Tool
    return true, nil
end



--[[ MAILBOX FRUIT BACKPACK HOLD FIX V3
     Controller-based correction:
     - GiftingController only checks for any truthy Tool attribute named HarvestedFruit/Fruit category, not HarvestedFruit == true.
     - Backpack/Main equips fruit by promoting the fruit Id, then parenting the resulting Tool to Character.
     - Seeds are always rejected for FRUITS TO SEND, even if their name contains the selected fruit name.
]]
function HaimiyachGAG2_MailToolLooksSeed(tool)
    if not tool then return false end
    if tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil then return true end
    local n = string.lower(tostring(tool.Name or ""))
    if string.find(n, "seed", 1, true) and not string.find(n, "seed pack", 1, true) then return true end
    return false
end

function HaimiyachGAG2_RealMailboxFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if HaimiyachGAG2_MailToolLooksSeed(tool) then return false end
    if tool:GetAttribute("Pet") ~= nil or tool:GetAttribute("PetId") ~= nil then return false end
    if tool:GetAttribute("Sprinkler") ~= nil or tool:GetAttribute("WateringCan") ~= nil or tool:GetAttribute("Trowel") ~= nil or tool:GetAttribute("Shovel") ~= nil or tool:GetAttribute("Crate") ~= nil or tool:GetAttribute("Egg") ~= nil or tool:GetAttribute("Mushroom") ~= nil or tool:GetAttribute("Gnome") ~= nil or tool:GetAttribute("Raccoon") ~= nil then
        return false
    end
    if tool:GetAttribute("HarvestedFruit") ~= nil then return true end
    if tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("FruitName") ~= nil or tool:GetAttribute("FruitProxy") ~= nil then return true end
    local c = string.lower(tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or ""))
    if c == "fruit" or c == "fruits" or c == "harvestedfruit" or c == "harvestedfruits" then return true end
    if HaimiyachGAG2_ParseKgText and HaimiyachGAG2_ParseKgText(tool.Name, false) then return true end
    return false
end

function HaimiyachGAG2_MailFruitToolIds(tool)
    local ids, seen = {}, {}
    local function add(v)
        if v ~= nil then
            local s = tostring(v)
            if s ~= "" and s ~= "true" and s ~= "false" and not seen[s] then
                seen[s] = true
                ids[#ids + 1] = s
            end
        end
    end
    if not tool then return ids end
    for _, attr in ipairs({ "Id", "ID", "InventoryKey", "ItemKey", "HarvestedFruit", "HarvestedFruitId", "FruitId", "UUID", "Uuid", "Guid", "UID", "ItemId" }) do
        add(tool:GetAttribute(attr))
    end
    return ids
end

function HaimiyachGAG2_MailFruitToolHasId(tool, fruitId)
    if not HaimiyachGAG2_RealMailboxFruitTool(tool) then return false end
    fruitId = tostring(fruitId or "")
    if fruitId == "" then return false end
    for _, id in ipairs(HaimiyachGAG2_MailFruitToolIds(tool)) do
...
function HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
    ...
    return scan(LocalPlayer.Character) or scan(LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
end
...
function HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
    ...
    pcall(function()
        if type(fpu.Pending) == "table" then
            ...
            if type(fpu.Pending.Slots) == "table" then
                fpu.Pending.Slots[fruitId] = fpu.Pending.Slots[fruitId] or 1
                local num = tonumber(fruitId)
                if num then fpu.Pending.Slots[num] = fpu.Pending.Slots[num] or 1 end
            end
        end
    end)
    return pcall(function()
...
function HaimiyachGAG2_WaitRealMailboxFruitToolById(fruitId, timeoutSec)
    fruitId = tostring(fruitId or "")
    timeoutSec = tonumber(timeoutSec) or 6
    local deadline = os.clock() + timeoutSec
    while os.clock() < deadline do
        local tool = HaimiyachGAG2_FindRealMailboxFruitToolById(fruitId)
        if tool then return tool end
...
function HaimiyachGAG2_EquipMailToolNow(tool)
    if not HaimiyachGAG2_RealMailboxFruitTool(tool) then return false, "selected tool is not fruit" end
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(3)
    if not char then return false, "character not found" end
    local backpack = LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack")
    local hum = char:FindFirstChildOfClass("Humanoid")
    local wantedIds = {}
    for _, id in ipairs(HaimiyachGAG2_MailFruitToolIds(tool)) do wantedIds[id] = true end

    for _ = 1, 48 do
        if not tool.Parent then return false, "fruit removed from backpack" end
        pcall(function() if hum and hum.UnequipTools then hum:UnequipTools() end end)
        if char then
            for _, other in ipairs(char:GetChildren()) do
                if other:IsA("Tool") and other ~= tool and backpack then
                    pcall(function() other.Parent = backpack end)
                end
            end
        end
        task.wait(0.04)
        pcall(function()
            if hum and hum.EquipTool and tool.Parent == backpack then
                hum:EquipTool(tool)
            end
        end)
        task.wait(0.06)
        ...
        task.wait(0.09)
        local held = char:FindFirstChildWhichIsA("Tool")
        if held and HaimiyachGAG2_RealMailboxFruitTool(held) then
            if held == tool then task.wait(0.45); return true, nil end
            for _, id in ipairs(HaimiyachGAG2_MailFruitToolIds(held)) do
                if wantedIds[id] then task.wait(0.45); return true, nil end
            end
        end
    end
    local held = char:FindFirstChildWhichIsA("Tool")
    if held then return false, "still holding: " .. tostring(held.Name) end
    return false, "fruit not held from backpack"
end
...
function HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    ...
    for _, fruitId in ipairs(ids) do
        ...
        if fruitId ~= "" then
            ...
            if not tool then
                HaimiyachGAG2_RequestPromoteMailboxFruit(fruitId)
                tool = HaimiyachGAG2_WaitRealMailboxFruitToolById(fruitId, 6.5)
            end
            if tool then
                local ok, err = HaimiyachGAG2_EquipMailToolNow(tool)
                if ok then return true, nil, tool end
                ...
                lastErr = "fruit tool/proxy not found: " .. fruitId
            end
        end
    end
    return false, lastErr
...
function HaimiyachGAG2_HoldMailboxFruitById(fruitId)
    return HaimiyachGAG2_HoldMailboxFruitByCandidateIds({ fruitId })
end

function HaimiyachGAG2_FindSelectedMailFruitTool(selected)
    if type(selected) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return nil end
    local function matchName(tool)
        if not HaimiyachGAG2_RealMailboxFruitTool(tool) then return false end
        if HaimiyachGAG2_MailSelectedAllFruits and HaimiyachGAG2_MailSelectedAllFruits(selected) then return true end
        local nm = HaimiyachGAG2_MailLooseName(HaimiyachGAG2_MailFruitToolName and HaimiyachGAG2_MailFruitToolName(tool) or tostring(tool.Name or ""))
        if nm == "" then return false end
        for sel, enabled in pairs(selected) do
            if enabled == true then
                local want = HaimiyachGAG2_MailLooseName(sel)
                if want ~= "" and (nm == want or string.find(nm, want, 1, true)) then
                    if not (want == "dragon fruit" and string.find(nm, "dragon breath", 1, true)) then
                        return true
                    end
                end
            end
    ...
    local function scan(container)
        if not container then return nil end
        for _, tool in ipairs(container:GetChildren()) do
            if matchName(tool) then return tool end
        end
        return nil
    end
    return scan(LocalPlayer.Character) or scan(LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
...
function HaimiyachGAG2_EquipSelectedMailFruit(selected)
    ...
    local key = nil
    local value = nil
    if HaimiyachGAG2_FindSelectedMailFruitEntry then
        key, value = HaimiyachGAG2_FindSelectedMailFruitEntry(selected)
    end
    if not key then return nil, "selected fruit not found in HarvestedFruits" end
    local ids = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(key)
    local ok, err, heldTool = HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    if not ok then return nil, err end
    return heldTool or HaimiyachGAG2_FindSelectedMailFruitTool(selected), nil
...
function HaimiyachGAG2_FindFruitToolForMailItem(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return nil end
    if item.__Tool and item.__Tool.Parent and HaimiyachGAG2_RealMailboxFruitTool(item.__Tool) then return item.__Tool end
    local ids = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(tostring(item.ItemKey or ""))
    for _, id in ipairs(ids) do
        local tool = HaimiyachGAG2_FindRealMailboxFruitToolById(id)
        if tool then return tool end
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    if not HaimiyachGAG2_IsFruitMailItem(item) then return false, "not fruit item" end
    if item.__Tool and item.__Tool.Parent and HaimiyachGAG2_RealMailboxFruitTool(item.__Tool) then
        local ok, err = HaimiyachGAG2_EquipMailToolNow(item.__Tool)
        if ok then return true, nil end
    end
    local ids = HaimiyachGAG2_MailFruitIdCandidatesForItemKey(tostring(item.ItemKey or ""))
    local ok, err, heldTool = HaimiyachGAG2_HoldMailboxFruitByCandidateIds(ids)
    if ok then item.__Tool = heldTool; return true, nil end
    return false, err
end
...
--[[ MAILBOX FRUIT FIX - MANUAL HOLD PRIORITY V4
     Core rule from controller/video: fruit mail only works reliably when the
     actual fruit Tool is held first. This patch prioritizes the currently held
     fruit Tool, then backpack/proxy promotion. It never treats seed tools as fruit. ]]

function HMY_MAIL_V4_Text(v)
    v = tostring(v or "")
    v = v:gsub("%c", " "):gsub("^%s+", ""):gsub("%s+$", "")
    return v
end

function HMY_MAIL_V4_Loose(v)
    v = HMY_MAIL_V4_Text(v):lower()
    v = v:gsub("%b[]", " ")
    v = v:gsub("%b()", " ")
    v = v:gsub("%s+%d+%.?%d*%s*kg%s*$", "")
    v = v:gsub("%s+x%d+%s*$", "")
...
function HMY_MAIL_V4_SelectedAll(selected)
    if type(selected) ~= "table" then return false end
    for k, v in pairs(selected) do
        if v == true and HMY_MAIL_V4_Loose(k) == "all fruits" then return true end
    end
    return false
end

function HMY_MAIL_V4_SelectedList(selected)
    local out = {}
    if type(selected) == "table" then
        for k, v in pairs(selected) do if v == true then out[#out + 1] = tostring(k) end end
    end
    table.sort(out)
    return table.concat(out, ", ")
end

function HMY_MAIL_V4_AnySelection(selected)
    if type(selected) ~= "table" then return false end
    for _, v in pairs(selected) do if v == true then return true end end
    return false
end

function HMY_MAIL_V4_BlockedTool(tool)
    if not (tool and tool:IsA("Tool")) then return true end
    if tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil then return true end
    local n = tostring(tool.Name or ""):lower()
    if n:find("seed", 1, true) and not n:find("seed pack", 1, true) then return true end
    for _, attr in ipairs({
        "Sprinkler", "WateringCan", "Trowel", "Shovel", "Crate", "Egg", "Mushroom", "Gnome", "Raccoon",
        "Pet", "PetId", "Build", "Crowbar", "Rake", "PowerHose", "Teleporter", "Wheelbarrow", "Ladder",
        "FreezeRay", "Flashbang", "MagicDice", "WeatherStaff", "StrawberrySniper", "Bird"
    }) do
...
function HMY_MAIL_V4_ToolFruitName(tool)
    if not tool then return "" end
    for _, attr in ipairs({ "FruitName", "Fruit", "Name", "ItemName", "PlantName", "CropName" }) do
        local v = tool:GetAttribute(attr)
        if type(v) == "string" and v ~= "" and v ~= "true" and v ~= "false" then
            -- avoid returning pure uuid-like long strings as display name
            if not (v:find("%-") and #v > 20) then return v end
        end
    end
    return HMY_MAIL_V4_Text(tool.Name)
end

function HMY_MAIL_V4_IsFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if HMY_MAIL_V4_BlockedTool(tool) then return false end
    if tool:GetAttribute("Fruit") ~= nil or tool:GetAttribute("FruitName") ~= nil then return true end
    if tool:GetAttribute("HarvestedFruit") ~= nil or tool:GetAttribute("HarvestedFruitId") ~= nil then return true end
    local c = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or ""):lower()
    if c == "fruit" or c == "fruits" or c == "harvestedfruit" or c == "harvestedfruits" then return true end
    local n = tostring(tool.Name or ""):lower()
    if n:find("kg", 1, true) then return true end
    if tool:GetAttribute("Id") ~= nil and (tool:GetAttribute("SizeMultiplier") ~= nil or tool:GetAttribute("Mutation") ~= nil) then return true end
    return false
end

function HMY_MAIL_V4_ToolMatchesSelected(tool, selected)
    if not HMY_MAIL_V4_IsFruitTool(tool) then return false end
    if not HMY_MAIL_V4_AnySelection(selected) then return false end
    if HMY_MAIL_V4_SelectedAll(selected) then return true end
    local tn = HMY_MAIL_V4_Loose(HMY_MAIL_V4_ToolFruitName(tool))
    local rawName = HMY_MAIL_V4_Loose(tool.Name)
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local want = HMY_MAIL_V4_Loose(sel)
            if want ~= "" then
                if tn == want or rawName == want then return true end
                if tn ~= "" and (tn:find(want, 1, true) or want:find(tn, 1, true)) then
                    if not (want == "dragon fruit" and tn:find("dragon breath", 1, true)) then return true end
                end
                if rawName ~= "" and rawName:find(want, 1, true) then
                    if not (want == "dragon fruit" and rawName:find("dragon breath", 1, true)) then return true end
                end
            end
        end
    end
...
function HMY_MAIL_V4_CurrentHeldTool()
    local char = LocalPlayer and LocalPlayer.Character
    return char and char:FindFirstChildWhichIsA("Tool") or nil
end

function HMY_MAIL_V4_ToolIds(tool)
    local ids, seen = {}, {}
    local function add(v)
        if v ~= nil then
            local s = tostring(v)
    ...
    if not tool then return ids end
    for _, attr in ipairs({ "InventoryKey", "ItemKey", "Id", "ID", "HarvestedFruit", "HarvestedFruitId", "FruitId", "UUID", "Uuid", "Guid", "UID", "ItemId" }) do
        add(tool:GetAttribute(attr))
    end
    return ids
end

function HMY_MAIL_V4_InventoryKeyFromTool(tool)
    if not HMY_MAIL_V4_IsFruitTool(tool) then return nil, nil end
    local ids = HMY_MAIL_V4_ToolIds(tool)
    local idSet = {}
    for _, id in ipairs(ids) do idSet[tostring(id)] = true end

    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" then
            if idSet[tostring(key)] then return tostring(key), value end
            for _, field in ipairs({ "Id", "ID", "UUID", "Uuid", "Guid", "UID", "ItemId", "HarvestedFruitId", "FruitId" }) do
                local v = value[field]
                if v ~= nil and idSet[tostring(v)] then return tostring(key), value end
            end
        end
    end

    -- name fallback only when there is a single matching inventory fruit name
    local toolName = HMY_MAIL_V4_Loose(HMY_MAIL_V4_ToolFruitName(tool))
    local foundKey, foundValue = nil, nil
    if toolName ~= "" then
        for key, value in pairs(inv("HarvestedFruits")) do
            if type(value) == "table" then
                local nm = HMY_MAIL_V4_Loose(value.FruitName or value.Name or value.ItemName or key)
                if nm ~= "" and (nm == toolName or toolName:find(nm, 1, true)) then
                    if foundKey then foundKey = nil; foundValue = nil; break end
                    foundKey, foundValue = tostring(key), value
                end
            end
        end
        if foundKey then return foundKey, foundValue end
    end

    -- last fallback: some games use Id itself as table key
    if ids[1] then return tostring(ids[1]), nil end
    return nil, nil
end

function HMY_MAIL_V4_ItemFromTool(tool)
    local key, value = HMY_MAIL_V4_InventoryKeyFromTool(tool)
    if not key or key == "" then return nil, "held fruit has no ItemKey/Id" end
    return {
        Category = "HarvestedFruits",
        ItemKey = tostring(key),
        Count = 1,
        __Tool = tool,
        __EntryValue = value,
        __ManualHeld = (tool.Parent == (LocalPlayer and LocalPlayer.Character))
    }, nil
end

function HMY_MAIL_V4_FindBackpackTool(selected)
    local bp = LocalPlayer and (LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
    if not bp then return nil end
    for _, tool in ipairs(bp:GetChildren()) do
        if HMY_MAIL_V4_ToolMatchesSelected(tool, selected) then return tool end
    end
    return nil
end

function HMY_MAIL_V4_GetFruitProxyUtil()
    local ok, result = pcall(function()
        local sm = ReplicatedStorage and ReplicatedStorage:FindFirstChild("SharedModules")
        local mod = sm and sm:FindFirstChild("FruitProxyUtil")
        if mod then return require(mod) end
    end)
    if ok then return result end
...
function HMY_MAIL_V4_IsFruitProxy(obj)
    local fpu = HMY_MAIL_V4_GetFruitProxyUtil()
    if fpu and type(fpu.IsFruitProxy) == "function" then
        local ok, res = pcall(fpu.IsFruitProxy, obj)
        if ok and res then return true end
    end
    return false
end

function HMY_MAIL_V4_ProxyMatchesSelected(obj, selected)
    if not obj then return false end
    if not HMY_MAIL_V4_AnySelection(selected) then return false end
    if HMY_MAIL_V4_SelectedAll(selected) then return true end
    local names = {}
    for _, attr in ipairs({ "FruitName", "Fruit", "Name", "ItemName", "PlantName" }) do
        local v = obj:GetAttribute(attr)
        if type(v) == "string" then names[#names + 1] = v end
    end
    names[#names + 1] = tostring(obj.Name or "")
    for _, nm in ipairs(names) do
        local loose = HMY_MAIL_V4_Loose(nm)
        for sel, enabled in pairs(selected) do
            if enabled == true then
                local want = HMY_MAIL_V4_Loose(sel)
                if want ~= "" and loose ~= "" and (loose == want or loose:find(want, 1, true)) then return true end
            end
        end
    end
    return false
...
function HMY_MAIL_V4_RequestPromoteById(id)
    id = tostring(id or "")
    ...
    pcall(function()
        if type(fpu.Pending) == "table" then
            fpu.Pending.Equip = id
            if type(fpu.Pending.Slots) == "table" then fpu.Pending.Slots[id] = fpu.Pending.Slots[id] or 1 end
        end
    end)
    local ok = pcall(function()
        if type(fpu.RequestPromote) == "function" then
            fpu.RequestPromote(id)
        end
    end)
    return ok
end

function HMY_MAIL_V4_FindAndPromoteProxy(selected)
    local bp = LocalPlayer and (LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
    if not bp then return nil end
    for _, obj in ipairs(bp:GetChildren()) do
        if HMY_MAIL_V4_IsFruitProxy(obj) and HMY_MAIL_V4_ProxyMatchesSelected(obj, selected) then
            local id = obj:GetAttribute("Id") or obj:GetAttribute("ID")
            if id then
                HMY_MAIL_V4_RequestPromoteById(id)
                return tostring(id)
            end
        end
    end
...
function HMY_MAIL_V4_WaitToolAfterPromote(selected, timeoutSec)
    local deadline = os.clock() + (tonumber(timeoutSec) or 6)
    while os.clock() < deadline do
        local held = HMY_MAIL_V4_CurrentHeldTool()
        if HMY_MAIL_V4_ToolMatchesSelected(held, selected) then return held end
        local tool = HMY_MAIL_V4_FindBackpackTool(selected)
        if tool then return tool end
        task.wait(0.08)
    end
    return nil
...
function HMY_MAIL_V4_EquipTool(tool)
    if not HMY_MAIL_V4_IsFruitTool(tool) then return false, "not a fruit tool" end
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(3)
    if not char then return false, "character not found" end
    if tool.Parent == char and char:FindFirstChildWhichIsA("Tool") == tool then
        return true, nil
    end
    local bp = LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack")
    local hum = char:FindFirstChildOfClass("Humanoid")
    for _ = 1, 28 do
        if not tool.Parent then return false, "fruit tool disappeared" end
        pcall(function() if hum and hum.UnequipTools then hum:UnequipTools() end end)
        task.wait(0.05)
        if tool.Parent == bp and hum and hum.EquipTool then
        ...
        task.wait(0.12)
        local held = char:FindFirstChildWhichIsA("Tool")
        if held == tool or HMY_MAIL_V4_IsFruitTool(held) then
            task.wait(0.35)
            return true, nil
        end
    end
...
function HMY_MAIL_V4_GetOrHoldSelectedFruit(selected)
    local held = HMY_MAIL_V4_CurrentHeldTool()
    if HMY_MAIL_V4_ToolMatchesSelected(held, selected) then return held, nil end
    if held and HMY_MAIL_V4_IsFruitTool(held) and not HMY_MAIL_V4_ToolMatchesSelected(held, selected) then
        return nil, "holding " .. tostring(HMY_MAIL_V4_ToolFruitName(held)) .. ", selected " .. HMY_MAIL_V4_SelectedList(selected)
    end
    local tool = HMY_MAIL_V4_FindBackpackTool(selected)
    if not tool then
        HMY_MAIL_V4_FindAndPromoteProxy(selected)
        tool = HMY_MAIL_V4_WaitToolAfterPromote(selected, 6.5)
    end
    if not tool then return nil, "fruit not found in hand/backpack: " .. HMY_MAIL_V4_SelectedList(selected) end
    local ok, err = HMY_MAIL_V4_EquipTool(tool)
    if not ok then return nil, err end
    local held2 = HMY_MAIL_V4_CurrentHeldTool()
    if HMY_MAIL_V4_ToolMatchesSelected(held2, selected) then return held2, nil end
    if HMY_MAIL_V4_ToolMatchesSelected(tool, selected) then return tool, nil end
    return nil, "fruit was not held after equip"
end

function HaimiyachGAG2_EquipSelectedMailFruit(selected)
    return HMY_MAIL_V4_GetOrHoldSelectedFruit(selected)
end

function HaimiyachGAG2_AddMailHarvestedFruits(items, selected)
    if type(items) ~= "table" then return 0 end
    if not HMY_MAIL_V4_AnySelection(selected) then return 0 end
    if #items >= 20 then return 0 end
    local tool, err = HMY_MAIL_V4_GetOrHoldSelectedFruit(selected)
    if not tool then
        _G.HMY_MAIL_LAST_FRUIT_ERR = err
        return 0
    end
    local item, itemErr = HMY_MAIL_V4_ItemFromTool(tool)
    if not item then
        _G.HMY_MAIL_LAST_FRUIT_ERR = itemErr
        return 0
    end
    items[#items + 1] = item
    return 1
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    if type(item) ~= "table" or tostring(item.Category or "") ~= "HarvestedFruits" then return true, nil end
    local tool = item.__Tool
    if tool and tool.Parent and HMY_MAIL_V4_IsFruitTool(tool) then
        if tool.Parent == (LocalPlayer and LocalPlayer.Character) then return true, nil end
        return HMY_MAIL_V4_EquipTool(tool)
    end
    local held = HMY_MAIL_V4_CurrentHeldTool()
    if HMY_MAIL_V4_IsFruitTool(held) then
        item.__Tool = held
        return true, nil
    end
    return false, "fruit must be held first"
end

function HMY_MAIL_V4_AddNonFruitItems(items, maxEach)
    HaimiyachGAG2_AddMailFromInventory(items, "Seeds", S.mailSeedTargets, maxEach, false)
    HaimiyachGAG2_AddMailGearFromInventory(items, maxEach)
    HaimiyachGAG2_AddMailFromInventory(items, "Pets", S.mailPetTargets, 1, true)
    HaimiyachGAG2_AddMailFromInventory(items, "Crates", S.mailCrateTargets, maxEach, false)
...
function HaimiyachGAG2_BuildMailItems()
    ...
    if maxEach > 20 then maxEach = 20 end
    HaimiyachGAG2_AddMailHarvestedFruits(items, S.mailFruitTargets)
    HMY_MAIL_V4_AddNonFruitItems(items, maxEach)
    return items
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    if type(S.mailFruitTargets) == "table" and HMY_MAIL_V4_AnySelection(S.mailFruitTargets) and S.mailHarvestGarden == true then
        local harvestedForMail = HaimiyachGAG2_HarvestMailFruitsFromGarden and HaimiyachGAG2_HarvestMailFruitsFromGarden() or 0
        if harvestedForMail and harvestedForMail > 0 then task.wait(0.85) end
    end

    _G.HMY_MAIL_LAST_FRUIT_ERR = nil
    local items = HaimiyachGAG2_BuildMailItems()
    if #items == 0 then
        if showNotify ~= false then
            local ferr = _G.HMY_MAIL_LAST_FRUIT_ERR
            if ferr and tostring(ferr) ~= "" then
                notify("Mailbox", tostring(ferr), 4)
            else
                notify("Mailbox", "No selected gift item found.", 3)
            end
        end
    ...
    local function cleanBatch(batch)
        local out = {}
        for _, it in ipairs(batch) do
            if type(it) == "table" and it.Category and it.ItemKey and it.Count then
                out[#out + 1] = { Category = tostring(it.Category), ItemKey = tostring(it.ItemKey), Count = math.max(1, math.floor(tonumber(it.Count) or 1)) }
            end
        end
        return out
    end

    -- Send held fruit first, one-by-one. Do not unequip it before firing SendBatch.
    for _, it in ipairs(items) do
        if type(it) == "table" and tostring(it.Category or "") == "HarvestedFruits" then
            local okHold, holdErr = HaimiyachGAG2_EquipFruitForMailboxSend(it)
            if not okHold then
                if showNotify ~= false then notify("Mailbox", "Hold fruit failed: " .. tostring(holdErr or it.ItemKey), 4) end
                return false
            end
            task.wait(0.2)
            local ok, success, message = pcall(function()
                return send:Fire(userId, cleanBatch({ it }), note)
            end)
            if ok and success == true then
                sent = sent + 1
                task.wait(0.45)
            else
                lastErr = tostring(message or success or "Could not send fruit")
                if showNotify ~= false then notify("Mailbox", lastErr, 4) end
                return false
            end
        end
    end
    ...
    local other = {}
    for _, it in ipairs(items) do
        if type(it) == "table" and tostring(it.Category or "") ~= "HarvestedFruits" then other[#other + 1] = it end
    end
    if #other > 0 then
        local ok, success, message = pcall(function()
            return send:Fire(userId, cleanBatch(other), note)
        end)
        if ok and success == true then
            sent = sent + #other
        else
            lastErr = tostring(message or success or "Could not send mail")
            if showNotify ~= false then notify("Mailbox", lastErr, 4) end
            return false
        end
    end
    ...
    if sent > 0 then
        if showNotify ~= false then
            local mailText = "Gift sent to @" .. tostring(targetName) .. " (" .. tostring(sent) .. " item)"
            notify("Mailbox", mailText, 4)
            pcall(function()
                local nc = require(LocalPlayer.PlayerScripts.Controllers.NotificationController)
                if nc and nc.CreateNotification then nc:CreateNotification(mailText) end
    ...
    if showNotify ~= false then notify("Mailbox", tostring(lastErr or "Could not send mail"), 4) end
    return false
end

print("[HAIMIYACH HUB] Mailbox fruit manual-hold priority patch loaded.")

--[[ MAILBOX FRUIT FIX - HELD TOOL ID PRIORITY V5
     Based on uploaded controllers:
     - Mailbox uses HarvestedFruits ItemKey/Count through Mailbox.SendBatch.
     - Backpack promotes fruit proxies using FruitProxyUtil.Pending.Equip + RequestPromote(Id).
     - When a fruit is already held manually, use that held Tool first and keep it held while sending.
]]

function HMY_MAIL_V5_Norm(s)
    s = tostring(s or "")
    s = s:gsub("%c", " ")
    s = s:gsub("%b[]", " ")
    s = s:gsub("%b()", " ")
    s = s:gsub("%s+%d+%.?%d*%s*kg%s*$", "")
    s = s:gsub("%s+x%d+%s*$", "")
    s = s:gsub("^%s+", ""):gsub("%s+$", "")
    s = s:lower():gsub("%s+", " ")
    return s
...
function HMY_MAIL_V5_HasSelected(selected)
    if type(selected) ~= "table" then return false end
    for _, v in pairs(selected) do if v == true then return true end end
    return false
end

function HMY_MAIL_V5_SelectedAll(selected)
    if type(selected) ~= "table" then return false end
    for k, v in pairs(selected) do
        if v == true and HMY_MAIL_V5_Norm(k) == "all fruits" then return true end
    end
    return false
end

function HMY_MAIL_V5_SelectedText(selected)
    local t = {}
    if type(selected) == "table" then
        for k, v in pairs(selected) do if v == true then t[#t+1] = tostring(k) end end
    end
    table.sort(t)
    return table.concat(t, ", ")
end

function HMY_MAIL_V5_IsBlockedNonFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return true end
    if tool:GetAttribute("SeedTool") ~= nil or tool:GetAttribute("Seed") ~= nil then return true end
    local lname = tostring(tool.Name or ""):lower()
    if lname:find("seed", 1, true) and not lname:find("seed pack", 1, true) then return true end
    for _, a in ipairs({
        "Sprinkler","WateringCan","Trowel","Shovel","Crate","Egg","Mushroom","Gnome","Raccoon","Pet","PetId",
        "Build","Crowbar","Rake","PowerHose","Teleporter","Wheelbarrow","Ladder","FreezeRay","Flashbang",
        "MagicDice","WeatherStaff","StrawberrySniper","Bird","SeedPack"
    }) do
        if tool:GetAttribute(a) ~= nil then return true end
    end
    return false
end

function HMY_MAIL_V5_IsFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if HMY_MAIL_V5_IsBlockedNonFruitTool(tool) then return false end
    if tool:GetAttribute("HarvestedFruit") ~= nil then return true end
    if tool:GetAttribute("Fruit") ~= nil then return true end
    if tool:GetAttribute("FruitName") ~= nil then return true end
    local cat = tostring(tool:GetAttribute("MainCategory") or tool:GetAttribute("Category") or tool:GetAttribute("ItemCategory") or ""):lower()
    if cat == "fruit" or cat == "fruits" or cat == "harvestedfruit" or cat == "harvestedfruits" then return true end
    if tool:GetAttribute("Id") ~= nil and (tool:GetAttribute("Weight") ~= nil or tool:GetAttribute("SizeMultiplier") ~= nil or tool:GetAttribute("Mutation") ~= nil) then return true end
    return false
end

function HMY_MAIL_V5_ToolFruitName(tool)
    if not tool then return "" end
    for _, a in ipairs({"FruitName","Fruit","SeedName","PlantName","CropName","ItemName","Name"}) do
...
function HMY_MAIL_V5_ToolMatchesSelected(tool, selected)
    if not HMY_MAIL_V5_IsFruitTool(tool) then return false end
    if not HMY_MAIL_V5_HasSelected(selected) then return false end
    if HMY_MAIL_V5_SelectedAll(selected) then return true end
    local tn = HMY_MAIL_V5_Norm(HMY_MAIL_V5_ToolFruitName(tool))
    local rn = HMY_MAIL_V5_Norm(tool.Name)
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local want = HMY_MAIL_V5_Norm(sel)
            if want ~= "" and want ~= "all fruits" then
                if tn == want or rn == want then return true end
                if tn ~= "" and (tn:find(want, 1, true) or want:find(tn, 1, true)) then
                    if not (want == "dragon fruit" and tn:find("dragon breath", 1, true)) then return true end
                end
                if rn ~= "" and rn:find(want, 1, true) then
                    if not (want == "dragon fruit" and rn:find("dragon breath", 1, true)) then return true end
                end
            end
        end
    end
...
function HMY_MAIL_V5_CurrentHeldFruitTool()
    local char = LocalPlayer and LocalPlayer.Character
    local tool = char and char:FindFirstChildWhichIsA("Tool")
    if HMY_MAIL_V5_IsFruitTool(tool) then return tool end
    return nil
end

function HMY_MAIL_V5_AddUnique(list, seen, v)
    if v ~= nil then
        local s = tostring(v)
        if s ~= "" and s ~= "true" and s ~= "false" and not seen[s] then
            seen[s] = true
...
function HMY_MAIL_V5_ToolIdList(tool)
    local ids, seen = {}, {}
    if not tool then return ids end
    for _, a in ipairs({"Id","ID","InventoryKey","ItemKey","HarvestedFruit","HarvestedFruitId","FruitId","UUID","Uuid","Guid","UID","ItemId"}) do
        HMY_MAIL_V5_AddUnique(ids, seen, tool:GetAttribute(a))
    end
    return ids
end

function HMY_MAIL_V5_InventoryCandidatesForTool(tool)
    local out, seen = {}, {}
    local ids = HMY_MAIL_V5_ToolIdList(tool)
    local idSet = {}
    for _, id in ipairs(ids) do idSet[tostring(id)] = true end
    ...
    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" then
            local matched = false
            if idSet[tostring(key)] then matched = true end
            for _, field in ipairs({"Id","ID","UUID","Uuid","Guid","UID","ItemId","HarvestedFruitId","FruitId"}) do
                if value[field] ~= nil and idSet[tostring(value[field])] then matched = true end
            end
            if matched then
                HMY_MAIL_V5_AddUnique(out, seen, key)
                for _, field in ipairs({"Id","ID","UUID","Uuid","Guid","UID","ItemId","HarvestedFruitId","FruitId"}) do
                    HMY_MAIL_V5_AddUnique(out, seen, value[field])
                end
            end
        end
    end

    for _, id in ipairs(ids) do HMY_MAIL_V5_AddUnique(out, seen, id) end
    return out
end

function HMY_MAIL_V5_InventoryFruitMatchesSelection(key, value, selected)
    if not HMY_MAIL_V5_HasSelected(selected) then return false end
    if HMY_MAIL_V5_SelectedAll(selected) then return true end
    if type(value) ~= "table" then return false end
    local nm = HMY_MAIL_V5_Norm(value.FruitName or value.Name or value.ItemName or value.SeedName or value.PlantName or key)
    for sel, enabled in pairs(selected) do
        if enabled == true then
            local want = HMY_MAIL_V5_Norm(sel)
            if want ~= "" and want ~= "all fruits" then
                if nm == want or nm:find(want, 1, true) or want:find(nm, 1, true) then
                    if not (want == "dragon fruit" and nm:find("dragon breath", 1, true)) then return true end
                end
            end
        end
    end
...
function HMY_MAIL_V5_FindSelectedInventoryFruit(selected)
    for key, value in pairs(inv("HarvestedFruits")) do
        if type(value) == "table" and value.Id ~= nil and HMY_MAIL_V5_InventoryFruitMatchesSelection(key, value, selected) then
            return tostring(key), value
        end
    end
    return nil, nil
...
function HMY_MAIL_V5_GetFruitProxyUtil()
    local ok, result = pcall(function()
        local sm = ReplicatedStorage and ReplicatedStorage:FindFirstChild("SharedModules")
        local mod = sm and sm:FindFirstChild("FruitProxyUtil")
        if mod then return require(mod) end
...
function HMY_MAIL_V5_RequestPromote(id)
    id = tostring(id or "")
    if id == "" then return false end
    local fpu = HMY_MAIL_V5_GetFruitProxyUtil()
    if not fpu then return false end
    pcall(function()
        if type(fpu.Pending) == "table" then
            fpu.Pending.Equip = id
    ...
    local ok = pcall(function()
        if type(fpu.RequestPromote) == "function" then fpu.RequestPromote(id) end
    end)
    return ok
end

function HMY_MAIL_V5_FindFruitToolByIds(ids)
    local idSet = {}
    if type(ids) == "table" then for _, id in ipairs(ids) do idSet[tostring(id)] = true end end
    local char = LocalPlayer and LocalPlayer.Character
    local bp = LocalPlayer and (LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack"))
    for _, parent in ipairs({char, bp}) do
        if parent then
            for _, tool in ipairs(parent:GetChildren()) do
                if HMY_MAIL_V5_IsFruitTool(tool) then
                    for _, id in ipairs(HMY_MAIL_V5_ToolIdList(tool)) do
                        if idSet[tostring(id)] then return tool end
                    end
                end
            end
        end
...
function HMY_MAIL_V5_EquipTool(tool)
    if not HMY_MAIL_V5_IsFruitTool(tool) then return false, "not a fruit tool" end
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(3)
    if not char then return false, "character not found" end
    if tool.Parent == char then return true, nil end
    local bp = LocalPlayer:FindFirstChild("Backpack") or LocalPlayer:FindFirstChildOfClass("Backpack")
    local hum = char:FindFirstChildOfClass("Humanoid")
    for i = 1, 20 do
        if not tool.Parent then return false, "fruit disappeared" end
        for _, t in ipairs(char:GetChildren()) do
            if t:IsA("Tool") and t ~= tool then pcall(function() t.Parent = bp end) end
        end
        pcall(function() if hum and hum.UnequipTools then hum:UnequipTools() end end)
        task.wait(0.04)
        if hum and hum.EquipTool and tool.Parent == bp then
            pcall(function() hum:EquipTool(tool) end)
        end
    ...
    local held = char:FindFirstChildWhichIsA("Tool")
    return false, "still holding: " .. tostring(held and held.Name or "nothing")
end

function HMY_MAIL_V5_HoldSelectedFruit(selected)
    -- If the player manually holds a fruit, trust the held fruit first.
    local held = HMY_MAIL_V5_CurrentHeldFruitTool()
    if held then return held, nil end

    local key, value = HMY_MAIL_V5_FindSelectedInventoryFruit(selected)
    if not key then return nil, "selected fruit not found in HarvestedFruits: " .. HMY_MAIL_V5_SelectedText(selected) end
    local ids = {}
    HMY_MAIL_V5_AddUnique(ids, {}, key)
    if type(value) == "table" then
        for _, field in ipairs({"Id","ID","UUID","Uuid","Guid","UID","ItemId","HarvestedFruitId","FruitId"}) do
            if value[field] ~= nil then ids[#ids+1] = tostring(value[field]) end
        end
    end

    local tool = HMY_MAIL_V5_FindFruitToolByIds(ids)
    if not tool then
        local promoteId = (type(value) == "table" and (value.Id or value.ID or value.UUID or value.Uuid or value.Guid or value.ItemId)) or key
        HMY_MAIL_V5_RequestPromote(promoteId)
        local deadline = os.clock() + 7
        while os.clock() < deadline do
            local h = HMY_MAIL_V5_CurrentHeldFruitTool()
            if h then return h, nil end
            tool = HMY_MAIL_V5_FindFruitToolByIds(ids)
            if tool then break end
            task.wait(0.08)
        end
    end
    if not tool then return nil, "fruit tool/proxy not ready: " .. HMY_MAIL_V5_SelectedText(selected) end
    local ok, err = HMY_MAIL_V5_EquipTool(tool)
    if not ok then return nil, err end
    return tool, nil
end

function HMY_MAIL_V5_BuildFruitItemFromTool(tool)
    if not HMY_MAIL_V5_IsFruitTool(tool) then return nil, "held item is not fruit" end
    local keys = HMY_MAIL_V5_InventoryCandidatesForTool(tool)
    if #keys == 0 then return nil, "held fruit has no Id/ItemKey" end
    return { Category = "HarvestedFruits", ItemKey = tostring(keys[1]), Count = 1, __Keys = keys, __Tool = tool }, nil
end

function HaimiyachGAG2_AddMailHarvestedFruits(items, selected)
    if type(items) ~= "table" then return 0 end
    if not HMY_MAIL_V5_HasSelected(selected) then return 0 end
    if #items >= 20 then return 0 end
    local tool, err = HMY_MAIL_V5_HoldSelectedFruit(selected)
    if not tool then _G.HMY_MAIL_LAST_FRUIT_ERR = err; return 0 end
    local item, itemErr = HMY_MAIL_V5_BuildFruitItemFromTool(tool)
    if not item then _G.HMY_MAIL_LAST_FRUIT_ERR = itemErr; return 0 end
    items[#items + 1] = item
    return 1
end
...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    ...
    if HMY_MAIL_V5_IsFruitTool(tool) then
        if tool.Parent == (LocalPlayer and LocalPlayer.Character) then return true, nil end
        return HMY_MAIL_V5_EquipTool(tool)
    end
    local held = HMY_MAIL_V5_CurrentHeldFruitTool()
    if held then item.__Tool = held; item.__Keys = HMY_MAIL_V5_InventoryCandidatesForTool(held); return true, nil end
    return false, "fruit must be held first"
end

function HMY_MAIL_V5_CleanItem(it, overrideKey)
    return { Category = tostring(it.Category), ItemKey = tostring(overrideKey or it.ItemKey), Count = math.max(1, math.floor(tonumber(it.Count) or 1)) }
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    if type(S.mailFruitTargets) == "table" and HMY_MAIL_V5_HasSelected(S.mailFruitTargets) and S.mailHarvestGarden == true then
        local harvestedForMail = HaimiyachGAG2_HarvestMailFruitsFromGarden and HaimiyachGAG2_HarvestMailFruitsFromGarden() or 0
        if harvestedForMail and harvestedForMail > 0 then task.wait(0.85) end
    end
    ...
    local items = HaimiyachGAG2_BuildMailItems()
    if #items == 0 then
        if showNotify ~= false then notify("Mailbox", tostring(_G.HMY_MAIL_LAST_FRUIT_ERR or "No selected gift item found."), 4) end
        return false
    end
    ...
    -- Fruits first: keep the fruit held and try every valid key candidate until the server accepts.
    for _, it in ipairs(items) do
        if type(it) == "table" and tostring(it.Category or "") == "HarvestedFruits" then
            local okHold, holdErr = HaimiyachGAG2_EquipFruitForMailboxSend(it)
            if not okHold then
                if showNotify ~= false then notify("Mailbox", "Hold fruit failed: " .. tostring(holdErr), 4) end
                return false
            end
            task.wait(0.25)
            local keys = it.__Keys or { it.ItemKey }
            local successAny = false
            local lastMessage = nil
            for _, key in ipairs(keys) do
                local ok, success, message = pcall(function()
                    return send:Fire(userId, { HMY_MAIL_V5_CleanItem(it, key) }, note)
                end)
                if ok and success == true then
                    successAny = true
                    sent = sent + 1
                    ...
                    lastMessage = tostring(message or success or "Could not send fruit")
                    task.wait(0.15)
                end
            end
            if not successAny then
                if showNotify ~= false then notify("Mailbox", tostring(lastMessage or "Could not send fruit"), 4) end
                return false
            end
        end
    end
    ...
    for _, it in ipairs(items) do
        if type(it) == "table" and tostring(it.Category or "") ~= "HarvestedFruits" then
            other[#other + 1] = HMY_MAIL_V5_CleanItem(it)
        end
    end
    if #other > 0 then
        local ok, success, message = pcall(function() return send:Fire(userId, other, note) end)
        if ok and success == true then
            sent = sent + #other
        else
            if showNotify ~= false then notify("Mailbox", tostring(message or success or "Could not send mail"), 4) end
            return false
        end
    end

    if sent > 0 then
        if showNotify ~= false then notify("Mailbox", "Gift sent to @" .. tostring(targetName) .. " (" .. tostring(sent) .. " item)", 4) end
        return true
    end
    if showNotify ~= false then notify("Mailbox", "Could not send mail", 4) end
    return false
end

print("[HAIMIYACH HUB] Mailbox fruit held-ID priority patch V5 loaded.")
