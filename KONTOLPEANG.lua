...
local function crateCatalog()
    ...
    pcall(function()
        ...
        if stockValues then
            ...
            for _, shop in ipairs(stockValues:GetChildren()) do
                if string.find(string.lower(shop.Name), "crate", 1, true) then
            ...
            for _, folderName in ipairs({ "Crates", "GuildCrates" }) do
                ...
                if folder then
                    for _, item in ipairs(folder:GetChildren()) do
                        if item:IsA("Model") or item:IsA("Folder") then add(item.Name) end
                    end
                end
            end
        end
    ...
    if #out == 0 then
        for _, name in ipairs({ "Arch Crate", "Wooden Crate", "Stone Crate", "Golden Crate", "Guild Crate" }) do add(name) end
    end
...
local function stockOfAnyCrate(name)
    for _, shopName in ipairs({ "CrateShop", "CratesShop", "GuildCrateShop", "GuildCrates", "Crates" }) do
...
local S = {
    ...
    crateBuy = {}, autoBuyCrates = false, autoBuyAllCrates = false, crateBuyInterval = 10,
    -- steal
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "crateBuy", "autoBuyCrates", "autoBuyAllCrates", "crateBuyInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
...
local function fireCratePurchase(name)
    ...
    return false, "no crate purchase remote"
end

loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return math.max(2, tonumber(S.crateBuyInterval) or 10) end, function()
    local targets = {}
    if S.autoBuyAllCrates then
        for _, name in ipairs(CRATE_NAMES) do targets[#targets + 1] = name end
    else
        for name in pairs(S.crateBuy) do targets[#targets + 1] = name end
        table.sort(targets)
    end
    ...
    for _, name in ipairs(targets) do
        ...
        local stock = stockOfAnyCrate(name)
        if stock == nil or stock > 0 then
            fireCratePurchase(name)
            task.wait(jitter(0.2, 0.45))
        end
    end
end)
...
local function hidePlantObject(obj)
    if not isPlantVisualObject(obj) then return end
    if HiddenPlantsBackup[obj] == nil then
        HiddenPlantsBackup[obj] = {}
        if obj:IsA("BasePart") then
            pcall(function() HiddenPlantsBackup[obj].LocalTransparencyModifier = obj.LocalTransparencyModifier end)
            pcall(function() HiddenPlantsBackup[obj].CastShadow = obj.CastShadow end)
        elseif obj:IsA("Decal") or obj:IsA("Texture") then
            pcall(function() HiddenPlantsBackup[obj].Transparency = obj.Transparency end)
        else
            pcall(function() HiddenPlantsBackup[obj].Enabled = obj.Enabled end)
        end
    end
...
local function scanAndHidePlants()
    local gardens = Workspace:FindFirstChild("Gardens")
...
local function RestoreHiddenPlants()
    for obj, backup in pairs(HiddenPlantsBackup) do
        if obj and obj.Parent then
            if obj:IsA("BasePart") then
                if backup.LocalTransparencyModifier ~= nil then
                    SafeSetProperty(obj, "LocalTransparencyModifier", backup.LocalTransparencyModifier)
                end
                if backup.CastShadow ~= nil then
                    SafeSetProperty(obj, "CastShadow", backup.CastShadow)
                end
            elseif obj:IsA("Decal") or obj:IsA("Texture") then
                if backup.Transparency ~= nil then
                    SafeSetProperty(obj, "Transparency", backup.Transparency)
                end
            else
                if backup.Enabled ~= nil then
                    SafeSetProperty(obj, "Enabled", backup.Enabled)
                end
            end
        end
    end
...
local function SetHidePlantsEnabled(state)
    ...
    if S.hidePlants then
        ...
        local gardens = Workspace:FindFirstChild("Gardens")
        if gardens then
            HidePlantsConnection = gardens.DescendantAdded:Connect(function(obj)
                if S.hidePlants then
                    task.delay(0.05, function()
                        if S.hidePlants then hidePlantObject(obj) end
                    end)
                end
            end)
        end
...
addButton(ShopPage, "REFRESH CRATE LIST", function()
    ...
    notify("HAIMIYACH HUB", "Crate list refreshed.", 2)
end)
