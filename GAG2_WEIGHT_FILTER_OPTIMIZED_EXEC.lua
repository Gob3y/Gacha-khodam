...
-- PERF/FILTER FIX:
-- Prefer the real fruit node (PlantId + FruitId). Returning the parent plant
-- makes ABOVE/BELOW KG compare against plant weight and also causes heavy scans.
local function promptCarrier(prompt)
    ...
    local plantNode = nil
    local guard = 0
    while node and node ~= Workspace and guard < 14 do
        guard = guard + 1
        local hasPlantId = node:GetAttribute("PlantId") ~= nil
        local hasFruitId = node:GetAttribute("FruitId") ~= nil
        if hasPlantId and hasFruitId then
            fruitNode = node
            break
        end
        if hasPlantId and not plantNode then
            plantNode = node
        end
        node = node.Parent
    end
    if fruitNode then return fruitNode end
    if plantNode then return plantNode end
...
HaimiyachGAG2_KgCache = HaimiyachGAG2_KgCache or setmetatable({}, { __mode = "k" })
HaimiyachGAG2_KgCacheTtl = 1.75

function HaimiyachGAG2_GetCachedKg(obj)
    local rec = obj and HaimiyachGAG2_KgCache and HaimiyachGAG2_KgCache[obj]
    if rec and (os.clock() - (rec.t or 0)) <= HaimiyachGAG2_KgCacheTtl then
        return rec.kg -- number or false sentinel
    end
    return nil
end

function HaimiyachGAG2_SetCachedKg(obj, kg)
    if obj and HaimiyachGAG2_KgCache then
        HaimiyachGAG2_KgCache[obj] = { t = os.clock(), kg = kg or false }
    end
    return kg
end
...
function HaimiyachGAG2_IsFruitWeightObject(obj)
    if not obj then return false end
    if obj:GetAttribute("FruitId") ~= nil then return true end
    if obj:GetAttribute("Fruit") ~= nil or obj:GetAttribute("FruitName") ~= nil then return true end
    if obj.Parent and tostring(obj.Parent.Name) == "Fruits" then return true end
    return false
end

function HaimiyachGAG2_DescendantKg(obj, maxCheck)
    if not (obj and obj.GetDescendants) then return nil end
    local checked = 0
    for _, d in ipairs(obj:GetDescendants()) do
        checked = checked + 1
        local kg = HaimiyachGAG2_InstanceKg(d)
        if kg then return kg end
        if checked >= (maxCheck or 45) then break end
    end
    return nil
end
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    local cached = HaimiyachGAG2_GetCachedKg(obj)
    if cached ~= nil then
    ...
    if kg then return HaimiyachGAG2_SetCachedKg(obj, kg) end

    local isFruit = HaimiyachGAG2_IsFruitWeightObject(obj)

    -- Fruit filter must NOT read the parent plant KG. That caused ABOVE KG to
    -- harvest small fruits when the parent plant was heavy, and it made scans laggy.
    if isFruit then
        kg = HaimiyachGAG2_DescendantKg(obj, 45)
        if kg then return HaimiyachGAG2_SetCachedKg(obj, kg) end

        local fruitId = obj:GetAttribute("FruitId")
        local node = obj.Parent
        local guard = 0
        while node and node ~= Workspace and guard < 5 do
            guard = guard + 1
            if fruitId ~= nil and node:GetAttribute("FruitId") == fruitId then
                kg = HaimiyachGAG2_InstanceKg(node) or HaimiyachGAG2_DescendantKg(node, 30)
                if kg then return HaimiyachGAG2_SetCachedKg(obj, kg) end
            elseif node:GetAttribute("PlantId") ~= nil and node:GetAttribute("FruitId") == nil then
                break
            end
            node = node.Parent
        end

        return HaimiyachGAG2_SetCachedKg(obj, nil)
    end

    -- Plant / inventory fallback. Kept smaller than before for mobile FPS.
    local node = obj.Parent
    local guard = 0
    while node and node ~= Workspace and guard < 6 do
        guard = guard + 1
    ...
    kg = HaimiyachGAG2_DescendantKg(obj, 55)
    if kg then return HaimiyachGAG2_SetCachedKg(obj, kg) end
...
function HaimiyachGAG2_BestKgModel(obj)
    if not obj then return nil end
    local node = obj
    local fruitNode = nil
    local plantNode = nil
    ...
    return fruitNode or plantNode or obj
end

function HaimiyachGAG2_LightGardenKg(model)
    if not model then return nil end

    -- Throttle module init. Calling GardenESP.initModules for every harvest prompt
    -- is the main reason HARVEST WEIGHT FILTER feels laggy on mobile/PC.
    if type(GardenESP) == "table" and type(GardenESP.initModules) == "function" then
        if not HaimiyachGAG2_LastWeightModuleInit or (os.clock() - HaimiyachGAG2_LastWeightModuleInit) > 5 then
            HaimiyachGAG2_LastWeightModuleInit = os.clock()
            pcall(GardenESP.initModules)
        end
    end

    local name = nil
    if type(GardenESP) == "table" and type(GardenESP.name) == "function" then
        local ok, res = pcall(GardenESP.name, model)
        if ok then name = res end
    end
    name = name or HaimiyachGAG2_ObjectName(model)
    if not name or name == "" then return nil end

    local baseWeight = nil
    if type(GardenESP) == "table" then
        if HaimiyachGAG2_IsFruitWeightObject(model) and type(GardenESP.fruitBaseWeight) == "table" then
            baseWeight = tonumber(GardenESP.fruitBaseWeight[tostring(name)])
        end
        if not baseWeight and type(GardenESP.plantBaseWeight) == "table" then
            baseWeight = tonumber(GardenESP.plantBaseWeight[tostring(name)])
        end
        if not baseWeight and type(GardenESP.fruitBaseWeight) == "table" then
            baseWeight = tonumber(GardenESP.fruitBaseWeight[tostring(name)])
        end
    end
    if not baseWeight or baseWeight <= 0 then return nil end

    local sizeMulti = tonumber(model:GetAttribute("SizeMultiplier")
        or model:GetAttribute("SizeMulti")
        or model:GetAttribute("Scale")
        or model:GetAttribute("Size")
        or 1) or 1

    local overtime = tonumber(model:GetAttribute("OvertimeGrowth")
        or model:GetAttribute("GrowthMultiplier")
        or model:GetAttribute("Overtime")
        or 1) or 1

    if overtime < 1 then overtime = 1 end
    return baseWeight * sizeMulti * overtime
end
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    local model = HaimiyachGAG2_BestKgModel(obj) or obj

    -- First use the cheap extractor. It no longer reads parent plant KG for fruit.
    local kg = HaimiyachGAG2_RawObjectKg and HaimiyachGAG2_RawObjectKg(model) or nil
    if kg then return HaimiyachGAG2_SetCachedKg(obj, kg) end

    -- Lightweight formula fallback from controller data:
    -- BaseWeight * SizeMultiplier * OvertimeGrowth
    kg = HaimiyachGAG2_LightGardenKg(model)
    if kg and kg > 0 then return HaimiyachGAG2_SetCachedKg(obj, kg) end

    -- Only use the full controller calculate as last fallback and cache the result.
    -- This avoids huge repeated lag while still keeping compatibility.
    if model and model.IsDescendantOf and model:IsDescendantOf(Workspace) and type(GardenESP) == "table" and type(GardenESP.calculateWeight) == "function" then
        local ok, weight = pcall(function()
            return GardenESP.calculateWeight(model)
        end)
        weight = tonumber(weight)
        if ok and weight and weight > 0 then
            return HaimiyachGAG2_SetCachedKg(obj, weight)
        end
    end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    return HaimiyachGAG2_ObjectKg(tool)
end
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        local keep = false
        if hasMutationKeep and HaimiyachGAG2_SelectedMutationOk(tool, S.sellKeepMutations) then
            keep = true
        end
...
function HaimiyachGAG2_HarvestAllowed(obj)
    -- PERF FIX: check selected fruit / harvest-all BEFORE KG.
    -- Before this, the script calculated KG for every prompt even if the fruit
    -- was not selected, causing heavy lag when HARVEST WEIGHT FILTER was ON.
