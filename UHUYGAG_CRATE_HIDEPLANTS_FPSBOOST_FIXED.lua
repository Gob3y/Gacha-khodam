...
local function crateCatalog()
    ...
    local function scanFolder(folder)
        ...
        for _, obj in ipairs(folder:GetChildren()) do
            if obj:IsA("Model") or obj:IsA("Folder") or obj:IsA("ModuleScript") or obj:IsA("ValueBase") then
                addName(obj.Name)
            end
        end
    end
    pcall(function()
        ...
        if assets then
            scanFolder(assets:FindFirstChild("Crates"))
            scanFolder(assets:FindFirstChild("GuildCrates"))
        end
    end)
    pcall(function()
        local stock = ReplicatedStorage:FindFirstChild("StockValues")
        if stock then
            for _, shopName in ipairs({ "CrateShop", "CratesShop", "GuildCrateShop", "GuildCrates" }) do
    ...
    if #out == 0 then
        for _, n in ipairs({ "Arch Crate", "Basic Crate", "Uncommon Crate", "Rare Crate", "Epic Crate", "Legendary Crate", "Mythical Crate" }) do
            addName(n)
        end
    end
    table.sort(out)
...
local S = {
    ...
    autoBuyCrates = false, autoBuyAllCrates = false, buyCrates = {}, crateBuyInterval = 10,
    -- steal
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoBuyCrates", "autoBuyAllCrates", "buyCrates", "crateBuyInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
...
local function crateStockOf(name)
    for _, shopName in ipairs({ "CrateShop", "CratesShop", "GuildCrateShop", "GuildCrates", "GearShop" }) do
...
local function buyCrate(name)
    for _, path in ipairs({
        ...
        "CrateShop.BuyCrate",
        "CratesShop.PurchaseCrate",
        "CratesShop.BuyCrate",
        "Crates.PurchaseCrate",
        "Crates.BuyCrate",
        "Crate.PurchaseCrate",
        "Crate.BuyCrate",
        "Crate.Purchase",
        ...
        if ok then
            if type(res) == "table" and res.Success == false then
                -- remote exists but server refused this crate; try the next known signature
            else
                return true, res, path
            end
        end
    end
    return false, "No crate purchase remote accepted " .. tostring(name)
end

loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return S.crateBuyInterval end, function()
    if not (S.autoBuyAllCrates or picked(S.buyCrates)) then return end
    for _, name in ipairs(CRATE_NAMES) do
        if not (S.autoBuyCrates or S.autoBuyAllCrates) then break end
        if S.autoBuyAllCrates or S.buyCrates[name] then
            local stock = crateStockOf(name)
            if stock == nil or stock > 0 then
                local ok = buyCrate(name)
...
-- FPS BOOST disables expensive shadows, particles, lights, water and post effects.
-- It keeps characters, hitboxes, prompts and GUI usable; HIDE PLANTS is a separate toggle.
...
local HidePlantsScanToken = 0
local HiddenPlantsBackup = {}
...
local function CaptureWorkspaceEffectBackup(obj)
    ...
    elseif obj:IsA("Light") then
        if GraphicsBackup.WorkspaceLights[obj] == nil then
            pcall(function()
                GraphicsBackup.WorkspaceLights[obj] = obj.Enabled
            end)
        end
    end
end
...
local function CapturePartBackup(obj)
    if not obj or not obj:IsA("BasePart") or GraphicsBackup.PartProps[obj] ~= nil then return end
    pcall(function()
        GraphicsBackup.PartProps[obj] = {
...
local function OptimizeFPSPart(obj)
    if not obj or not obj:IsA("BasePart") then return end
    if IsCharacterObject(obj) or obj:FindFirstAncestorWhichIsA("Tool") then return end
    CapturePartBackup(obj)
...
local function IsPlantVisualObject(obj)
    if not obj then return false end
    local node = obj
    while node and node ~= Workspace do
        local n = string.lower(tostring(node.Name or ""))
        if string.sub(n, 1, 6) == "plant_" or node:GetAttribute("PlantId") ~= nil or node:GetAttribute("PlantName") ~= nil then
            return true
        end
        if n == "plants" then
            local parent = node.Parent
            local pn = parent and tostring(parent.Name or "") or ""
            if string.match(pn, "^Plot%d+") or node:IsDescendantOf(Workspace:FindFirstChild("Gardens") or Workspace) then
                return true
            end
        end
        node = node.Parent
...
local function CaptureHiddenPlantBackup(obj, props)
    if HiddenPlantsBackup[obj] ~= nil then return end
    local data = {}
    ...
    HiddenPlantsBackup[obj] = data
end

local function HidePlantVisual(obj)
    if not obj or not IsPlantVisualObject(obj) then return end
    if obj:IsA("BasePart") then
        CaptureHiddenPlantBackup(obj, { "Transparency", "LocalTransparencyModifier", "CanCollide", "CastShadow" })
        SafeSetProperty(obj, "Transparency", 1)
        SafeSetProperty(obj, "LocalTransparencyModifier", 1)
        SafeSetProperty(obj, "CanCollide", false)
        SafeSetProperty(obj, "CastShadow", false)
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        CaptureHiddenPlantBackup(obj, { "Transparency" })
        SafeSetProperty(obj, "Transparency", 1)
    elseif obj:IsA("BillboardGui") or obj:IsA("SurfaceGui") then
        CaptureHiddenPlantBackup(obj, { "Enabled" })
        SafeSetProperty(obj, "Enabled", false)
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") or obj:IsA("Light") then
        CaptureHiddenPlantBackup(obj, { "Enabled" })
        SafeSetProperty(obj, "Enabled", false)
    end
end
...
local function ApplyHidePlants()
    HidePlantsScanToken = HidePlantsScanToken + 1
    local scanToken = HidePlantsScanToken
    S.hidePlants = true
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(descendants) do
            if scanToken ~= HidePlantsScanToken or not S.hidePlants or S.killed then break end
            HidePlantVisual(obj)
    ...
    HidePlantsConnection = Workspace.DescendantAdded:Connect(function(obj)
        if S.hidePlants then
            task.delay(0.1, function()
                if S.hidePlants and not S.killed then HidePlantVisual(obj) end
            end)
        end
    end)
end
...
local function RestoreWorkspaceEffects()
    ...
    for obj, enabled in pairs(GraphicsBackup.WorkspaceLights) do
        if obj and obj.Parent then
            SafeSetProperty(obj, "Enabled", enabled)
        end
    end
...
local function RestoreDefaultGraphics()
    ...
    for obj, props in pairs(GraphicsBackup.PartProps) do
        if obj and obj.Parent then
            for prop, value in pairs(props) do
                SafeSetProperty(obj, prop, value)
            end
...
local function ApplyFPSBoostGraphics()
    ...
    -- Mobile ultra-light graphics: remove expensive lighting, shadows, water, effects and map part shadows.
    -- Characters, prompts, hitboxes and GUI are skipped so gameplay remains usable.
...
local crateDrop = addDropdown(ShopPage, "CRATES TO BUY", CRATE_NAMES, S.buyCrates, true, function(sel) pickMulti(sel, S.buyCrates) end)
addButton(ShopPage, "REFRESH CRATE LIST", function()
    CRATE_NAMES = crateCatalog()
    crateDrop.SetOptions(CRATE_NAMES)
end)
...
addSlider(ShopPage, "CRATE BUY DELAY", 2, 60, S.crateBuyInterval, 0, function(v) S.crateBuyInterval = v end)
-- STEAL
