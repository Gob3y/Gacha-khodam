...
local function GetPlotAncestor(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then return nil end
    local cur = obj
    while cur and cur ~= Workspace do
        if cur.Parent == gardens then
            return cur
        end
        cur = cur.Parent
    end
...
local function IsDecorVisual(obj)
    local cur = obj
    while cur and cur ~= Workspace do
        local n = string.lower(tostring(cur.Name or ""))
        if string.find(n, "decor") or string.find(n, "cosmetic") or string.find(n, "effect") or string.find(n, "vfx") or string.find(n, "leaf") or string.find(n, "leaves") then
            return true
        end
        cur = cur.Parent
    end
...
local function CaptureWorkspacePartBackup(part)
    if not part or GraphicsBackup.WorkspaceParts[part] ~= nil then return end
    pcall(function()
        GraphicsBackup.WorkspaceParts[part] = {
...
local function CaptureTextureBackup(obj)
    if not obj or GraphicsBackup.WorkspaceTextures[obj] ~= nil then return end
    pcall(function()
        GraphicsBackup.WorkspaceTextures[obj] = {
            Transparency = obj.Transparency,
        }
    end)
end

local function ApplyMobileFPSObject(obj, fpsMode)
    if not obj then return end
    DisableClientVisualEffect(obj)

    if fpsMode == "LIGHT" then return end

    if obj:IsA("BasePart") then
        if IsCharacterDescendant(obj) then return end
        CaptureWorkspacePartBackup(obj)
        ...
        if fpsMode == "ULTRA" then
            ...
            local plotAncestor = GetPlotAncestor(obj)
            if (plotAncestor and ownPlot and plotAncestor ~= ownPlot) or IsDecorVisual(obj) then
                SafeSetProperty(obj, "LocalTransparencyModifier", 1)
            end
        end
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        CaptureTextureBackup(obj)
        if fpsMode == "ULTRA" then
            SafeSetProperty(obj, "Transparency", 1)
        else
            SafeSetProperty(obj, "Transparency", math.max(tonumber(obj.Transparency) or 0, 0.35))
        end
    end
end
...
local function RestoreDefaultGraphics()
    ...
    for part, values in pairs(GraphicsBackup.WorkspaceParts) do
    ...
    for obj, values in pairs(GraphicsBackup.WorkspaceTextures) do
        if obj and obj.Parent then
            for prop, value in pairs(values) do
                SafeSetProperty(obj, prop, value)
            end
...
local function ApplyFPSBoostGraphics()
    ...
    -- Mobile optimized:
    -- LIGHT    = disable effects only, still normal visual.
    -- BALANCED = LIGHT + flat lighting, terrain/water off, part shadow off, decals slightly reduced.
    -- ULTRA    = BALANCED + smooth plastic, hidden textures, hide other gardens/decor visuals client-side.
    ...
    elseif fpsMode == "BALANCED" then
        ...
        SafeSetProperty(Lighting, "Brightness", 0.75)
        SafeSetProperty(Lighting, "FogStart", 0)
        SafeSetProperty(Lighting, "FogEnd", 250000)
        SafeSetProperty(Lighting, "EnvironmentDiffuseScale", 0)
        SafeSetProperty(Lighting, "EnvironmentSpecularScale", 0)
        SafeSetProperty(Lighting, "Ambient", Color3.fromRGB(128, 128, 128))
        SafeSetProperty(Lighting, "OutdoorAmbient", Color3.fromRGB(128, 128, 128))
        pcall(function() Lighting.Technology = Enum.Technology.Compatibility end)
        pcall(function() UserSettings():GetService("UserGameSettings").SavedQualityLevel = Enum.SavedQualitySetting.QualityLevel1 end)
    end
    ...
    -- Batch scan supaya HP tidak freeze di server publik.
    task.spawn(function()
        local descendants = {}
        pcall(function()
        ...
        for _, obj in ipairs(descendants) do
            ...
            ApplyMobileFPSObject(obj, fpsMode)
    ...
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost then
                    ApplyMobileFPSObject(obj, NormalizeFPSBoostMode(S.fpsBoostMode))
                end
            end)
        end
    end)
