...
function HaimiyachGAG2_FindHarvestedFruitRecord(tool)
    if not tool then return nil, nil end
    local fruitId = tool:GetAttribute("Id") or tool:GetAttribute("FruitId") or tool:GetAttribute("HarvestedFruitId")
    local fruits = inv("HarvestedFruits")
    if type(fruits) ~= "table" then return nil, nil end
    if fruitId ~= nil then
        local direct = fruits[fruitId] or fruits[tostring(fruitId)]
        if direct ~= nil then return direct, fruitId end
    end
    for key, rec in pairs(fruits) do
        if fruitId ~= nil then
            if tostring(key) == tostring(fruitId) then return rec, key end
            if type(rec) == "table" then
                if tostring(rec.Id or rec.FruitId or rec.HarvestedFruitId or "") == tostring(fruitId) then
                    return rec, key
                end
            end
        end
...
function HaimiyachGAG2_FruitRecordName(rec, key)
    if type(rec) == "table" then
        return rec.FruitName or rec.Seed or rec.SeedName or rec.PlantName or rec.Name or rec.ItemName or rec.Type or tostring(key or "")
    end
    return tostring(key or "")
end

function HaimiyachGAG2_FruitRecordKg(rec, key)
    if type(rec) ~= "table" then return nil end
    for _, field in ipairs({ "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }) do
        local kg = HaimiyachGAG2_ParseKgText(rec[field], true)
        if kg then return kg end
    end
    local name = HaimiyachGAG2_FruitRecordName(rec, key)
    local size = tonumber(rec.SizeMultiplier or rec.SizeMulti or rec.WeightMultiplier)
    local over = tonumber(rec.OvertimeGrowth) or 1
    if name and size and size > 0 and GardenESP and type(GardenESP.fruitBaseWeight) == "table" then
        local base = tonumber(GardenESP.fruitBaseWeight[name])
        if base and base > 0 then
            return base * size * over
        end
    end
    return nil
end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    if kg then return kg end
    local rec, key = HaimiyachGAG2_FindHarvestedFruitRecord(tool)
    return HaimiyachGAG2_FruitRecordKg(rec, key)
end
...
function HaimiyachGAG2_BuyCrateViaGui(crateName)
    ...
    if type(result) == "table" then
        local __unpack = table and table.unpack or unpack
        if type(__unpack) == "function" then return __unpack(result) end
