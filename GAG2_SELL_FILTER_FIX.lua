...
local HaimiyachGAG2_KgCache = setmetatable({}, { __mode = "k" })
...
function HaimiyachGAG2_DirectChildrenKg(obj)
    if not obj or not obj.GetChildren then return nil end
    local checked = 0
    for _, child in ipairs(obj:GetChildren()) do
        checked = checked + 1
        local kg = HaimiyachGAG2_InstanceKg(child)
        if kg then return kg end
        if checked >= 35 then break end
    end
    return nil
end

function HaimiyachGAG2_GardenCalcKg(obj)
    if not (obj and obj.IsDescendantOf and obj:IsDescendantOf(Workspace)) then return nil end
    local node = obj
    local guard = 0
    while node and node ~= Workspace and guard < 8 do
        guard = guard + 1
        local plantId = node:GetAttribute("PlantId")
        local fruitId = node:GetAttribute("FruitId")
        if plantId ~= nil or fruitId ~= nil then
            if GardenESP and type(GardenESP.calculateWeight) == "function" then
                local ok, weight = pcall(function()
                    return GardenESP.calculateWeight(node)
                end)
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    local now = os.clock()
    local cached = HaimiyachGAG2_KgCache[obj]
    if cached and cached.t and (now - cached.t) < 1.25 then
        return cached.kg
    end
    ...
    if not kg then
        ...
        while node and node ~= Workspace and guard < 6 do
            ...
            if kg then break end
            node = node.Parent
        end
    end
    if not kg then
        kg = HaimiyachGAG2_DirectChildrenKg(obj)
    end
    if not kg then
        kg = HaimiyachGAG2_GardenCalcKg(obj)
    end

    HaimiyachGAG2_KgCache[obj] = { t = now, kg = kg }
    return kg
end
...
local HaimiyachGAG2_MutationCache = setmetatable({}, { __mode = "k" })

function HaimiyachGAG2_ObjectMutation(obj)
    if not obj then return nil end
    local now = os.clock()
    local cached = HaimiyachGAG2_MutationCache[obj]
    if cached and cached.t and (now - cached.t) < 1.25 then
        return cached.v
    end
    ...
    local function scan(inst)
        ...
        if inst:IsA("StringValue") then
            ...
            if string.find(n, "mutation", 1, true) or string.find(n, "variant", 1, true) then
                return clean(inst.Value)
            end
        end
        return nil
    end
    ...
    if not v and obj.GetChildren then
        local checked = 0
        for _, child in ipairs(obj:GetChildren()) do
            checked = checked + 1
            v = scan(child)
    ...
    HaimiyachGAG2_MutationCache[obj] = { t = now, v = v }
    return v
end
...
-- Fast ObjectKg already includes GardenESP fallback. Do not wrap it again, to avoid lag when filters are ON.
...
function HaimiyachGAG2_FruitItemIsInstance(item)
    return type(item) == "userdata" and item.IsA ~= nil
end

function HaimiyachGAG2_FruitItemAttr(item, attr)
    if not item then return nil end
    if HaimiyachGAG2_FruitItemIsInstance(item) then
...
function HaimiyachGAG2_FruitItemId(item)
    local id = HaimiyachGAG2_FruitItemAttr(item, "Id") or HaimiyachGAG2_FruitItemAttr(item, "FruitId") or HaimiyachGAG2_FruitItemAttr(item, "__Id")
    if id ~= nil and tostring(id) ~= "" then return id end
    return nil
end

function HaimiyachGAG2_InventoryFruitRecords()
    local out = {}
    local data = inv("HarvestedFruits")
    if type(data) ~= "table" then return out end
    for key, rec in pairs(data) do
        if type(rec) == "table" then
            if rec.__Id == nil then rec.__Id = rec.Id or key end
            out[#out + 1] = rec
        else
            out[#out + 1] = { __Id = key, Id = key, FruitName = tostring(key) }
        end
    end
    return out
end

function HaimiyachGAG2_GetInventoryFruitItems()
    local out, seen = {}, {}
    for _, rec in ipairs(HaimiyachGAG2_InventoryFruitRecords()) do
        local id = HaimiyachGAG2_FruitItemId(rec)
        if id then seen[tostring(id)] = true end
        out[#out + 1] = rec
    end
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        local id = HaimiyachGAG2_FruitItemId(tool)
        if not id or not seen[tostring(id)] then
            if id then seen[tostring(id)] = true end
            out[#out + 1] = tool
        end
    end
...
function HaimiyachGAG2_FruitItemName(item)
    if HaimiyachGAG2_FruitItemIsInstance(item) then
        return HaimiyachGAG2_ObjectName(item)
    end
    return tostring((type(item) == "table" and (item.FruitName or item.Fruit or item.Seed or item.Name or item.ItemName or item.__Id)) or "")
end

function HaimiyachGAG2_FruitItemMutation(item)
    if HaimiyachGAG2_FruitItemIsInstance(item) then
        return HaimiyachGAG2_ObjectMutation(item)
    end
    return (type(item) == "table" and (item.Mutation or item.Mutations or item.FruitMutation or item.Variant)) or nil
end

function HaimiyachGAG2_FruitItemFavorite(item)
    if HaimiyachGAG2_FruitItemIsInstance(item) then
        return item:GetAttribute("IsFavorite") == true
    end
    if type(item) == "table" then
        return item.IsFavorite == true or item.Favorite == true
    end
    return false
end

function HaimiyachGAG2_GetFruitToolKg(item)
    if type(item) == "table" and not HaimiyachGAG2_FruitItemIsInstance(item) then
        local attrs = { "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }
        for _, a in ipairs(attrs) do
            local kg = HaimiyachGAG2_ParseKgText(item[a], true)
            if kg then return kg end
        end
        return nil
    end
...
function HaimiyachGAG2_FruitItemSelectedNameOk(item, selected)
    if type(selected) ~= "table" or not picked(selected) then return true end
    return HaimiyachGAG2_SelectedNameMatch(HaimiyachGAG2_FruitItemName(item), selected)
end

function HaimiyachGAG2_FruitItemSelectedMutationOk(item, selected)
    if type(selected) ~= "table" or not picked(selected) then return false end
    local mut = string.lower(tostring(HaimiyachGAG2_FruitItemMutation(item) or ""))
    if mut == "" then return false end
    for name, enabled in pairs(selected) do
        if enabled == true then
            local key = string.lower(tostring(name or ""))
            if key ~= "" and string.find(mut, key, 1, true) then return true end
        end
    end
    return false
end

function HaimiyachGAG2_ShouldFavoriteFruit(item)
    if not item then return false end
    local fruitId = HaimiyachGAG2_FruitItemId(item)
    if not fruitId then return false end
    local fruit = HaimiyachGAG2_FruitItemName(item)
    if fruit == "" then return false end

    if picked(S.favoriteFruitTargets) then
        if not HaimiyachGAG2_FruitItemSelectedNameOk(item, S.favoriteFruitTargets) then return false end
    elseif not HaimiyachGAG2_TextListMatch(fruit, S.favoriteFruitFilter) then
        return false
    end

    if picked(S.favoriteMutations) then
        if not HaimiyachGAG2_FruitItemSelectedMutationOk(item, S.favoriteMutations) then return false end
    else
        local mutFilter = tostring(S.favoriteMutationFilter or "")
        if mutFilter ~= "" and mutFilter ~= "all" then
            local mutation = tostring(HaimiyachGAG2_FruitItemMutation(item) or "")
            if not HaimiyachGAG2_TextListMatch(mutation, mutFilter) then return false end
        end
    end

    local kg = HaimiyachGAG2_GetFruitToolKg(item)
...
function HaimiyachGAG2_SetFruitFavorite(item, enabled)
    if not item then return false end
    local fruitId = HaimiyachGAG2_FruitItemId(item)
    if not fruitId then return false end
    if HaimiyachGAG2_FruitItemIsInstance(item) then
        item:SetAttribute("IsFavorite", enabled and true or nil)
    elseif type(item) == "table" then
        item.IsFavorite = enabled and true or nil
    end
...
function HaimiyachGAG2_AutoFavoriteStep()
    local changed = 0
    for _, item in ipairs(HaimiyachGAG2_GetInventoryFruitItems()) do
        local shouldFav = HaimiyachGAG2_ShouldFavoriteFruit(item)
        if shouldFav and not HaimiyachGAG2_FruitItemFavorite(item) then
            if HaimiyachGAG2_SetFruitFavorite(item, true) then changed = changed + 1 end
            task.wait(0.12)
        elseif (not shouldFav) and S.unfavoriteNotMatching and HaimiyachGAG2_FruitItemFavorite(item) then
            if HaimiyachGAG2_SetFruitFavorite(item, false) then changed = changed + 1 end
            task.wait(0.12)
        end
    end
    return changed
...
function HaimiyachGAG2_SellKeepItem(item)
    if not item then return true end
    if picked(S.sellKeepMutations) and HaimiyachGAG2_FruitItemSelectedMutationOk(item, S.sellKeepMutations) then
        return true
    end
    if HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue) then
        local kg = HaimiyachGAG2_GetFruitToolKg(item)
        if not kg then return true end
        if HaimiyachGAG2_KgValueOk(kg, S.sellKeepKgMode, S.sellKeepKgValue) then return true end
    end
    if HaimiyachGAG2_FruitItemFavorite(item) then return true end
    return false
end

function HaimiyachGAG2_SellFiltersActive()
    return picked(S.sellKeepMutations) or HaimiyachGAG2_KgModeActive(S.sellKeepKgMode, S.sellKeepKgValue)
end

function HaimiyachGAG2_ProtectSellMutations()
    if not HaimiyachGAG2_SellFiltersActive() then return 0 end
    local protected = 0
    for _, item in ipairs(HaimiyachGAG2_GetInventoryFruitItems()) do
        if HaimiyachGAG2_SellKeepItem(item) and not HaimiyachGAG2_FruitItemFavorite(item) then
            if HaimiyachGAG2_SetFruitFavorite(item, true) then protected = protected + 1 end
            task.wait(0.08)
        end
    end
    return protected
...
function HaimiyachGAG2_SellFilteredFruits()
    ...
    for _, item in ipairs(HaimiyachGAG2_GetInventoryFruitItems()) do
        local fruitId = HaimiyachGAG2_FruitItemId(item)
        if fruitId and not HaimiyachGAG2_SellKeepItem(item) then
            local ok, res = fireFast("NPCS.SellFruit", fruitId)
            if ok and type(res) == "table" and res.Success then
                sold = sold + (tonumber(res.SoldCount) or 1)
                earned = earned + (tonumber(res.SellPrice) or tonumber(res.Price) or 0)
                task.wait(0.04)
            end
        end
    end
    if sold > 0 or earned > 0 then
        ...
        Stats.earned = Stats.earned + earned
...
local function sellAllNow()
    if HaimiyachGAG2_SellFiltersActive and HaimiyachGAG2_SellFiltersActive() then
