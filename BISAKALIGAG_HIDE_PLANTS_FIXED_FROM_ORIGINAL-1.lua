...
-- HIDE PLANTS
-- Client-side visual hide only. It only runs after the toggle is clicked.
local HidePlantsActive = false
local function SetHidePlantsEnabled(state, silent)
    HidePlantsActive = state and true or false

    local plot = myPlot()
    local plants = plot and plot:FindFirstChild("Plants")
    if not plants then
        if HidePlantsActive and not silent then
            notify("HIDE PLANTS", "Plants folder not found in your garden.", 3)
        end
        return
    end

    local value = HidePlantsActive and 1 or 0
    local changed = 0
    ...
    for _, obj in ipairs(plants:GetDescendants()) do
        ...
        if changed > 0 and changed % 300 == 0 then
            task.wait()
        end
    end

    if not silent then
        notify("HIDE PLANTS", HidePlantsActive and "Plants hidden." or "Plants shown.", 2)
    end
end
...
addButton(SettingsPage, "UNLOAD HUB", function()
    ...
    task.spawn(function() SetHidePlantsEnabled(false, true) end)
