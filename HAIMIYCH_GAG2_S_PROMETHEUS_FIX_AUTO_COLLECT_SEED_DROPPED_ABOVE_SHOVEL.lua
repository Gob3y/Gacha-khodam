...
local S = {
    ...
    autoCollectSeed = false, collectSeedDelay = 1, collectSeedTeleport = true, collectSeedReturn = true,
...
function GardenESP.isFruit(model)
    ...
    -- FRUITS ONLY:
    -- Do not show plant/crop carrier models like Pineapple plant.
    -- Those can be harvestable crops, but they are not real fruit objects.
...
-- // ============================================================ \ --
-- //              AUTO COLLECT DROPPED SEEDS (NOT EVENT)         \ --
-- // ============================================================ \ --
...
local function HaimiyachGAG2_IsDroppedSeed(obj)
    ...
    local cat = tostring(obj:GetAttribute("ItemCategory") or "")
...
function HaimiyachGAG2_CollectDroppedSeedsStep()
    ...
    local collected = 0
    local oldCf = root and root.CFrame or nil

    for _, item in ipairs(folder:GetChildren()) do
        if not S.autoCollectSeed or S.killed then break end
        if item and item.Parent and HaimiyachGAG2_IsDroppedSeed(item) and HaimiyachGAG2_CanCollectDroppedItem(item) then
            ...
            if prompt and pos then
                ...
                if ok then
                    ...
                    Stats.lastCollectedSeed = HaimiyachGAG2_DropItemText(item)
    ...
    if oldCf and root and root.Parent and S.collectSeedTeleport and S.collectSeedReturn then
...
addSection(UI.Pages["FARM"], "DROPPED SEEDS")
addMiniNote(UI.Pages["FARM"], "Auto collect dropped seeds only. This is not Seed Event claim.")
addToggle(UI.Pages["FARM"], "AUTO COLLECT SEED", S.autoCollectSeed, function(v) S.autoCollectSeed = v; saveConfig(true) end)
addToggle(UI.Pages["FARM"], "TELEPORT TO SEED", S.collectSeedTeleport, function(v) S.collectSeedTeleport = v; saveConfig(true) end)
addToggle(UI.Pages["FARM"], "RETURN AFTER COLLECT", S.collectSeedReturn, function(v) S.collectSeedReturn = v; saveConfig(true) end)
addSlider(UI.Pages["FARM"], "COLLECT SEED DELAY (SEC)", 1, 30, S.collectSeedDelay, 0, function(v) S.collectSeedDelay = v; saveConfig(true) end)
addButton(UI.Pages["FARM"], "COLLECT SEEDS NOW", function()
    local n = HaimiyachGAG2_CollectDroppedSeedsStep()
...
-- AUTO COLLECT DROPPED SEED loop (normal dropped seeds, not Seed Event)
loopOn(function() return S.autoCollectSeed end, function() return math.max(1, tonumber(S.collectSeedDelay) or 1) end, function()
    HaimiyachGAG2_CollectDroppedSeedsStep()
end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        claimSeedEvent = claimSeedEvent, collectDroppedSeeds = HaimiyachGAG2_CollectDroppedSeedsStep, flyToPosition = flyToPosition, SetCutsceneDisabled = SetCutsceneDisabled,
