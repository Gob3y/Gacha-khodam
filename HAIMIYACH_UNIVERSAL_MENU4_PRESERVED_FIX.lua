...
local Menu4RushexButton = nil
local RushexFloatingButton = nil
local RushexEnabled = false
...
local Menu4FlyVel = Vector3.new(0, 0, 0)
...
local function MakeGuiDraggable(guiObject)
    ...
    guiObject.InputChanged:Connect(function(input)
        ...
        if isMouseMove or isTouch then
            dragInput = input
        end
    end)

    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and draggingObject and dragStart and startPosition then
...
local function UpdateRushexFloatingButton()
    local theme = GetTheme()
    local buttonVisible = RushexFloatingButton and RushexFloatingButton.Visible

    if Menu4RushexButton then
        Menu4RushexButton.Text = buttonVisible and "RUSHEX BUTTON: HIDE" or "RUSHEX BUTTON: SHOW"
    end

    if RushexFloatingButton then
        RushexFloatingButton.Text = RushexEnabled and "Rushex ON" or "Rushex OFF"
        RushexFloatingButton.BackgroundColor3 = RushexEnabled and theme.Accent or theme.Button
        RushexFloatingButton.TextColor3 = RushexEnabled and theme.Background or theme.Text
    end
end

local function CreateRushexFloatingButton()
    if RushexFloatingButton then
        return
    end

    RushexFloatingButton = New("TextButton", {
    ...
    Corner(RushexFloatingButton, 14)
    Stroke(RushexFloatingButton, GetTheme().Accent, 1, 0.35)

    MakeGuiDraggable(RushexFloatingButton)

    RushexFloatingButton.MouseButton1Click:Connect(function()
        RushexEnabled = not RushexEnabled
        UpdateRushexFloatingButton()
        Notify("Rushex", RushexEnabled and "ON" or "OFF", 2)

        -- Masukkan logic legal milik game kamu di sini.
        local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
local function ToggleRushexFloatingButton()
    if not RushexFloatingButton then
        CreateRushexFloatingButton()
        RushexFloatingButton.Visible = true
    else
        RushexFloatingButton.Visible = not RushexFloatingButton.Visible
    end
...
Menu4RushexButton = MakeButton("RUSHEX BUTTON: SHOW", Menu4Scroll, function()
    ToggleRushexFloatingButton()
end)
Menu4RushexButton.Size = UDim2.new(1, 0, 0, 34)
...
--// SETTINGS BUTTONS
function AddSizeButton(text, callback)
...
function AddThemeButton(themeName)
...
FlingActive = false
FlingConnection = nil
HaimiyachEnv.OldPos = nil
HaimiyachEnv.FPDH = workspace.FallenPartsDestroyHeight
...
if Character and Humanoid and RootPart then  
    if RootPart.Velocity.Magnitude < 50 then  
        HaimiyachEnv.OldPos = RootPart.CFrame  
    end  
    ...
    if HaimiyachEnv.OldPos then  
        repeat  
            RootPart.CFrame = HaimiyachEnv.OldPos * CFrame.new(0, .5, 0)  
            Character:SetPrimaryPartCFrame(HaimiyachEnv.OldPos * CFrame.new(0, .5, 0))  
            ...
            task.wait()  
        until (RootPart.Position - HaimiyachEnv.OldPos.p).Magnitude < 25  
        workspace.FallenPartsDestroyHeight = HaimiyachEnv.FPDH  
    end  
else  
