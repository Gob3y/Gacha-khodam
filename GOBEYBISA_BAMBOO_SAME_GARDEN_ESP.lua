...
function GardenESP.initModules()
    if GardenESP.modulesReady then return end
    GardenESP.modulesReady = true
    ...
    pcall(function()
        ...
        if visualizer then
            ...
            if ok and type(mod) == "table" then GardenESP.visualizer = mod end
        end
...
function GardenESP.calculateWeight(model, sizeMulti)
    ...
    if visualizer then
        if GardenESP.isSingleHarvestPlant(model) and visualizer.CalculatePlantWeight then
        ...
        elseif visualizer.CalculateFruitWeight then
            local ok, weight = pcall(function()
...
function GardenESP.valueFor(model, name, sizeMulti)
    ...
    if GardenESP.valueCalc then
        local ok, value = pcall(GardenESP.valueCalc, name, finalSize, model and model:GetAttribute("Mutation") or nil, LocalPlayer, model and model:GetAttribute("DecayAlpha") or nil)
        if ok and tonumber(value) then return tonumber(value) end
    end

    return GardenESP.value(name, finalSize)
end
