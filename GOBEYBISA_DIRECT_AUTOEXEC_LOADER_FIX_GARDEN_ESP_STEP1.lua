...
local CONFIG_KEYS = {
    ...
    "espGardenFruit", "espGardenValue", "autoExecute", "autoExecuteUrl", "uiScale", "uiKeybindName"
}
local CONFIG_KEYSET = {}
...
function GardenESP.name(model)
    ...
    local attrs = { "CorePartName", "FruitName", "Fruit", "PlantName", "CropName" }
...
function GardenESP.make(parent)
    ...
    limit.MinTextSize = 9
    limit.MaxTextSize = mobile and 14 or 15
    limit.Parent = label
...
function GardenESP.scan()
    ...
    if gardens then
        for _, inst in ipairs(gardens:GetDescendants()) do
            if inst:IsA("Model") and inst:GetAttribute("FruitId") ~= nil and not seen[inst] then
                seen[inst] = true
                out[#out + 1] = inst
            end
        end
...
function GardenESP.update()
    ...
    for _, model in ipairs(GardenESP.scan()) do
        ...
        if GardenESP.isFruit(model) then
            local hp = GardenESP.part(model)
            if hp and hp.Parent then
                ...
                local text = tostring(name) .. " | " .. GardenESP.sizeName(sm) .. " | x" .. string.format("%.2f", sm)
                if S.espGardenValue then
                    local value = GardenESP.value(name, sm)
                    if value then text = text .. " | " .. GardenESP.money(value) .. "¢" end
                end
    ...
    for model, gui in pairs(GardenESP.labels) do
        if not alive[model] or not model.Parent or not S.espGardenFruit then
            pcall(function()
                if gui then gui:Destroy() end
            end)
            GardenESP.labels[model] = nil
...
function GardenESP.setValue(state)
    ...
    if S.espGardenFruit then pcall(GardenESP.update) end
end
...
addToggle(UI.Pages["VISUALS"], "ESP GARDEN FRUIT", S.espGardenFruit, function(v) GardenESP.set(v); saveConfig(true) end)
