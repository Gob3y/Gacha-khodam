...
local S = {
    ...
    codeText = "", autoCodes = false, antiAfk = true, disableLoadingScreen = false, disableCutscene = false,
    -- perf / webhook
...
-- // ============================================================ \ --
-- //                   LOADING SCREEN DISABLER                   \ --
-- // ============================================================ \ --
local LoadingScreenBlocker = {
    lastRestore = 0,
    lastRemove = 0,
    lastInput = 0,
}

local function SendLoadingScreenInput()
    local now = os.clock()
    if now - (LoadingScreenBlocker.lastInput or 0) < 0.75 then return end
    LoadingScreenBlocker.lastInput = now
...
local function RestoreAfterLoadingScreenBlock()
    local now = os.clock()
    if now - (LoadingScreenBlocker.lastRestore or 0) < 0.25 then return end
    LoadingScreenBlocker.lastRestore = now
    ...
    pcall(function()
        local depth = Lighting:FindFirstChild("DepthOfField")
        if depth then depth.Enabled = true end
        local sharedBlur = ReplicatedStorage:FindFirstChild("ClientModules") and ReplicatedStorage.ClientModules:FindFirstChild("Blur")
        if sharedBlur then
            local blurModule = require(sharedBlur)
            if blurModule and type(blurModule.SetBlur) == "function" then
                blurModule.SetBlur(0, 0)
            end
        end
    end)
end

local function IsLoadingScreenObject(obj)
    if not obj then return false end
    local name = string.lower(tostring(obj.Name or ""))
    if name == "loadingscreenmenu" or name == "loadinggui" then return true end

    local ok, result = pcall(function()
        local hasVariant = obj:FindFirstChild("Variant1Frame", true) ~= nil
        if not hasVariant then return false end
        local hasLoadingText = obj:FindFirstChild("PressAnyTxt", true) ~= nil
            or obj:FindFirstChild("CounterTxt", true) ~= nil
            or obj:FindFirstChild("TipLabel", true) ~= nil
            or obj:FindFirstChild("SkipTxt", true) ~= nil
        return hasLoadingText
    end)
    return ok and result or false
end

local function GetLoadingScreenRoot(obj)
    if not obj then return nil end
    ...
    local cur = obj
    while cur and cur ~= game and cur ~= Workspace and cur ~= playerGui and cur ~= coreGui do
        if tostring(cur.Name) == "LoadingScreenMenu" then
            return cur
        end
        local ok, hasLoadingGui = pcall(function()
            return cur:FindFirstChild("LoadingGui", true) ~= nil and cur:FindFirstChild("Variant1Frame", true) ~= nil
        end)
        if ok and hasLoadingGui then
            return cur
        end
        cur = cur.Parent
    end
    ...
    if obj:IsA("ScreenGui") or obj:IsA("SurfaceGui") or obj:IsA("BillboardGui") then
        if IsLoadingScreenObject(obj) then return obj end
    end

    return nil
end

local function RemoveActiveLoadingScreens()
    local now = os.clock()
    if now - (LoadingScreenBlocker.lastRemove or 0) < 0.15 then return false end
    LoadingScreenBlocker.lastRemove = now
    ...
    local function collect(parent)
        ...
        pcall(function()
            if IsLoadingScreenObject(parent) then
                local root = GetLoadingScreenRoot(parent) or parent
                if root then roots[root] = true end
            end
            for _, obj in ipairs(parent:GetDescendants()) do
                if IsLoadingScreenObject(obj) then
                    local root = GetLoadingScreenRoot(obj) or obj
                    if root then roots[root] = true end
                end
            end
        end)
    ...
    if removed then
        SendLoadingScreenInput()
        RestoreAfterLoadingScreenBlock()
    end

    return removed
end

local function SetLoadingScreenDisabled(enabled, silent)
    S.disableLoadingScreen = enabled and true or false
    if S.disableLoadingScreen then
        SendLoadingScreenInput()
        RemoveActiveLoadingScreens()
        RestoreAfterLoadingScreenBlock()
        if not silent then notify("Loading Screen", "Loading screen disabled.", 3) end
    else
        if not silent then notify("Loading Screen", "Loading screen disabler off.", 3) end
    end
end

loopOn(function() return S.disableLoadingScreen end, 0.4, function()
    SendLoadingScreenInput()
    RemoveActiveLoadingScreens()
    RestoreAfterLoadingScreenBlock()
end)
...
if S.disableLoadingScreen then
    SetLoadingScreenDisabled(true, true)
end
...
createTab("MAILBOX", 13)
createTab("SETTINGS", 14)
...
addToggle(UI.Pages["MISC"], "AUTO RECONNECT", S.autoReconnect, function(v) S.autoReconnect = v; saveConfig(true) end)
addSlider(UI.Pages["MISC"], "RECONNECT DELAY (SEC)", 3, 60, S.reconnectDelay, 0, function(v) S.reconnectDelay = v; saveConfig(true) end)
...
addLabel(UI.Pages["PROTECT"], "Note: Protect features are client-side and only protect your avatar. They do not delete or damage other players.")
...
addSlider(UI.Pages["MAILBOX"], "MAIL SEND DELAY", 5, 300, S.mailSendDelay, 0, function(v)
    S.mailSendDelay = v
    saveConfig(true)
end)
...
addToggle(UI.Pages["VISUALS"], "DISABLE LOADING SCREEN", S.disableLoadingScreen, function(v) SetLoadingScreenDisabled(v); saveConfig(true) end)
...
addButton(UI.Pages["SETTINGS"], "UNLOAD HUB", function()
    S.killed = true
    UI.Unloaded = true
    pcall(function() SetLoadingScreenDisabled(false, true) end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        claimSeedEvent = claimSeedEvent, flyToPosition = flyToPosition, SetLoadingScreenDisabled = SetLoadingScreenDisabled, SetCutsceneDisabled = SetCutsceneDisabled,
