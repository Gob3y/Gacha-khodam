...
-- // ============================================================ \ --
-- //                 LOADING SCREEN SKIPPER SAFE                 \ --
-- // ============================================================ \ --
local function HideLoadingTextObjects()
    local pg = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
    ...
    pcall(function()
        for _, d in ipairs(pg:GetDescendants()) do
            if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                ...
                if string.find(txt, "spawning garden", 1, true) or string.find(txt, "fruits need to be ripe", 1, true) then
                    local obj = d
                    for _ = 1, 5 do
                        ...
                        obj = obj.Parent
                        if not obj or obj:IsA("ScreenGui") then break end
                    end
                end
            end
        end
...
local function SkipGardenLoadingNow()
    pcall(function()
    ...
    HideLoadingTextObjects()
end

local function SetLoadingScreenDisabled(enabled, silent)
    S.disableLoading = enabled and true or false
    if S.disableLoading then
        SkipGardenLoadingNow()
        if not silent then notify("Loading Screen", "Garden loading disabled.", 3) end
    else
        if not silent then notify("Loading Screen", "Garden loading enabled.", 3) end
    end
end
...
addToggle(UI.Pages["VISUALS"], "DISABLE LOADING SCREEN", S.disableLoading, function(v)
    SetLoadingScreenDisabled(v)
    saveConfig(true)
end)

loopOn(function() return S.disableLoading end, 0.35, function()
    SkipGardenLoadingNow()
end)
if S.disableLoading then
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        unload = function()
            ...
            UI.Unloaded = true
            pcall(function() SetCutsceneDisabled(false, true) end)
            pcall(function() S.disableLoading = false end)
            pcall(RestoreDefaultGraphics)
