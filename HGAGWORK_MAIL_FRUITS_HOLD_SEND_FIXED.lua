...
function HaimiyachGAG2_EquipFruitForMailboxSend(item)
    ...
    while tool.Parent ~= char and os.clock() - start < 1.5 do
        task.wait(0.08)
        pcall(function()
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum and hum.EquipTool then hum:EquipTool(tool) else tool.Parent = char end
        end)
    end

    if tool.Parent ~= char then return false, "failed to equip fruit" end
    task.wait(0.35)
    return true, nil
end
...
function HaimiyachGAG2_SendSelectedMailNow(showNotify)
    ...
    local function sendMailBatch(batch)
        ...
        local ok, success, message = pcall(function()
            return send:Fire(userId, batch, note)
        end)
