...
-- LIGHT    : effects/particles/post effects OFF, visual masih normal.
...
local function CaptureLightingObjectBackup(obj)
    if not obj then return end
...
local function CaptureGraphicsBackup()
    ...
    for _, obj in ipairs(Lighting:GetDescendants()) do
        CaptureLightingObjectBackup(obj)
    end
...
local function CaptureMeshTextureBackup(obj)
    ...
    if obj:IsA("SpecialMesh") then
        if GraphicsBackup.MeshTextures[obj] == nil then
            pcall(function()
                GraphicsBackup.MeshTextures[obj] = { TextureId = obj.TextureId }
            end)
        end
...
local function NameHasAny(obj, words)
    local n = string.lower(tostring(obj and obj.Name or ""))
...
local function IsUltraGardenDecorVisual(obj)
    if not obj then return false end
    if IsCharacterDescendant(obj) then return false end
...
local function ApplyBalancedObjectFPS(obj, fpsMode)
    if not obj or fpsMode == "LIGHT" then return end
    if IsCharacterDescendant(obj) then return end
    ...
    if obj:IsA("BasePart") then
        ...
        if fpsMode == "ULTRA" then
            ...
            if IsUltraGardenDecorVisual(obj) then
    ...
    elseif obj:IsA("SurfaceAppearance") then
        ...
        if fpsMode == "ULTRA" and IsUltraGardenDecorVisual(obj) then
...
local function RestoreFPSBoostObjectChanges()
    ...
    for obj, values in pairs(GraphicsBackup.SurfaceAppearances) do
        if obj and values and values.Parent then
            pcall(function()
                if values.Parent.Parent ~= nil and obj.Parent ~= values.Parent then
    ...
    for obj, values in pairs(GraphicsBackup.MeshTextures) do
        if obj and obj.Parent and values then
            if values.TextureId ~= nil then
                SafeSetProperty(obj, "TextureId", values.TextureId)
            end
            if values.TextureID ~= nil then
                SafeSetProperty(obj, "TextureID", values.TextureID)
            end
...
local function ApplyFPSBoostGraphics()
    ...
    -- LIGHT tetap aman, BALANCED lebih ringan, ULTRA potato mode.
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(descendants) do
            ...
            ApplyBalancedObjectFPS(obj, fpsMode)
            processed = processed + 1

            if processed % 80 == 0 then
                task.wait()
            end
        end
    end)
    ...
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost then
                    DisableClientVisualEffect(obj)
                    ApplyBalancedObjectFPS(obj, NormalizeFPSBoostMode(S.fpsBoostMode))
                end
            end)
        end
    end)
