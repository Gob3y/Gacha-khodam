...
function HaimiyachGAG2_GetFruitToolId(tool)
    ...
    for _, attr in ipairs({ "Id", "FruitId", "HarvestedFruitId", "UID", "Uuid", "UUID" }) do
...
function HaimiyachGAG2_GetInventoryFruitTools()
    local out = {}
    local function badCategory(tool)
        local c = tostring(tool:GetAttribute("ItemCategory") or tool:GetAttribute("Category") or tool:GetAttribute("ToolType") or tool:GetAttribute("Type") or "")
        c = string.lower(c)
        if c == "" then return false end
        if string.find(c, "seed", 1, true) or string.find(c, "gear", 1, true) or string.find(c, "pet", 1, true) then return true end
        if string.find(c, "egg", 1, true) or string.find(c, "crate", 1, true) or string.find(c, "sprinkler", 1, true) then return true end
        return false
    end
    local function isFruitTool(tool)
        if not (tool and tool:IsA("Tool")) then return false end
        if tool:GetAttribute("PetId") ~= nil then return false end
        if badCategory(tool) then return false end
        ...
        if HaimiyachGAG2_GetFruitToolId(tool) ~= nil then
            if HaimiyachGAG2_ParseKgText(tool.Name, false) then return true end
            if HaimiyachGAG2_ObjectKg(tool) then return true end
            if HaimiyachGAG2_ObjectMutation(tool) then return true end
        end
        return false
    end
...
function HaimiyachGAG2_FindInventoryFruitRecord(id)
    if id == nil then return nil end
    local want = tostring(id)
    local data = inv("HarvestedFruits")
    if type(data) ~= "table" then return nil end
    for k, v in pairs(data) do
        if tostring(k) == want then return v end
        if type(v) == "table" then
            for _, key in ipairs({ "Id", "FruitId", "HarvestedFruitId", "UID", "Uuid", "UUID" }) do
                local got = v[key]
                if got ~= nil and tostring(got) == want then return v end
            end
        end
    end
    return nil
...
function HaimiyachGAG2_RecordKg(record)
    if type(record) ~= "table" then return nil end
    for _, key in ipairs({ "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }) do
        local kg = HaimiyachGAG2_ParseKgText(record[key], true)
        if kg then return kg end
    end
    for _, key in ipairs({ "Name", "DisplayName", "ItemName" }) do
        local kg = HaimiyachGAG2_ParseKgText(record[key], false)
        if kg then return kg end
    end
    return nil
end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    if kg then return kg end
    local record = HaimiyachGAG2_FindInventoryFruitRecord(HaimiyachGAG2_GetFruitToolId(tool))
    return HaimiyachGAG2_RecordKg(record)
end
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        ...
        if hasKgKeep then
            local kg = HaimiyachGAG2_GetFruitToolKg(tool)
            if kg == nil or HaimiyachGAG2_KgValueOk(kg, S.sellKeepKgMode, S.sellKeepKgValue) then
                keep = true
            end
        end
...
local function sellAllNow()
    local protected = HaimiyachGAG2_ProtectSellMutations()
    if protected and protected > 0 then task.wait(0.25) end
