...
local S = {
    ...
    -- esp
    espGardenFruit = false,
    espGardenValue = false,
    -- config / auto execute
    autoExecute = false,
    uiScale = 0,
...
-- // ============================================================ \ --
-- //          VISUALS ESP GARDEN LITE - GLOBAL SAFE              \ --
-- // ============================================================ \ --
_G.HaimiyachGardenESP = _G.HaimiyachGardenESP or { labels = {}, looping = false }
HaimiyachGardenESP = _G.HaimiyachGardenESP
HaimiyachGardenESP.Values = HaimiyachGardenESP.Values or {
    Carrot=5, Strawberry=3, Tomato=9, Blueberry=5, Apple=12, Pinetree=100, Bamboo=800, Pumpkin=350, Cactus=40, Pineapple=30,
    ["Green Bean"]=10, Banana=35, Grape=45, Mushroom=13000, Coconut=60, Mango=90, ["Thorn Rose"]=140, ["Dragon Fruit"]=150,
    Acorn=200, Cherry=350, Sunflower=1750, ["Venus Fly Trap"]=3000, Lotus=6500, Pomegranate=900, Beanstalk=2000,
    ["Poison Apple"]=900, ["Moon Bloom"]=9000, ["Dragon's Breath"]=3400, ["Poison Ivy"]=1700, ["Glow Mushroom"]=700,
    ["Ghost Pepper"]=2500, ["Horned Melon"]=200, Corn=34, ["Baby Cactus"]=70, Tulip=60, Romanesco=1500,
    ["Venom Spitter"]=4000, Hypnobloom=2000,
}

function HaimiyachGardenESP_Money(n)
    n = math.floor(tonumber(n) or 0)
    if math.abs(n) >= 1000000000 then return string.format("%.1fB", n / 1000000000) end
...
function HaimiyachGardenESP_Name(model)
    if not model then return nil end
    for _, attr in ipairs({ "CorePartName", "FruitName", "Fruit", "PlantName", "CropName" }) do
...
function HaimiyachGardenESP_Size(sizeMulti)
...
function HaimiyachGardenESP_Value(name, sizeMulti)
    name = tostring(name or "")
    local base = HaimiyachGardenESP.Values and HaimiyachGardenESP.Values[name]
    if not base then return nil end
    sizeMulti = tonumber(sizeMulti) or 1
    local exp = 2.65
    if name == "Mushroom" then exp = 1.9 end
    if name == "Bamboo" then exp = 1.75 end
    local value = math.floor(base * (sizeMulti ^ exp))
    if value < 1 then value = base end
    return value
end

function HaimiyachGardenESP_HasTag(inst, tag)
    local ok, res = pcall(function() return CollectionService:HasTag(inst, tag) end)
    return ok and res == true
end

function HaimiyachGardenESP_Part(model)
    if not model then return nil end
...
function HaimiyachGardenESP_IsFruit(model)
    if not (model and model:IsA("Model")) then return false end
    if not HaimiyachGardenESP_Name(model) then return false end
    if model:GetAttribute("FruitId") == nil and model:GetAttribute("PlantId") == nil then return false end
    if HaimiyachGardenESP_HasTag(model, "Harvestable") then return true end
    local age = tonumber(model:GetAttribute("Age"))
    local maxAge = tonumber(model:GetAttribute("MaxAge"))
    if age and maxAge and maxAge > 0 and age >= maxAge then return true end
    return model:FindFirstChild("HarvestPart", true) ~= nil
end

function HaimiyachGardenESP_Clear()
    if not HaimiyachGardenESP then return end
    for model, gui in pairs(HaimiyachGardenESP.labels or {}) do
        pcall(function() if gui then gui:Destroy() end end)
        HaimiyachGardenESP.labels[model] = nil
    end
    local gardens = Workspace:FindFirstChild("Gardens")
    if gardens then
        pcall(function()
...
function HaimiyachGardenESP_Update()
    if not (S and S.espGardenFruit) then return end
    HaimiyachGardenESP.labels = HaimiyachGardenESP.labels or {}
    local seen, list = {}, {}
    pcall(function()
        for _, inst in ipairs(CollectionService:GetTagged("Harvestable")) do
            if inst and inst:IsA("Model") then list[#list + 1] = inst end
        end
    end)
    if #list == 0 then
        local gardens = Workspace:FindFirstChild("Gardens")
        if gardens then
            for _, inst in ipairs(gardens:GetDescendants()) do
                if inst:IsA("Model") and inst:GetAttribute("FruitId") ~= nil then list[#list + 1] = inst end
            end
        end
    end
    local shown = 0
    for _, model in ipairs(list) do
        ...
        if HaimiyachGardenESP_IsFruit(model) then
            local hp = HaimiyachGardenESP_Part(model)
            if hp and hp.Parent then
                shown = shown + 1
                seen[model] = true
                local gui = HaimiyachGardenESP.labels[model]
                if not (gui and gui.Parent) then
                    local mobile = false
                    pcall(function() mobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled end)
                    ...
                    gui.Size = UDim2.fromOffset(mobile and 220 or 240, mobile and 48 or 50)
                    gui.Parent = hp
                    ...
                    sizeLimit.MinTextSize = 9
                    sizeLimit.MaxTextSize = mobile and 13 or 15
                    sizeLimit.Parent = label
                    HaimiyachGardenESP.labels[model] = gui
                elseif gui.Parent ~= hp then
                    gui.Parent = hp
                end
                local name = HaimiyachGardenESP_Name(model) or "Fruit"
                local sm = tonumber(model:GetAttribute("SizeMulti") or model:GetAttribute("SizeMultiplier") or 1) or 1
                local text = tostring(name) .. " | " .. HaimiyachGardenESP_Size(sm) .. " | x" .. string.format("%.2f", sm)
                if S.espGardenValue then
                    local value = HaimiyachGardenESP_Value(name, sm)
                    if value then text = text .. " | " .. HaimiyachGardenESP_Money(value) .. " Sheckles" end
                end
    ...
    for model, gui in pairs(HaimiyachGardenESP.labels or {}) do
        if (not seen[model]) or (not model.Parent) or (not S.espGardenFruit) then
            pcall(function() if gui then gui:Destroy() end end)
            HaimiyachGardenESP.labels[model] = nil
        end
    end
...
function HaimiyachGardenESP_Start()
    if HaimiyachGardenESP.looping then return end
    HaimiyachGardenESP.looping = true
    task.spawn(function()
        while HaimiyachGardenESP.looping and S and not S.killed do
            if S.espGardenFruit then
                pcall(HaimiyachGardenESP_Update)
                task.wait(1.25)
            else
                task.wait(0.75)
            end
        end
        pcall(HaimiyachGardenESP_Clear)
    end)
end

function SetGardenFruitESP(state)
    S.espGardenFruit = state and true or false
    if S.espGardenFruit then
        HaimiyachGardenESP_Start()
        pcall(HaimiyachGardenESP_Update)
    else
        pcall(HaimiyachGardenESP_Clear)
    end
...
function SetGardenFruitValueESP(state)
    S.espGardenValue = state and true or false
    if S.espGardenFruit then pcall(HaimiyachGardenESP_Update) end
end
...
addToggle(UI.Pages["VISUALS"], "ESP GARDEN FRUIT", S.espGardenFruit, function(v) SetGardenFruitESP(v); saveConfig(true) end)
addToggle(UI.Pages["VISUALS"], "GARDEN VALUE SHECKLES", S.espGardenValue, function(v) SetGardenFruitValueESP(v); saveConfig(true) end)
...
if S.espGardenFruit then SetGardenFruitESP(true) end
