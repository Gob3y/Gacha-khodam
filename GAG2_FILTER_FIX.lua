...
local HaimiyachGAG2_FruitVisualizerController
local function HaimiyachGAG2_ReadKgValue(v)
    if type(v) == "number" then
        if v > 0 then return v end
        return nil
    end
    if type(v) == "string" then
        local clean = string.lower(string.gsub(v, ",", ""))
        local n = string.match(clean, "([%d%.]+)")
        n = tonumber(n)
        if n and n > 0 then
            -- UI labels may show grams (250g), but filters are in KG.
            if string.find(clean, "kg", 1, true) then
                return n
            elseif string.find(clean, "g", 1, true) then
                return n / 1000
            end
            return n
        end
...
local function HaimiyachGAG2_ReadKgAttrs(inst)
    if not inst then return nil end
    for _, attr in ipairs({ "Weight", "WeightKg", "KG", "Kg", "Kilogram", "Kilograms", "Mass" }) do
        local kg = HaimiyachGAG2_ReadKgValue(inst:GetAttribute(attr))
        if kg then return kg end
    end
    local kg = HaimiyachGAG2_ReadKgValue(string.match(tostring(inst.Name or ""), "([%d%.,]+)%s*[Kk][Gg]"))
    if kg then return kg end
    return nil
end

local function HaimiyachGAG2_GetFruitVisualizerController()
    if HaimiyachGAG2_FruitVisualizerController then return HaimiyachGAG2_FruitVisualizerController end
    ...
    if mod then
        ...
        if ok and type(controller) == "table" then
            HaimiyachGAG2_FruitVisualizerController = controller
            return controller
        end
    end
    return nil
...
local function HaimiyachGAG2_CalcKgWithController(inst)
    local controller = HaimiyachGAG2_GetFruitVisualizerController()
    if not (controller and inst) then return nil end
    if type(controller.CalculateFruitWeight) == "function" then
        local ok, kg = pcall(function() return controller:CalculateFruitWeight(inst) end)
        kg = HaimiyachGAG2_ReadKgValue(ok and kg or nil)
        if kg then return kg end
    end
    if type(controller.CalculatePlantWeight) == "function" then
        local ok, kg = pcall(function() return controller:CalculatePlantWeight(inst) end)
        kg = HaimiyachGAG2_ReadKgValue(ok and kg or nil)
        if kg then return kg end
    end
    return nil
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    while node and node ~= Workspace and guard < 8 do
        guard = guard + 1
        local kg = HaimiyachGAG2_CalcKgWithController(node)
        if kg then return kg end
        kg = HaimiyachGAG2_ReadKgAttrs(node)
        if kg then return kg end
        node = node.Parent
    end
    ...
    for _, d in ipairs(obj:GetDescendants()) do
        checked = checked + 1
        local kg = HaimiyachGAG2_ReadKgAttrs(d)
        if kg then return kg end
...
function HaimiyachGAG2_NormalizeMutationValue(v)
    if type(v) ~= "string" then return nil end
    v = string.gsub(v, "^%s+", "")
    v = string.gsub(v, "%s+$", "")
    if v == "" or v == "None" or v == "none" or v == "Normal" or v == "normal" then return nil end
    return v
end

function HaimiyachGAG2_SplitMutations(v)
    local out = {}
    v = HaimiyachGAG2_NormalizeMutationValue(v)
    if not v then return out end
    for item in string.gmatch(v, "[^%+%,]+") do
        item = HaimiyachGAG2_NormalizeMutationValue(item)
...
function HaimiyachGAG2_ObjectMutation(obj)
    ...
    while node and node ~= Workspace and guard < 10 do
        guard = guard + 1
        local v = HaimiyachGAG2_NormalizeMutationValue(node:GetAttribute("Mutation"))
    ...
    for _, d in ipairs(obj:GetDescendants()) do
        checked = checked + 1
        local mv = HaimiyachGAG2_NormalizeMutationValue(d:GetAttribute("Mutation"))
...
function HaimiyachGAG2_MutationListMatch(mutations, selected)
    if type(mutations) ~= "table" or not HaimiyachGAG2_HasSelection(selected) then return false end
...
function HaimiyachGAG2_SelectedMutationOk(obj, selected)
    if not HaimiyachGAG2_HasSelection(selected) then return false end
    return HaimiyachGAG2_MutationListMatch(HaimiyachGAG2_SplitMutations(HaimiyachGAG2_ObjectMutation(obj)), selected)
end
...
-- //                    INVENTORY FAVORITE                        \\ --
-- // ============================================================ \\ --
function HaimiyachGAG2_IsInventoryFruitTool(tool)
    if not (tool and tool:IsA("Tool")) then return false end
    if not tool:GetAttribute("Id") then return false end
    if tool:GetAttribute("Fruit") then return true end
...
function HaimiyachGAG2_GetInventoryFruitTools()
    ...
    if bp then
        for _, tool in ipairs(bp:GetChildren()) do
            if HaimiyachGAG2_IsInventoryFruitTool(tool) then
                out[#out + 1] = tool
            end
        end
    end
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    ...
    local kg = HaimiyachGAG2_ReadKgAttrs(tool)
    if kg then return kg end
    local checked = 0
    for _, d in ipairs(tool:GetDescendants()) do
        checked = checked + 1
        kg = HaimiyachGAG2_ReadKgAttrs(d)
        if kg then return kg end
...
function HaimiyachGAG2_GetFruitToolMutation(tool)
    if not tool then return nil end
    local v = HaimiyachGAG2_NormalizeMutationValue(tool:GetAttribute("Mutation"))
    if v then return v end
    local checked = 0
    for _, d in ipairs(tool:GetDescendants()) do
        checked = checked + 1
        v = HaimiyachGAG2_NormalizeMutationValue(d:GetAttribute("Mutation"))
        if v then return v end
        if checked >= 80 then break end
    end
    return nil
...
function HaimiyachGAG2_FruitToolMutationSelected(tool, selected)
    if type(selected) ~= "table" or not picked(selected) then return false end
    return HaimiyachGAG2_MutationListMatch(HaimiyachGAG2_SplitMutations(HaimiyachGAG2_GetFruitToolMutation(tool)), selected)
end

function HaimiyachGAG2_FruitToolMutationTextOk(tool, filter)
    filter = tostring(filter or "")
    if filter == "" or string.lower(filter) == "all" then return true end
    local mutations = HaimiyachGAG2_SplitMutations(HaimiyachGAG2_GetFruitToolMutation(tool))
    for _, mutation in ipairs(mutations) do
        if HaimiyachGAG2_TextListMatch(mutation, filter) then return true end
    end
    return false
end
...
function HaimiyachGAG2_ShouldFavoriteFruit(tool)
    local fruit = tool and (tool:GetAttribute("Fruit") or tool:GetAttribute("FruitName") or tool:GetAttribute("Seed") or tool.Name)
    ...
    if picked(S.favoriteMutations) then
        if not HaimiyachGAG2_FruitToolMutationSelected(tool, S.favoriteMutations) then return false end
    else
        local mutFilter = tostring(S.favoriteMutationFilter or "")
        if mutFilter ~= "" and string.lower(mutFilter) ~= "all" then
            if not HaimiyachGAG2_FruitToolMutationTextOk(tool, mutFilter) then return false end
        end
    end
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        local keep = false
        if hasMutationKeep and HaimiyachGAG2_FruitToolMutationSelected(tool, S.sellKeepMutations) then
            keep = true
        end
