...
local function worldPetNames()
    local ok, result = pcall(function()
        ...
        for _, w in ipairs(wildPets()) do
            local nm = tostring(w and w.name or "")
...
local function ensureWorldPetSelection()
    if type(S.buyWorldPets) ~= "table" then S.buyWorldPets = {} end
    return S.buyWorldPets
end
...
loopOn(function() return S.autoBuyPets end, function() return S.petBuyInterval end, function()
    local selectedPets = ensureWorldPetSelection()
    if not picked(selectedPets) then return end
    for _, w in ipairs(wildPets()) do
        if not S.autoBuyPets then break end
        local petName = tostring(w and w.name or "")
        if petName ~= "" and selectedPets[petName] and w.owner == 0 and w.price > 0 and w.price <= S.maxPetPrice and getSheckles() >= w.price then
...
ensureWorldPetSelection()
UI.worldPetDrop = addDropdown(UI.Pages["PETS"], "WORLD PETS TO BUY", worldPetNames(), S.buyWorldPets, true, function(sel)
    ensureWorldPetSelection()
    pickMulti(sel, S.buyWorldPets)
end)
addButton(UI.Pages["PETS"], "REFRESH WORLD PET LIST", function()
    if UI.worldPetDrop and UI.worldPetDrop.SetOptions then
        UI.worldPetDrop.SetOptions(worldPetNames())
    end
end)
