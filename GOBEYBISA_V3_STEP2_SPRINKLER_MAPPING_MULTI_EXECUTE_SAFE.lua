...
do
    ...
    for _, n in ipairs(GEAR_NAMES) do
        if string.find(string.lower(tostring(n)), "sprinkler", 1, true) and not seen[n] then
            seen[n] = true
            SPRINKLER_NAMES[#SPRINKLER_NAMES + 1] = n
        end
    end
    if #SPRINKLER_NAMES == 0 then
        for _, n in ipairs({ "Basic Sprinkler", "Advanced Sprinkler", "Godly Sprinkler" }) do
            if not seen[n] then
                seen[n] = true
                SPRINKLER_NAMES[#SPRINKLER_NAMES + 1] = n
            end
...
function HaimiyachGAG2_ProjectToPlantArea(pos)
    ...
    for _, area in ipairs(areas) do
        ...
        local x = clampNumber(lp.X, -size.X / 2 + 1, size.X / 2 - 1)
        local z = clampNumber(lp.Z, -size.Z / 2 + 1, size.Z / 2 - 1)
        local top = (cf * CFrame.new(x, size.Y / 2, z)).Position
        local hit = Workspace:Raycast(top + Vector3.new(0, 12, 0), Vector3.new(0, -40, 0), params)
        local p = hit and hit.Position or top
        local d = flatDistance(pos, p)
        if d < bestDist then
            bestDist = d
            best = p
        end
...
function HaimiyachGAG2_ExistingSprinklerPositions()
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        ...
        if type(attr) == "string" and string.find(string.lower(attr), "sprinkler", 1, true) then
            isSprinkler = true
        elseif string.find(string.lower(obj.Name), "sprinkler", 1, true) then
            isSprinkler = true
        end
        if isSprinkler then
            local pos
...
function HaimiyachGAG2_SprinklerCandidatePoints(mode)
    ...
    if mode == "AVATAR POSITION" then
        ...
        local p = HaimiyachGAG2_ProjectToPlantArea(hrp and hrp.Position or center)
        if p then out[#out + 1] = p end
        local grid = plantGrid(8)
        table.sort(grid, function(a, b) return flatDistance(a, p or center) < flatDistance(b, p or center) end)
        for _, gp in ipairs(grid) do out[#out + 1] = gp end
    elseif mode == "MAPPING" then
        local grid = plantGrid(12)
        table.sort(grid, function(a, b)
            local da, db = flatDistance(a, center), flatDistance(b, center)
            if math.abs(da - db) < 0.01 then return tostring(a) < tostring(b) end
            return da < db
        end)
        for _, gp in ipairs(grid) do out[#out + 1] = gp end
        if center then table.insert(out, 1, center) end
    else
        if center then out[#out + 1] = center end
        local grid = plantGrid(8)
        table.sort(grid, function(a, b) return flatDistance(a, center) < flatDistance(b, center) end)
        for _, gp in ipairs(grid) do out[#out + 1] = gp end
    end
    return out
end
...
function HaimiyachGAG2_ChooseSprinklerPosition(placed)
    ...
    local minGap = S.sprinklerTargetMode == "MAPPING" and 10 or 8
    for _, p in ipairs(points) do
        local ok = true
        for _, ex in ipairs(placed) do
            if flatDistance(p, ex) < minGap then ok = false break end
        end
...
function HaimiyachGAG2_SelectedSprinklerTools()
    ...
    for _, tool in ipairs(toolsByAttr("Sprinkler")) do
        local n = tostring(tool:GetAttribute("Sprinkler") or tool.Name or "")
        if (not useSelected) or selected[n] or selected[tool.Name] then
            out[#out + 1] = tool
        end
    end
