...
-- FPS BOOST LIGHT/BALANCED keeps visuals safe; ULTRA hides textures/plants/decorations client-side.
...
local function IsGardenPlantVisual(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
    if not gardens then return false end
    local current = obj
    while current and current ~= Workspace do
        local parent = current.Parent
        if parent and parent.Name == "Plants" and parent.Parent and parent.Parent.Parent == gardens then
            return true
        end
        current = parent
    end
...
local function IsDecorationLike(obj)
    local current = obj
    while current and current ~= Workspace do
        local n = string.lower(current.Name or "")
        if string.find(n, "decor") or string.find(n, "cosmetic") or string.find(n, "vfx") or string.find(n, "visual") or string.find(n, "grass") or string.find(n, "bush") or string.find(n, "leaf") or string.find(n, "leaves") or string.find(n, "rock") or string.find(n, "stone") then
            return true
        end
        current = current.Parent
    end
...
local function CaptureUltraPartBackup(part)
    if not part or GraphicsBackup.UltraParts[part] then return end
    pcall(function()
        GraphicsBackup.UltraParts[part] = {
...
local function CaptureUltraVisualBackup(obj)
    if not obj or GraphicsBackup.UltraVisuals[obj] then return end
    if obj:IsA("Decal") or obj:IsA("Texture") then
        pcall(function()
            GraphicsBackup.UltraVisuals[obj] = { Transparency = obj.Transparency }
        end)
    elseif obj:IsA("SurfaceAppearance") then
        pcall(function()
            GraphicsBackup.UltraVisuals[obj] = { Parent = obj.Parent }
        end)
    end
end

local function RestoreUltraVisuals()
    for part, values in pairs(GraphicsBackup.UltraParts) do
        if part and part.Parent then
            for prop, value in pairs(values) do
                SafeSetProperty(part, prop, value)
            end
        end
    end
    for obj, values in pairs(GraphicsBackup.UltraVisuals) do
        if obj then
            if values.Transparency ~= nil and obj.Parent then
                SafeSetProperty(obj, "Transparency", values.Transparency)
            elseif values.Parent and not obj.Parent then
                pcall(function() obj.Parent = values.Parent end)
            end
        end
    end
    table.clear(GraphicsBackup.UltraParts)
    table.clear(GraphicsBackup.UltraVisuals)
end

local function ApplyBalancedObject(obj)
    DisableClientVisualEffect(obj)
    if obj and obj:IsA("BasePart") and not IsPlayerCharacterDescendant(obj) then
        CaptureUltraPartBackup(obj)
        SafeSetProperty(obj, "CastShadow", false)
    elseif obj and (obj:IsA("Decal") or obj:IsA("Texture")) then
        CaptureUltraVisualBackup(obj)
        local t = 0.65
        pcall(function()
            if obj.Transparency < t then obj.Transparency = t end
        end)
    end
end

local function ApplyUltraObject(obj)
    ApplyBalancedObject(obj)
    ...
    if obj:IsA("BasePart") then
        ...
        if IsGardenPlantVisual(obj) or IsDecorationLike(obj) then
...
local function ApplyFPSBoostObject(obj, mode)
    if mode == "LIGHT" then
        DisableClientVisualEffect(obj)
    elseif mode == "ULTRA" then
        ApplyUltraObject(obj)
    else
        ApplyBalancedObject(obj)
    end
end
...
local function ApplyFPSBoostGraphics()
    ...
    -- LIGHT = effects only. BALANCED = low graphics. ULTRA = potato mode: smooth plastic, hidden textures, plants/fruits/decorations.
    ...
    -- LIGHT only touches effects; BALANCED reduces shadows/textures; ULTRA applies potato visuals.
    task.spawn(function()
        local descendants = {}
        pcall(function()
