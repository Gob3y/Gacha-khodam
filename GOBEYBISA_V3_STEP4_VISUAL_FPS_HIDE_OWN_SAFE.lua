...
local CONFIG_KEYS = {
    ...
    "fpsBoost", "fpsBoostMode", "highGraphics", "hideOwnPlants", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
local function OptimizeFPSBasePart(obj)
    if not obj or not obj:IsA("BasePart") then return end
    if IsPlayerCharacterPart(obj) then return end
    if GraphicsBackup.WorkspaceParts[obj] == nil then
        pcall(function()
            GraphicsBackup.WorkspaceParts[obj] = {
...
local function RestoreWorkspaceEffects()
    ...
    for obj, props in pairs(GraphicsBackup.WorkspaceParts) do
        if obj and obj.Parent and type(props) == "table" then
            if props.CastShadow ~= nil then SafeSetProperty(obj, "CastShadow", props.CastShadow) end
            if props.Reflectance ~= nil then SafeSetProperty(obj, "Reflectance", props.Reflectance) end
        end
    end
end
...
local function ApplyFPSBoostGraphics()
    ...
    -- LIGHT only removes expensive effects and softens shadows.
    -- BALANCED also lowers terrain/water/lighting.
    -- ULTRA keeps BALANCED and also disables basepart shadows in the workspace scan.
    SafeSetProperty(Lighting, "GlobalShadows", false)
    SafeSetProperty(Lighting, "ExposureCompensation", 0)
    SafeSetProperty(Lighting, "EnvironmentDiffuseScale", fpsMode == "LIGHT" and 0.35 or 0)
    SafeSetProperty(Lighting, "EnvironmentSpecularScale", fpsMode == "LIGHT" and 0.2 or 0)
    SafeSetProperty(Lighting, "ShadowSoftness", 0)
    pcall(function()
        UserSettings():GetService("UserGameSettings").SavedQualityLevel = (fpsMode == "LIGHT") and Enum.SavedQualitySetting.QualityLevel3 or Enum.SavedQualitySetting.QualityLevel1
    end)
    ...
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost then
                    DisableClientVisualEffect(obj)
                    if NormalizeFPSBoostMode(S.fpsBoostMode) == "ULTRA" then
                        OptimizeFPSBasePart(obj)
                    end
                end
            end)
...
-- // ----------------------- HIDE VISUALS ----------------------- \ --
-- Client-side only. It never destroys plants; it uses transparency so it can restore.
local HideOwnPlantsConnection = nil
local HideVisualBackup = {}

local function BackupHideVisual(obj)
    if not obj or HideVisualBackup[obj] ~= nil then return end
    local data = {}
    if obj:IsA("BasePart") then
        pcall(function()
            data.LocalTransparencyModifier = obj.LocalTransparencyModifier
    ...
    HideVisualBackup[obj] = data
end

local function HideVisualObject(obj)
    if not obj then return end
    if obj:IsA("BasePart") then
        BackupHideVisual(obj)
...
local function RestoreHideVisuals()
    for obj, data in pairs(HideVisualBackup) do
        if obj and obj.Parent and type(data) == "table" then
            if obj:IsA("BasePart") then
                if data.LocalTransparencyModifier ~= nil then SafeSetProperty(obj, "LocalTransparencyModifier", data.LocalTransparencyModifier) end
                if data.CastShadow ~= nil then SafeSetProperty(obj, "CastShadow", data.CastShadow) end
            elseif obj:IsA("Decal") or obj:IsA("Texture") then
                if data.Transparency ~= nil then SafeSetProperty(obj, "Transparency", data.Transparency) end
            elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
                if data.Enabled ~= nil then SafeSetProperty(obj, "Enabled", data.Enabled) end
            end
        end
    end
    table.clear(HideVisualBackup)
end

local function ApplyHideOwnPlantsOnce()
    local plot = myPlot()
    local plants = plot and plot:FindFirstChild("Plants")
...
local function SetHideOwnPlantsEnabled(state)
    S.hideOwnPlants = state and true or false
    if HideOwnPlantsConnection then
        pcall(function() HideOwnPlantsConnection:Disconnect() end)
        HideOwnPlantsConnection = nil
    end
    RestoreHideVisuals()
    if not S.hideOwnPlants then return end

    task.spawn(function()
        local plants = nil
        for _ = 1, 20 do
            if not S.hideOwnPlants then return end
            plants = ApplyHideOwnPlantsOnce()
            if plants then break end
            task.wait(0.5)
        end
        if not (S.hideOwnPlants and plants) then return end
        HideOwnPlantsConnection = plants.DescendantAdded:Connect(function(obj)
            if S.hideOwnPlants then
                task.defer(function()
                    if S.hideOwnPlants then HideVisualObject(obj) end
                end)
            end
        end)
    end)
...
addSection(UI.Pages["VISUALS"], "HIDE VISUALS")
addToggle(UI.Pages["VISUALS"], "HIDE OWN PLANTS", S.hideOwnPlants, function(v) SetHideOwnPlantsEnabled(v); saveConfig(true) end)
...
if S.fpsBoost then SetFPSBoostEnabled(true) end
if S.hideOwnPlants then SetHideOwnPlantsEnabled(true) end
...
addButton(UI.Pages["SETTINGS"], "UNLOAD HUB", function()
    ...
    pcall(function() SetHideOwnPlantsEnabled(false) end)
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        claimSeedEvent = claimSeedEvent, flyToPosition = flyToPosition, SetCutsceneDisabled = SetCutsceneDisabled, SetHideOwnPlantsEnabled = SetHideOwnPlantsEnabled,
