...
local function crateCatalog()
    ...
    if assets then
        for _, folderName in ipairs({ "Crates", "GuildCrates" }) do
    ...
    local stockRoot = ReplicatedStorage:FindFirstChild("StockValues")
    if stockRoot then
        for _, shopName in ipairs({ "CrateShop", "Crates", "Crate", "GuildCrates", "GuildCrate" }) do
            local shop = stockRoot:FindFirstChild(shopName)
            local items = shop and shop:FindFirstChild("Items")
            if items then
                for _, item in ipairs(items:GetChildren()) do addName(item.Name) end
            end
        end
    end

    for nm in pairs(invNames("Crates")) do addName(nm) end
    table.sort(out)
    return out
end
...
local S = {
    ...
    autoBuyCrates = false, autoBuyAllCrates = false, crateBuy = {}, crateInterval = 10,
    -- steal
    ...
    -- perf / webhook
    fpsBoost = false, highGraphics = false, hidePlants = false,
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoBuyCrates", "autoBuyAllCrates", "crateBuy", "crateInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay",
    "autoMail", "autoAcceptGift", "autoHop", "hopInterval", "codeText", "autoCodes", "antiAfk",
    "fpsBoost", "highGraphics", "hidePlants", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
local CRATE_BUY_PATHS = {
    "CrateShop.PurchaseCrate", "CrateShop.Purchase", "CrateShop.BuyCrate", "CrateShop.Buy",
    "Crates.PurchaseCrate", "Crates.BuyCrate",
    "Crate.PurchaseCrate", "Crate.Purchase", "Crate.BuyCrate", "Crate.Buy",
    "GuildCrates.PurchaseCrate", "GuildCrate.PurchaseCrate", "Shop.PurchaseCrate", "Shop.BuyCrate",
}

local function stockOfCrate(name)
    for _, shopName in ipairs({ "CrateShop", "Crates", "Crate", "GuildCrates", "GuildCrate" }) do
        local stock = stockOf(shopName, name)
        if stock ~= nil then return stock end
    end
    return nil
end

local function buyCrateByName(name)
    name = tostring(name or "")
    if name == "" then return false, "empty crate name" end
    for _, path in ipairs(CRATE_BUY_PATHS) do
        if action(path) then
            local ok, res = fire(path, name)
            if ok then
    ...
    return false, "crate buy remote not found"
end

loopOn(function() return S.autoBuyCrates or S.autoBuyAllCrates end, function() return S.crateInterval end, function()
    if not (S.autoBuyAllCrates or picked(S.crateBuy)) then return end
    if #CRATE_NAMES == 0 then CRATE_NAMES = crateCatalog() end

    if S.autoBuyAllCrates then
        for _, name in ipairs(CRATE_NAMES) do
            if not (S.autoBuyCrates or S.autoBuyAllCrates) then break end
            local stock = stockOfCrate(name)
            if stock == nil or stock > 0 then
                local ok = buyCrateByName(name)
                if ok then Stats.bought = Stats.bought + 1 end
                task.wait(jitter(0.25, 0.55))
            end
        end
    else
        for name, enabled in pairs(S.crateBuy) do
            if not S.autoBuyCrates then break end
...
local function ApplyFPSBoostGraphics()
    ...
    -- Ultra-light mobile mode: no full Workspace scan on enable.
    -- Full map scans can make Android executor freeze or look like it cannot execute.
    -- Existing Lighting/Terrain are lowered above; new heavy effects are disabled as they spawn.
    FPSBoostDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if FPSBoostActive and S.fpsBoost then
            task.delay(0.15, function()
                if FPSBoostActive and S.fpsBoost and scanToken == FPSBoostScanToken then
...
local HidePlantsActive = false
local HidePlantsToken = 0
local HidePlantsConnection = nil
local HidePlantsBackup = {}

local function BackupHidePlantProperty(obj, prop)
    if not obj then return end
    local data = HidePlantsBackup[obj]
    if not data then
        data = {}
        HidePlantsBackup[obj] = data
    end
    if data[prop] == nil then
        pcall(function() data[prop] = obj[prop] end)
    end
end

local function RestoreHiddenPlants()
    if HidePlantsConnection then
        pcall(function() HidePlantsConnection:Disconnect() end)
        HidePlantsConnection = nil
    end
    for obj, props in pairs(HidePlantsBackup) do
        if obj and obj.Parent then
    ...
    HidePlantsBackup = {}
    HidePlantsActive = false
end

local function HidePlantVisualObject(obj)
    if not obj then return end
    if obj:IsA("BasePart") then
        BackupHidePlantProperty(obj, "LocalTransparencyModifier")
        BackupHidePlantProperty(obj, "CastShadow")
        SafeSetProperty(obj, "LocalTransparencyModifier", 1)
        SafeSetProperty(obj, "CastShadow", false)
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        BackupHidePlantProperty(obj, "Transparency")
        SafeSetProperty(obj, "Transparency", 1)
    elseif obj:IsA("ParticleEmitter") then
        BackupHidePlantProperty(obj, "Enabled")
        BackupHidePlantProperty(obj, "Rate")
        SafeSetProperty(obj, "Enabled", false)
        SafeSetProperty(obj, "Rate", 0)
...
local function IsPlantObjectNamed(obj)
    if not obj then return false end
    local n = string.lower(tostring(obj.Name or ""))
    return string.find(n, "plant", 1, true) ~= nil
        or string.find(n, "fruit", 1, true) ~= nil
        or string.find(n, "crop", 1, true) ~= nil
        or string.sub(n, 1, 6) == "plant_"
end

local function IsInsidePlantObject(obj)
    local cur = obj
    while cur and cur ~= Workspace do
        if IsPlantObjectNamed(cur) then return true end
        cur = cur.Parent
    end
    return false
end

local function GetPlantRoots()
    local roots, seen = {}, {}
    local function addRoot(root)
        if root and root.Parent and not seen[root] then
            seen[root] = true
            roots[#roots + 1] = root
        end
    end
    ...
    if gardens then
        for _, plot in ipairs(gardens:GetChildren()) do
            local plants = plot:FindFirstChild("Plants") or plot:FindFirstChild("Plant")
            if plants then addRoot(plants) end
            for _, child in ipairs(plot:GetChildren()) do
                if IsPlantObjectNamed(child) then addRoot(child) end
            end
        end
    end

    -- fallback from screenshot/list: plant_* objects may exist directly in Workspace folders
...
local function ProcessPlantRoot(root, token)
    if not (root and root.Parent) then return end
    HidePlantVisualObject(root)
    local descendants = {}
    pcall(function() descendants = root:GetDescendants() end)
    for i, obj in ipairs(descendants) do
        if token ~= HidePlantsToken or not HidePlantsActive or not S.hidePlants then break end
        HidePlantVisualObject(obj)
        if i % 60 == 0 then task.wait() end
    end
end

local function SetHidePlantsEnabled(state)
    HidePlantsToken = HidePlantsToken + 1
    local token = HidePlantsToken
    ...
    RestoreHiddenPlants()
    HidePlantsActive = true
    S.hidePlants = true

    task.spawn(function()
        for _, root in ipairs(GetPlantRoots()) do
            if token ~= HidePlantsToken or not HidePlantsActive or not S.hidePlants then break end
            ProcessPlantRoot(root, token)
            task.wait()
        end
    end)

    HidePlantsConnection = Workspace.DescendantAdded:Connect(function(obj)
        if not (HidePlantsActive and S.hidePlants) then return end
        task.defer(function()
            if HidePlantsActive and S.hidePlants and IsInsidePlantObject(obj) then
                HidePlantVisualObject(obj)
            end
        end)
    end)
...
addSection(ShopPage, "CRATE SHOP")
local crateDropdown = addDropdown(ShopPage, "CRATES TO BUY", CRATE_NAMES, S.crateBuy, true, function(sel) pickMulti(sel, S.crateBuy) end)
addButton(ShopPage, "REFRESH CRATE LIST", function()
    CRATE_NAMES = crateCatalog()
    if crateDropdown and crateDropdown.SetOptions then crateDropdown.SetOptions(CRATE_NAMES) end
    notify("Crates", "Crate list refreshed.", 2)
end)
addToggle(ShopPage, "AUTO BUY SELECTED CRATES", S.autoBuyCrates, function(v) S.autoBuyCrates = v end)
addToggle(ShopPage, "AUTO BUY ALL CRATES", S.autoBuyAllCrates, function(v) S.autoBuyAllCrates = v end)
addSlider(ShopPage, "CRATE BUY DELAY", 2, 60, S.crateInterval, 0, function(v) S.crateInterval = v end)
-- STEAL
...
addToggle(VisualsPage, "HIDE PLANTS", S.hidePlants, function(v)
    SetHidePlantsEnabled(v)
end)
...
if S.hidePlants then SetHidePlantsEnabled(true) end
...
pcall(function()
    if getgenv then getgenv().HaimiyachGAG2 = {
        ...
        catalog = CATALOG, gearNames = GEAR_NAMES, crateNames = CRATE_NAMES, myPlot = myPlot, replica = replica,
...
local __haimiyach_loader = loadstring or load
if type(__haimiyach_loader) ~= "function" then
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "HAIMIYACH HUB",
            Text = "Executor has no loadstring/load support.",
            Duration = 8
        })
    end)
    warn("[HAIMIYACH HUB] loadstring/load is not available in this executor")
else
    local __chunk, __compileErr = __haimiyach_loader(__HAIMIYACH_GAG2_SOURCE)
    if not __chunk then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "HAIMIYACH HUB",
                Text = "Compile error. Check console.",
                Duration = 8
            })
        end)
        warn("[HAIMIYACH HUB] Compile error: " .. tostring(__compileErr))
    else
        ...
        if not __ok then
            pcall(function()
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = "HAIMIYACH HUB",
                    Text = "Runtime error. Check console.",
                    Duration = 8
                })
            end)
            warn("[HAIMIYACH HUB] Runtime error: " .. tostring(__runtimeErr))
        end
    end
end
