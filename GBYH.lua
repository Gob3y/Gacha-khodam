...
local S = {
    ...
    clickDeleteMode = false,   -- mode klik-hapus tanaman
}
...
S.clickDeletePlant = function()
    if not S.clickDeleteMode then return false end
        ...
        if plot:IsA("Model") then
            ...
            if plantsFolder then
                for _, plant in ipairs(plantsFolder:GetChildren()) do
                    if plant:IsA("Model") and plant:GetAttribute("PlantId") then
                        table.insert(plantModels, plant)
                    end
                end
            end
        end
    ...
    if #plantModels == 0 then return false end

    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Include
    params.FilterDescendantsInstances = plantModels

    local hit = workspace:Raycast(ray.Origin, ray.Direction * 1000, params)
    if not hit then return false end

    local plantModel = hit.Instance
    while plantModel and plantModel ~= workspace do
        if plantModel:IsA("Model") and plantModel:GetAttribute("PlantId") then
            break
        end
        plantModel = plantModel.Parent
    end

    if not plantModel or not plantModel:GetAttribute("PlantId") then
        return false
    end

    local userId = tonumber(plantModel:GetAttribute("UserId"))
    if userId == LocalPlayer.UserId then
        notify("CLICK DELETE", "This is your own plant!", 3)
        return false
    end

    local plantId = plantModel:GetAttribute("PlantId")
    if plantId then
        local ok = S.useShovel(plantId, "")
        if ok then
            notify("CLICK DELETE", "Plant successfully deleted!", 3)
            return true
        else
            notify("CLICK DELETE", "Failed to delete plant", 3)
        end
    end
    return false
end
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        if S.clickDeleteMode then
            task.spawn(S.clickDeletePlant)
        end
    end
end)
...
addSection(UI.Pages["AUTO"], "DELETE PLANTS")
addToggle(UI.Pages["AUTO"], "CLICK DELETE MODE (Click enemy plant)", S.clickDeleteMode, function(v)
    S.clickDeleteMode = v
end)
