...
local baseW = isMobile and 620 or 650
...
local Content = new("Frame", {
    ...
    Position = UDim2.fromOffset(156, 56),
    BackgroundColor3 = T.Panel,
    BorderSizePixel = 0,
    ClipsDescendants = true,
}, Main)
...
createTab("BOOSTS", 3)
createTab("SHOP", 4)
createTab("PETS & CRATES", 5)
createTab("ESP", 6)
createTab("EVENTS", 7)
createTab("MAILBOX", 8)
createTab("STEAL", 9)
createTab("VISUALS", 10)
createTab("PROTECT", 11)
...
addLabel(UI.Pages["EVENTS"], "Weather/Event detector needs extra game data before it can be added safely.")
...
-- PETS
addSection(UI.Pages["PETS & CRATES"], "PETS")
addToggle(UI.Pages["PETS & CRATES"], "AUTO EQUIP PETS", S.autoEquipPets, function(v) S.autoEquipPets = v end)
addToggle(UI.Pages["PETS & CRATES"], "AUTO BUY PET SLOTS", S.autoPetSlot, function(v) S.autoPetSlot = v end)
UI.worldPetDrop = addDropdown(UI.Pages["PETS & CRATES"], "WORLD PETS TO BUY", HaimiyachGAG2_WorldPetNames(), S.buyWorldPets, true, function(sel)
    if type(S.buyWorldPets) ~= "table" then S.buyWorldPets = {} end
    pickMulti(sel, S.buyWorldPets)
end)
addToggle(UI.Pages["PETS & CRATES"], "AUTO BUY SELECTED WORLD PETS", S.autoBuyPets, function(v) S.autoBuyPets = v end)
addToggle(UI.Pages["PETS & CRATES"], "TELEPORT TO PET", S.petTeleport, function(v) S.petTeleport = v end)
addSlider(UI.Pages["PETS & CRATES"], "PET BUY DELAY (SEC)", 2, 60, S.petBuyInterval, 0, function(v) S.petBuyInterval = v end)
addSection(UI.Pages["PETS & CRATES"], "SELL PETS")
UI.petDrop = addDropdown(UI.Pages["PETS & CRATES"], "PETS TO SELL", ownedPetNames(), S.sellPets, true, function(sel) pickMulti(sel, S.sellPets) end)
addButton(UI.Pages["PETS & CRATES"], "REFRESH PET LIST", function() UI.petDrop.SetOptions(ownedPetNames()) end)
addToggle(UI.Pages["PETS & CRATES"], "AUTO SELL SELECTED PETS", S.autoSellPets, function(v) S.autoSellPets = v end)

-- OPEN
addSection(UI.Pages["PETS & CRATES"], "OPEN ITEMS")
addToggle(UI.Pages["PETS & CRATES"], "AUTO OPEN EGGS", S.autoEgg, function(v) S.autoEgg = v end)
addToggle(UI.Pages["PETS & CRATES"], "AUTO OPEN CRATES", S.autoCrate, function(v) S.autoCrate = v end)
addToggle(UI.Pages["PETS & CRATES"], "AUTO OPEN SEED PACKS", S.autoPack, function(v) S.autoPack = v end)
addSlider(UI.Pages["PETS & CRATES"], "OPEN DELAY (SEC)", 1, 30, S.openInterval, 0, function(v) S.openInterval = v end)
...
addLabel(UI.Pages["SHOP"], "Shop timers use in-game stock refresh data when available.")
-- STEAL
...
addLabel(UI.Pages["STEAL"], "Trap/Gnome protection needs extra game data before it can be added safely.")
...
addLabel(UI.Pages["ESP"], "Note: Turn off FPS BOOST before using ESP.")
