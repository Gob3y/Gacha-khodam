...
local function BackupFPSObjectProperty(obj, prop)
    if not obj then return end
    local backup = FPSBoostObjectBackup[obj]
    if not backup then
        backup = {}
        FPSBoostObjectBackup[obj] = backup
    end
    if backup[prop] == nil then
        pcall(function()
...
local function SetFPSObjectProperty(obj, prop, value)
    BackupFPSObjectProperty(obj, prop)
    SafeSetProperty(obj, prop, value)
end

local function RestoreFPSBoostObjectProperties()
    for obj, props in pairs(FPSBoostObjectBackup) do
...
local function IsGardenVisual(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
...
local function IsDecorationVisual(obj)
    local current = obj
    while current and current ~= Workspace do
        local name = string.lower(tostring(current.Name or ""))
        if string.find(name, "decor") or string.find(name, "cosmetic") or string.find(name, "grass") or string.find(name, "bush") or string.find(name, "leaf") or string.find(name, "leaves") or string.find(name, "rock") or string.find(name, "stone") or string.find(name, "vfx") or string.find(name, "effect") then
            return true
        end
        current = current.Parent
    end
...
local function ApplyFPSBoostObject(obj, mode)
    ...
    if obj:IsA("BasePart") then
        if IsCharacterVisual(obj) then return end
        SetFPSObjectProperty(obj, "CastShadow", false)
        if mode == "ULTRA" then
            SetFPSObjectProperty(obj, "Reflectance", 0)
            SetFPSObjectProperty(obj, "Material", Enum.Material.SmoothPlastic)
            if IsGardenVisual(obj) or IsDecorationVisual(obj) then
                SetFPSObjectProperty(obj, "LocalTransparencyModifier", 1)
            end
        end
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        if mode == "ULTRA" then
            SetFPSObjectProperty(obj, "Transparency", 1)
        else
            BackupFPSObjectProperty(obj, "Transparency")
            pcall(function()
                if obj.Transparency < 0.65 then
...
local function RestoreDefaultGraphics()
    ...
    RestoreFPSBoostObjectProperties()
...
local function ApplyFPSBoostGraphics()
    ...
    -- LIGHT = effects only. BALANCED = low graphics. ULTRA = potato mode with smooth plastic, hidden textures, plants/fruits/decorations.
    ...
    -- LIGHT touches effects only; BALANCED lowers shadows/textures; ULTRA applies potato visuals.
    task.spawn(function()
        local descendants = {}
        pcall(function()
