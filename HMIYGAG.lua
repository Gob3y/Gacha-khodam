...
if GardenESP.isSingleHarvestPlant(model) then return false end
...
local label = Instance.new("TextLabel")
label.Name = "Text"
label.Text = "Total Value Fruits Garden: 0¢"
label.Size = UDim2.new(1, 0, 1, 0)
label.BackgroundTransparency = 1
label.BorderSizePixel = 0
...
if label then
label.Text = "Total Value Fruits Garden: " .. GardenESP.money(totalValue) .. "¢"
end
end
...
or (inst.Parent and inst.Parent.Name == "Fruits") then
add(inst)
end
end
end
...
GardenESP.initModules()
if not S.espGardenFruit and not S.espGardenGrowing and not S.espGardenValue then return end
local alive = {}
...
local nameForValue = GardenESP.name(model) or "Fruit"
...
if S.espGardenFruit or S.espGardenGrowing or S.espGardenValue then
...
local __C={BackpackESP=BackpackESP,CATALOG=CATALOG,DisconnectWebhookSent=DisconnectWebhookSent,GEAR_NAMES=GEAR_NAMES,GardenESP=GardenESP,KG_MODE_OPTIONS=KG_MODE_OPTIONS,LocalPlayer=LocalPlayer,Net=Net,NormalizeFPSBoostMode=NormalizeFPSBoostMode,RestoreDefaultGraphics=RestoreDefaultGraphics,RunService=RunService,S=S,SEED_NAMES=SEED_NAMES,SetCutsceneDisabled=SetCutsceneDisabled,SetFPSBoostEnabled=SetFPSBoostEnabled,SetHighGraphicsEnabled=SetHighGraphicsEnabled,Stats=Stats,action=action,active=active,an=an,arr=arr,before=before,bought=bought,box=box,child=child,claimSeedEvent=claimSeedEvent,clear=clear,code=code,color=color,corner=corner,current=current,currentFps=currentFps,data=data,delta=delta,err=err,fill=fill,fire=fire,fireFast=fireFast,first=first,flyToPosition=flyToPosition,fmt=fmt,fpsConnection=fpsConnection,fruitCount=fruitCount,getPingText=getPingText,getSeedEventAvailableText=getSeedEventAvailableText,getSeedEventClaimText=getSeedEventClaimText,getSeedEventLastText=getSeedEventLastText,getSheckles=getSheckles,getTokens=getTokens,hit=hit,hms=hms,hp=hp,item=item,keep=keep,label=label,left=left,list=list,loadConfig=loadConfig,loader=loader,loopOn=loopOn,maxFruitCap=maxFruitCap,mobile=mobile,myBasePos=myBasePos,myPlot=myPlot,name=name,nextRefresh=nextRefresh,note=note,notify=notify,obj=obj,ok=ok,owned=owned,ownedPetNames=ownedPetNames,parent=parent,parts=parts,pct=pct,pickMulti=pickMulti,pid=pid,plantGrid=plantGrid,plants=plants,plot=plot,pos=pos,prompt=prompt,props=props,raw=raw,replica=replica,res=res,ripeHarvests=ripeHarvests,saveConfig=saveConfig,saved=saved,scroll=scroll,sellAllNow=sellAllNow,sendDisconnectWebhook=sendDisconnectWebhook,sendWebhook=sendWebhook,serverNow=serverNow,set=set,shouldHop=shouldHop,size=size,state=state,status=status,stealable=stealable,stepHarvest=stepHarvest,text=text,toolsByAttr=toolsByAttr,top=top,total=total,url=url,value=value,wildPets=wildPets,writeAutoExecuteLoader=writeAutoExecuteLoader}
__C.__loadedAt=os.clock()
__C.HaimiyachGAG2_GetCurrentFps=function() return currentFps end
__C.HaimiyachGAG2_ResetDisconnectWebhook=function() DisconnectWebhookSent=false end
__G.HaimiyachGAG2Core=__C
end
local function HaimiyachGAG2_LoadGui()
local __G=(getgenv and getgenv()) or _G
local url=__G.HaimiyachGAG2GuiUrl or "https://raw.githubusercontent.com/HAIMIYACH/Haimiyach1/refs/heads/main/HMIYGUI.lua"
local code=nil
if type(url)=="string" and url~="" and url~="PASTE_GUI_RAW_URL_HERE" then
local ok,res=pcall(function() return game:HttpGet(url) end)
if ok and type(res)=="string" and #res>50 then code=res end
end
if not code and type(readfile)=="function" then
local ok,res=pcall(function() return readfile("HMIYGUI.lua") end)
if ok and type(res)=="string" and #res>50 then code=res end
end
if not code then notify("HAIMIYACH HUB","GUI file not loaded. Check GUI raw URL or same folder GUI file.",7) return false end
local fn,err=loadstring(code)
if not fn then notify("HAIMIYACH HUB","GUI syntax error: "..tostring(err),8) return false end
local ok,runErr=pcall(fn)
if not ok then notify("HAIMIYACH HUB","GUI runtime error: "..tostring(runErr),8) return false end
return true
end
HaimiyachGAG2_LoadGui()
