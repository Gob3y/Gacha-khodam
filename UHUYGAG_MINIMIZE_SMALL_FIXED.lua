...
local function getMiniBarSize()
    ...
    local w = math.floor((470 * scale) + 0.5)
    local h = math.floor((44 * scale) + 0.5)
    return w, h
end
...
local function setMinimized(v)
    ...
    if UI.Minimized then
        do local mw, mh = getMiniBarSize(); MiniBar.Size = UDim2.fromOffset(mw, mh) end
...
addSlider(MiscPage, "HOP INTERVAL MINUTES", 0, 120, S.hopInterval / 60, 0, function(v) S.hopInterval = v * 60 end)
