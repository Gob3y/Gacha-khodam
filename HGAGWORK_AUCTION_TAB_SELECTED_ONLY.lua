...
local S = {
    ...
    -- auction
    autoBuyAuction = false, auctionBuyItems = {}, auctionBuyDelay = 3, auctionShowActive = true,
    -- steal
...
-- Reads the same Auctioneer snapshot used by the game UI and buys only selected lots.
...
function HaimiyachGAG2_AuctionDisplayName(lot)
    ...
    local name = tostring(lot.item or lot.Name or lot.FruitName or lot.name or "Unknown")
    if lot.mutation and tostring(lot.mutation) ~= "" then name = tostring(lot.mutation) .. " " .. name end
    if lot.count and tonumber(lot.count) and tonumber(lot.count) > 1 then name = name .. " (x" .. tostring(math.floor(tonumber(lot.count))) .. ")" end
    return name
end
...
function HaimiyachGAG2_AuctionCurrentPrice(lot)
    ...
    return tonumber(lot.currentPrice or lot.price or lot.startPrice or lot.startingPrice or lot.buyPrice or lot.PurchasePrice) or 0
end
    ...
    if type(snapshot.manifest) == "table" and type(snapshot.manifest.lots) == "table" then
        for _, lot in pairs(snapshot.manifest.lots) do
            if type(lot) == "table" and lot.lotId then
                table.insert(state.lots, lot)
            end
        end
    end
    table.sort(state.lots, function(a, b)
        return tostring(a.lotId or "") < tostring(b.lotId or "")
    end)
    state.stock = type(snapshot.stock) == "table" and snapshot.stock or {}
    state.rollIntervalSeconds = tonumber(snapshot.rollIntervalSeconds) or state.rollIntervalSeconds or 0
    state.rollWindowUnix = tonumber(snapshot.rollWindowUnix) or state.rollWindowUnix or 0
    state.timerShiftSeconds = tonumber(snapshot.timerShiftSeconds) or 0
    state.lastSnapshotAt = os.clock()
    state.optionVersion = (tonumber(state.optionVersion) or 0) + 1
    state.status = #state.lots > 0 and ("LOADED " .. tostring(#state.lots) .. " LOTS") or "NO ACTIVE AUCTIONS"
    return true
end
        ...
        if ev and ev.OnClientEvent then
            ev.OnClientEvent:Connect(function(data)
                if type(data) == "table" and type(data.stock) == "table" then
                    HaimiyachGAG2_AuctionState.stock = data.stock
...
function HaimiyachGAG2_AuctionRefresh(force)
    ...
    local state = HaimiyachGAG2_AuctionState
    local now = os.clock()
    if not force and state.lastFetch and now - state.lastFetch < 6 then return false end
    state.lastFetch = now
    ...
    if not ok then state.status = "SNAPSHOT FAILED: " .. tostring(result) end
    return false
end
...
function HaimiyachGAG2_AuctionStock(lot)
    ...
    local stock = nil
    if HaimiyachGAG2_AuctionState and type(HaimiyachGAG2_AuctionState.stock) == "table" then
        stock = HaimiyachGAG2_AuctionState.stock[lot.lotId]
    end
    if stock == nil then stock = lot.stock or lot.stockQuantity or lot.quantity end
...
function HaimiyachGAG2_AuctionStockText(lot)
    ...
    if stock == nil then return "Unlimited" end
...
function HaimiyachGAG2_AuctionMatchesSelection(lot)
    ...
    local rawName = HaimiyachGAG2_NameKey(lot and (lot.item or lot.Name or lot.name) or "")
...
function HaimiyachGAG2_AuctionListText()
    ...
    for _, lot in ipairs(lots) do
        ...
        local remain = HaimiyachGAG2_AuctionTimeText((tonumber(lot.expiresAt) or now) - now)
...
function HaimiyachGAG2_AuctionBuyLot(lot)
    if type(lot) ~= "table" or not lot.lotId then return false, "bad lot" end
    if not HaimiyachGAG2_AuctionIsActive(lot) then return false, "inactive" end
    local price = HaimiyachGAG2_AuctionCurrentPrice(lot)
    local ok, res = fire("Auctioneer.PurchaseLot", lot.lotId, price)
    if ok then
...
addMiniNote(UI.Pages["AUCTION"], "Uses the live Auctioneer snapshot. Select target items first; ALL ACTIVE AUCTION ITEMS will buy any active lot.")
