...
local function HaimiyachGAG2_ReadKgValue(v)
    ...
    if type(v) == "string" then
        ...
        local n = tonumber(string.match(clean, "([%d%.]+)"))
        if n and n > 0 then
            if string.find(clean, "kg", 1, true) then return n end
            if string.find(clean, "g", 1, true) then return n / 1000 end
            return n
        end
    end
    return nil
...
local function HaimiyachGAG2_ReadKgAttrs(inst)
    ...
    local kg = HaimiyachGAG2_ReadKgValue(tostring(inst.Name or ""))
    if kg then return kg end
    return nil
end
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    while node and node ~= Workspace and guard < 8 do
        guard = guard + 1
        local kg = HaimiyachGAG2_ReadKgAttrs(node)
        if kg then return kg end
        node = node.Parent
    end
    local checked = 0
...
function HaimiyachGAG2_ObjectMutation(obj)
    ...
    while node and node ~= Workspace and guard < 10 do
        guard = guard + 1
        local v = node:GetAttribute("Mutation")
        if type(v) == "string" and v ~= "" and v ~= "None" and v ~= "Normal" then return v end
        node = node.Parent
    end
    local checked = 0
    for _, d in ipairs(obj:GetDescendants()) do
        ...
        if type(mv) == "string" and mv ~= "" and mv ~= "None" and mv ~= "Normal" then return mv end
        if checked >= 80 then break end
    end
    return nil
end
...
function HaimiyachGAG2_SelectedMutationOk(obj, selected)
    ...
    for name, enabled in pairs(selected) do
        if enabled == true then
            local want = HaimiyachGAG2_NameKey(name)
            if want ~= "" and (key == want or string.find(key, want, 1, true)) then return true end
        end
    end
    return false
end
...
function HaimiyachGAG2_GetFruitToolMutation(tool)
    if not tool then return nil end
    local v = tool:GetAttribute("Mutation")
    if type(v) == "string" and v ~= "" and v ~= "None" and v ~= "Normal" then return v end
    local checked = 0
    for _, d in ipairs(tool:GetDescendants()) do
        checked = checked + 1
        v = d:GetAttribute("Mutation")
        if type(v) == "string" and v ~= "" and v ~= "None" and v ~= "Normal" then return v end
        if checked >= 80 then break end
    end
    return nil
...
function HaimiyachGAG2_SelectedToolMutationMatch(tool, selected)
    if type(selected) ~= "table" or not picked(selected) then return false end
    local mutation = HaimiyachGAG2_GetFruitToolMutation(tool)
    if not mutation then return false end
    local key = HaimiyachGAG2_NameKey(mutation)
    if key == "" then return false end
    for name, enabled in pairs(selected) do
...
function HaimiyachGAG2_ShouldFavoriteFruit(tool)
    ...
    local mutation = tostring(HaimiyachGAG2_GetFruitToolMutation(tool) or "")
    if picked(S.favoriteMutations) then
        if not HaimiyachGAG2_SelectedToolMutationMatch(tool, S.favoriteMutations) then return false end
    else
...
function HaimiyachGAG2_ProtectSellMutations()
    ...
    for _, tool in ipairs(HaimiyachGAG2_GetInventoryFruitTools()) do
        local keep = false
        if hasMutationKeep and HaimiyachGAG2_SelectedToolMutationMatch(tool, S.sellKeepMutations) then
            keep = true
        end
