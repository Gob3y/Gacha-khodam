...
local S = {
    ...
    -- perf / webhook
    fpsBoost = false, hidePlants = false, highGraphics = false,
...
local CONFIG_KEYS = {
    ...
    "fpsBoost", "hidePlants", "highGraphics", "webhookEnabled", "webhookUrl", "webhookInterval", "webhookDisconnect",
...
local HidePlantsDescendantConnection = nil
...
local function IsGardenPlantObject(obj)
    ...
    while node and node ~= Workspace do
        if node.Name == "Plants" and node:IsDescendantOf(gardens) then
            return true
        end
        if node == gardens then break end
        node = node.Parent
    end
...
local function ResetHidePlantsBackup()
    HidePlantsBackup.BaseParts = {}
    HidePlantsBackup.Decals = {}
    HidePlantsBackup.Effects = {}
    HidePlantsBackup.ParticleRates = {}
end

local function HidePlantObject(obj)
    if not obj or not IsGardenPlantObject(obj) then return end

    if obj:IsA("BasePart") then
        if HidePlantsBackup.BaseParts[obj] == nil then
            pcall(function()
                HidePlantsBackup.BaseParts[obj] = {
    ...
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        if HidePlantsBackup.Decals[obj] == nil then
            pcall(function()
                HidePlantsBackup.Decals[obj] = obj.Transparency
            end)
        end
    ...
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
        if HidePlantsBackup.Effects[obj] == nil then
            pcall(function()
                HidePlantsBackup.Effects[obj] = obj.Enabled
            end)
        end
        if obj:IsA("ParticleEmitter") and HidePlantsBackup.ParticleRates[obj] == nil then
            pcall(function()
                HidePlantsBackup.ParticleRates[obj] = obj.Rate
            end)
        end
...
local function HidePlantTree(obj)
    HidePlantObject(obj)
    local descendants = {}
    pcall(function()
    ...
    for _, child in ipairs(descendants) do
        HidePlantObject(child)
    end
end

local function RestoreHiddenPlants()
    if HidePlantsDescendantConnection then
        pcall(function() HidePlantsDescendantConnection:Disconnect() end)
        HidePlantsDescendantConnection = nil
    end

    HidePlantsScanToken = HidePlantsScanToken + 1

    for obj, props in pairs(HidePlantsBackup.BaseParts) do
        if obj and obj.Parent and type(props) == "table" then
            if props.LocalTransparencyModifier ~= nil then
                SafeSetProperty(obj, "LocalTransparencyModifier", props.LocalTransparencyModifier)
            end
            if props.CastShadow ~= nil then
                SafeSetProperty(obj, "CastShadow", props.CastShadow)
            end
        end
    end

    for obj, transparency in pairs(HidePlantsBackup.Decals) do
        if obj and obj.Parent then
            SafeSetProperty(obj, "Transparency", transparency)
        end
    end

    for obj, enabled in pairs(HidePlantsBackup.Effects) do
        if obj and obj.Parent then
            SafeSetProperty(obj, "Enabled", enabled)
        end
    end

    for obj, rate in pairs(HidePlantsBackup.ParticleRates) do
        if obj and obj.Parent then
            SafeSetProperty(obj, "Rate", rate)
        end
    end

    ResetHidePlantsBackup()
...
local function ApplyHidePlants()
    ...
    task.spawn(function()
        ...
        local descendants = {}
        pcall(function()
            descendants = gardens:GetDescendants()
        end)

        local processed = 0
        for _, obj in ipairs(descendants) do
            if scanToken ~= HidePlantsScanToken or not HidePlantsActive or not S.hidePlants then
                break
            end
            ...
            if processed % 90 == 0 then
                task.wait()
            end
        end
    end)

    HidePlantsDescendantConnection = Workspace.DescendantAdded:Connect(function(obj)
        if HidePlantsActive and S.hidePlants and IsGardenPlantObject(obj) then
            task.delay(0.1, function()
                if HidePlantsActive and S.hidePlants and obj and obj.Parent then
                    HidePlantTree(obj)
                end
            end)
        end
...
local function SetHidePlantsEnabled(state)
    if state then
        ...
        if HidePlantsActive or S.hidePlants then
            RestoreHiddenPlants()
        else
            S.hidePlants = false
        end
...
local fpsBoostToggle, hidePlantsToggle, highGraphicsToggle
...
hidePlantsToggle = addToggle(VisualsPage, "HIDE PLANTS", S.hidePlants, function(v)
    S.hidePlants = v
