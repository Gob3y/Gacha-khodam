...
-- Forward declaration: KG filters are defined before GardenESP, so they must
-- reference this same local table after GardenESP is initialized below.
local GardenESP
...
local function promptCarrier(prompt)
    -- Prefer the real fruit model/part when a prompt is nested inside HarvestPart.
    -- The old version stopped at the first PlantId, which could be HarvestPart or
    -- the parent plant; that made KG filters read plant KG and harvest small fruits.
    local node = prompt and prompt.Parent or nil
    local fallback = nil
    local guard = 0
    while node and node ~= Workspace and guard < 14 do
        ...
        if node:IsA("Model") and node.Parent and tostring(node.Parent.Name) == "Fruits" then
            return node
        end
        node = node.Parent
    end
    if fallback then return fallback end
    return prompt and prompt:FindFirstAncestorWhichIsA("Model") or nil
end
...
function HaimiyachGAG2_ParseKgText(value, allowPlainNumber)
    ...
    for _, pat in ipairs(patterns) do
        local n = string.match(text, pat)
        local v = n and tonumber(n)
        if v and v > 0 then return v end
    end

    -- Grams are only accepted when the unit is exactly g, not kg.
    local g = string.match(text, "^([%d]+%.?[%d]*)g$")
    local gv = g and tonumber(g)
    if gv and gv > 0 then return gv / 1000 end
...
function HaimiyachGAG2_IsWeightFieldName(name)
    ...
    return string.find(name, "weight", 1, true)
        or string.find(name, "kg", 1, true)
        or string.find(name, "kilogram", 1, true)
        or string.find(name, "mass", 1, true)
end
...
function HaimiyachGAG2_InstanceKg(inst)
    ...
    local attrs = {
        "Weight", "WeightKg", "WeightKG", "KG", "Kg",
        "Kilogram", "Kilograms", "Mass", "FruitWeight", "FruitKg", "FruitKG"
    }

    for _, attr in ipairs(attrs) do
        local ok, raw = pcall(function() return inst:GetAttribute(attr) end)
        if ok then
            local kg = HaimiyachGAG2_ParseKgText(raw, true)
...
function HaimiyachGAG2_ObjectSizeMultiplier(obj)
    if not obj then return nil end
    local attrs = { "SizeMultiplier", "SizeMulti", "Size", "Scale", "FruitSize", "GrowthMultiplier", "OvertimeGrowth" }
    for _, attr in ipairs(attrs) do
        local ok, raw = pcall(function() return obj:GetAttribute(attr) end)
...
function HaimiyachGAG2_KgGardenTarget(obj)
    ...
    while node and node ~= Workspace and guard < 14 do
        ...
        local hasPlant = node:GetAttribute("PlantId") ~= nil
        local hasFruit = node:GetAttribute("FruitId") ~= nil
        ...
        if hasPlant and not fallback then
            fallback = node
        end
    ...
    return fallback or obj, fallback ~= nil
end

function HaimiyachGAG2_GardenCalcKg(obj)
    if not obj then return nil end
    if GardenESP and type(GardenESP.calculateWeight) == "function" then
        local sizeMulti = HaimiyachGAG2_ObjectSizeMultiplier(obj)
        local ok, weight = pcall(function()
            return GardenESP.calculateWeight(obj, sizeMulti)
        end)
...
function HaimiyachGAG2_ObjectKg(obj)
    ...
    if cached and cached.t and (now - cached.t) < 0.6 then
        return cached.kg
    end
    ...
    if not kg then
        local target, isGarden = HaimiyachGAG2_KgGardenTarget(obj)

        if target and target ~= obj then
            kg = HaimiyachGAG2_InstanceKg(target)
            if not kg then kg = HaimiyachGAG2_DirectChildrenKg(target) end
            if not kg then kg = HaimiyachGAG2_GardenCalcKg(target) end
        elseif isGarden then
            kg = HaimiyachGAG2_DirectChildrenKg(obj)
            if not kg then kg = HaimiyachGAG2_GardenCalcKg(obj) end
        end

        -- Non-garden inventory tools can store KG in direct child values/labels.
        -- For garden fruits, do NOT scan parent plant as fallback because that causes
        -- ABOVE KG to harvest tiny fruits when only the parent plant is heavy.
        if not kg and not isGarden then
            kg = HaimiyachGAG2_DirectChildrenKg(obj)
            if not kg then kg = HaimiyachGAG2_GardenCalcKg(obj) end
        end
    end
...
function HaimiyachGAG2_NormalizeKgMode(mode)
    mode = string.upper(tostring(mode or "OFF"))
    if string.find(mode, "BELOW", 1, true) then return "BELOW KG" end
    if string.find(mode, "ABOVE", 1, true) then return "ABOVE KG" end
    return "OFF"
end

function HaimiyachGAG2_KgModeActive(mode, value)
    mode = HaimiyachGAG2_NormalizeKgMode(mode)
...
function HaimiyachGAG2_KgValueOk(kg, mode, value)
    ...
    if mode == "BELOW KG" then
        return kg <= limit
    elseif mode == "ABOVE KG" then
        return kg >= limit
    end
...
function GardenESP.initModules()
    ...
    pcall(function()
        ...
        if not GardenESP.visualizer then
            -- Latest controller data uses FruitVisualizerController for
            -- CalculateFruitWeight / CalculatePlantWeight. Older dumps may still
            -- expose PlantVisualizerController, so keep it as fallback.
            local visualizer = controllers and (controllers:FindFirstChild("FruitVisualizerController") or controllers:FindFirstChild("PlantVisualizerController"))
            if visualizer then
...
function HaimiyachGAG2_GetFruitToolKg(tool)
    local kg = HaimiyachGAG2_ObjectKg(tool)
    if kg then return kg end

    -- Inventory fruit tools often only expose SizeMultiplier, while visible KG is
    -- shown in BackpackGui. Calculate from GrowData so SELL KEEP / FAVORITE weight
    -- filters still work even without a literal Weight attribute.
    local sizeMulti = HaimiyachGAG2_ObjectSizeMultiplier(tool)
    if GardenESP and type(GardenESP.calculateWeight) == "function" then
        local ok, weight = pcall(function()
            return GardenESP.calculateWeight(tool, sizeMulti)
        end)
        weight = tonumber(weight)
        if ok and weight and weight > 0 then return weight end
    end

    if GardenESP and type(GardenESP.initModules) == "function" then
        pcall(GardenESP.initModules)
        local name = HaimiyachGAG2_ObjectName(tool)
        local baseWeight = (GardenESP.fruitBaseWeight and GardenESP.fruitBaseWeight[name])
            or (GardenESP.plantBaseWeight and GardenESP.plantBaseWeight[name])
        baseWeight = tonumber(baseWeight)
        sizeMulti = tonumber(sizeMulti) or 1
        if baseWeight and baseWeight > 0 and sizeMulti > 0 then
