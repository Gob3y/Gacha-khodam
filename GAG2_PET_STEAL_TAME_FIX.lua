...
local function HaimiyachGAG2_AncestorGarden(obj)
    local gardens = Workspace:FindFirstChild("Gardens")
    local cur = obj
    while cur and cur ~= Workspace do
        if gardens and cur.Parent == gardens then return cur end
        cur = cur.Parent
    end
    return nil
end

local function HaimiyachGAG2_StealFruitValue(obj)
    if not obj then return 0 end
    local attrs = { "FinalSellValue", "SellValue", "Value", "Price", "Worth" }
    for _, a in ipairs(attrs) do
        local v = tonumber(obj:GetAttribute(a))
        if v and v > 0 then return v end
    end
    local kg = HaimiyachGAG2_ObjectKg and HaimiyachGAG2_ObjectKg(obj) or nil
    if kg and kg > 0 then return kg * 1000 end
    local name = HaimiyachGAG2_ObjectName and HaimiyachGAG2_ObjectName(obj) or tostring(obj.Name or "")
    if GardenESP and type(GardenESP.estimateValue) == "function" then
        local ok, v = pcall(function() return GardenESP.estimateValue(name, obj) end)
...
local function stealable()          -- other players' ripe fruit (tag "StealPrompt")
    ...
    for _, pr in ipairs(CollectionService:GetTagged("StealPrompt")) do
        if pr:IsA("ProximityPrompt") and pr.Enabled and pr:IsDescendantOf(Workspace) then
            ...
            if pid then
                ...
                if owner ~= 0 and owner ~= LocalPlayer.UserId then
                    local garden = HaimiyachGAG2_AncestorGarden(m)
                    out[#out + 1] = {
                        owner = owner,
                        ...
                        obj = m,
                        garden = garden,
                        gardenName = garden and garden.Name or tostring(owner),
                        value = HaimiyachGAG2_StealFruitValue(m),
                        name = HaimiyachGAG2_ObjectName and HaimiyachGAG2_ObjectName(m) or tostring(m.Name or "Fruit"),
                    }
                end
            end
        end
    ...
    table.sort(out, function(a, b)
        local ga = tostring(a.gardenName or "")
        local gb = tostring(b.gardenName or "")
        if ga ~= gb then
            local totalA, totalB = 0, 0
            for _, x in ipairs(out) do
                if tostring(x.gardenName or "") == ga then totalA = totalA + (tonumber(x.value) or 0) end
                if tostring(x.gardenName or "") == gb then totalB = totalB + (tonumber(x.value) or 0) end
            end
...
local function HaimiyachGAG2_WildPetFolders()
    local map = Workspace:FindFirstChild("Map")
    local ref = map and (map:FindFirstChild("WildPetRef") or map:FindFirstChild("WildPets")) or nil
    local spawns = map and map:FindFirstChild("WildPetSpawns") or nil
    return ref, spawns
end

local function HaimiyachGAG2_FindWildPetVisual(refPart, petName)
    local _, spawns = HaimiyachGAG2_WildPetFolders()
    if not spawns then return nil end
    local refName = refPart and refPart.Name or ""
    local wanted = tostring(petName or "")
    local best = nil
    for _, obj in ipairs(spawns:GetChildren()) do
        if obj:IsA("Model") then
            local n = tostring(obj.Name or "")
            local attrName = tostring(obj:GetAttribute("PetName") or "")
            if (refName ~= "" and string.find(n, refName, 1, true)) or (wanted ~= "" and (attrName == wanted or string.find(n, wanted, 1, true))) then
                best = obj
                if refName ~= "" and string.find(n, refName, 1, true) then return obj end
            end
        end
    end
    return best
...
local function HaimiyachGAG2_WildPetPromptFromVisual(model)
...
local function HaimiyachGAG2_WildPetPosition(refPart, visual)
    if visual then
        local pp = visual.PrimaryPart or visual:FindFirstChild("PrimaryPart") or visual:FindFirstChild("RootPart") or visual:FindFirstChildWhichIsA("BasePart")
        if pp and pp:IsA("BasePart") then return pp.Position end
        local ok, cf = pcall(function() return visual:GetPivot() end)
        if ok and cf then return cf.Position end
    end
    if refPart and refPart:IsA("BasePart") then return refPart.Position end
    return nil
end

local function wildPets()
    local out = {}
    local ref = HaimiyachGAG2_WildPetFolders()
    if ref then
        for _, p in ipairs(ref:GetChildren()) do
            if p:IsA("BasePart") then
                local petName = p:GetAttribute("PetName") or p:GetAttribute("Name") or p.Name
                local visual = HaimiyachGAG2_FindWildPetVisual(p, petName)
                local prompt = HaimiyachGAG2_WildPetPromptFromVisual(visual)
                ...
                out[#out + 1] = {
                    ...
                    prompt = prompt,
                    name = petName,
                    rarity = p:GetAttribute("Rarity") or (visual and visual:GetAttribute("Rarity")),
                    price = tonumber(p:GetAttribute("Price")) or tonumber(visual and visual:GetAttribute("Price")) or 0,
                    owner = tonumber(p:GetAttribute("OwnerUserId")) or tonumber(p:GetAttribute("Owner")) or 0,
                    spawnedAt = spawnedAt,
                    lifetime = lifetime,
                    state = p:GetAttribute("State") or p:GetAttribute("PetState"),
                    pos = HaimiyachGAG2_WildPetPosition(p, visual),
                }
            end
        end
    end
    table.sort(out, function(a, b)
        local ar = tostring(a.rarity or "Common")
        local br = tostring(b.rarity or "Common")
        if type(HaimiyachGAG2_GetWildPetRarity) == "function" then
            ar = HaimiyachGAG2_GetWildPetRarity(a)
            br = HaimiyachGAG2_GetWildPetRarity(b)
        end
        local ra = HaimiyachGAG2_RarityScore and (HaimiyachGAG2_RarityScore[ar] or 0) or 0
        local rb = HaimiyachGAG2_RarityScore and (HaimiyachGAG2_RarityScore[br] or 0) or 0
...
local S = {
    ...
    -- steal
    autoSteal = false, stealTeleport = true, stealReturnBase = true, stealDelay = 0.05, stealOwnerRange = 45,
    -- misc
...
local CONFIG_KEYS = {
    ...
    "autoGear", "autoBuyAllGear", "gearBuy", "gearInterval", "autoSteal", "stealTeleport", "stealReturnBase", "stealDelay", "stealOwnerRange",
...
local function HaimiyachGAG2_TriggerPromptObject(prompt)
    ...
    local ok2 = pcall(function()
        prompt:InputHoldBegin()
        task.wait(math.max(0.2, tonumber(prompt.HoldDuration) or 0.2) + 0.1)
        prompt:InputHoldEnd()
    end)
    return ok2 == true
end
...
local function HaimiyachGAG2_TriggerWildPetPrompt(w)
    ...
    if type(w) == "table" then
        part = w.part
        visual = w.visual or HaimiyachGAG2_FindWildPetVisual(w.part, w.name)
        prompt = w.prompt or HaimiyachGAG2_WildPetPromptFromVisual(visual)
    else
        part = w
        visual = HaimiyachGAG2_FindWildPetVisual(part, part and (part:GetAttribute("PetName") or part.Name) or nil)
        prompt = HaimiyachGAG2_WildPetPromptFromVisual(visual)
    end
    if HaimiyachGAG2_TriggerPromptObject(prompt) then return true end
    for _, rootObj in ipairs({ visual, part }) do
        if rootObj then
            for _, d in ipairs(rootObj:GetDescendants()) do
                if d:IsA("ProximityPrompt") and HaimiyachGAG2_TriggerPromptObject(d) then
                    return true
                end
            end
        end
...
local function HaimiyachGAG2_WaitWildPetConfirmed(w, beforeCount)
    ...
    local part = w.part
    local visual = w.visual or HaimiyachGAG2_FindWildPetVisual(w.part, w.name)
    local petName = tostring(w.name or "")
    local oldOwner = tonumber(w.owner or 0) or 0
    local deadline = os.clock() + 4.5

    while os.clock() < deadline do
        if part and (not part.Parent or not part:IsDescendantOf(Workspace)) then
            return true
        end
        if visual and (not visual.Parent or not visual:IsDescendantOf(Workspace)) then
            return true
        end
        ...
        if part then
            ...
            if oldOwner == 0 and owner ~= 0 then return true end
            local state = string.lower(tostring(part:GetAttribute("State") or part:GetAttribute("PetState") or ""))
            if string.find(state, "tame", 1, true) or string.find(state, "own", 1, true) or string.find(state, "sold", 1, true) or string.find(state, "bought", 1, true) then
                return true
            end
        end
...
local function HaimiyachGAG2_ReturnToGardenAfterPet(savedCf, gardenPos, moved, statusText)
    if not (moved and S.petTeleport) then return end
    ...
    if gardenPos then
        ...
        Stats.lastReturnGardenStatus = statusText or "PET OK"
    elseif savedCf then
...
local function HaimiyachGAG2_TameWildPet(w)
    ...
    w.visual = w.visual or HaimiyachGAG2_FindWildPetVisual(w.part, w.name)
    w.prompt = w.prompt or HaimiyachGAG2_WildPetPromptFromVisual(w.visual)
    w.pos = HaimiyachGAG2_WildPetPosition(w.part, w.visual) or w.pos
    ...
    if S.petTeleport and w.pos and hrp then
        pcall(function() hrp.CFrame = CFrame.new(w.pos + Vector3.new(0, 3.5, 0)) end)
        moved = true
        task.wait(0.75)
    end
    ...
    for attempt = 1, 4 do
        ...
        fired = fired or ok
        HaimiyachGAG2_TriggerWildPetPrompt(w)
        if HaimiyachGAG2_WaitWildPetConfirmed(w, beforeCount) then
            HaimiyachGAG2_ReturnToGardenAfterPet(savedCf, gardenPos, moved, "PET OK")
            return true
        end
        task.wait(0.55)
    end
...
-- //                     STEAL (PvP, night)                      \\ --
-- // ============================================================ \\ --
-- Smart steal: choose high-value gardens first, steal highest-value fruit, and return only after confirmed carry.
local function hrpNow() local c = LocalPlayer.Character; return c and c:FindFirstChild("HumanoidRootPart") end
HaimiyachGAG2_StealFailUntil = HaimiyachGAG2_StealFailUntil or {}

local function HaimiyachGAG2_StealKey(f)
    return tostring(f.owner or "") .. ":" .. tostring(f.plantId or "") .. ":" .. tostring(f.fruitId or "")
end

local function HaimiyachGAG2_WaitStealCarry(timeout)
    local deadline = os.clock() + (tonumber(timeout) or 2.5)
    while os.clock() < deadline do
        if LocalPlayer:GetAttribute("CarryingStolenFruit") or LocalPlayer:GetAttribute("CarryingFruit") then
            return true
        end
        task.wait(0.1)
    end
...
local function HaimiyachGAG2_WaitStealDelivered(timeout)
    local deadline = os.clock() + (tonumber(timeout) or 3)
    while os.clock() < deadline do
        if not (LocalPlayer:GetAttribute("CarryingStolenFruit") or LocalPlayer:GetAttribute("CarryingFruit")) then
            return true
        end
        task.wait(0.12)
    end
...
local function HaimiyachGAG2_OwnerNearby(f, range)
    range = tonumber(range) or 45
    if not (f and f.pos) then return false end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and (tonumber(f.owner) == 0 or plr.UserId == tonumber(f.owner)) then
            local c = plr.Character
            local r = c and c:FindFirstChild("HumanoidRootPart")
            if r and (r.Position - f.pos).Magnitude <= range then return true end
        end
    end
    return false
end

local function HaimiyachGAG2_BestStealTargets()
    local targets = {}
    local now = os.clock()
    for _, f in ipairs(stealable()) do
        local key = HaimiyachGAG2_StealKey(f)
        if not HaimiyachGAG2_StealFailUntil[key] or HaimiyachGAG2_StealFailUntil[key] <= now then
            local ownerNear = HaimiyachGAG2_OwnerNearby(f, tonumber(S.stealOwnerRange) or 45)
            if not ownerNear then
                targets[#targets + 1] = f
            end
        end
    end
...
loopOn(function() return S.autoSteal end, 1.5, function()
    ...
    local targets = HaimiyachGAG2_BestStealTargets()
    for _, f in ipairs(targets) do
        if not (S.autoSteal and isNight()) then break end
        local key = HaimiyachGAG2_StealKey(f)
        if f.pos and S.stealTeleport then
            local hrp = hrpNow()
            if hrp then
        ...
        if f.prompt and HaimiyachGAG2_TriggerPromptObject then HaimiyachGAG2_TriggerPromptObject(f.prompt) end
        task.wait(0.12)
        fire("Steal.CompleteSteal")

        if HaimiyachGAG2_WaitStealCarry(2.75) then
            Stats.stolen = (tonumber(Stats.stolen) or 0) + 1
            if Stats.SetLastAction then Stats.SetLastAction("STEAL", tostring(f.name or "Fruit") .. " " .. fmt(tonumber(f.value) or 0)) end
            if S.stealReturnBase then
                ...
                if base and hrp then
                    ...
                    Stats.lastReturnGardenAt = os.clock()
                    HaimiyachGAG2_WaitStealDelivered(3.5)
                end
            end
            break
        else
            HaimiyachGAG2_StealFailUntil[key] = os.clock() + 8
            if Stats.SetWarning then Stats.SetWarning("STEAL", "Not confirmed: " .. tostring(f.name or "Fruit")) end
        end
